...
8. **Directory hygiene.** Rows come from the organization's own domain (or a government domain) only; https-only; registration *kinds* stated, numbers never transcribed; receipts always come from the organization; `verifiedOn` older than 18 months drops the row to draft at the accessor (`isOrgRowStale`). Non-monetary hand-offs (e-RaktKosh) say so on the row.
...
11. **Tests.** `daanContent.test.ts` pins registry shape, bilingual parity, the source threshold, draft invisibility, live-solver rule-id integrity, exact-beats-suffix matching, directory hygiene + staleness, the gupt guarantee, CSV escaping, and the copy guard; `DaanLedgerContext.test.tsx` pins the versioned payload and the write-side gupt strip; `DaanScreens.test.tsx` pins the §2.7 surface contract and the journey's terminal gating. Typecheck, Jest/lint and the Maestro flow are required before release.
