...
**No native store-review module.** The rating hand-off uses `Linking.openURL` to the store listing, not `expo-store-review` / `SKStoreReviewController` / Play In-App Review. Those are native modules: adding one moves the feature out of the bundle and behind a store rebuild, which the roadmap's bundle-only constraint forbids. The same reasoning governs the Instagram row's `https://` URL (design.md §37). Since the OS is not throttling us, the throttling in `data/ratingPrompt.ts` (2 lifetime asks, 10-day cooldown, engagement floor) **is** the user protection — do not loosen a threshold without an explicit product decision. **`MAX_ASKS` is the load-bearing one:** it caps lifetime exposure at two cards no matter what the user taps, which is what lets the cooldown be short. Shortening the cooldown further is a scheduling change; raising `MAX_ASKS` changes how many times a user can be interrupted, and is the one to argue about.
