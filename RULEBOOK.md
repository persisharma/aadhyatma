...
12. **Personalised Tarabala/Chandrabala (Phase 4) — annotate-only, private by construction.** The strip's source of truth is the saved Kundali profile (`@vedansh:kundali-birth-profile:v1`) and nothing else: never re-asked anywhere in the finder, nothing derived from it persisted (one per-session memo keyed on the profile record; a corrupt profile is a guest state, never a rendered guess). Five hard lines, all test-pinned:
