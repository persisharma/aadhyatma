...
## 27. Daan-punya (PRD-26) — the giving-layer contract

A daan entry is **source-reviewed tradition with its stated reason** — never a fruit-promise, a score, or a solicitation. Section §11 applies in full; the additional contracts below are what make this feature *not* a donate button.

1. **Educate-first is the default (§2.7 relaxed).** Educate is what opens first — not a wall in front of the door. `DaanPunyaScreen` carries a **standing door block**: a filled `दान करें` button (`daan-home-donate` → `DaanJourney`, occasion-aware) and a quiet direct `दान-द्वार` link (`daan-home-dwaar` → `DaanDirectory`), both present every day. The directory is thus reachable from the journey terminal, the journey skip, OR the home's direct link — but it is still never a tab. The app **still never transacts** (Resolution A, links-only): every hand-off is `Linking.openURL` to the organization's own site behind the honest interstitial, and the ledger is never gated. The journey is one skippable scroll — its terminal actions are NO LONGER step-gated, but the reading still comes first (a quiet `daan-journey-skip` jumps past it for the informed) and the record action always precedes the directory hand-off. Pinned by `DaanScreens.test.tsx` — a change that removes the home doors, or that makes the app collect payment, is a contract break, not a tweak.
2. **The app never touches money (Resolution A, links-only).** No payment collection, processing, confirmation, commission, suggested amounts, targets or progress bars — the hand-off is `Linking.openURL` to the organization's own official channel behind the honest interstitial, and the return-offer renders at most once. No UPI VPAs anywhere (web-only hand-off; open decision #4). **PRD-26/PRD-40 boundary:** the PRD-40 dakshina/donation rail is ON HOLD; the daan directory (`directory.ts`) must **never contain a Vedansh-owned recipient** — every row's `officialUrl` is the organization's own (or a government) domain, never ours (§27.8, directory hygiene). If PRD-40 ever resumes, it owns dakshina-to-Vedansh / partner-temples on its own surfaces and is never merged into this directory.
3. **Punya is not a number.** No totals, streaks, scores, leaderboards or comparisons anywhere — the ledger is a smaran register; `buildLedgerCsv` via the OS share sheet is the single aggregation surface. No helper that sums amounts may be added to `data/daan/ledger.ts`.
4. **The gupt guarantee is two-sided.** `sanitizeLedgerEntry` strips note/amount/occasion from gupt entries before persistence AND `isDaanLedgerEntry` rejects unsanitized gupt rows on hydration; the CSV keeps gupt rows bare. Amounts never render outside the ledger and its export.
5. **Verified-only accessors.** Every registry (`principles`, `occasions`, `kathas`, `directory`) exposes `status: 'verified'` rows only; drafts are indistinguishable from absence (the §20/§21/§22 pattern).
6. **Source threshold.** A verified row needs ≥2 concordant references with a dated `verificationNote` — shipped verified in-repo content that itself carries the tradition counts as one (named by path), an external published domain as another. Contested enumerations (the daśa-dāna list is the recorded example) state the variance or stay draft.
7. **Coverage discipline.** `occasions.ts` rows key to REAL solver rule ids (exact `ruleIds` beat `ruleIdSuffixes` families; `shattila-ekadashi` over `-ekadashi`, `makar-sankranti` over `-sankranti`). A day with no attested daan tradition gets NO row, and hosts (Observance Detail's last-section door, the home's आज card) render nothing for it — absent, never a placeholder. Grahan-daan stays out until the engine computes eclipses.
8. **Directory hygiene.** Each row carries exactly ONE outbound link, `officialUrl` — the organization's own domain (or a government domain), https, pointing at the page where the giving actually happens; never an aggregator, never a gateway of ours. `verifiedOn` older than 18 months drops the row to draft at the accessor (`isOrgRowStale`). Non-monetary hand-offs (e-RaktKosh) say so on the row.
9. **Stance guard (hard).** No fear copy, no "daan removes dosha", no guilt ("you haven't given"), no fruit-claims beyond what a cited verse itself states, and reminders may name the occasion and tradition only — never an amount or recipient. The copy-guard test bans the score/streak/fear vocabulary across all `data/daan/` copy.
10. **Store-policy gate.** The release that first carries the directory rides a store build with the PRD-26 §6.1 written self-review appended to the PRD, even though the code is OTA-safe JS. If review objects, the directory degrades to information-only rows (no `openURL`); educate/ledger are never hostage to this gate.
11. **Tests.** `daanContent.test.ts` pins registry shape, bilingual parity, the source threshold, draft invisibility, live-solver rule-id integrity, exact-beats-suffix matching, directory hygiene + staleness, the cause-mahatva invariants of §27.14, the gupt guarantee, CSV escaping, and the copy guard; `DaanLedgerContext.test.tsx` pins the versioned payload and the write-side gupt strip; `DaanScreens.test.tsx` pins the §2.7 surface contract (educate-first as default) — including the home's standing doors (`daan-home-donate` → journey, `daan-home-dwaar` → directory) — the journey's always-present terminal actions in both occasion and daily modes, the द्वार's cause grid (`daan-directory-grid` of `daan-cause-tile-*` with no counts) drilling into the filtered single-cause view with its cause-mahatva above the places, and the detail screen's thinness (no paperwork, no verification badge, the hand-off opening `officialUrl` only). Typecheck, Jest/lint and the Maestro flow are required before release.
12. **Two axes, never conflated.** `DaanCategory` (ledger) is the **dravya** — what was given; `DaanCause` (`causes.ts`) is the **प्रयोजन** — whom it serves, and it is the द्वार's only grouping axis. A cause chip renders only where a verified org row carries that cause, so the taxonomy can never show an empty shelf; every occasion's `causes` must resolve to real causes, and an occasion with no honest cause carries none rather than an invented one.
13. **The purpose bridge is the honest subset.** A `DaanCause` may declare a `purposeId` from the shipped text-intent taxonomy ONLY where that taxonomy already names the seva (विद्या→`knowledge`, आरोग्य→`health`). `wealth`/`prosperity` must stay unbridged — a give-door on a prosperity intent is the fruit-promise §27.9 bans. The bridge door opens the educate home, never the directory; both rules are pinned in tests.

14. **Explain the daan, do not profile the org.** The **mahatva belongs to the cause**: every `DaanCause` carries `mahatvaHi/mahatvaEn` — why this giving is held dear — and the द्वार renders it ABOVE the places that serve it, in both the grouped and the filtered state. A mahatva that makes a scriptural or textual claim MUST carry the paired `citeHi`/`citeEn` and a review-only `source` meeting §27.6; one written as plain tradition-register prose carries neither — an invented citation is worse than none.
   A **directory row is a place, not a profile**: a name, ONE line of what they do, and `officialUrl`. `DaanOrgEntry` may not carry account numbers, UPI ids, registration/80G/paperwork text, or scale figures the app would be asserting on an organization's behalf — whatever receipt or exemption exists is between the giver and the organization. `verifiedOn` and `source` stay EDITORIAL: they drive the staleness drop and the review, and are never rendered as a trust badge at the user. The banned field names are pinned against `types.ts` by test, so re-adding one fails the suite rather than shipping.
