...
- **Every reader carries read-aloud, exposed only through the shared hook + button, never a raw `Speech.speak`.** Read-aloud is part of the reader shell, not an opt-in: all 21 `<Pascal>ReaderScreen`s (verse readers, the registry readers, chaptered texts and the prose Vrat Katha reader) call `useReaderReadAloud({ sourceId, data, offset, verseCount, currentIndex, listRef })` (`mobile/src/screens/_useReaderReadAloud.ts`) and render `ReadAloudButton` inside a `readAloudSlot` view pinned to the **right edge of the language-toggle row** (`position: absolute`, `right: 16`, matching the row's vertical padding) — never by importing `expo-speech` in a screen, never in `ReaderHeader`'s `right` slot (it cramps the counter and widens `sideWidth`, shifting the centred title), and never in a verse page's `topActions` (which renders once per page). Call the hook **before** any null-chapter early return so hook order stays stable. A new reader ships with the pill and a row in `mobile/src/screens/__tests__/readerReadAloud.test.tsx`, the same way `readerAutoAdvance.test.tsx` tables the chaptered readers — the hook is lenient without a provider, so a missing pill is otherwise invisible. The one sanctioned surface without read-aloud is **Japam** (a counter with its own recorded loop, not a reader). (`design.md` §56)
...
**The rating prompt opens only from a reported moment.** It has no cold-start trigger (Sept 2026), and exactly one moment ships: routine completion, reported by the celebration overlay's `onDone`. A surface where the user has just finished something calls `useRatingAsk()('<trigger>')` from **`contexts/ratingAsk.ts`** — the light module, never `RatingPromptContext` itself, which pulls `expo-notifications` into any unit test that mounts the surface standalone. Adding a moment means: (1) add the literal to `RatingAskTrigger` / `RATING_ASK_TRIGGERS` in `data/ratingPrompt.ts` (the parser only keeps known triggers), (2) call `requestAsk` at the point where the moment's own feedback has **finished** (an animation's `onDone`, a screen unmount, a resolved share — never mid-interaction and never on top of a celebration), (3) add the row to the design.md §54 trigger table. The gate, the once-per-session guard and the settle delay are the provider's job, not the host's; a host never checks eligibility itself.
...
**The ask currently has no user-reachable off switch. Know this before you ship it.** Two product decisions compose into one consequence: `MAX_ASKS` is `null` (ask every 5 days until rated) and the sheet carries two actions only — "now" and "later" (Aug 2026). So a user who never rates is asked every 5 days for as long as they keep the app, and nothing in the UI stops it. Moving the trigger off the cold start (Sept 2026) softens *when* it lands, not *how often*: a user who completes a routine daily still meets the card every 5 days. "Maybe later" defers; Android back maps to the same thing; the More row is an entry point, not an exit.
...
### 14.5 Gochar, personal guidance, and the compiled report (PRD-20)

- The PRD-20 engines (`gochar.ts`, `dashaReading.ts`, `weeklyOutlook.ts`, `kundaliReport.ts`, `kundaliReportModel.ts`) live beside `kundali.ts` and inherit its purity contract exactly: explicit `Date` + `KundaliChart` in, typed data out; no React, storage, wall-clock reads, randomness, or network. Source-purity tests pin each file.
- Every daily quantity is evaluated at the shared 06:00 IST anchor (`indiaDayAnchor`), one answer per India civil day. Nothing is persisted and no `panchangDayStore`/cache version is touched — transit quantities are location-free and cheap. Ingress day-walks and the Sade Sati boundary scan defer behind `InteractionManager` in the UI, never on the render path.
- `computePersonalGuidance` must remain a strict superset of `computeRashifal` for the chart's janma rashi — the favour/pause/reflect fields byte-identical (pinned by test) — so the contractual landing rows never change meaning for existing users. Personal extras (Lagna-house context, dasha note, tara bala, Sade Sati flag) layer only on saved-profile surfaces, and on the Rashifal screen only when the selected sign IS the natal Moon sign.
- **Tara/chandra bala has ONE implementation** — `panchang/taraChandraBala.ts` (`tarabala`, `chandrabala`, and its class tables, PRD-16 Phase 4). PRD-20's daily and weekly surfaces delegate to it and map its `favourable | unfavourable | contested` classes onto their own display tones; never add a second table or re-derive the counts (§14.1's no-second-calculation rule). The mapping is `favourable → favourable`, `unfavourable → reflective`, `contested → steady`, and `gochar.engine.test.ts` pins that gochar imports the shared module and declares no tara table of its own.
- Tone vocabulary is closed: `favourable | steady | reflective`. No luck scores, percentages, good/bad day verdicts, or ranked days. Weekly-outlook copy must name its traditional basis (chandra bala house + tara) in the line itself, and the strip carries the explicit "not a rating or verdict" basis note.
- Sade Sati is framed as a traditional observation with guidance copy (patience, steady practice) — never doom, fear, or event claims; the dhaiya (4th/8th) is reported as a secondary observation, never as a phase. Its practice link is `shani-ashtakam` via `buildEntryStartTarget()` and appears only while a phase is active.
- Dosha triage in the compiled report: Sade Sati included; **Mangal Dosha engine + tests ship but the section is display-gated off** (`includeMangalDosha`) until an explicit product/content review — if enabled, prevalence-normalizing copy is mandatory, the "dosha" label is avoided in customer copy, and it must never surface in or link from Guna Milan (§15.3); **Kaal Sarp is excluded by decision**, and a test pins its absence from emitted models. Revisiting either is a recorded product decision, not a code change.
- `KundaliReportModel` is versioned (`reportVersion`) and fully serializable plain JSON — no `Date`/function/class instances; a `JSON.parse(JSON.stringify())` round-trip test pins it. This is also the designed grounding object for the future AI phase (PRD-20 §5); enabling any AI/network path remains banned by §14.3 until that section is explicitly rewritten.
- Sharing boundaries: Gochar has **no share card**; report sections and the report summary share only through the warned Kundali path (birth details named in the warning), never through the Rashifal no-birth-details path. The report's **full-text handoff** (`buildKundaliHandoffText`) is the one sanctioned complete export — pure, rendered from the chart + `KundaliReportModel`, offered ONLY as the secondary action inside the report's warned share sheet (one Share button; the sheet's privacy line must name the birth details both the card and the text carry), and delivered only through the OS share sheet (the user carries the text to notes or an AI assistant themselves; the app still makes no network call — §14.3's AI/network ban is untouched). The `JyotishShareSheet` detail props are optional extensions — never fork the sheet, and omitting them must keep the card-only sheet unchanged. Its framing line must keep stating the interpretive sections are guidance, not predictions, and its JSON tail must stay the parse-back-equal report model (pinned by test).
- Required checks per PRD-20 change: `npm run typecheck`, `npm run test:engine` (purity, determinism, superset lock, banned-vocabulary scans, 27×27 tara grid, pinned 2025 Saturn ingress, report serde + all-12-lagna sweep), targeted Jest (`GocharExperience`, `KundaliReportExperience`, extended `KundaliExperience`), and `.maestro/gochar-smoke.yaml` + `kundali-report-smoke.yaml` with `kundali-smoke.yaml` kept green. If Maestro is not run, state that explicitly before merge.
...
9. **Published registry.** Seven entries are required in `VIDHI_ENTRIES` (the verified-only view of `ALL_VIDHI_ENTRIES`, which also holds the §26 drafts): the six festive vidhis plus `shraddha-tarpan-vidhi`, a narrow, mantra-free household tila-tarpana remembrance guide. It must never claim to be complete Shraddha or absorb pinda/bhojana/homa and branch-specific formulas. `vidhiContent.test.ts` pins the exact registry, hooks, no-mantra boundary and Gita chapters 15/2.
...
5. **Stance guard (hard) — amended for Phase 2 (owner decision, 2026-09-05).** The stance moved from "never a verdict" to **weighted findings**: a placement may be read against the convention as one of five closed classes — निषिद्ध स्थान · विधान से भिन्न · श्रेयस् अनुपलब्ध · परंपरागत विकल्प · मेल (+ `unmeasured`) — each rule carrying its weight (`vidhana`/`shreyas`; `avoidDirections` is always the निषेध class). What remains forbidden: any **composite score, percentage, total, rank or "n of m"**, any home-level verdict, "dosha detected", fear copy, remedies, products, expert referrals, and pseudo-scientific rationale (magnetic-field sleep claims are the recorded example). Every convention that has a stated traditional accommodation carries it (`accommodation*` fields) and the accommodation travels with every reading of that finding — the register is understanding, not compliance.
...
7. **True north stays honest.** Headings are corrected coords-first by `getDeclination` — bilinear over the bundled 1°×1° WMM grid (`data/vastu/declinationGrid.ts`, 6–38°N/66–100°E, integer tenths), then the per-city table (`data/vastu/declination.ts`), both regenerated per WMM epoch via `mobile/scripts/generate-declination.md`; outside both, silently magnetic — the app never invents a correction. The grid regression test holds every bundled city within 0.2° of the city table (the oracle); a breach means re-verify that city, never loosen the tolerance. The UI never claims per-degree precision — the classical unit is the 45° sector (the pada ring's 11.25° unit renders as NAMES, never degrees).
...
10. **Tests and device gate.** `compass.test.ts` pins the heading math (wrap smoothing, sector mapping, declination, tilt, wall/pada boundaries); `useCompassHeading.test.tsx` pins the source ladder (query-never-request, trueHeading −1 fallback, watchdog, Hold's `enabled=false`, tilt precedence); `vastuContent.test.ts` pins registry shape, bilingual fields, the two-domain threshold, draft invisibility (rooms AND door padas), the copy guard, and full declination coverage; `declinationGrid.test.ts` the grid-vs-table oracle; `VastuDishaScreen.test.tsx` the sensor-unavailable manual path + Hold pins; `GharVastuScreens.test.tsx` the frozen group order, five-pills-with-zero, no-n-of-m/percent guards, the sensor-free 3BHK walk and compare guardrails; `homeHandoff.test.ts` the share text incl. its stance sweep; `myhome.test.ts` + the corpus the Ask behaviour. Typecheck, Jest/lint and the three Maestro flows are required before release; live fused-vs-magnetometer agreement, tilt feel, haptic rate and VoiceOver announcements remain a two-phone device gate before the release is cut.
11. **The source ladder never prompts.** `useCompassHeading` climbs fused (expo-location OS heading) → magnetometer → unavailable. The location permission is only ever **queried** — the Panchang flow owns the prompt; on the magnetometer rung the screen may carry the copy-only hint to enable the Panchang location. A fused subscription that never emits must fall down the ladder (watchdog), not hold the dial at `starting`.
12. **Tilt honesty is the magnetometer rung's.** The flat-pose assumption belongs to the raw-sample math alone: >20° from flat sustained 5 samples → `tilted` (outranks `unreliable`); the dial keeps moving and the warning line says why it may be wrong. The fused rung is OS-tilt-compensated and never reports `tilted`.
13. **Five classes, frozen order.** An assessment renders findings grouped exactly forbidden → differs → preferred-unmet → alternate → in-keeping → unmeasured (`FINDING_CLASS_ORDER`), registry order within a group; the class-count strip always shows the five class pills (a zero is information), `unmeasured` joining only when > 0. No surface may re-rank, net, total or percentage them.
14. **The roster is private, NON-cache user data.** `@vedansh:vastu-homes:v1` sits in `derivedCacheReset`'s MUST_SURVIVE list and is never swept; cap 12; a retired registry id is dropped on read, never a crash; an unknown template degrades to `custom` with placements intact. Home data renders on the vastu screens and the `vastu.myhome` Ask answer ONLY — never Home/Today, widgets, notifications, or any share the user did not explicitly invoke. The compare surface reads `considering` homes only and never names a winner; its closing line hands the decision back verbatim.
15. **Pada ring scope.** Door padas belong to the door flow alone (setup facing step + the assessment's facing caption) — never to room placement or the reading mandala. All four `doorPadas.ts` walls ship `draft`; the verified-only accessors make a draft wall indistinguishable from absence (facing-only, NO placeholder), and every flip to `verified` is a reviewed data-only OTA meeting §22.3. A pada renders as its name — the 1–32 index never reaches copy.
16. **Templates are seed lists, never layouts.** A `homeTemplates.ts` entry only pre-populates which room chips exist (verified rooms only, ordinal-expanded); it makes no assumption about shape, size or adjacency, and the engine consumes only `{roomId, zone}` — room size/shape/position-within-a-cell (`at`) is a user sketch that never enters `classifyPlacement`.
17. **The engine judges; a model never does.** `assessHome`/`classifyPlacement` are the only producers of findings, classes and weights. The full-text handoff's framing instructs any downstream reader (human or AI) to stay inside the findings — no additions, no re-weighting, no remedies — and the future F2 vision pre-read may only PROPOSE pin positions for user confirmation, never touch the rule engine or its output.

## 23. Observance day selection (vyapini) — which civil day a rule falls on

A lunar rule names a **tithi**, not a date. The civil day it lands on depends on which instant of that day the tithi must cover — the vyapini convention. `ObservanceRule.dayRule` (`panchang/types.ts`) declares it; the matcher lives in `festivalEngine.ts` and nowhere else.

1. **`udaya` is the default and stays implicit.** A rule with no `dayRule` matches the tithi running at **sunrise** — correct for the large majority (Ekadashi, Purnima, Amavasya, Navratri, Dussehra), and the convention the whole Panchang tab labels days by. Do not add `dayRule: 'udaya'` to a rule; absence *is* the declaration.
2. **`chandrodaya` is for the moonrise vrats.** A vrat whose defining act is the evening moon that ends the fast is fixed at **moonrise**: `sankashti-chaturthi-vrat`, `karwa-chauth`, `bahula-chaturthi` (Bol Choth, concluded after Godhuli puja and moonrise), and `bhadwa-chauth` (the Rajasthani Chauth Mata vrat on the same tithi, concluded with the evening moon arghya). Krishna Chaturthi typically opens mid-morning and closes before the next mid-morning, so sunrise matching named the day AFTER the night the moon is worshipped (Bhadrapada 2026: 1 Sep, whose 9:22 PM moonrise falls in Panchami, instead of 31 Aug's 8:39 PM moonrise inside Chaturthi). Over 2025–2026 moonrise matching agrees with published dates in 23 of 25 lunations against sunrise's 12.
3. **Three cases, in this order, and all three are load-bearing.** (a) This day's moonrise falls inside the target tithi ⇒ this is the day, *unless* yesterday's did too — a tithi spanning two moonrises is kept on the **first** (the same "first of two" convention udaya matching uses for vriddhi; Mar 2025, Jun 2026). (b) Yesterday's moonrise fell inside it ⇒ yesterday owns this lunation. (c) **No** moonrise falls inside it at all (Kartik 2025: Chaturthi ran 9 Oct 10:54 PM → 10 Oct 7:38 PM) ⇒ fall back to the udaya matcher, which is what published almanacs do there too. Dropping (a)'s dedupe emits the vrat twice; dropping (c) deletes a lunation's vrat outright.
4. **Sibling rules on one tithi must share a `dayRule`.** Karwa Chauth *is* its month's Sankashti Chaturthi (both Kartik Krishna Chaturthi), and Bahula Chaturthi is Bhadrapada's; a day apart would put the same tithi's vrats on different nights. `observanceDates.test.ts` asserts both coincidences for 2024–2028.
5. **Moonrise solves are gated, never swept.** `tithiAtMoonrise` (`engine.ts`) takes the expected tithi index as a **required** argument and answers `null` without a rise/set root-find whenever that index is neither the day's sunrise tithi nor its successor — a moonrise can carry no other. A year scan therefore pays ~50 moonrise solves, not 365 per rule. Both it and `computePanchangForDate` read the shared `moonriseCache` memo.
6. **Regenerate the precomputed table in the same commit.** Observance dates ship baked (`scripts/gen-precomputed-observances.mts` → `precomputedObservances.ts`, 2024–2031 × purnimant/amanta); a matcher change that skips `TZ=Asia/Kolkata npx tsx scripts/gen-precomputed-observances.mts` leaves the app on the old dates. Diff the regenerated table by rule id and state in the PR which rules moved and by how much — nothing but the rules you retagged may change.
7. **`madhyahna` is for the midday worship.** Ganesh Chaturthi (sthapana), Ram Navami (janma) and the monthly `vinayaka-chaturthi-vrat` are fixed by the tithi at the sunrise–sunset midpoint, through the same three-case instant matcher as chandrodaya (`matchesInstantVyapiniRuleOnDate` with `tithiAtMadhyahna`). Udaya matching named the day after the worshipped midday whenever the tithi opens shortly after sunrise — Ganesh Chaturthi 2026 resolved to 15 Sep against every published almanac's 14 Sep, Ram Navami 2026 to 27 Mar against 26 Mar. The remaining conventions — pradosh (Diwali, Dhanteras) and nishita (Maha Shivaratri) — are NOT modelled: they still resolve at sunrise and still show the ±1-day shift `VERIFICATION.md` documents and `npm run verify:observances` reports as Class B. Adding one is the same three-part job: a `dayRule` value, its instant solver, and published-date tests across several years — never a per-rule date override.
8. **Tests.** `observanceDates.test.ts` (tsx, in `test:engine`) pins the full 2025 Sankashti series and Karwa Chauth 2024–2026 against **published** dates (never engine output), the reported 31 Aug 2026 case, the one-per-lunation invariant with 28–31-day gaps for 2024–2028, and the live matcher against the shipped table so a stale `precomputedObservances.ts` fails CI.

## 24. Shubh Yoga (PRD-27) — the additive-vocabulary rule-table contract

The शुभ योग tables (`mobile/src/panchang/shubhYoga.ts`; design.md §69, §33, §60) name what is specially right about a day from the shipped panchang primitives. They are **religious content** and carry the same obligations as the §17 occasion tables, plus the annotate-only hard lines below.

1. **§10 applies to every table.** The सर्वार्थ सिद्धि vāra×nakshatra rows, the अमृत सिद्धि pairs, the रवि योग counts and the द्विपुष्कर/त्रिपुष्कर factor sets must each be verified against **≥2 independent authoritative concordant sources** (edition/page or stable URL + retrieval date — a search-index snippet does not qualify), recension variance recorded, a named reviewer. The contract is `docs/roadmap/conventions/shubh-yoga-v1.md`, pinned **row-for-row** by `shubhYoga.test.ts`; until it clears, `SHUBH_YOGA_SOURCE.verified` stays the literal `false` (test-pinned) and **any store build exposing the chips is release-blocked**. These tables are printed in every published panchang — this is the cheapest §10 in the roadmap; do it, don't waive it. A table change requires a new convention id, never a silent edit.
2. **Annotate-only is structural, not editorial.** A yoga must never re-rank, re-tier, exclude, offset, or otherwise move anything the finder produces — `eventMuhurat.ts` must not import `shubhYoga.ts` (direction pinned by test), `verdictForDate` and every rider (share card, reminder scheduler, ★ chip, month overlay, abujh resolve) stay yoga-blind, and the annotation is computed beside the verdict (`shubhYogasForDate`) on the surfaces alone. Tradition does allow certain yogas to lift doshas; adopting that is a **product reversal requiring a new convention id**, because it retroactively changes every ranking the shipped finder has produced.
3. **No score, ever, in either direction.** Present-or-absent with a window — no day-quality number, percentage, "luckiest day", netting of doshas against yogas, or visual hierarchy between yogas (one chip treatment; no filled/"strong" variant). A day carries its dosha exclusion and its yoga chip together when both are true. Round 1 §3 (no standalone fear copy) and round 2 §3.3 (no day score) bound this feature on both sides.
4. **The योग naming collision is a hard rule.** The 27-cycle nitya yoga field renders as **नित्य योग / Nitya Yoga** on every surface — never bare योग — and a shubh yoga always renders under a **शुभ योग** group label carrying its full "… योग" name. Neither system's value may render in the other's slot (one nitya yoga is literally named सिद्धि; the ambiguity is real, reintroducing it is a defect).
5. **Windows are nakshatra-to-nakshatra and honest about midnight.** Evaluation covers the vedic vāra-day [sunrise, next sunrise), cut at every solved anga end via the kshaya-aware `angaAt` (never `index + 1` across a kshaya, never a fresh ephemeris re-match); every rendered end goes through the shipped `formatEndInstant` (12-hour clock + short-date suffix on a different civil day). The printed-panchang extended-hour style (26:12) must not be introduced. End-only rendering is permitted solely for sunrise-start windows (the anga-row convention); a mid-day onset always shows its start.
6. **Purity and the store.** `shubhYoga.ts` stays free of wall-clock reads, randomness, network, storage, and React (source-guard test); the one astronomy import is `getSiderealPlanetLongitude` for रवि योग's Sun nakshatra, floor-divided by the shipped `NAKSHATRA_SPAN` — never a second 13°20′ constant. Surfaces read through the shared `panchangDayStore` (`useShubhYoga` over `useMuhurat`; `shubhYogasForDate` beside `verdictForDate`) — no private cache (§17.6). Nothing in `DayInputs` changes, so this feature owns **no `PANCHANG_DAY_CACHE_VERSION` bump**; a future change that moves the computation into `DayInputs` inherits §17.6 in full.
...
11. **Verification.** `npm run test:ask` (tsx) + `components/__tests__/AskAnswerCard.test.tsx` (Jest) + the three `.maestro/ask-*-smoke.yaml` flows; typecheck and `npm run lint` at 0 errors; design.md §71 updated in the same PR.

## 26. Visarjan vidhi family + festival arcs (PRD-28) — the concluding-rite content contract

A visarjan vidhi is the **concluding half** of a rite the app already knows how to open (`ganesh-chaturthi-sthapana`, `navratri-ghatasthapana`). It is sourced liturgical **procedure** and carries §19 in full (§11 applies), plus the rules below — which follow §19's shape and add the one thing §19 lacked: a verification boundary the code enforces.

1. **Data shape.** A `VidhiEntry` (§19.1) with **`status: 'draft' | 'verified'`** set explicitly (§19's seven predate the field and are verified by omission). `festivalIds` names the arc's **customary** visarjan rule (`anant-chaturdashi`, `dussehra`) — the door the arc strip opens on the family's *own* visarjan day is `ArcDefinition.visarjanVidhiId` in `panchang/arcs.ts`, so a 5-day Ganesh family reaches the same guide on a day that is no rule at all. Current members: `ganesh-visarjan-uttar-puja`, `durga-visarjan`.
2. **Verified or invisible — enforced in code, not by review discipline.** `VIDHI_ENTRIES`, `VIDHI_BY_ID`, `getVidhiById` and `getVidhiForFestival` (`data/vidhi/index.ts`) expose verified entries only; `ALL_VIDHI_ENTRIES` / `VIDHI_DRAFTS` exist for review and tests. A draft is therefore indistinguishable from no entry on the catalog, the §36 search rows, the §33 day-panel pill, the Observance Detail card and the §65.2 arc door, and it contributes nothing to `VIDHI_ENTRIES.length`. Screens render no draft/review/"coming soon" placeholder. New content-bearing vidhi families enter the repo as `'draft'`.
3. **The flip is a reviewed content change.** `status: 'verified'` requires: the canonical edition opened and the instruction-only sequence collated (`canonicalEditionStatus` records what was checked and when), **≥ 2 independent published `referenceUrls` actually opened** and concordant on the sequence (§11.1 — a candidate URL never counts), regional scope in `notes`/a variant note, a named reviewer and a dated note. A draft's `canonicalEditionStatus` must begin `DRAFT — NOT VERIFIED` and state exactly what is outstanding. Green tests never authorize the flip.
4. **Mantras are transcribed or absent (§11.3, §19.2).** The visarjan family ships **instruction-only** until a formula is verbatim-verified: the customary *punarāgamanāya ca* utthapana formula, the uttarapuja upachara formulas and any regional visarjan liturgy are described as acts, never quoted or paraphrased. Aartis hand off to the shipped sections by `ref` (§11.11) — never re-typed.
5. **Scope stays narrow and named.** A visarjan guide states the **festival-murti vs permanent-murti** boundary in both languages (test-pinned), names its regional form in `conventionLine*`, and does not absorb rites it did not source (Aparajita/Shami puja, Bengali dhunuchi/sindoor khela, Marathi/Konkani regional sequences). Home immersion and returning clay to a plant are recorded as practice, not rule.
6. **The arc relation is additive data.** `ObservanceRule.arcId` / `arcRole: 'sthapana' | 'day' | 'visarjan'` / `arcOrdinal` are present together or absent together and never change a rule's tithi, month, paksha or `dayRule` (`arcs.test.ts` pins the eight members' fields). The arc's shape (roster, `durationChoices`, gap labels, `prepare`, `visarjanVidhiId`) lives in `ARC_DEFINITIONS`; `festivals.ts` must not import `arcs.ts` (direction pinned). Adding an arc means: set the three fields on **existing** rules, add one definition, extend the test pins — never a new rule to make a strip look complete (Diwali's day 2 is a gap label, not a fabricated observance).
7. **Duration is the family's; the app records and computes.** The offered set is `ARC_DURATION_CHOICES` (1½/3/5/7/10); there is **no default** and no recommendation copy on any surface. The choice is occurrence-scoped user state at `@vedansh/parv-arc` (`arcChoiceStore.ts` — the §19 checklist-store shape, keyed per arc per sthapana date); a stale or malformed record parses to "no choice". No choice ⇒ the arc degrades to independent days, no visarjan slot, **no reminder, no re-ask**. Ordinals and totals come from resolved dates, never the customary count.
8. **Reminders ride a shipped family.** A chosen visarjan schedules through the vrat-reminder family (`visarjanReminderInputs` → `VratReminderScheduler`), under `VRAT_REMINDER_CAP`, gated on OS permission already granted — the scheduler never prompts, and the family's identifier prefix and deep link are unchanged. No new notification family, channel or cap.
9. **Purity.** `panchang/arcs.ts` reads no clock, storage, React or expo (`today` is a parameter; guard test); it consumes the shipped festival engine like every other occurrence surface and owns no cache.
10. **Tests.** `arcs.test.ts` (tsx — relation, solver, occurrences, `arcDayFor`, preparation window, reminder inputs, purity/direction guards), `vidhiContent.test.ts` (tsx — drafts meet the full §19 contract, are hidden by every accessor, carry no mantra, state the outstanding review and the murti boundary), `vratReminderPure.test.ts` (title override), `arcChoiceStore.jest.test.ts`, `ArcStrip.test.tsx`, `ObservanceDetailScreen.test.tsx` (Jest); `.maestro/parv-arc-smoke.yaml` (e2e — date-independent surfaces; the date-dependent ones are unit-pinned).
