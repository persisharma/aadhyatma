...
2. **Convention is declared, not implied.** The finder follows **DrikPanchang** (like the rest of the Panchang subsystem): factor model = nakshatra + vara + tithi + masa shuddhi; asta bar per Muhurta Chintamani/Dharmasindhu. The results/day-detail UI names the convention on the surface (`· दृक्पंचांग पद्धति`). Changing convention is a product decision, not a table edit.
3. **Consume the engines; never re-match.** Chaturmas derives from the sunrise anga (kshaya-safe, purnimant-normalised); abujh days resolve through `festivalEngine` (`ABUJH_RULE_IDS`) — re-implementing tithi matching re-introduces the kshaya/vriddhi bugs the engine already fixed. Asta uses `getSiderealPlanetLongitude`; orbs are 10° (Shukra) / 11° (Guru), flat.
...
6. **One shared, versioned day-store — no per-surface panchang caches.** The per-day solve is cached in `panchangDayStore` per (`locationKey`, calendar system), keyed by absolute civil date, persisted by `panchangDayCache`, and read by EVERY panchang surface: the finder scans, the abujh calendar, the day detail, Home's Today strip, the daily Muhurat card, and the Panchang tab (design.md §60). Four obligations follow, and all of them are release-blocking:
...
4. **Corpus gate.** Every production record has a stable unique id, Devanagari name, pronunciation aid, gender class, charana ids, attested Hindi and English meaning, syllable count, and only resolving deity ids. Its Devanagari initial must mechanically match a primary or explicitly accepted alternate syllable. Every non-thin charana requires at least six boy and six girl names; a thin charana uses a declared nakshatra-level fallback and never an invented filler. Generated indexes must be reproducible, and all raw Namkaran JSON together stays ≤512 KB unless a separately reviewed sharding decision changes the budget.
...
9. **Published v1 registry.** Six entries are required: Satyanarayan, Diwali Lakshmi-Ganesha, Ganesh Chaturthi Sthapana, Navratri Ghatasthapana, Karwa Chauth, and Maha Shivaratri. `vidhiContent.test.ts` pins the exact registry and every festival hook. Search-index rows (a Vidhi is not a `LibraryEntry`), the ObservanceDetail "How to observe" slot, keep-awake, the DISCOVER card, and `AddToRoutineButton` remain deferred follow-ups.
