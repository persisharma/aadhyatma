...
1. **Declare its routes once**, in a shared param list (`VidhiStackParamList` in `navigation/types.ts`), and **intersect** it into each hosting stack's param list — `export type HomeStackParamList = VidhiStackParamList & { … }`. `tsc` then keeps the two registrations from drifting; a param list that re-declares the routes inline does not.
2. **Register the same components in both navigators**, and type the screens against the shared list (`NativeStackScreenProps<VidhiStackParamList, …>`), never against one stack — one component serves both mountings.
3. **Every door pushes in place.** No `navigate('<Other>Tab', …)` from a door on a tab that hosts the flow.
4. **A genuine cross-stack hop stays runtime-checked.** Where a flow hands off to a screen that legitimately lives on one stack only (the vidhi conduct → shipped readers), use `navigateToHomeStackTarget` (`navigation/entryRoutes.ts`): it pushes in place when the enclosing stack registers the target route and falls back to the tab hop when it does not — so the same component behaves correctly under either mounting.
...
1. **Data shape.** `VidhiEntry` (`data/vidhi/types.ts`): `id`, bilingual title, `festivalIds[]` (observance rule ids), `deities[]` (the shared `Deity` union — §11.4), bilingual `conventionLine*`, `durationHintMin`, `samagri[{itemHi/En, qty?, optional?}]`, `steps[{id, phase 'prep'|'main'|'closing', titleHi/En, instructionHi/En, mantra?, ref?}]`, `source`. Registry: `VIDHI_ENTRIES` / `VIDHI_BY_ID` / `getVidhiForFestival` in `data/vidhi/index.ts`. Each Vidhi remains an independently sourced and reviewable content module even when several are delivered together.
...
3. **Reuse by reference, never by duplication (§11.11).** Steps that are a shipped text (katha, aarti, sahasranama…) carry `ref {kind: 'katha'|'section', id}` and render a hand-off card into the existing reader — the text is never re-typed into the vidhi. Refs must resolve: `katha` ids into `KATHA_CONTENT_BY_ID`, `section` ids into an active `library` entry.
...
6. **Festival hook.** Observance rules gain an optional `vidhiId` (`panchang/types.ts`) — the identical mechanism as `kathaId`. The day panel's ObservanceCard renders the `॥ पूजा विधि` pill only when the id resolves in `VIDHI_BY_ID`; `vidhiContent.test.ts` pins the round-trip in both directions (every `rule.vidhiId` resolves; every `vidhi.festivalIds` entry is a real rule).
...
9. **Published v1 registry.** Six entries are required: Satyanarayan, Diwali Lakshmi-Ganesha, Ganesh Chaturthi Sthapana, Navratri Ghatasthapana, Karwa Chauth, and Maha Shivaratri. `vidhiContent.test.ts` pins the exact registry and every festival hook.
