...
4. App boots in Expo dev client; the new card is visible on the **पाठ library** (Home → पाठ, design.md §72 — not Home itself since TRD-42) below the existing active sections; tapping navigates to a working reader; every page shows a background image; every verse has `meaningHi` and `meaningEn` populated.
5. The new section appears correctly under its category tile (Home → पाठ → tap the tile → item is listed), and the tile's derived count went up by one. If deity tags are set, also verify the item shows under those deity chips (the Library's देवता rail, or its `All N deities` overflow).
...
11. **Per-section device check.** If the PR adds N sections, every one of them must be opened in the Expo dev client individually — Home → पाठ → tile → reader → toggle language → swipe to last verse → back. A single "tested locally" sign-off does not cover N sections; capture one screenshot per section and paste them in the PR description.
12. **Both listings reach the reader.** Open the section from Home → पाठ → its category tile **and** from Home → पाठ → the देवता rail → its deity card. Both paths must land on the same reader. If the section appears as a card but tapping is a no-op, the routing helper (`entryRoutes.ts`) is missing a case.
...
14. **Section is reachable from search.** `mobile/src/data/__tests__/searchIndex.test.ts` already enforces that every active `library` entry produces verse entries in the search index — but verify manually: open the global search (Home's floating ⌕, or the Library's search row), type a unique word from the section's first verse, confirm the result row tap lands on the correct reader page. See §7 for the per-shape integration paths.
...
New sections are registered in `mobile/src/navigation/HomeStackNavigator.tsx` (not the old `RootNavigator.tsx`). The **Library screen** (`LibraryScreen.tsx`, pushed from Home's पाठ door since TRD-42) dynamically renders categories from `mobile/src/data/categories.ts`, deities from `deities.ts` and purposes from `purposes.ts` — adding a new section only requires:
...
No manual routing in either screen is needed — `CategoryListScreen` filters items by their `category` field automatically, and `data/libraryCounts.ts` derives the tile count.

**Home's own rows are a different, deliberately closed registry.** `data/home/tools.ts` (the eight उपकरण tiles) is static, ordered and hand-curated — it is *not* driven by `categories.ts` and must not be made to be. A new content category surfaces on the Library, never on Home. Adding a ninth tool is a product decision with a doc update (design.md §18.7), not a side effect of shipping content.
...
1. App boots; the Theerth tile is visible on the पाठ library under पाठ के प्रकार, and Home keeps its तीर्थ उपकरण tile (design.md §18.7/§72).
...
- A Maestro flow should cover Home → पाठ → a उद्देश्य chip → purpose list → text → reader before merge (`discovery-purpose-smoke.yaml`); if it is not run, the PR must state that explicitly.
...
- Home keeps a permanent Kundali launcher — the third **उपकरण** tile (design.md §18.7); Panchang keeps Jyotish as a peer of Panchang and Vrat–Parv. Do not bury Kundali only in a carousel, More, or a second-level catalog. (There is no carousel to bury it in since TRD-42, and the tools row carries no NEW badge — the one it used to wear was hardcoded and could never clear.)
...
- Discovery surfaces: the `होम-स्क्रीन विजेट` More row (one-release NEW), the in-app **Widget Gallery** (`WidgetGalleryScreen.tsx`, previews from the same validated payload the native consumers read + accurate platform add-instructions — no promise of a system widget-picker jump), and a Home **नया** card (design.md §73 — per-user, gone on first open; it was a shuffled DISCOVER spotlight until TRD-42). No new permission prompt is introduced.
