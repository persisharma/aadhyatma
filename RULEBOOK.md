...
6. **One shared, versioned day-store — no per-surface panchang caches.** The per-day solve is cached in `panchangDayStore` per (`locationKey`, calendar system), keyed by absolute civil date, persisted by `panchangDayCache`, and read by EVERY panchang surface: the finder scans, the abujh calendar, the day detail, Home's Today strip, the daily Muhurat card, and the Panchang tab (design.md §60). Three obligations follow, and all three are release-blocking:
   - **Version it.** Any change to the panchang engine, to `DayInputs`, or to the asta flags MUST bump `PANCHANG_DAY_CACHE_VERSION` in `panchangDaySerde.ts` — otherwise already-scanned devices keep serving the old engine's days and the fix reaches only fresh installs (the same failure mode as `observanceCache`'s `CACHE_VERSION`). Adding a city needs no bump.
...
8. **Tests.** `eventMuhurat.engine.test.ts` (tsx) pins the Chaturmas boundaries (incl. the 2026 kshaya Dev Uthani), amanta invariance, the validated asta windows, the 26 Nov 2026 Griha Pravesh golden, the zero-result 90-day scan, and the draft flag. Cache correctness is pinned by `dayCacheParity.e2e.test.ts` (fresh == cached == serialize→revive over a year × 3 locations × 2 systems) and `panchangDayStore.test.ts`; storage by `__tests__/jest/panchangDayCache.jest.test.ts`; the no-mutation invariant by `panchangDayImmutability.test.ts`; and the daily surfaces' zero-solve routing by `__tests__/jest/panchangDayRouting.jest.test.ts`. Screen smokes in `MuhuratFinderScreens.test.tsx`; e2e in `.maestro/muhurat-finder-smoke.yaml` and `.maestro/panchang-day-cache-smoke.yaml`.

## 18. Puja Vidhi (PRD-19) — guided-puja content contract
...
4. **Convention is declared in review data, not implied.** Every Vidhi carries a `conventionLine*` naming the tradition it follows. It remains internal review metadata and does not render on catalog, detail, conduct, or completion surfaces (design.md §61.2). Instructions are authored fresh in both languages, 1–2 lines, *what + why* (§9).
...
7. **Completion is quiet.** The conduct screen's completion page is a static ॐ seal — the routine celebration animation is deliberately not wired (design.md §61.3), and wiring it is a product decision, not a polish task.
