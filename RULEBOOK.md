...
## 23a. Adding an observance to the catalog — the regional-coverage contract

§23 says which civil day a rule lands on. This section says what it takes to put a rule in `festivals.ts` at all. It was written after the Sept 2026 audit found Goga Navami, Teja Dashami, Chaiti Chhath and Kartik Purnima absent, and five Rajasthani observances shipping a full katha with **no tithi on the rule**.

1. **A katha, a bhog or an upvas entry is not an observance.** `hidden()` / `ruleType: 'catalog-only'` produces a rule the engine can never resolve — `resolveRuleDates` returns `[]` for it — so it is searchable and readable and appears on **no day**. Attaching content to such a rule looks like shipping the festival and is not. If the observance has a tithi, give the rule its `lunarMonth` / `paksha` / `tithi`; `catalog-only` is for things that genuinely have no single date (`chaturmasa`, the weekday-fast families).
2. **Two independent published sources, and the second one lives in a comment.** §11.1 applies in full. `ObservanceRule.sourceUrl` holds ONE URL and is never rendered, so the second reading is recorded in the comment directly above the rule, together with the **published civil date** used to pin it. `observances.test.ts` keeps a closed allowlist of source hosts — Drik for the pan-Hindu calendar, the state tourism/culture portals for folk-deity mela days Drik does not index. Adding a host is a review decision, not a drive-by.
3. **Pin published dates, never engine output.** Every new rule gets an entry in `observanceDates.test.ts` (`REGIONAL_PUBLISHED`) for at least one year whose date you actually read in a source, plus the once-a-year invariant across 2024–2031. Add it to `scripts/verify-observances.mts` `ANNUAL` too, with its **real** muhurta — including when that muhurta is one the engine does not model, so the shift is reported on every run instead of forgotten (`bachh-baras` is the worked example).
4. **A rule that shares a tithi with a shipped rule must be tested against it.** Sakat Chauth *is* Magha's Sankashti, Chitragupta Puja *is* the Bhai Dooj day, Madhushravani closes on the Hariyali Teej day, Kartik Purnima is a Purnima. Assert the coincidence (§23.4) — a silent one-day divergence puts one household observance on two nights.
5. **`visibility: 'regional'` is retired; calendar scope is `lens[]`.** No `lens` means universal and always present. A non-empty lens list is additive OR metadata: passive presentation includes the rule when any named calendar is active, while direct search/detail, `getNextOccurrence(s)`, ★ follows/reminders and explicit arc choices stay lens-independent. Never retag a universal rule merely because one region values it; use `shortDescription*` + `searchTerms` for regional identity and reserve lenses for calendar depth that is genuinely noise elsewhere.
6. **A regional name for a day that already ships is an alias, not a rule.** Bhadrapada Shukla Ekadashi is जलझूलनी / देव झूलनी ग्यारस in Rajasthan and Parivartini Ekadashi in the pan-Hindu calendar — one tithi, one rule, extra `searchTerms` (`EKADASHI_EXTRA_SEARCH_TERMS`, `SANKRANTI_ALIASES`). Duplicating the rule duplicates the day on the calendar.
7. **Every `vrat`/`upavas` rule needs verified food guidance (§21).** `bhogContent.test.ts` counts them; a new vrat with no honest profile either joins a shared one whose tradition note is extended to name it, or the observance is not a `vrat`. Adding a rule to a shared profile means adding its id to that profile's `observanceIds` *and* the `bhogId` hook to the rule — the test checks both directions.
8. **Regenerate and bump, both** — `TZ=Asia/Kolkata npx tsx scripts/gen-precomputed-observances.mts` (§23.6) **and** `CACHE_VERSION` in `observanceCache.ts`. A catalog addition needs the bump even though no matcher changed: a device that already scanned its city hydrates the old year and never sees the new rules. Diff the regenerated table by rule id and state in the PR that only the new ids were added and **no existing date moved**.
9. **The default-on festive-reminder catalog is a separate, narrower decision.** `festiveReminders.ts` ships enabled for everyone, so its curation rule is "famous enough that a typical user is already observing it". A state or community festival does not qualify; its path is the ★ follow → vrat reminders, which a dated rule enables for free.
10. **Docs in the same PR** — design.md §33's regional-coverage block (the catalog counts and the wave list) and `VERIFICATION.md` when the rule carries a known muhurta shift.
11. **Raw tables, filtered reads.** Generation and per-location persistence scan all date-bearing rules and never store a filtered table. Panchang day/month/upcoming, catalog counts/lists, Home Today, generic Ask and widgets filter only after the raw read, and presentation memo keys include the canonical sorted lens set. A preference toggle must never trigger astronomy.
12. **No passive notification leakage.** Both the daily-verse title picker and its resolver independently reject every rule carrying `lens`, regardless of active preferences. `festive-reminder` remains a separate unchanged allowlist; only an explicit ★ follow may schedule a lensed observance.
