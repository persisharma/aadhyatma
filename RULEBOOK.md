...
**Because asks are uncapped, "Don't ask again" is load-bearing.** It is the only permanent escape a user has: `outcome: 'declined'` is the sole state that stops an otherwise indefinite 5-day cadence. It must stay a **visible, first-class button** on the sheet — never demoted to a long-press, a swipe-to-dismiss, an overflow menu, or a Settings-only toggle, and never removed "to simplify the card". A future variant that drops the third button (the inline designs in `docs/rating-ask-prototype.html` propose exactly that) must either restore a lifetime ceiling or keep an equivalent permanent opt-out; shipping neither turns the feature into unbounded nagging. The engagement floor gates who enters the cadence, not how long it runs, so it is not a substitute.
