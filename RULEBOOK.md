...
1. **Educate-first is structural.** `DaanPunyaScreen` renders no external-linking affordance and no directory door; `DaanDirectoryScreen` is reachable only from `DaanJourneyScreen`'s terminal step, whose record action always precedes the hand-off. Pinned by `DaanScreens.test.tsx` — a change that adds a give affordance to the home or a second directory door is a contract break, not a tweak.
2. **The app never touches money.** No payment collection, processing, confirmation, commission, suggested amounts, targets or progress bars — the hand-off is `Linking.openURL` to the organization's own official channel behind the honest interstitial, and the return-offer renders at most once. No UPI VPAs anywhere (web-only hand-off; open decision #4).
...
11. **Tests.** `daanContent.test.ts` pins registry shape, bilingual parity, the source threshold, draft invisibility, live-solver rule-id integrity, exact-beats-suffix matching, directory hygiene + staleness, the cause-mahatva invariants of §27.14, the gupt guarantee, CSV escaping, and the copy guard; `DaanLedgerContext.test.tsx` pins the versioned payload and the write-side gupt strip; `DaanScreens.test.tsx` pins the §2.7 surface contract, the journey's terminal gating, the द्वार's cause-mahatva rendering, and the detail screen's thinness (no paperwork, no verification badge, the hand-off opening `officialUrl` only). Typecheck, Jest/lint and the Maestro flow are required before release.
