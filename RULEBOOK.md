...
4. **Convention is declared, not implied.** Every vidhi carries a `conventionLine*` naming the tradition it follows, rendered under the detail-screen title (design.md §61.2) — never an anonymous procedure. Instructions are authored fresh in both languages, 1–2 lines, *what + why* (§9).
...
9. **Deferred, recorded:** search-index rows (a vidhi is not a `LibraryEntry`, so §7's coverage gate does not fire; indexing needs a new entry kind), the ObservanceDetail "How to observe" slot, keep-awake in conduct mode (`expo-keep-awake` is not a dependency), the DISCOVER card, and `AddToRoutineButton` on recurring vidhis — all Phase-2 items in the PRD's phasing.
