...
| 7 | `deities` | **yes** | `['hanuman', 'rama']` | Array of deity tags from: `rama`, `krishna`, `vishnu`, `shiva`, `hanuman`, `durga`, `ganesha`, `savitr` (Maa Gayatri), `saraswati`, `lakshmi` (Maa Lakshmi), `surya` (Surya Dev) — PRD-A deity expansion §A.4.2, 9 → 21, ongoing (source of truth: `mobile/src/data/deities.ts`). The section appears under each tagged deity in the Deity Index. At least one required. |
