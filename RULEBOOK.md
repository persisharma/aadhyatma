...
| 7 | `deities` | **yes** | `['hanuman', 'rama']` | Array of deity tags from the `Deity` union in `mobile/src/data/texts.ts` and display metadata in `deities.ts`: `rama`, `krishna`, `vishnu`, `shiva`, `hanuman`, `durga`, `ganesha`, `savitr` (Maa Gayatri), `saraswati`, `lakshmi`, `surya`, `radha`, `kartikeya`, `kubera`, `ganga`, `parvati`, `narasimha`, `dattatreya`, `shani`, `kali`, `navagraha`. The section appears under each tagged deity in the Deity Index/Detail surfaces. At least one required. |
