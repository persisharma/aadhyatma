...
5. **Purity.** `eventMuhurat.ts`/`abujhMuhurat.ts` stay free of wall-clock reads, randomness, network, storage, and React (source-guard test). All clock/scan behaviour lives in `useMuhuratFinder`. The shared day store (`muhuratDayStore.ts`), its serde (`muhuratDaySerde.ts`) and the scan core (`muhuratFinderScan.ts`) stay RN-free for the same reason — AsyncStorage lives only in `muhuratDayCache.ts`, so the tsx engine suite can import the rest.
6. **Persisted days are versioned.** The per-day panchang solve is cached on disk per (location, calendar system) and hydrated on entry (design.md §60). **Any change to the panchang engine, to `DayInputs`, or to the asta flags MUST bump `MUHURAT_DAY_CACHE_VERSION` in `muhuratDaySerde.ts`** — otherwise already-scanned devices keep serving the old engine's days and the fix reaches only fresh installs (the same failure mode as `observanceCache`'s `CACHE_VERSION`). Adding a city needs no bump. The cache may only be keyed by the canonical `locationKey`, and no consumer may mutate a returned `PanchangData` — the store hands out shared instances.
...
8. **Tests.** `eventMuhurat.engine.test.ts` (tsx) pins the Chaturmas boundaries (incl. the 2026 kshaya Dev Uthani), amanta invariance, the validated asta windows, the 26 Nov 2026 Griha Pravesh golden, the zero-result 90-day scan, and the draft flag. Cache correctness is pinned by `dayCacheParity.e2e.test.ts` (fresh == cached == serialize→revive over a year × 3 locations × 2 systems) and `muhuratDayStore.test.ts`; storage by `__tests__/jest/muhuratDayCache.jest.test.ts`. Screen smokes in `MuhuratFinderScreens.test.tsx`; e2e in `.maestro/muhurat-finder-smoke.yaml` and `.maestro/muhurat-day-cache-smoke.yaml`.
