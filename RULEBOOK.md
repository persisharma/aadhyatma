...
7. **Other non-sunrise conventions are NOT modelled** — madhyahna (Ganesh Chaturthi, Ram Navami), pradosh (Diwali, Dhanteras) and nishita (Maha Shivaratri) still resolve at sunrise and still show the ±1-day shift `VERIFICATION.md` documents and `npm run verify:observances` reports as Class B. Adding one is the same three-part job as this: a `dayRule` value, its matcher case, and published-date tests across several years — never a per-rule date override.
