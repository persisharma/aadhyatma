...
5. **`visibility: 'regional'` means invisible, so do not reach for it as a label.** `getObservanceCatalog()` returns `default` only and nothing opts back in. An observance a whole state or community keeps ships **`default`**, carrying its identity in `shortDescriptionHi/En` (name the state, the local name, the mela) and in `searchTerms` (every spelling and regional alias). Reserve `regional` for entries genuinely waiting on the opt-in surface (`docs/roadmap/prds/42-regional-parv.md`), and never let a `regional` rule be the only home of a real date.
