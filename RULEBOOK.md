...
- **Tokens, not literals.** Every colour, spacing, radius, font family, **and font size** must come from `mobile/src/theme/{colors,spacing,typography}.ts` via the `useTheme()` hook. No hex codes in component files. No hardcoded `fontFamily`. No hardcoded `fontSize`/`lineHeight` on reading-content text. (`design.md` §13)
...
1. **Version bump discipline.** `APP_TOUR_VERSION` in `data/tour/whatsNew.ts` **must equal** `app.json` `expo.version`. On every version bump: update `APP_TOUR_VERSION` and add a `whatsNew[version]` entry listing **only** that release's new features (bilingual `titleHi/En` + `bodyHi/En`); omit the entry (or leave `items` empty) to intentionally skip the sheet. `getWhatsNewForVersion` returns null for unknown/empty versions → sheet suppressed. The `src/data/__tests__/tourContent.jest.test.ts` gate fails a bump that forgets the entry.
