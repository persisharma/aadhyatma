...
2. **`chandrodaya` is for the moonrise vrats.** A vrat whose defining act is the evening moon that ends the fast is fixed at **moonrise**: `sankashti-chaturthi-vrat`, `karwa-chauth`, and `bahula-chaturthi` (Bol Choth, concluded after Godhuli puja and moonrise). Krishna Chaturthi typically opens mid-morning and closes before the next mid-morning, so sunrise matching named the day AFTER the night the moon is worshipped (Bhadrapada 2026: 1 Sep, whose 9:22 PM moonrise falls in Panchami, instead of 31 Aug's 8:39 PM moonrise inside Chaturthi). Over 2025–2026 moonrise matching agrees with published dates in 23 of 25 lunations against sunrise's 12.
