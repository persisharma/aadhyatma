...
- [[backup]] — Backup & Restore (PRD-06 Track C): the user-authored key registry + mechanical completeness gate, the `vedansh-backup` JSON envelope (parse failures named, unknown keys dropped), share-sheet export / document-picker restore with confirm + reload, and why OS-mirror bookkeeping never travels.
