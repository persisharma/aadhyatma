...
- Vertical space is the scarce resource on a widget — a card is about four text lines. The `ॐ वेदांश़` mark used to spend one of them on itself; it now rides the right end of each surface's eyebrow row (iOS `eyebrowRow {}`, Android a horizontal row with `@id/widget_brand`) for free, since both are 10 pt. Anything else tempted to claim a whole line should pair with an existing row the same way. The Android recovery card hides the mark, so **every content render must set `widget_brand` back to `VISIBLE`** — a reapplied `RemoteViews` keeps whatever the last one set (the same trap as recovery's `maxLines` reset).
