...
**Festive reminders** (`festiveReminders.ts` catalog + `festiveReminderPure.ts` planner + `festiveScheduler.ts` glue, armed by `<FestiveReminderScheduler>`) — the **default-ON** counterpart to the opt-in vrat family: the user follows nothing and configures nothing.
...
- `festive-reminder` → **the reading the message named**, via `buildEntryStartTarget(entry)` on the payload's `sourceId`; unknown ids fall back to `PanchangTab → ObservanceDetail`.
...
- **A festive tap opens a reader on purpose**, unlike `daily-verse` (which stays on the Daily Bhakti tab so a glance can't clobber the resume position). The body is an explicit invitation to read a named text, so moving reading progress is the normal consequence of a user-initiated "open this section" tap.
