...
**Hast Rekha (palm reading)** (`panchang/hastRekha.ts` + `useHastRekha.ts` + `screens/HastRekhaScreen.tsx`, design.md §55): the third Jyotish tool. A guided self-observation — no camera or image analysis — of the four classical lines (heart/head/life/fate), each picked from three observable forms (3⁴ = 81 profiles). The engine is pure and deterministic like `computeRashifal`: tradition-attributed reflection copy only, no predictions, and the life line is always framed as vitality, never lifespan (`hastRekha.engine.test.ts` guards the framing, the `navagraha-stotram` practice allow-list, and source purity). `useHastRekha` mirrors `useKundali` (`@vedansh:hast-rekha-profile:v1`, loading/guest/saved/error, corrupt storage → recovery not silence); the screen auto-persists once all four lines are chosen and hydrates the pickers on return. Jyotish landing carries its `JyotishToolCard` in guest, saved ("More tools"), and error branches — it needs no birth details, so it stays reachable while a Kundali profile is broken. Route `HastRekha` in the Panchang stack. No share card in v1 (deliberate; design.md §55).
