...
│   ├── milestone-stores-portrait.png  # ↑ its visual, 4:5 — THIS is the one to post
│   └── milestone-stores.png           # same card, 2.27:1 landscape (desktop-only; thin strip on mobile)
└── README.md
```
...
```bash
python3 make-store-card.py <play-screenshot.png> <ios-screenshot.png>   # → posts/milestone-stores{,-portrait}.png
```
Post the **4:5 portrait** one. 4:5 is the tallest ratio LinkedIn shows uncropped and it wins the most
feed height on mobile; the landscape variant shrinks to an unreadable strip on a phone.
