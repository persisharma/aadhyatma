...
**What a calendar brings — a lens and a tag, one answer.** `ObservanceRule.lens` decides **who sees** a rule (additive, gates visibility); the region-tag table `panchang/regionTags.ts` (keyed by rule id, reached through a `require()` thunk because `festivals.ts` is on the launch graph) says **whose** a universal rule is (a highlight, gates nothing — Gangaur is `['rajasthan','bundelkhand-malwa']` and stays on everyone's calendar). "What this calendar brings" everywhere below is the union (`vratCatalog.getRulesForLens`): the hidden rules a lens adds plus the universal rules tagged as its own. Only calendars that bring ≥ 1 rule are **offered** (`getLensesWithContent`, today all 22; `lens.test.ts` pins it) — a switch that changes nothing is never shown, and if the offered count ever reached zero the ledger column, the quiet-day row and the More row would not render at all. `useLenses().lenses` is the stored set narrowed to offered ids; `availableCount` is the offered count; city seeding and सभी चुनें use offered ids only.

**Structure — one sheet, four doors, and the marks.** The sheet is identical from every entry; only the doorway moves.

| Surface | Where | What it says |
|---|---|---|
| **The sheet** (`LensPickerSheet.tsx`) | Bottom sheet, `parchmentHighlight`, 22 pt top radii, grabber | Title `क्षेत्रीय पंचांग`, then the promise line at 12.5/19 `inkSoft` — "its festivals get marked as yours, and days are only ever added, never removed" — then `n सक्रिय · m और उपलब्ध` at 11 pt `inkMuted` (`सभी n सक्रिय` once every offered calendar is on). Under the count, **two pill buttons** — `सभी चुनें` and `सभी हटाएँ` — 36 pt tall, 1 pt `saffron` outline, 12.5 pt `saffronDeep` label, centred with a 10 pt gap; **each hides when it would be a no-op**, so the header never offers a dead button. Rows are the **offered** calendars, grouped उत्तर · पूर्व · पश्चिम · दक्षिण · परंपरा under 10 pt/1 pt-tracked `inkMuted` eyebrows (an empty group is not rendered). Each row: name at 15 pt `ink`, its two or three recognisable observances at 11 pt `inkMuted` (one line, ellipsised), then a **third line counting what it brings** — `14 पर्व: गणगौर · गोगा नवमी · तेजा दशमी +11` at 11 pt `saffronDeep`, two lines max, first three names then `+n` — and a 22 pt circular check filled `saffron` when on. **Checkboxes, not radios** — lenses compose — and a tap applies immediately with no confirm step, because the calendar behind the sheet answers it. 52 pt row floor, ≥ 44 pt tap target. |
| **The ledger column** (`PanchangScreen.tsx`, व्रत-पर्व) | Third column of the §33 ledger bar, after मेरा व्रत and पितृ स्मरण | A `CatalogLedgerCell` inside the shared `goldTint`-on-`gold` bar: ❖ glyph, `क्षेत्र` at 14 pt, and one 11 pt line of state that still **names a calendar** — `राजस्थान +1` (the first active calendar plus a count of the rest), `सभी n` when every offered calendar is on, or `n उपलब्ध` with nothing on. The column holds ~106 pt, so the names are truncated to one plus `+n` rather than turned into a bare count: a count does not tell a user where their extra dates came from, and the leading name still does. Renders even with nothing on — for a user whose city seeded nothing it is the only path to the sheet — but not at all when the offered count is zero. |
| **आपके पंचांग से** (`PanchangScreen.tsx`, व्रत-पर्व, `LensAdditionsSection`) | Card directly under the ledger bar, **only with ≥ 1 lens on** | The pointer to what the active calendars actually bring. `parchmentSoft` on `divider`, `radii.md`, `elevation.card`, 14 pt gutters. Head: `आपके पंचांग से` at 14 pt `ink` over an 11 pt `inkMuted` count (`23 पर्व जो आपके क्षेत्र या परंपरा के हैं`), with a `बदलें` text button (12 pt Inter SemiBold `saffronDeep`) that opens the sheet. Then **one group per active calendar in registry order**: the sheet's 10 pt uppercase eyebrow with the group count (`राजस्थान · 14`), the **four soonest** rules as `resultRow`s (15 pt `ink` name, 13 pt `inkMuted` other-script caption, the next date as an 11 pt `saffronDeep` uppercase short date, `›`) opening `ObservanceDetail`, and — when there are more — a right-aligned `सभी 14 देखें ›` (12 pt Inter SemiBold `saffronDeep`, 40 pt row) into `ObservanceList { region }`. Data: `getLensAdditions` + `getNextOccurrence`. |
| **The per-calendar list** (`ObservanceListScreen.tsx`, `ObservanceList { region }`) | Pushed from `सभी n देखें` | The same screen as the type lists (§33): title `राजस्थान पंचांग` / `Rajasthan calendar` over the other-script caption and count, the in-list search, the soonest-first rows with next date and the ★ follow. Rows are `getRulesForLens(region)`. |
| **The day-view chip** (`PanchangScreen.tsx`, `ObservanceCard`) | Beside the व्रत/पर्व category pill on a day card | `❖ राजस्थान` (`+n` past one) on `goldTint`, `radii.pill`, 10 pt `saffronDeep` — **only** when the rule belongs to one of the user's ACTIVE calendars (`activeCalendarsOf`). A plain universal day, or a user with no lens on, gets no chip: the card is byte-identical to what shipped. The upcoming cards on the व्रत-पर्व landing carry a bare 11 pt `gold` ❖ after the category glyph under the same rule. |
| **The detail hero** (`ObservanceDetailScreen.tsx`) | In the `heroTags` row after the category pill and deity, wrapping | One `❖ <calendar>` `goldTint` pill per calendar the rule belongs to (lens and tags alike), **all of them, not only the user's** — the detail is where "whose day is this?" is answered. |
...
**The seed notice.** When a chosen city seeds its calendars — only calendars that bring something are ever seeded — one line appears on the पंचांग day view — where the new dates are — on `goldTint`, naming the **city's** calendar and never the person: `बेंगलुरु का कर्नाटक · तेलुगु पंचांग जोड़ा गया — बदलें`, with a ✕. Tapping the line opens the sheet. Never a modal, never a question.
...
**A11y.** Each lens row is `accessibilityRole="checkbox"` with `checked`, labelled in English even in a Hindi UI, naming the observances and what it brings (`Rajasthan calendar. Goga Navami · Gangaur · Khetsimavas. Brings 14: Gangaur, Goga Navami, Teja Dashami and 11 more`), so two regional calendars are distinguishable by ear. The bulk pills are buttons `Select all n calendars` / `Clear all calendars` (testIDs `lens-select-all` / `lens-clear-all`). The ledger, quiet-day and More rows are buttons labelled `Regional calendars, n on` / `, all n on` / `, none selected`. The आपके पंचांग से card is `testID="lens-additions"`; its rows are buttons `<NameEn>, next <date>`, its `बदलें` is `Change regional calendars`, its group link `See all n <Calendar> observances`. Day-view and hero chips are labelled `<Calendar> calendar` (`and n more`).

**Files.** `mobile/src/panchang/regionTags.ts` (`REGION_TAGS`, rule id → calendars, with the source note per row — off the launch graph), `lenses.ts` (type, ids, set, parse/serialise — I/O-free and launch-path-safe), `lensRegistry.ts` (names, examples, `CITY_LENS`/`STATE_LENS` — behind a `require()` thunk), `lensStore.ts` (AsyncStorage + once-per-install seeding of offered calendars + `setAllLenses`/`clearLenses`), `useLenses.ts` (display set, `availableCount`, `selectAll`/`clearAll`/`allSelected`; reaches `vratCatalog` through a `require()` thunk), `vratCatalog.ts` (`regionsOf`, `ruleCalendars`, `activeCalendarsOf`, `getRulesForLens`, `getLensesWithContent`, `withContentOnly`, `getLensAdditions`); `mobile/src/components/LensPickerSheet.tsx` (`React.lazy` in both screens); `PanchangScreen.tsx` (entry points, `LensAdditionsSection`, the `ObservanceCard` chip), `ObservanceListScreen.tsx` (`region` mode), `ObservanceDetailScreen.tsx` (hero chips), `MoreScreen.tsx`; the gate in `festivals.ts` (`getObservanceCatalog`) and `festivalEngine.ts` (day/month/upcoming). Tests: `panchang/__tests__/lens.test.ts` (tags valid and non-gating, per-lens agreement with the catalog gate, every calendar offered), `.maestro/regional-lens-smoke.yaml` (authored; device run owed). Contract: RULEBOOK §23a.5, §23a.11–12.
