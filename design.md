...
**Observance Detail** (`ObservanceDetailScreen`) — hero (category pill + deity, name at 24 pt centred, other-language caption, and a `saffron-tint` "अगला · Next · date · in N days" pill), then an action row: **Follow** (outline `saffron` pill; fills `saffron` with `parchment` text when following — following also feeds vrat reminders, §38) and **॥ Read Katha** (filled `saffron`). Following shows a transient (3.5 s) `gold-tint` "Added to My Vrat — View →" bar. Below: **महत्व · About** prose, a katha card, and — for observances whose rule carries a resolving `vidhiId` (PRD-19 Phase 2B) — a **पूजा विधि · How to observe** block: the same card shell as the katha card (`parchment-soft`, `divider`, `radii.lg`, `elevation.card`) with a saffron ॥ glyph, the vidhi's bilingual title, a `N चरण · लगभग M मिनट` meta line, and › — tap → `VidhiDetail` (§62) carrying the next occurrence's `dateMs` so the samagri checklist keys to the right festival date. The section renders only when a published vidhi exists; there is never a "coming soon" placeholder here.
...
**Surfaces.**
- **आज का श्लोक — `VedanshVerseWidget` / `VedanshVerseWidgetProvider` (wide · large · small; best at wide).** The deterministic Daily-Bhakti selection for the device-local day. **Wide** (iOS `systemMedium`, Android 4×2 default cell) gives the verse the line width it was written for — three lines of the planner's excerpt plus the source; **large** (iOS `systemLarge`, Android any tall cell) drops the excerpt and renders the verse **line-by-line from `lines`**, which the payload has always carried; **small** keeps the two-line excerpt (`twoLineExcerpt`) and drops the source, the only thing that fits. The full verse + source stay in the accessibility label. Tap → the exact Daily Bhakti entry.
...
**Twelve occasions, grouped picker.** `EVENT_RULES` now carries भवन (Griha Pravesh, Bhumi Pujan) · संस्कार (Namkaran, Vidyarambh, **Mundan, Annaprashan, Karnavedha, Upanayana**) · क्रय व आरम्भ (Vahan, Vyapar, **Sampatti, Swarna**); the picker renders three `sectionLabel` groups over the same `ListCard` rows — no new card grammar, अबूझ door still last. Annaprashan's 6th–8th-month guidance is caption copy (the same plain-occasion treatment Namkaran shipped with — a within-window mode remains future work). All twelve tables are DRAFT (`verified: false`).
...
**Known limits (post-Phase 2).** Rule tables are DRAFT pending §10 review (RULEBOOK §17 — release gate), now including the masa tables and the six Phase-2 occasions; a Vishti karana that STARTS during daylight (sunrise karana ends, Vishti follows) is still invisible — only sunrise-Vishti is solved; yoga end-times stay unsolved by design (Vyatipata/Vaidhriti remain day-level); windows straddling an anga changeover are graded at their start, not split (Phase 3). **The abujh↔finder contradiction is only partly closed**: seasonal bars now yield, but an abujh day can still be excluded by a per-day dosha (Akshaya Navami on a rikta tithi) or by failing the occasion's nakshatra/tithi/vara match (Dhanteras, excluded for all six on factors alone) — while the Abujh screen calls the same day auspicious in its entirety. §4.2's wording is stronger than what ships; RULEBOOK §17.8 carries the decision to §10. Following an **occasion** as a standing interest ("tell me when a श्रेष्ठ day appears") remains out of scope — this slice follows *days*.
...
**Back always retraces the journey — the flow is registered on BOTH stacks.** The three routes are declared once, in the shared `VidhiStackParamList` (`navigation/types.ts`), which both `HomeStackParamList` and `PanchangStackParamList` intersect, and both `HomeStackNavigator` and `PanchangStackNavigator` register. Every door therefore **pushes in place**: the Home DISCOVER card, search rows and routine items push on the Home stack; the day-panel pill, the Browse-by-type tile and the Observance Detail card push on the Panchang stack. The vidhi screens type against `VidhiStackParamList` rather than one stack, so a single component serves both registrations. This replaced a cross-tab `navigate('PanchangTab', panchangTabTarget(…))` from the Home-side doors: back from the catalog popped to the **Panchang calendar** — a tab the user never chose, and one whose default (calendar) mode carries no vidhi door at all, so the flow they had just opened became unreachable without knowing about the व्रत-पर्व tab's tile. The one cross-stack hop that remains is the conduct hand-off: the readers live only on the Home stack, so `navigateToHomeStackTarget` (`navigation/entryRoutes.ts`) pushes in place when the enclosing stack already registers the target route and falls back to `HomeTab` when it does not.
...
- **Hand-off card** (steps that ARE a shipped text): 1.4px `saffron` border, `radii.lg` — the target's title + a "returns here after" caption that is category-aware (कथा पढ़कर… for kathas, आरती पूर्ण कर… for aarti sections, पाठ पूर्ण कर… for stotram/sanskar sections); katha refs deep-link to `VratKathaReader {kathaId}`, section refs route through `buildEntryStartTarget` — the vidhi never re-types a shipped text (RULEBOOK §11.11). Both go out through `navigateToHomeStackTarget`, so a puja opened from Home pushes the reader onto the same stack (back returns to the step) instead of bouncing through `HomeTab` and rebuilding it.
...
**Files.** `mobile/src/data/vidhi/{types,index,checklistStore,satyanarayan-puja,diwali-lakshmi-ganesh-puja,ganesh-chaturthi-sthapana,navratri-ghatasthapana,karwa-chauth-puja,maha-shivaratri-puja}.ts` · `mobile/src/screens/Vidhi{Catalog,Detail,Conduct}Screen.tsx` · pill + tile in `mobile/src/screens/PanchangScreen.tsx` · routes in `mobile/src/navigation/{types.ts,PanchangStackNavigator.tsx,HomeStackNavigator.tsx}` (one shared `VidhiStackParamList`, registered on both stacks) · `vidhiId` hooks in `mobile/src/panchang/{types,festivals}.ts`. Phase 2B surfaces: search rows in `data/searchIndex.ts` + routing in `screens/SearchScreen.tsx` · the How-to-observe card in `screens/ObservanceDetailScreen.tsx` · the DISCOVER spotlight in `screens/HomeScreen.tsx` · keep-awake in `VidhiConductScreen.tsx` (`expo-keep-awake`) · the `vidhi` routine-item kind in `data/routine/{types,units}.ts`, `components/AddToRoutineSheet.tsx`, `navigation/entryRoutes.ts` and the detail-header `AddToRoutineButton`. Tests: `src/data/__tests__/vidhiContent.test.ts` (tsx — six-entry registry, refs + Phase 2B liturgy hand-offs, source contract + Devanagari well-formedness), `src/data/__tests__/searchIndex.test.ts` (vidhi rows), `src/screens/__tests__/VidhiScreens.test.tsx` (Jest — all screens + registry-wide source privacy, both hand-off mountings), `src/navigation/__tests__/vidhiBackNavigation.test.ts` (Jest — the dual registration, the single shared param list, and that every door pushes in place), `src/navigation/entryRoutes.test.ts` (tsx — the routine-item route and both `navigateToHomeStackTarget` branches), `.maestro/vidhi-smoke.yaml` (e2e).
...
4. **Detail** (`PitruSmaranDetailScreen`) — the §33 ObservanceDetail hero: ॥ ॐ ॥ ornament (`gold`, letter-spacing 6), name 24 pt centred, "श्राद्ध तिथि: <words>" caption, and the `saffronTint` **"अगला · <date> · in N days"** pill. Rows (`parchmentSoft`, `divider`, `radii.md`): अगले वर्ष (weekday + full date), पितृ पक्ष `<year>` में (`<tithi> श्राद्ध — <date>` via `pakshaShraddhaDay`), a dedicated **स्मरण अनुस्मारक** switch (new saves default ON after the shared OS grant; one-day-before 18:00 + day-of 07:00), and two **गीता पाठ** rows deep-linking the shipped Gita reader — अध्याय 15 and अध्याय 2 (`HomeTab → GitaReader { chapter }`). A refused/hard-blocked OS grant writes the new preference off instead of showing a switch that cannot deliver; old entries with an omitted preference remain off. The switch stays independently controllable per person; deletion removes the entry and the headless re-arm cancels its queued slots. शेष: an `सम्पादन` header action → edit, and a centred underlined delete link whose confirm is a **full modal card** over `modalBackdrop` (`radii.lg`, `elevation.raised` — never a swipe-only gesture, §12). **The four solved dates arrive in two stages, in reading order:** the hero pill is published on its own solve, and अगले वर्ष + पितृ पक्ष follow. Both stages are usually invisible because the answers are cached (below), but on a miss the pill must never wait on a date further down the page — the row shells are absent until solved, never a spinner or a placeholder date.
5. **Pitru Paksha overview** (`PitruPakshaOverviewScreen`) — title पितृ पक्ष `<year>` (rolls to next year once the amavasya passes), a date-range hero + "भाद्रपद कृष्ण पक्ष · आपके परिवार की श्राद्ध-तिथियाँ" caption, then the fortnight as **§33.6 Upcoming rows**: 8 px marker dot (`saffron` when a family entry maps to the day, `gold` default), short date (52 pt column, 11), the day's tithi-shraddha name from its sunrise tithi (a kshaya day carries both names — "चतुर्थी व पंचमी श्राद्ध"), and each matched person beneath as `॥ <name>` in `saffronDeep` 12. The पूर्णिमा श्राद्ध row leads; the last row is "सर्वपितृ अमावस्या — अज्ञात तिथियों हेतु", where unknown-tithi entries collect.
6. **Panchang day chips** (`PitruSmaranDayChip` / `PitruPakshaDayChip` + `usePitruSmaranForDate`, §33 day panel) — on a saved annual observance **or the person's mapped Pitru-Paksha day**, a muted **"॥ स्मरण — <relation>"** pill joins the day's observance area: `goldTint` fill, 1 px `gold` border, `radii.pill`, `inkSoft` text at 12, min-height 32 — never the festive saffron pill style. Tap → that person's detail (`MoreTab → PitruSmaranDetail`). During the public fortnight, the saffron-tint **"पितृ पक्ष — <tithi> श्राद्ध"** chip also appears (including combined kshaya names) → the overview. Matching remains deferred and costs nothing when no entries exist.
