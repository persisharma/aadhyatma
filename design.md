...
**Notification tap → deep link** (`notifications/deepLink.ts`). A module-level `navigationRef` (attached to the `NavigationContainer` in `App.tsx`) lets `handleNotificationResponse` dispatch from outside the React tree; `App.tsx` wires both the cold-start response and the live `addNotificationResponseReceivedListener`. **Cold start (Sept 2026):** the tap that *launched* the app is read (`getLastNotificationResponseAsync`) in the same pre-mount race as a cold widget URL, under the shared `INITIAL_TARGET_TIMEOUT_MS` = 1 s, and `coldStartTargetFromNotification` turns a daily-verse payload into the `StartTarget` (`navigation/startTarget.ts`) that `TabNavigator` takes as its **initial route** — so Daily Bhakti is the first committed tab and Home never mounts as an intermediate screen (the widget path already worked this way; the notification path used to dispatch after mount and visibly landed on Home first). A widget URL wins outright and never waits on the notification read; a consumed launch tap is not re-dispatched by the retry loop; if the read misses the timeout the post-mount dispatch still catches it. Warm taps and every other family keep the post-mount dispatch. Routing by payload type:
