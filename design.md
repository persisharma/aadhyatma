...
- **Themed readings over a bundled corpus (`gita-saar/`, §75)** — a chapter is a *theme*, a verse is a `{chapter, verse}` **ref** into `gita/` plus the theme's own plain-language सार. The loader pulls the Sanskrit/IAST from the corpus at open time; the theme files carry no scripture (RULEBOOK §29). Routes, bookmarks, progress, search and the routine picker treat it as one more chaptered text.
...
## 75. गीता सार — themed readings over the Gita (Gita Saar)

**Purpose.** A place to keep adding *readings*: one question ("what is true prema?"), the Gita's own verses that answer it, in a considered order, each with its sense in plain words. The Gita reader (§9/§10) carries the whole text with bhāvārtha and commentary; गीता सार carries the *thread*. It is a `granth` catalog entry (`gita-saar`, deity `krishna`, thumb `सा`) and behaves like every chaptered text — a **chapter is a theme**, a **page is one shloka inside that theme**. The first theme is **सच्चा प्रेम · True Prema** (19 shlokas in six groups).

**Scripture is pointed at, never re-typed.** A theme file (`data/gita-saar/themes/<id>.ts`) holds only refs (`{chapter, verse}` as printed), the group structure, and the सार lines. At open time the loader resolves each ref against `data/gita/` and attaches the corpus Sanskrit + IAST to the page, so a verse can never drift from the Gita reader's text, and a mistyped ref fails loudly (loader + `gitaSaarContent.test.ts`) rather than rendering the wrong shloka. Every page hands off into the Gita reader on its own verse. (RULEBOOK §11.11 duplicate-content rule and §28.2 precedent.)

**Structure.**

1. **Themes index** (`GitaSaarThemesScreen`, route `GitaSaarChapters`) — the §15 chapters index verbatim: `ReaderHeader variant="index"` titled `गीता सार` / `Gita Saar`, the toggle row, the `gita-saar` source sketch under the parchment overlay, then one `GitaChapterCard` per theme with the tag `विषय N` / `THEME N` and `<n> श्लोक` / `<n> verses`, resuming at the theme's last-read page. A one-line lede sits above the cards (`subtitle` face 15/23 `inkSoft`, italic in English): *एक विषय, गीता के चुने हुए श्लोक, सरल अर्थ। हर श्लोक से पूरा अर्थ गीता में खुलता है।* `entryRoutes` skips this screen while the manifest holds one theme (the §38 single-chapter rule) and opens the reader directly; the index becomes the landing surface the day a second theme is appended.
2. **Reader** (`GitaSaarReaderScreen`, route `GitaSaarReader {chapter, initialIndex?}`) — the Gita reader shell component for component (§9): `ReaderHeader` (title `गीता सार · <theme>` / `Gita Saar · <theme>`, italic `n / m` counter, `sideWidth` 60), `ReadingProgressBar`, the toggle row with `LanguageToggle` + `AddToRoutineButton` (chapter = theme) + the read-aloud pill pinned right, the horizontal `pagingEnabled` `FlatList`, `JumpToStartButton`, the §5 pager dots. Bookmark and share sit in the page's `topActions`; `sourceId` is `gita-saar`, bookmark ids `gita-saar:<theme>:<pageIndex>`. Swiping past the last page crosses into the next theme through `NextChapterCard` / `PrevChapterCard` exactly as the Gita crosses adhyāyas (no cards render while one theme ships).
3. **Page** (`GitaSaarVersePage`) — top to bottom: the **`श्लोक · c.v` / `Shloka · c.v` pill** (the Gita's own vocabulary — it names the shloka as printed, not the page number) with bookmark + share; a **group eyebrow** (`sectionLabel` size in the script's eyebrow face, `inkMuted`) naming which section of the reading order the page sits in, and — on a group's first page only — the group's one-line intro (15/23 `inkSoft`, italic in English); the **corpus verse** at the shared `verse` / `verseLatin` token; `Ornament`; the label **`सार` / `Essence` / `સાર` / `ಸಾರ`** in the `meaningLabel` idiom; the page's **theme line** (`titleFontByLang`, 17/25, `saffronDeep`, centred — the one idea this verse carries); the **सार** body at the shared `meaning` / `meaningEnglish` token (RULEBOOK §3 — no per-section sizes); the **hand-off pill** `पूरा अर्थ गीता में पढ़ें ›` / `Read the full meaning in the Gita ›` (`parchmentSoft` fill, 1 px `divider`, `radii.pill`, 14 pt `latinSemiBold` / script title face, `saffronDeep`; a11y `Open Gita reader for c.v`) → `GitaReader {chapter: c, initialIndex: v − 1}`. The theme's **last page** adds a second `Ornament`, the label **`एक पंक्ति में` / `In one line`**, and the theme's closing line centred at the meaning token.

**Language.** Every string routes through `contentByLang` / `meaningByLang` / `pick`; gu/kn re-script the Hindi सार. The English pill and labels carry no Devanagari (pinned by `GitaSaarReaderScreen.test.tsx`).

**Data (`mobile/src/data/gita-saar/`).** `types.ts` (`GitaSaarTheme` → `groups[]` → `verses[] {ref, themeHi/En, saarHi/En}`, `status`, review-only `source`), `themes/<id>.ts` + `themes/index.ts` (the registry, in reading order — position is the chapter number, so **append only**), `index.ts` (eager `gitaSaarChaptersManifest` mirrored by hand and pinned to the registry by test; lazy `getGitaSaarChapter(n)` behind a `require()` thunk; `resolveGitaSaarRef`; verified-only `getGitaSaarThemes()`). The catalog row's `sub`/`verseCount` derive from the manifest.

**Adding a theme.** Write `themes/<id>.ts`, append it to `themes/index.ts`, add its manifest row to `index.ts` (id, titles, verse count), and run the suite — the content test pins ref resolution, parity, Devanagari well-formedness, the source threshold and the manifest mirror. Nothing else changes: routing, search, bookmarks, resume, the routine picker and the themes index pick the theme up from the manifest.

**Files.** `data/gita-saar/{types,index}.ts`, `data/gita-saar/themes/{index,true-prema}.ts` · `components/GitaSaarVersePage.tsx` · `screens/GitaSaarReaderScreen.tsx`, `screens/GitaSaarThemesScreen.tsx` · rows in `data/texts.ts`, `navigation/{types,HomeStackNavigator,entryRoutes}.ts`, `data/{searchIndex,backgrounds,discoveryMeta}.ts`, `data/routine/chapters.ts`. Tests: `data/gita-saar/__tests__/gitaSaarContent.test.ts`, `screens/__tests__/GitaSaarReaderScreen.test.tsx`, rows in `readerReadAloud.test.tsx`, `readerTypeScale.test.tsx`, `entryRoutes.test.ts`, `searchIndex.test.ts`; e2e `.maestro/granth-smoke.yaml`. Contract: RULEBOOK §29.
