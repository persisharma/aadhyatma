...
**Discovery and landing state.** Kundali is a permanent `CategoryCard variant="launcher"` on Home (`कुंडली · Kundali`, insight glyph, NEW badge), not a shuffled Discover card. It deep-links to `PanchangHome({ initialTab: 'jyotish' })`. Panchang's top peer selector is `Panchang | Vrat & Parv | Jyotish`; it remains the fixed first control in every mode, while location/calendar-system/My Vrat controls appear beneath it only for the two Panchang-derived modes. A guest sees Create Kundali, Daily Rashifal, a Palm Reading tool card (§55), and one Navagraha practice card. Once a birth profile is saved, the landing becomes daily-first: the full Favour/Pause/Reflect Rashifal card leads, a compact Kundali reference follows, a "More tools" Palm Reading card comes next, and the same single practice card closes the page. The persistence-error landing also keeps Palm Reading reachable (it needs no birth details). Returning from creation must refresh this saved state immediately. Birth city remains independent of the current Panchang location.
...
## 55. Hast Rekha — Palm Reading (हस्तरेखा दर्शन)

**Purpose.** A guided self-observation of the four classical palm lines (हृदय/heart, मस्तिष्क/head, जीवन/life, भाग्य/fate) that returns deterministic, tradition-attributed reflection copy. It is the third Jyotish tool beside Kundali and Rashifal and follows the same content contract: **pure engine, no camera or image analysis, no randomness, no AI, no network, no predictive verdicts.** Every insight body attributes its association to tradition ("Tradition links/reads …"), invites reflection ("Consider …"), and closes on a not-a-verdict clause. The life line is **always** framed as vitality — the copy states "reflects vitality, never length of life" (hi: "जीवन की लम्बाई नहीं") and lifespan language is a hard reject, pinned by `hastRekha.engine.test.ts`.

**Entry points.** A `JyotishToolCard` (`हस्तरेखा दर्शन · Palm Reading`, glyph `ह`, NEW badge) on the Jyotish landing: after Daily Rashifal for guests, under a "और उपकरण · More tools" section label on the saved-profile landing, and on the persistence-error landing (palm reading needs no birth details, so it stays available while the Kundali profile is being repaired). Route: `HastRekha` in `PanchangStackParamList`, pushed within the Panchang stack.

**Structure** (`HastRekhaScreen.tsx`, RashifalScreen's shell):
1. *Top bar* — 44 back circle (§12), title swaps with the reading language, `Hast Rekha Shastra` caption, and a "फिर से · Start over" pill (visible only with a complete reading) that clears the stored profile.
2. *Disclaimer note* — the Jyotish info-mark row: "पारम्परिक हस्तरेखा-आधारित चिंतन—निश्चित भविष्यवाणी नहीं। · Traditional palm-line reflection—not a certain prediction." Part of the surface, not fine print.
3. *Observe card* — tells the user to look at their **active (dominant) hand** in good light, with an `n/4` progress counter.
4. *Four line pickers* — one section per line in fixed order (heart → head → life → fate): section label (line name), a locate sentence (where the line is), and a 3-option `radiogroup` of picker tiles (label + observational description, `hastrekha-<line>-<trait>` testIDs, selected = `saffronTint` fill + accent border). Options describe **observable forms only** (curved/straight/chained; long/short/sloping; broad/close/faint; defined/faint/absent) — never evaluative wording.
5. *Reading card* — appears once all four are chosen (before that, a dashed empty-state card): `cardActiveFrom` block with an "आपका पाठ · Your reading" header, four left-accent insight rows (line · chosen form label, then the guidance body), and a closing saffron-tint **Reflect** row whose question derives from the fate-line choice. Selections auto-persist (`@vedansh:hast-rekha-profile:v1`) — the header notes "Saved on this device"; a failed save shows a recovery note but never hides the reading.
6. *Practice* — the shared `JyotishPracticeCard` routing to the allow-listed `navagraha-stotram` reader via `buildEntryStartTarget` (same allow-list rule as Rashifal's `sourceId`).

**States.** `useHastRekha` mirrors `useKundali`: `loading` (JyotishStateCard) / `guest` / `saved` (pickers hydrate from storage, reading shows immediately) / `error` (corrupt storage → recovery copy, "Nothing was deleted", pickers still usable). No implicit profile: an empty store renders no preselected traits.

**Camera guide (Tier 1 assist).** The observe card carries a "कैमरा गाइड से देखें · Use camera guide" pill → `HastRekhaCamera` (`HastRekhaCameraScreen.tsx`). It is an **observation aid, not an analyzer**: a live `expo-camera` preview with a stylised SVG framing overlay (`PalmGuideOverlay.tsx` — dashed palm silhouette + the four guide lines, each accent double-stroked over a white underlay so it survives any photo background; heart=gold, head=saffron, life=parchment, fate=avoid), a capture shutter (74, §12-clearing), then a review step where the captured photo — with the guides re-overlaid, the first unanswered line highlighted — sits above four compact confirm radiogroups (`hastrekha-cam-<line>-<trait>` testIDs). Confirming all four hands the choices back as the `HastRekha` route's `prefill` param, which the picker applies once, persists, and clears (`navigation.setParams`) so back-navigation cannot replay it. The manual picker remains the default and the complete fallback (permission denied → explanation + "Choose manually instead"). This screen is the app's one dark surface: chrome sits on `ink` with `parchment` text because it frames a live viewfinder — do not generalise this outside camera surfaces.
- **Privacy invariants (non-negotiable).** The photo is a cache-directory temp file, deleted (best-effort, `idempotent`) on retake, confirm, and unmount; it is never written to app storage, never uploaded, and never part of the saved profile — only the four trait strings persist. The permission copy and the on-screen note both state "never saved, never uploaded". Suggestion hook: `panchang/palmSuggestions.ts` is the seam for a future **on-device** model (TFLite/MediaPipe hand landmarks) that may pre-select options; a suggestion is an assist the user still confirms per line, never an auto-reading. v1 ships the null provider. Any provider MUST run on-device with no network and no image persistence — a cloud path is a contract break, not a config change.
- **Delivery note.** `expo-camera` is a **native module**: this feature reaches users through a store/dev-client build, not OTA (same constraint class as §6.2's no-native-store-review rule — here the cost was accepted deliberately). `app.json` carries the plugin with the camera-only permission string; microphone and Android audio are explicitly off.

**No sharing in v1.** The reading is personal-observation data; a share card would need its own privacy framing and the §51 fixed-height fit work. Adding one requires updating this section first.

**Files.** `mobile/src/panchang/hastRekha.ts` (pure engine + line/option catalog), `useHastRekha.ts` (storage hook), `palmSuggestions.ts` (on-device suggestion seam); `components/PalmGuideOverlay.tsx`; `screens/HastRekhaScreen.tsx`, `screens/HastRekhaCameraScreen.tsx`; Panchang navigation types/stack; Jyotish landing cards in `PanchangScreen.tsx`; `app.json` expo-camera plugin. Tests: `panchang/__tests__/hastRekha.engine.test.ts` (all 81 profiles, determinism, framing/lifespan guards, allow-list, purity), `screens/__tests__/HastRekhaScreen.test.tsx` (guest/saved/error states, selection flow, persistence, prefill apply/clear/reject, English-leak guard), `screens/__tests__/HastRekhaCameraScreen.test.tsx` (permission fallback, capture→review→prefill hand-off, temp-photo deletion on confirm/retake, suggestion pre-select + override, capture failure). E2E: `.maestro/hast-rekha-smoke.yaml` (manual path + camera permission-fallback leg; live capture is unit-tested — Maestro cannot drive a camera).
