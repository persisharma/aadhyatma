...
- **waiting** (calendar-gated: weekday off-day / festival window not open) — the resting copy ("Your sankalp begins 11 Oct." / "Resting today — …") **plus the next selected unit as a tap-to-read preview**, so an upcoming sankalp never opens onto an empty dead end. The preview row carries **no check circle** — the day is not committable until the gate opens, and a visible-but-dead circle reads as a broken control (July 2026 review).
