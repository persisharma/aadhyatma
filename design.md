...
**Known Phase-1 limits (PRD-16).** Rule tables are DRAFT pending §10 review (RULEBOOK §14 — release gate); Bhadra is the sunrise karana, not a window; angas are evaluated at sunrise, not at the candidate window; follow/remind is the next slice (it reuses the vrat `VratReminderPref` model when it lands).