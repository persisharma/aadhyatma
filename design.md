...
2. **Top bar**:
   ...
   - **Share circle** (§39, `Share theerth`) in the slot the mirror spacer used to hold → a prose share on the temple's own plate, scopes *महिमा व कथा* (significance + origin story) and, for temples with the §12.6 extended reading, *पूरा परिचय* (§39.4–§39.6).
...
**Regional coverage of the observance catalog** (Sept 2026 — the “Goga Navami is missing” audit). The catalog is **147 rules, 133 of them default-visible**; the rest are `advanced`/`regional` and reachable only through catalog search. Two rules about what goes where:
...
- Placement: in the verse page's **header row**, right of the verse-type pill, alongside the Bookmark button (readers pass both via the verse page's `topActions` slot). Every reader carries one — all 20 verse readers plus Daily Bhakti and the Japam counter. Because the picker lives inside the provider, every call site gained the Instagram destination without a call-site change; the button's own props are unchanged. It is also the **only** share affordance elsewhere in the app: the **Today's Panchang** (Muhurat detail) header uses this same circle rather than a bespoke button, so the share glyph reads identically everywhere.
- **Placement on non-verse surfaces (PRD-45)** — the rule is *on the content unit, not the screen*: **Vrat Katha reader** — toggle row, pinned left (mirror of the read-aloud pill on the right) · **Theerth detail** — top bar right, where the back button's mirror spacer was · **Daan katha** — `ReaderHeader` right slot (`sideWidth` 44) · **Daan principle cards** (home carousel) — the card's action row, trailing · **Daan journey शास्त्र card** — top-right of the card · **Vidhi conduct** — the mantra section's label row, pinned right (the unit is the mantra, not the step) · **Observance detail** — top bar right · **Ask answer** (Search) — right of the tag chip. a11y labels: `Share katha`, `Share theerth`, `Share verse` (Daan principles), `Share mantra`, `Share this day`, `Share answer`, each with its own hint.
- **Never on:** Pitru Smaran and the पितृ पक्ष परिचय layer (§63, §74 lock "no share surface"), and the personal-tithi (tila-tarpana) Vidhi, which keeps that register. Deity-page essays are one-line browse copy and carry none.
...
**Series mode (§39.5).** When the content paginates to more than one card the sheet title follows the content (`कथा साझा करें` / `Share this katha`, `तीर्थ साझा करें`, `पर्व साझा करें`, `उत्तर साझा करें`), a sub-line says *"Too long for one card — split into a series"*, the pages strip and the two all-pages rows sit above an eyebrow **`JUST THIS PAGE · n`**, and the three rows above each gain a trailing `page n` chip. With one page none of that renders: a verse share and a short prose share are the sheet exactly as listed above. The body scrolls inside a `maxHeight: 92%` sheet. `components/ShareTargetSheet.tsx` (shared `TargetRow`).
...
### §39.4 Prose card + paginator (`ProseShareCard.tsx`, `utils/shareCardPages.ts`)

**Purpose.** Long prose cannot fit the verse card and must not be shrunk to fit (§39's auto-fit lesson). It is cut into a series of fixed-size cards instead.

**Card.** Same 540×675 dp chrome as the verse card, captured at 1080×1350: `parchment` + the content's plate (`background` — the Theerth plate for temples, else the plain gradient), 1 px `divider`, padding 28/28/22; header band 13 pt `saffron-deep` (Cormorant + 2.4 tracking for `en`, script serif and no tracking otherwise); then a **fixed-height body box of exactly `proseBodyHeight` (459 dp)**; a 26 dp **page row**; and the shared branding footer (`ShareBrandFooter.tsx`, now used by both cards). Body faces (`proseType`): Indic body **18/29**, title **21/32** bold, in-flow heading **17/28** bold `saffron-deep` centred; Latin body **19/28**, title **22/31**, heading **18/26**. Paragraph gap 10, title margin 10. Page 1 carries the title; a paragraph continued from the previous page opens with `…`. In a series (`pageCount > 1`) the page row shows a left cue `आगे पढ़ें →` / `continues →` (last page `॥ इति ॥`) and right-aligned dots (5 pt, current `saffron`) + `n / m`; the row's height is reserved on single pages too, so geometry never changes. No `adjustsFontSizeToFit`, no `numberOfLines` on body text, Indic never italic.

**Paginator.** Pure and deterministic. Sentences (split at `। ॥ . ? !`, closing quotes kept) are packed greedily into the body box using a word-wrap line estimate; a sentence that would overflow starts the next page; a sentence longer than a page splits at words; a heading never ends a page (keep-with-next needs heading + 2 lines); a last page under 3 lines borrows sentences from the page before. One body line per page is held back as slack. **Chars-per-line is fitted, not guessed:** 500 bundled katha paragraphs per language laid out in Chromium at 484 dp with the app's own TTFs gave the smallest advance at which the estimate never under-counts (hi 0.390 · gu 0.410 · kn 0.540 · en 0.420); the shipped values are those + 0.02 (≈ 11 % over-count — pages end slightly early, never clip). If a device ever clips, widen the advance. `MAX_SHARE_PAGES = 10`.

### §39.5 Pages strip + preview (`SharePagesStrip.tsx`, `SharePagePreview.tsx`, `ScaledShareCard.tsx`)

**Strip.** Eyebrow `PAGES · n` (indic-safe tag, 10, `ink-muted`); on multi-scope content a pill segment right-aligned — `यह प्रसंग | पूरी कथा` on kathas, `महिमा व कथा | पूरा परिचय` on temples with the extended reading — switching re-paginates and resets selection. Below, a horizontal row of **72×90** thumbnails (the real card at 0.133 scale; 1.5 px border, `saffron` when highlighted), each with a page-number badge (10 pt) and a 16 pt tick (`saffron` filled when included, outlined when not; excluded thumbs at 45 % opacity). **Tap = highlight** (the page the single-page rows export); **tap the highlighted thumb = include/exclude**; at least one page always stays in. Meta line `3 selected · page 2 highlighted` + **`Preview ›`**. Over the cap: a `gold-tint` note when the scope has more than 10 pages (the first 10 are pre-selected), an `avoid-tint` alert when more than 10 are selected (all-pages rows disable). a11y: `Page n` (state `selected`), `Scope: <label>, n pages`, `Preview page`.

**Preview.** Rendered in place of the rows inside the same sheet (a second RN `Modal` misbehaves on iOS): `‹` back (`Back to share options`) · `Page n / m` · include checkbox (`Include this page`); the card at `min(300, width − 96)` with `elevation.raised`; `‹ ›` paging (`Previous page` / `Next page`) around *"In the series" / "Left out"*; a full-width `Continue with this page` button. The scaled view is never captured — the export is always the unscaled off-screen mount.

### §39.6 All pages at once (`utils/multiShare.ts`)

**Rows** (series only, above `JUST THIS PAGE`, under an eyebrow `ALL SELECTED PAGES · n`): **Share all pages** (`⇶`, *"WhatsApp album, Messages, Save to Photos"*, hot `n` chip) and **Instagram carousel** (`▤`, *"All pages as one post — caption is copied"*, hot `n` chip). a11y `Share all pages` / `Share as Instagram carousel`.

**Flow.** Either row switches the sheet to a progress view (*"Rendering page 3 of 7…"*, a 6 dp `saffron` bar) and captures the selected pages **one at a time through the single off-screen mount** — never N bitmaps at once (§39.3's memory note). *Share all pages* then closes the sheet, waits 350 ms for the modal to finish dismissing, and opens **one** OS share sheet with every PNG (WhatsApp receives an album; the iOS sheet offers "Save N Images"). *Instagram carousel* copies the Instagram caption first and pauses on a **hand-off view**: *"n pages ready"*, three numbered steps (pick Instagram in the next sheet; if it takes only one image, save the images — iOS "Save N Images" / Android Photos-Gallery — then Instagram → + → Select multiple; paste the copied caption) and a filled `saffron` **Continue** (`Continue to Instagram`) that opens the same one-call sheet. A capture failure closes the sheet with the §39 "Couldn't share just now" alert.

**Why this route, and not an Instagram API.** No share intent creates an Instagram carousel on either platform, and whether Instagram's share target accepts several images is not something this build can promise — hence the hand-off steps. The Graph API's carousel publishing needs a Business account, a backend and hosted images; out of scope for an offline app.

**Native gate.** `RN Share.share` takes one `url` and `expo-sharing` one file, so multi-file share needs `react-native-share` (`open({ urls })`) — a **store build**. `multiShare.ts` probes `TurboModuleRegistry.get('RNShare')` before lazily requiring the package (its codegen spec throws at import on an older binary); absent, both rows render **disabled** with *"Needs the latest app update"*. `expo-media-library` was deliberately not used: on Android 13+ its save path needs `READ_MEDIA_IMAGES`, which Google Play has required a core-use justification for since May 2025. `app.json` gains `NSPhotoLibraryAddUsageDescription` (add-only; *"Save the verse and katha cards you share to Photos."* — a consequence, never a privacy reassurance, per RULEBOOK §3 copy rule) — the iOS share sheet's own "Save Image(s)" action requires it, which the pre-PRD-45 single-card share also exposed.

### Share flow (`ShareProvider` / `useShare()`)

0. `share(content, lang)` takes a `ShareableContent` — a `ShareableVerse` (unchanged) or a `ShareableProse` (`kind: 'prose'`, one or more scopes of `heading`/`para` blocks; built per surface in `utils/shareContent.ts`). A verse with no `opts.target` opens the target picker (§39.1) and returns; the picker calls back with `'system'` or `'instagram'`. A caller that already knows the destination passes `opts.target` and skips the sheet. **Prose always opens the picker** (the reader picks pages there); its single-page rows export the highlighted page through the same capture + delivery code as a verse, with the caption label extended by ` · page n/m`, and its all-pages rows follow §39.6.
...
**Files:** `mobile/src/components/ShareButton.tsx`, `ShareCard.tsx`, `ShareBrandFooter.tsx`, `ShareStoryCanvas.tsx`, `ShareStoryFrame.tsx`, `ProseShareCard.tsx`, `ShareTargetSheet.tsx`, `SharePagesStrip.tsx`, `SharePagePreview.tsx`, `ScaledShareCard.tsx`, `mobile/src/utils/shareVerse.tsx`, `mobile/src/utils/shareContent.ts`, `mobile/src/utils/shareCardPages.ts`, `mobile/src/utils/shareCardType.ts`, `mobile/src/utils/shareStoryLayout.ts`, `mobile/src/utils/multiShare.ts`, `mobile/src/data/shareLinks.ts` (`tagNameHi/En` lets prose tag the work, not the shelf), `mobile/src/data/shareHashtags.ts`. `ShareProvider` mounts once in `App.tsx` and owns the off-screen card and both pickers. Tests: `utils/__tests__/shareHashtags.test.ts`, `shareStoryLayout.test.ts`, `shareVerseTarget.test.tsx`, `shareSeries.test.tsx` (series flow), `shareCardPages.test.ts` (paginator + every bundled katha fits), `shareContent.test.ts` (per-surface builders; every katha/temple fits), `multiShare.test.ts` (native gate), `components/__tests__/ShareTargetSheet.test.tsx`, `proseShareCardFit.test.tsx`; e2e `mobile/.maestro/share-target-smoke.yaml`, `share-katha-smoke.yaml`.
...
- **Mantra section** (steps carrying a transcribed mantra): the shared reader `Ornament`, then a quiet `मन्त्र · Mantra` label, Devanagari at `footerMantra` size with 2× leading, and IAST beneath (`latinItalic` 12.5/21, `ink-muted`) inside the same reading card — no nested bespoke mantra card. The label row carries the §39 share circle pinned right (`Share mantra`) → the verse card with the mantra lines, the step's instruction as meaning, header `<vidhi> · मन्त्र · <step>`. Absent on the personal-tithi vidhi (§63 register).
...
`snapshot` (**आपकी कुंडली, एक नज़र में · Your Kundali in sixty seconds** — 5–7 label:value rows, each a PROJECTION of a later section; the fact id IS the source section id, pinned by test; never a new claim) · `summary` (birth facts + **age as of this date**) · `lagna` · `moon` (now names the Moon's bhava) · `combinations` (**ग्रह एक साथ क्या कहते हैं · What the placements say together** — ≤6 lines from `kundaliYoga.ts`: the Lagna lord's seat always first, then conjunctions, allow-listed yogas, dignities, ranked by weight then id) · six life areas (**age-aware, ids stable**: for an adult `career` 10 · `relationships` 7 · `wealth` 2/11 · `wellbeing` 1/6 · `learning` 4/5 · `dharma` 9; for 13–17 `career` becomes *सीख, प्रतिभा और आगे की दिशा · Learning, talents and future direction* (10/5) and `relationships` becomes *मित्रता, सहकार्य और सामाजिक स्वभाव · Friends, teamwork and social nature* (7/3/11); under 13 the register is parent-facing throughout; each area closes with a REGISTER line — what to do with the section — never a disclaimer) · `observations` (Sade Sati always, **prefixed `As of <date>:`**, followed by Saturn's next sign-change date or an explicit "dated, not permanent" line; **Mangal Dosha display-gated off**; **Kaal Sarp excluded by decision**, pinned absent) · `vimshottari` (**विम्शोत्तरी दशा — तिथियों में जीवन-यात्रा · a dated life view**: three composed Maha×Antar pair paragraphs from `buildDashaPairReading` — both date ranges, the lords' naisargika relation, the Antar lord's natal house — then nine period lines that lead with **dates** (`4 Jul 2029 → 4 Jul 2046 (ages 15–32)`), the period running at birth labelled **`active at birth → <end> (until age 15 y 11 m) · balance at birth 16 y 2 m of the full 19 y`**, current flagged; facts carry both current date ranges) → **the full disclaimer exactly once**, at the foot. Section cards carry one full accessibility label each; a practice link renders only from the allow-listed `practiceSourceId`.
...
**Answer card (`components/AskAnswerCard.tsx`).** The §8 active-card gradient (`cardActiveFrom/To`, `cardActiveBorder`, `radii.lg`, `elevation.card`). Structure: a `goldChipBg` eyebrow **tag** (`eyebrowTextStyle` 10.5) · the **headline** in the language's title face, 19/28 · optional italic **sub** · **label · value rows** (78 pt micro label, value 14.5/22; a `tone: 'avoid'` row — the निषेध half — takes `avoid` / `avoidDeep`, the §65.1 split) · a hairline-ruled **`▸ गणना देखें / Show the working`** toggle revealing the answer's plain-language **basis** (`AskAnswer.basis`, localized, body face 11/17 muted; collapsed by default) — the raw engine-call `working` trail (`computeMuhuratDay(…)`, rule ids) is never shown to users: it renders only in `__DEV__` builds when an answer has no `basis`, and in release the toggle is hidden for such answers · the registry's own **tradition note** (12.5/19 muted) and a one-line **provenance** (11/16, never a URL) · up to **three actions**, the first a filled `saffron` pill with `onPrimary` text, the rest outlined in `cardActiveBorder`. All copy flows `contentByLang`, so gu/kn derive from the Devanagari half. `compact` tightens padding for the briefing. An optional `onShare` puts the §39 share circle right of the tag chip (`Share answer`) — Search passes it (a one-scope prose card: headline as title, sub, each `label — value` line, note); the card itself stays provider-free, and the compact briefing cards omit it.
...
- `शास्त्र क्या कहते हैं` — a label row with a quiet `स्वाइप करें →` hint, then the verse spine as a **horizontal carousel** (`daan-principle-carousel`: paged `ScrollView`, card width = screen − 2×`readingGutter` − 28pt peek, 10pt gap, `snapToInterval`, `decelerationRate="fast"`). Each `daan-principle-*` card (`parchmentSoft`, `divider`, `radii.lg`, `elevation.card`) shows the verse in full (14.5/24 title face, centred; Devanagari via `verseLinesByLang`), the cite at 10.5 uppercase in **`inkMuted`** (was `gold` — 3.1:1 at that size), and the meaning **clamped to two lines** with a `पूरा अर्थ ›` / `संक्षेप ‹` toggle (`daan-principle-meaning-*`, 32pt min-height) that unfolds it in place; the Gita card alone also carries the `गीता में पढ़ें ›` chip (`daan-gita-link`) deep-linking `GitaReader {chapter:17}`. Every card's action row ends with the §39 share circle (`Share verse`) → the verse card (shloka or, for a teaching row, its title; cite as label; meaning); the journey's शास्त्र card carries the same circle top-right, with its verse lines padded 36 each side to clear it. Position dots under the carousel (active 16×6 `saffron`, rest 6×6 `dotRest`; hidden from a11y).
