...
5. **Pitru Paksha overview** (`PitruPakshaOverviewScreen`) — title पितृ पक्ष `<year>`, then the **standing hero** (`pitru-paksha-standing`, Sept 2026 UX review): the screen answers *where today stands in the fortnight* before it answers *what the dates are*. One 21/30 title plus one pill, in three states — **during**: today's short tithi name (`heroHi`, the row label without its qualifier clause) over a `saffronTint` pill `दिन N / M · आज`; **before**: `पितृ पक्ष आरम्भ` over `N दिन शेष · <start>` (`कल से` at one day); **after the year has rolled**: `पितृ पक्ष <year+1>` over a *quiet* `goldTint` pill `अगले वर्ष · <start> से`, plus a second 12/18 `inkMuted` line naming the paksha that ended. The rolled year is never carried by a bare numeral in the title alone. Under it, the 12/18 `भाद्रपद कृष्ण पक्ष · <range>` line.
   ...
   Then the fortnight as §33.6 Upcoming rows — except **today's row, which opens where it sits** (`pitru-paksha-today`, 1 px `saffron` on `parchmentSoft`, `radii.md`): the short tithi name, an `आज · <date>` caption, the day's family names in `saffronDeep`, and two 44 pt `goldTint`/`gold` actions — `॥ तिल-तर्पण` → `VidhiDetail {vidhiId, dateMs: today}` and `इस तिथि का परिचय ›` → `PitruPakshaShiksha` (the latter only when `hasPitruShiksha()`). The card carries **no line on days no one is saved for** — silence is the absence, not a caption about it. Every other date stays a calendar row rather than growing a launcher.
...
**Structure — two routes in `MoreStackParamList` (`PitruPakshaShiksha: undefined` · `PitruKatha: {kathaId}`), registered on `MoreStackNavigator` after `PitruPakshaOverview` so Back retraces overview → परिचय → katha:**
...
1. **परिचय** (`PitruPakshaShikshaScreen`) — one reverent scroll in reading order under a gold `॥ ॐ ॥` (letter-spacing 6). Each section is present only when it has verified rows; section labels are the `typography.sectionLabel` uppercase idiom in `inkMuted`. **No lede line** — the *क्या है, क्यों है, किस दिन किसका — फिर स्मरण, फिर विधि* caption went in the Sept 2026 UX review: it described the screen's own section order, which is exactly the pattern §1's copy rule removes (pinned by a case in `PitruPakshaShikshaScreen.test.tsx`).
   ...
   - **पक्ष की तिथियाँ** — one §33.6-style row per fortnight day (title 14.5/22 · note 13/20 `inkSoft`, `divider` rule). **All sixteen days always render** — purnima, krishna 1–14, amavasya, in the order the fortnight is kept (Sept 2026 UX review; it used to render only the verified rows, which meant a section headed "Days of the fortnight" listing the first day and the last). A day the registry can teach carries its lesson title + body behind a filled 8 px `gold` marker (`pitru-tithi-<lessonId>`); a day it cannot carries the **engine's** name for it — `TITHI_NAMES_HI/EN`, krishna tithis at index `14 + day` — in `inkMuted` behind a hollow marker (transparent, 1 px `divider`), testID `pitru-tithi-day-<day>`. Named, not yet explained: no "locked", no "coming soon", no footer about source review. The names come from the engine and never from the draft rows, so the verified-only boundary is untouched and each verification upgrades a row in place with no UI change.
