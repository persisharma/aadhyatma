...
**Data (`mobile/src/data/pitru/`) — 58 rows authored, 38 verified.** `lessons.ts` (`kind: 'parichay' | 'tithi' | 'shabd'`, `fortnightDay` on tithi rows — 9 concept, all 16 fortnight tithis, 12 glossary), `principles.ts` (`ref: {kind:'gita'|'valmiki', chapter, verseIndex}` — Gita 1.42 / 9.16 / 9.25 / 10.29 / 2.20, Valmiki 2.102.27 / 1.41.19), `kathas.ts` (three verified, each retold from the bundled corpus and anchored by a `ref`: Ayodhya 102, Bala 38–44, Aranya 67–68), `prashna.ts` (9); `index.ts` exposes **verified-only** accessors behind `require()` thunks (the More stack imports the overview eagerly — launch-graph rule) and the single door predicate `hasPitruShiksha()`. Scripture is deep-linked, never re-typed; review-only `source` blocks are never rendered.
