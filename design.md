...
**Purpose.** The third limb of the japa–vrat–daan tripod: *educate first* (why the tradition gives, in the shastra's own words), a private on-device ledger second, and an external hand-off to verified organizations strictly last. The IA contract (PRD-26 §2.7) is structural, not tonal: the home carries **zero** give affordances, the directory is never a tab and is reachable ONLY from a journey's terminal step, and "record in my register" always precedes "give elsewhere". No punya scores, streaks, totals, goals or fear copy anywhere — pinned by the copy-guard test.

**दान-पुण्य home (`screens/DaanPunyaScreen.tsx`).** ReaderHeader (index). Sections in §4a order: `आज के दान का महत्व` — the first daan-covered observance of today (`useObservancesForDate` → `getDaanOccasionForRule`; card absent on uncovered days, never a placeholder) with the journey door; `इस वार का दान` — 7 vaar chips (`daan-vaar-*`, muhurat chip idiom) over the shared `data/daan/vaar.ts` table (one table with PRD-21); `शास्त्र क्या कहते हैं` — the verse spine rendered inline and in full (`daan-principle-*` cards: Devanagari via `verseLinesByLang`, gold uppercase cite, meaning via `meaningByLang`; the Gita card alone carries a `गीता में पढ़ें` chip deep-linking `GitaReader {chapter:17}`); `दान की कथाएँ` — the five teaching-katha rows; and one quiet `मेरा दान-पुण्य खाता` door (`daan-ledger-door`) at the end. Closing muted stance line.

**दान-यात्रा (`screens/DaanJourneyScreen.tsx`).** The §4a journey as a stepper — महत्व → शास्त्र → कथा (skipped when the occasion carries none) → क्या दें → संकल्प-भाव — with progress dots (active 22 pt saffron pill, rest 7 pt border dots) and पीछे/आगे pills. **Terminal actions exist in the tree only on the last step** (`daan-journey-record` filled saffron, leading; `daan-journey-directory` outlined, trailing, copy `दान-द्वार (बाहरी)`), preceded by the `इस दिन की सेवा — …` cause line (`daan-journey-causes`) and followed by the muted "रुक जाना भी पूर्ण है" line. The directory door passes the occasion's `causes` so the द्वार opens on the right प्रयोजन (§5.1); an occasion with no honest cause (कार्तिक दीप) passes none and the द्वार opens ungrouped-by-filter. Shipped kathas open `VratKathaReader` cross-tab; teaching-kathas open `DaanKatha`.
...
**दान-द्वार, by प्रयोजन (`screens/DaanDirectoryScreen.tsx` + `DaanDirectoryDetailScreen.tsx`).** The द्वार is organised on the **cause axis** (`data/daan/causes.ts` — अन्न-जल · गौ · बाल · वृद्ध · विद्या · आरोग्य · वस्त्र · जीव · आपदा), NOT the dravya axis the ledger records. A centred chip row (`daan-cause-*`, muhurat chip idiom) sits under the `देशे काले च पात्रे` line and the subhead that states the order of the screen — *first why each cause matters, then the places doing that seva, and the giving on their own website*; **chips are derived from the rows present**, so a cause with no verified row renders no chip and no shelf.

**The द्वार explains before it lists (RULEBOOK §27.14).** Every cause — grouped or filtered, identically — renders a `CauseHeading`: the uppercase cause label, then the **mahatva card** (`daan-cause-mahatva-*`, `parchment-soft` on `divider`, body 13/21 `ink-soft`) carrying `mahatva{Hi,En}` — why this daan is held dear — with a gold 11.5/17 citation line beneath it where the mahatva makes a textual claim (अन्न, गौ, बाल, विद्या, वस्त्र carry one; वृद्ध, आरोग्य, जीव, आपदा are plain tradition-register prose and carry none). Only then the muted `ये स्थान यह सेवा करते हैं — …` line and the short list of places. The teaching is the larger half of the screen by construction; the places are thin rows underneath.

Unfiltered, the list groups by cause in `DAAN_CAUSES` order, a row appearing under every cause it serves (the §12 theerth multi-group-tag precedent). Filtered (chip tap, or `route.params.causes` from an occasion) it is the flat set for that cause — still under its full mahatva — with a `saffron-deep` `इस दिन की सेवा — …` line naming the day's causes (`daan-directory-occasion-line`); tapping the active chip clears back to all. Each row shows name → its ONE about line → a gold `kind` meta line (अन्नक्षेत्र · देवस्थान ट्रस्ट · सेवा-संस्था · सेवा-पोर्टल, plus `· धन नहीं` where the hand-off is non-monetary). The footer states plainly that the app does not represent these places.
