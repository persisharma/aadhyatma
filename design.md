...
4. **Safe-area insets are seeded, not awaited**: the root `SafeAreaProvider` receives `initialMetrics={initialWindowMetrics}`. Without it the provider renders nothing until the first native inset event crosses the launch-busy JS thread, and the tree then mounts on whatever that event carried — which on a cold start can be **zero on either platform**: the iOS provider view reports its own `safeAreaInsets`, which UIKit leaves at zero until the view is attached to the window (the native guard only skips a zero *size*, so a sized-but-unattached view passes), and Android under `edgeToEdgeEnabled` can dispatch a pre-attach zero the same way. Home then painted flush under the notch/status bar, sat unresponsive behind the mount burst, and lurched down by the top inset when the correction event applied (the August 2026 "launch jerk" report, observed on iOS). `initialWindowMetrics` is read from the **key window's** insets — attached by definition when JS loads — so the seed gives the first committed frame its final insets and any later inset event is a no-op.
