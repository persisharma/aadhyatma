...
- `mobile/src/screens/LibraryScreen.tsx` — पाठ, the library index (§72).
...
**Purpose.** Answer "what is today, and what can I do with this app" in one screenful. It is **not** the library index — that is §72.
...
7. **उपकरण row** (`components/ToolsRow.tsx`) — **eight** launcher tiles (§19 launcher variant), four across (`flexBasis: '22%'`), two rows. Registry: `data/home/tools.ts`, **static and ordered**, data only — the routing lives in the component because each tool lands on a different stack.
...
8. **पाठ door** — one full-width row (`parchmentSoft`, `divider` border, radius 18, `minHeight 56`): a saffron `पा` thumb, the title `पाठ`, and a derived subtitle `{N} पाठ · {N} देवता · {N} उद्देश्य`. Tap → the **Library** screen (§72). This one row replaced the sixteen-tile CATEGORIES grid. The three counts are computed **once at module scope** from the bundled registries, never per render (§64).
9. **नया** (§73) — at most three cards for features this user has not opened yet, each gone for good once tapped. Renders **nothing at all**, heading included, when the list is empty. Replaced the DISCOVER carousel (§32).
10. Footer mantra (Section 7 — token `footerMantra`, 18 @ 55% opacity) at the end of the scroll
11. **Floating search button** (`SearchFloatingButton`) docked bottom-right → opens the Search screen (§36). Kept on Home deliberately: search is the fastest path to a named text, and pushing the library one screen away makes it *more* important, not less. (The Library carries its own search row too, §72.)

**What is NOT on Home.** The 16-tile CATEGORIES grid, the DISCOVER carousel, the देवता and उद्देश्य doors, and the daily verse. The first two were replaced (7–9 above); देवता/उद्देश्य moved to the Library (§72); the verse stays where it has always lived, the भक्ति tab (§23) — it was considered for Home and deliberately left alone. `screens/__tests__/HomeStructure.test.ts` fails if any of them comes back.
...
| Home-screen widgets · Guna Milan | **नया** (§73), which clears per user on open |
| दैनिक भक्ति · Daily Verse | nowhere on Home — it is a whole tab (§23) |

The general lesson is recorded in §73: novelty is a **lifecycle**, not a section. A surface that shows the same "new" thing forever is a banner.
...
**Spotlight set.** There is no longer a Home spotlight array. The card's live callers are the FOR TODAY row (§50, built from today's vaar deity and festival metadata) and nothing else. A **नया** card (§73) deliberately does *not* use this shell — it is a 3-per-user strip with its own compact row, not a hero card. Sibling-tab/More-stack targets still navigate via the **parent** (`useNavigation()` → bubble up), never the Home stack — same pattern as `RoutineBanner`/`PanchangScreen`, and always through a `*TabTarget` helper carrying `initial: false` (RULEBOOK §6.0).
...
## 72. Screen: पाठ — the Library index (TRD-42)

**Purpose.** Everything the Home grid used to hold, on one screen, organised by the axes a devotee actually thinks in. Pushed from Home's पाठ door (§18.8) on the **Home stack**, so Back returns to Home. `screens/LibraryScreen.tsx`.
...
## 73. नया — the feature-novelty lifecycle (TRD-42)
...
**Relationship to the other two novelty surfaces.** §44's `NEW` badge marks *texts* and clears per text (now on the Library grid, §72). The What's New sheet (§47) announces a *release* once, to everyone updating into it. नया is the per-user, per-feature middle: it survives the sheet being dismissed, and it clears per person rather than per version.
