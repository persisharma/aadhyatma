...
**Performance.** The occasion-independent per-day inputs (panchang solve + asta flags) live in the framework-free **`panchang/muhuratDayStore.ts`**, keyed by **absolute civil date** (`YYYY-MM-DD`) within a scope of **`locationKey` + calendar system** — the app's canonical location key (cityId, else `lat,lng@2dp`), the same one the observance cache and the engine's Observer cache use, so a GPS fix with no cityId can never alias another city's days. One store is shared by **all three surfaces** — the picker warmup (`useMuhuratFinderWarmup`), the occasion scan (`useMuhuratFinder`), and the abujh scan (`scanAbujhDays`) — so a day solved by any of them is free for the others. Absolute-date keying (rather than an index off the scan's start day) is what makes a day survive a **midnight rollover** and a re-entry that starts on a different day. The store is bounded to **`MAX_CITIES` = 5 scopes, LRU**: a 6th city evicts only the least-recently-used one and fires eviction listeners; touring cities never evicts the ones under the cap. Scans stay chunked behind `InteractionManager`, and yields are skipped for fully-cached chunks so a warm surface never flashes the spinner. The store and scan core live outside the React hook precisely so they are unit-testable under the tsx engine suite (`muhuratDayStore.test.ts`, `muhuratFinderScan.test.ts`).

**Persistence.** Those solves are also written to disk, so re-entering the finder — or **cold-starting the app** — no longer re-solves ~90–260 days. **`panchang/muhuratDayCache.ts`** is the only module that touches AsyncStorage (the same `observanceStore` ⇄ `observanceCache` split, and the reason the store stays RN-free): one key per `(scope, civil day)` under `@vedansh:muhurat-days:v<VERSION>:<scope>:<YYYY-MM-DD>`. All three hooks `await hydrateMuhuratDays(...)` the wanted range inside `runAfterInteractions` **before** scanning and fire-and-forget `persistMuhuratDays(...)` after; hydrate short-circuits with **no storage call at all** when the range is already warm in memory. Only today-or-later days are written, past-dated and stale-version keys are purged on hydrate, and an LRU eviction drops that city's disk keys too — so storage is bounded by 5 cities × one scan horizon. `Date` fields are tagged generically by `muhuratDaySerde.ts` (a plain `JSON.stringify` would flatten `sunrise`/`endTime` to strings that never revive, silently changing what the user sees). **Only a location or calendar-system change forces fresh solves** — a different scope.

> **RULE — bump `MUHURAT_DAY_CACHE_VERSION` (`panchang/muhuratDaySerde.ts`) whenever the panchang engine changes**, or whenever persisted days could otherwise serve stale results (a change to `DayInputs`' shape, to `computeDayInputs`, or to the asta flags). Devices that already scanned keep hydrating the OLD engine's days forever otherwise, and the fix ships only to fresh installs — the same trap as `observanceCache`'s `CACHE_VERSION` (§ the Panchang gotchas). Correctness of the cache itself is pinned by `dayCacheParity.e2e.test.ts`, which asserts *fresh == cached == serialize→revive* over a full year × 3 locations × 2 calendar systems; storage behaviour by `__tests__/jest/muhuratDayCache.jest.test.ts`; the cold-start journey by `.maestro/muhurat-day-cache-smoke.yaml`.
