...
**Files.** `mobile/src/panchang/hastRekha.ts` (pure engine + line/option catalog), `useHastRekha.ts` (storage hook); `screens/HastRekhaScreen.tsx`; Panchang navigation types/stack; Jyotish landing cards in `PanchangScreen.tsx`. Tests: `panchang/__tests__/hastRekha.engine.test.ts` (all 81 profiles, determinism, framing/lifespan guards, allow-list, purity), `screens/__tests__/HastRekhaScreen.test.tsx` (guest/saved/error states, selection flow, persistence, English-leak guard). E2E: `.maestro/hast-rekha-smoke.yaml`.
