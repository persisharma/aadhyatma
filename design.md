...
5. **Pitru Paksha overview** (`PitruPakshaOverviewScreen`) — title पितृ पक्ष `<year>`, date-range hero, then (PRD-44, §74) one quiet **परिचय door** — `parchmentSoft` on `divider`, `radii.md`, gold ॥ lead, "पितृ पक्ष क्या है, क्यों है" + "परिचय · तिथियाँ · शास्त्र-वचन · कथाएँ" → `PitruPakshaShiksha` — rendered only when `hasPitruShiksha()` is true (never a teaser), then the fortnight as §33.6 Upcoming rows. Beneath the calendar, one muted-gold **पितृ तिल-तर्पण स्मरण** door opens the guide for the first family-matched day, falling back to the fortnight start when the ledger is empty. Individual dates remain calendar rows rather than each growing a launcher. The public reminder tap therefore lands education-first, calendar second, vidhi third with no deep-link change. No engine/device reassurance footer.
...
1. **परिचय** (`PitruPakshaShikshaScreen`) — one reverent scroll in reading order under a gold `॥ ॐ ॥` (letter-spacing 6) and a 13 pt lede. Each section is present only when it has verified rows; section labels are the `typography.sectionLabel` uppercase idiom in `inkMuted`.
   ...
   - **पक्ष की तिथियाँ** — one §33.6-style row per fortnight day (8 px `gold` marker · title 14.5 · note 13/20 `inkSoft`, `divider` rule). **All sixteen days carry a row in the registry** (purnima, 1–14, amavasya — coverage is test-pinned); the section renders whichever are verified, and while any are draft an italic Latin footer (12/18 `inkMuted`) says the remaining tithis follow their source review.
   ...
   - **शब्द** — one glossary card; term 14 `ink`, one line 13/20.
   - **अब** — the three doors LAST, in the `goldTint` / 1.5 px `gold` door idiom (52 pt): **इस वर्ष की तिथियाँ** → `PitruPakshaOverview` · **॥ पितृ तिल-तर्पण स्मरण** → `VidhiDetail {vidhiId}` (undated; rendered only when `getVidhiById('shraddha-tarpan-vidhi')` resolves) · **॥ पितृ स्मरण** → `PitruSmaranList`. **No footer line** — the *यह परिचय है, विधान नहीं — परिवार की परम्परा सर्वोपरि है।* caption that used to close the scroll went in the Sept 2026 copy pass (§73 "Copy rule"): the doors are the end of the screen. The screen's honesty stays where it is load-bearing — the verified-only registry, the draft-tithi note, and the प्रश्नोत्तर answers themselves.
...
**A11y.** Every pressable carries an English constant label for Maestro: `Open Pitru Paksha introduction`, `Expand lesson <id>` / `Collapse lesson <id>`, `Open gita reader for <id>` / `Open valmiki reader for <id>`, `Open katha <id>`, `Open Pitru Paksha overview`, `Open Tila-Tarpana remembrance guide`, `Open Pitru Smaran list`. Cards are plain information panels; only interactive rows carry chevrons.
