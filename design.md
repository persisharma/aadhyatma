...
**Job-switch answer.** For the selected switch question, the first card asks “Should I change jobs?” and states a qualified decision before period details. The next card separates “In favour of a change” from “Reasons to pause,” with one or two plain-language reasons on each side and a compact birth-chart, current-period or current-transit reference under each. “What to do now” gives the offer-checking step. Empty opposition is said plainly; missing dasha support is uncertainty, not a negative prediction. The next-period and expandable-basis cards stay below. Allow all Hindi/English text to wrap; no fixed card heights.
...
**Freshness and integration.** `useJyotishNow` refreshes on focus, foreground, IST midnight and the next exact dasha boundary. Both Prashna and report rebuild against that instant. `gocharScanDays:0` skips future ingress scans, not the current snapshot. Ask uses the same phase engine for general career/business questions and recognizes explicit job-switch wording; other job wording remains general. Full reading passes `{purposeId, questionId}` and recomputes for the active chart. The warned text export includes the selected phase, decision when present, detailed signals, direction provenance and next period in a separate JSON block. The final report JSON tail remains unchanged and parse-back equal.
