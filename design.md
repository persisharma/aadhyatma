...
3. **आज · Today strip** (§48) — a one-card daily-panchang glance (vara + tithi headline, one lead-observance pill, a quiet Abhijit / Rahu Kaal windows line). Tap → Panchang tab.
...
3. **Meta row** (wraps, column-gap 14 / row-gap 6) — deliberately quiet: the July 2026 shipped version stacked up to four ALL-CAPS pills (2 observances + Abhijit + Rahu Kaal, each with a full time range) and read as a wall of text, so the pills were cut down to:
   - **One lead-observance pill** at most (`observances[0]` — the full list lives on the Panchang tab this card opens): `saffronTint` fill, `saffron-deep` text via `pillTextStyle()` 10.5.
   - **A dot-marked windows line** — per window a 6px dot (`gold` for Abhijit, `avoid` for Rahu Kaal — colour is supplementary, the text label still carries the meaning, §12) followed by `{label} {range}` in **`inkMuted` 12** (label: `latinSemiBold` for en, script serif for Indic; range always `latinSemiBold` — numerals never in the thin italic, §3). `inkMuted` is pinned AA against both `cardActive*` stops in `colors.contrast.test.ts`. Ranges use **`formatRangeCompact()`** (`muhuratFormat.ts`): the shared meridiem written once (`3:37 – 5:13 PM`), full form when the window crosses noon/midnight. The kaal label comes from the `KaalWindow`'s own `nameHi/nameEn` (KAAL_NAMES, `muhurat.ts`) — no duplicated literals.
...
- Accessibility: single button, label `"Today's Panchang. {vara}, {tithi}. {lead observance}. Tap to open."`.
