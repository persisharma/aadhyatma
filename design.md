...
## 61. Pitru Smaran (पितृ स्मरण, PRD-17 Phase 1)
...
4. **Detail** (`PitruSmaranDetailScreen`) — the §33 ObservanceDetail hero: ॥ ॐ ॥ ornament (`gold`, letter-spacing 6), name 24 pt centred, "श्राद्ध तिथि: <words>" caption, and the `saffronTint` **"अगला · <date> · in N days"** pill. Rows (`parchmentSoft`, `divider`, `radii.md`): अगले वर्ष (weekday + full date), पितृ पक्ष `<year>` में (`<tithi> श्राद्ध — <date>` via `pakshaShraddhaDay`), and two **गीता पाठ** rows deep-linking the shipped Gita reader — अध्याय 15 and अध्याय 2 (`HomeTab → GitaReader { chapter }`). शेष: an `सम्पादन` header action → edit, and a centred underlined delete link whose confirm is a **full modal card** over `modalBackdrop` (`radii.lg`, `elevation.raised` — never a swipe-only gesture, §12).
...
6. **Panchang day chip** (`PitruSmaranDayChip` + `usePitruSmaranForDate`, §33 day panel) — on a saved observance date a muted **"॥ स्मरण — <relation>"** pill joins the day's observance area: `goldTint` fill, 1 px `gold` border, `radii.pill`, `inkSoft` text at 12, min-height 32 — never the festive saffron pill style. Tap → that person's detail (`MoreTab → PitruSmaranDetail`). The hook defers its matching like `useObservancesForDate` and costs nothing when no entries exist; this chip is the entire Panchang integration.

**Privacy & non-goals.** Nothing syncs, nothing shares, no export in v1 (PRD-06 is the future transfer path); notifications are Phase 2 (opt-in per person, plus the default-on season reminder) and deliberately absent here. No Gregorian-anniversary mode, no gotra/genealogy fields.

**Files.** `panchang/pitruSmaran.ts` · `panchang/pitruSmaranDisplay.ts` · `panchang/usePitruSmaranForDate.ts` · `contexts/PitruSmaranContext.tsx` · `screens/PitruSmaranListScreen.tsx` / `PitruSmaranEditScreen.tsx` / `PitruSmaranDetailScreen.tsx` / `PitruPakshaOverviewScreen.tsx` · `components/PitruSmaranDayChip.tsx` · rows in `MoreScreen.tsx`, routes in `navigation/types.ts` + `MoreStackNavigator.tsx`. Tests: `panchang/__tests__/pitruSmaran.test.ts` (tsx), `contexts/__tests__/PitruSmaranContext.test.tsx`, `screens/__tests__/PitruSmaranScreens.test.tsx`, `components/__tests__/PitruSmaranDayChip.test.tsx`, `.maestro/pitru-smaran.yaml`. PRD: `docs/roadmap/prds/17-pitru-smaran.md`; prototype: `docs/pitru-smaran-prototype.html`.
