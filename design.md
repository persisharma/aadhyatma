...
- **The eyebrow row carries the ॐ mark (Aug 2026).** `ॐ वेदांश़` sat on its own line at the foot of every card, and on a surface only four text lines tall that line is expensive — on the wide verse cell it was a third of the shloka's body, so a verse needing three lines never got all three at full size. The mark now sits **right-aligned on the eyebrow row** (iOS `eyebrowRow {}` wrapping each surface's first line, `.fixedSize()` so a long eyebrow gives ground first; Android a horizontal `LinearLayout` with a `layout_weight="1"` eyebrow and `@id/widget_brand`), which costs no height at all — both are 10 pt and the eyebrow never fills the width. All three kinds and the in-app facsimile follow it; the Android recovery card hides the mark (`widget_brand` → `GONE`) because its eyebrow and both message lines already say वेदांश़, and every content render sets it back to `VISIBLE` — the same RemoteViews reapply trap as recovery's `maxLines` reset. `catalog.test.ts` fails if any surface puts `brand()` back on a line of its own.
