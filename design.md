...
**Remote push groundwork — device-token capture** (`notifications/pushTokenPure.ts` pure half + `pushToken.ts` glue, armed by the headless `<PushTokenRegistrar>` in `App.tsx`). Every family above is local and on-device; this is the one piece of *remote*-push plumbing, and it has **no visual surface at all** — no screen, no copy, no setting, no notification of its own. It exists so a server can one day address a single install.
- **Identity is two values, not one.** The **Expo push token** (`ExponentPushToken[…]`) is the address and it *rotates* (reinstall, device restore, an FCM refresh), so it is never the key. The **install id** (`@vedansh/install-id`) is ours: derived once from the first token via the repo's FNV-1a hash (deterministic — no entropy source, no native dependency) and then read back unchanged for the life of the install. No OS identifier is used: IDFV and `ANDROID_ID` both carry privacy-disclosure weight this needs none of.
...
- **No network call until an endpoint is configured**, and configuring it needs no code edit: `resolvePushEndpoint` takes the build-time `EXPO_PUBLIC_PUSH_REGISTRY_URL` (set per profile in `eas.json`, so preview and production can differ) and falls back to the `PUSH_REGISTRY_ENDPOINT` constant, which ships `null` — the pure suite fails if that changes. Until one is set the token is captured and stored locally (`@vedansh/push-token`, readable via `getCapturedPushToken()`), so the plumbing is verifiable on a real device *before* anything leaves it. Either source must be `https:` — a push token is enough to notify one identified person's phone. Because babel inlines `EXPO_PUBLIC_*` at build time, the read must stay the full literal; Jest needs `expo/virtual/env` stubbed (`jest.config.js` `moduleNameMapper`) for the same reason `expo-speech` is.
- **Uploads are deduped by fingerprint, not by token.** The stored fingerprint covers install id + token + platform + app version + reading language + IANA timezone, so a language switch or a move across time zones re-syncs (a server should push in the language the reader chose, at a sane local hour) while an unchanged install costs no request.
...
- **The status code is the protocol** (`classifyUploadResponse`), because it decides what the client does next: **2xx** accepted (so only return it once the row is durable — the client then stops re-sending an unchanged registration); **5xx / 429 / 408 / 425** retried with the backoff above; **any other 4xx** abandoned for this attempt without a retry, since an identical body would be refused identically. Only an accepted upload advances the fingerprint, so both a 4xx and an exhausted retry leave nothing recorded and are re-attempted on a later foreground — a fix on either side needs no client release. Response bodies are ignored.
- **Re-arms** on the grant arriving, on every foreground, on a reading-language change, and on an OS token-rotation event (`addPushTokenListener`). Concurrent callers collapse onto one attempt.
- **Payload is the minimum a server needs to address and segment an install** — install id, token, platform, app version, language, timezone, each a bounded (≤ 120 char) non-empty string. Nothing from the private Smaran/Kundali/Namkaran records and no location, ever (the field set is pinned by `pushTokenUpload.jest.test.ts`). The endpoint should **upsert on `installId`**: the same device re-POSTs with a new token after a rotation. Nothing in the request authenticates, so the body is untrusted input.
- **Not yet wired for delivery.** Receiving a remote push still needs a `type` in the `resolveNotificationTarget` table below — an unrecognised payload resolves to `null`, i.e. an ordinary Home launch — and an Android `channelId` on the send side. Switching the endpoint on is also a privacy-disclosure decision: it is this app's first outbound request, so it changes the store privacy label and the policy copy.
