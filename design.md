...
- **A line earns its place by telling the user something** (Sept 2026). The app grew a habit of closing screens with a muted caption restating its own stance — *"यहाँ रुक जाना भी पूर्ण है"*, *"यह कोई अंक नहीं है"*, *"यह परिचय है, विधान नहीं"*, *"निर्णय आपका है"*, *"यह सूची जान-बूझकर छोटी रखी गई है"*. The reader never asked, and under the very buttons they sat beneath they read as filler, so they are gone. A UI line now carries a **fact the user acts on** — what may be recorded, what a control does, where a hand-off leads, why a screen names no winner — or it does not ship. Nor does the app narrate its own implementation to buy trust: `userFacingImplementationCopy.test.ts` bans `on this device`, `offline`, `no cloud` and their Devanagari forms from every screen and component, so privacy is a property of the build, never a caption. The stance is enforced where it is load-bearing: in the data (no totals, verified-only registries, the gupt guarantee) and in the copy-guard tests' banned vocabulary. **This never touches content**: verse meanings, cause mahatva, katha शिक्षा, vidhi instructions and katha prose are what the reader came for and keep their full teaching register.
...
4. **Illustration plate** (only for temples with a dedicated commissioned sketch — `getTheerthIllustration(temple.id)`, i.e. the per-temple overrides in `backgrounds.ts`: Salasar Balaji, Khatu Shyam, Karni Mata, Jeen Mata, Ramdevra, Gogaji, Tejaji, Khandoba, Mahasu Devta, Sabarimala, Vetrimalai Murugan): the same sketch that sits faded in the background layer, shown at full strength inside the content column. Frame: 5:4 window, `parchmentSoft` fill, 1 px `divider` border, `radii.lg`, `spacing.sm` side inset, `overflow: hidden`; the square plate is anchored to the **top** of the window (artwork lives in the upper part of every plate) so the murti and mandap stay in view. Accessible as one image labelled with the temple name (`testID="theerth-illustration"`). Temples on a generic deity plate render nothing here — a deity image repeating across dozens of temples belongs only in the background. Since the September 2026 plate-decoupling decision (RULEBOOK §12.6) a temple may carry its full §12.6 reading *before* its plate is commissioned: it reads complete on the deity background and gains this illustration block when its plate lands.
...
**Regional coverage of the observance catalog** (Sept 2026 — the “Goga Navami is missing” audit). The catalog is **174 rules, 162 of them default-visible**; the rest are `advanced`/`regional` and reachable only through catalog search. Two rules about what goes where:

- **A regional observance that a whole state keeps is `default`, not `regional`.** `visibility: 'regional'` currently means *invisible everywhere* — `getObservanceCatalog()` filters to `default` and there is no surface that opts back in — so tagging Gangaur or Chhath “regional” would have hidden them from the users who actually keep them. The identity lives in the rule's **`shortDescription*` and `searchTerms`**, which name the state, the mela and the local name (the pattern `bhadwa-chauth` set): a Tamil user sees one extra dot on a day, a Rajasthani user sees their festival. The two rules that remain `visibility: 'regional'` (`karthigai-vrat`, `rohini-vrat`) are the monthly Krittika and Rohini nakshatra series, awaiting the opt-in surface designed in `docs/roadmap/prds/42-regional-parv.md`. Wave 2 gave both a **correct** rule (they were `catalog-only` *and* carried a 1-indexed nakshatra, so they resolved to nothing twice over) without making them visible: twelve rows a year each is exactly the universal-day load that PRD's lens exists to prevent. The Tamil date's default-visible home is the annual `karthigai-deepam`; `rohini-vrat` is knowingly still the only home of its own date, and is the Jain lens's job.
...
- **And the converse — a shipped date is not a shipped story.** तेजा दशमी shipped as a dated rule (Bhadrapada S10) with no katha behind its detail page, so the day named वीर तेजाजी and then had nothing to read. It now carries `teja-dashami-katha` — a six-part bilingual retelling (खरनाल · पनेर · जलती झाड़ी का नाग · लाछा की गायें · वचन का पालन · तांती और मेला) that reaches the Observance Detail katha card, the Katha Library tile and जिज्ञासा's *Read katha* action with no screen change. Drik indexes no Tejaji katha, so the catalog entry cites the same Rajasthan tourism portal the rule does — the §11.1 folk-deity carve-out, now closed over katha sources too (RULEBOOK §23a.2).
...
**Wave 2 additions (16 rules, Sept 2026)** — the universal gaps PRD-42 Appendix C listed as belonging to *everyone*, plus the first Tamil/Malayalam observances the engine can express. Universal — रथ सप्तमी (Magha S7; Surya Jayanti), रंग पंचमी (Chaitra K5; Indore's gair), शनि जयंती (Jyeshtha Amavasya, the Vat Savitri day), राधा अष्टमी (Bhadrapada S8 — Appendix C called it “the largest single universal gap left”), हनुमान जयंती · कार्तिक (Kartika K14; the Telugu–Tamil reckoning, a **second** rule beside the Chaitra one per §23.9), गोपाष्टमी (Kartika S8), चंपा षष्ठी (Margashirsha S6; Khandoba/Subrahmanya), सोमवती अमावस्या (the amavasya that lands on a Monday — 1–3 a year), विश्वकर्मा पूजा (Kanya Sankranti; a solar rule, not an alias, because the observance is its own). Tamil/Malayalam — कार्तिगई दीपम (Krittika in Vrischika), थै पूसम (Pushya in Makara), पंगुनि उत्तिरम (Uttara Phalguni in Meena), वैकासि विशाकम (Vishakha in Vrishabha), चित्रा पूर्णिमा (Purnima of the **Mesha solar month**, which is *not* Chaitra Purnima — 1 May vs 2 Apr in 2026), अवनि अविट्टम (Shravana Purnima; the upakarma), कर्किडक वावु (Amavasya in Karka; Kerala's tarpana day). Every one is pinned to a **published** civil date in `observanceDates.test.ts` — exactly, not ±1.

Two new mechanisms carry them, both narrow: a real **`nakshatra`** matcher (the day whose sunrise a nakshatra covers, with the same first-of-two and kshaya handling the tithi matcher uses), and two optional **constraints** on an existing rule — `solarMonth` (sidereal solar month, 0 = Mesha) and `weekday` on a lunar-tithi rule. Constraints rather than new rule types: they filter days a rule already claimed, so they can never move a rule off its tithi.

**Held back, and why.** **Onam**, **Thrissur Pooram** and **अट्टुकाल पोंगाल** are absent although the solver now exists: all three are Kerala pooram-class observances, and the two that can be checked resolve exactly one day late under the sunrise rule that is correct for all four Tamil rules (Thrissur Pooram 27 vs 26 Apr 2026; Attukal Pongala 4 vs 3 Mar). Two independent rules failing in the same direction is a **convention** difference, not a solver bug, and PRD-42 Open decision 6 says that belongs in a convention doc rather than being guessed at. **आडि पूरम** is held for a different reason: a solar month can contain Pooram twice and the almanacs take the second (14 Aug 2026, not 18 Jul), which `recurrence: 'annual'` cannot express — and sources themselves disagree, so it is a §23.9 two-rules case.

**Section A additions (27 rules, Sept 2026)** — found by a sweep of the standard pan-Hindu festival list (Drik, the Sri Mandir calendar) after a user asked why वराह जयंती was missing. All universal, all plain lunar-tithi rules, no engine work. Vishnu's avatars — मत्स्य (Chaitra S3), वराह (Bhadrapada S3), वामन (Bhadrapada S12), कल्कि (Shravana S6), हयग्रीव (Shravana S15) जयंती, and हलषष्ठी · ललही छठ (Bhadrapada K6, Balarama's day). Saints — सूरदास and आदि शंकराचार्य (both Vaishakha S5), तुलसीदास (Shravana S7, links to the Ramcharitmanas reader), वाल्मीकि (Ashvina S15, links to the Valmiki Ramayan reader), वल्लभाचार्य (Vaishakha K11), कबीर (Jyeshtha S15). Devi and sampradaya days — बगलामुखी, धूमावती, अन्नपूर्णा, नर्मदा, जानकी जयंती, महेश नवमी, फुलेरा दूज. Named days — नरक चतुर्दशी (now a card of its own, not only the दीपावली arc's gap label), काल भैरव जयंती, मौनी अमावस्या, गणेश जयंती (`madhyahna`), भीष्म अष्टमी (`madhyahna`), माघ पूर्णिमा, and both गुप्त नवरात्रि openings (Ashadha, Magha). **Varaha, Matsya, Kalki and Hayagriva carry `dayRule: 'aparahna'`**, because Drik fixes these days by an afternoon puja window. That matters in 2026 for two of them: Varaha is 13 Sep, not the sunrise 14th, and Hayagriva is 27 Aug, a day before Raksha Bandhan on the same Purnima. Each rule is pinned to a published date and to the sibling rule that shares its tithi (`observanceDates.test.ts`, SECTION_A_*). Sixteen forward-year dates (2027–2028) are pinned too (SECTION_A_FORWARD), and `VERIFY_YEARS=2025-2031 npm run verify:observances` re-derives every row independently through 2031 with 0 wrong-month results. The default year grows from 308 to 335 rows, about 0.08 more observances a day. **Held back:** कूर्म जयंती and छिन्नमस्ता जयंती (Drik keeps both at the evening/sayahna hour. In 2027 the tithi opens after 4 PM on the published day, so every day rule the engine has names the next day; they ship with the pradosh/sayahna rule, PRD-42 E-1), ललिता पंचमी (2025 published a day before the sunrise Panchami), शारदीय दुर्गा अष्टमी / महानवमी (published sources split 18/19 and 19/20 Oct 2026), वैकुंठ चतुर्दशी (nishita-vyapini, not modelled), बलराम जयंती (three dates across three traditions), गायत्री and रामानुज जयंती (contested tithi), and सर्वपितृ अमावस्या (already on the calendar through Pitru Paksha). **Enrichment (in progress):** गणेश जयंती now carries a full katha (`ganesh-jayanti-katha`, the king and Queen Ratnavali, plus a closing section that sets it apart from भाद्रपद's गणेश चतुर्थी and माघ कृष्ण's सकट चौथ) and its own til-gud bhog profile (`ganesh-jayanti-bhog`, kept separate from `ganesha-bhog` so the til offering doesn't show up on Ganesh Chaturthi). A second batch followed: नरक चतुर्दशी (`narak-chaturdashi-katha` — Narakasura's end and King Rantideva — plus `narak-chaturdashi-bhog`, which names Bengal's choddo shaak as regional), वराह and वामन जयंती (their own avatar kathas and bhog profiles: the kalash-and-coconut daan, the 52 pedas and curd-rice daan), मौनी अमावस्या (`mauni-amavasya-katha`, a mahatmya of the amrit kumbh, Manu and the silent snan; bhog reuses `pitru-offering`), and काल भैरव जयंती (reuses the Bhairava-origin `masik-kalashtami-katha` and `kalashtami-bhog`). The other 21 still carry only their short description; each gets katha and bhog as it is sourced.

**Sankranti day rule corrected.** `findSolarFestivalDate` returned the civil day whose *midnight followed* the ingress instant rather than the day *containing* it, so **all twelve sankrantis resolved one day late** — Makar Sankranti 2026 as 15 Jan against a published 14 Jan, Kanya Sankranti as 18 Sep against 17 Sep. Found because `vishwakarma-puja` rides Kanya Sankranti and inherited the shift. The loop now compares each day's midnight with the *next* day's and returns that day. Twelve shipped dates move one day earlier, onto their published dates; `CACHE_VERSION` 6 forces already-scanned cities to re-scan, and `observanceDates.test.ts` pins Makar/Mesha/Kanya against published almanacs plus the ascending-order invariant across all twelve.
...
**Area tags are display-only** (`panchang/observanceAreas.ts`). `OBSERVANCE_AREAS` maps a rule id to the regions its own description names; a lensed rule takes its lens as its area. A tag **never hides a rule**. Gangaur tagged `rajasthan` is still on every user's calendar, and the filter narrows only the list being viewed. Hiding stays the lens's job (§73, RULEBOOK §23a.5). The area ids are the 22 lens ids, so names come from the same lazily loaded registry. `observanceAreas.test.ts` checks that every tag names a real default-visible rule, that no tagged rule also carries a lens, that सर्वत्र plus the tagged rows adds up to the whole list, and that every chip it offers is non-empty.

**Observance Detail** (`ObservanceDetailScreen`) — hero (category pill + deity + a **क्षेत्र pill** for a tagged rule (1 pt `divider` outline, 11 pt `saffron-deep` area names; the tag row wraps and centres at phone width), name at 24 pt centred, other-language caption, and a `saffron-tint` "अगला · Next · date · in N days" pill), then an action row: **Follow** (outline `saffron` pill; fills `saffron` with `parchment` text when following — following also feeds vrat reminders, §38) and **॥ Read Katha** (filled `saffron`). Following shows a transient (3.5 s) `gold-tint` "Added to My Vrat — View →" bar. For rules inside a **festival arc** (PRD-28 — `ganesh-chaturthi`/`anant-chaturdashi`, `navratri-start`/`dussehra`, the four Diwali days) the **पर्व-अर्क strip** (§65.2) sits directly under the ask row; every other rule renders nothing there. Below: **महत्व · About** prose, a katha card, and — **last on the page** — the single **"How to observe" home** (PRD-09 Phase 4 §65 composed with PRD-19 Phase 2B), in four states keyed off a verified upvas entry (`upvasId` → `getUpvasInfo`, verified-only) and a resolving `vidhiId`: **(1) upvas only** → `उपवास विधि · How to observe` heading + the §65 fast-facts panel; **(2) vidhi only** → exactly the shipped `पूजा विधि · How to observe` block, unchanged — the same card shell as the katha card (`parchment-soft`, `divider`, `radii.lg`, `elevation.card`) with a saffron ॥ glyph, the vidhi's bilingual title, a `N चरण · लगभग M मिनट` meta line, and › — tap → `VidhiDetail` (§62) carrying the next occurrence's `dateMs` so the samagri checklist keys to the right festival date; **(3) both** → the `उपवास विधि` heading, fast facts first, and the vidhi card beneath them inside the same panel, its meta line prefixed `पूजा विधि · ` so the card keeps its own identity; **(4) neither** → no section at all. There is never a "coming soon" placeholder here — the parent PRD-09 §6.2 placeholder slot is retired.
...
**More responsiveness (Sept 2026).** Counts and navigation render without calendar calculations. `useMoreFamilySummary` runs only while MoreHome is focused: hydrate the shared tithi-only occurrence cache before solving, derive birth rules through the cooperative engine, deduplicate equal rules/dates, and request only the next occurrence. Cold scans share the background queue and cancel on blur, unmount or record changes. A timer around a synchronous annual scan is not sufficient. Optional dates may arrive after the count; failures keep the count usable. Language, reading-size and read-aloud sheets mount only while open. More stack destination screens use `getComponent` loaders so opening the hub does not evaluate every destination module; route names, parameters and back navigation remain unchanged.
...
**Purpose.** All notifications are **local and on-device** — scheduled with `expo-notifications` (plus the native alarm tiers of §35); no server push. Nine families, each owning an identifier prefix so cancel/re-arm cycles never touch each other's slots: daily verse (`daily-verse`), vrat reminders (`vrat-…`, PRD-09), muhurat reminders (`muhurat-reminder`, §60), festive reminders (`festive-reminder`), return reminders (`return-reminder`), sadhana reminders (`sadhana-reminder`, §46), routine reminders (`routine-reminder`, §45 — PRD-07 P3), Pitru Smaran / Pitru Paksha reminders (`pitru-smaran-reminder` / `pitru-paksha-reminder`, §63), and japam alarms. **One OS permission grant serves all of them** — the daily-verse and festive defaults request it at launch, the Pitru save flow and the routine-detail reminder toggle ask on their own opt-in moments, and the remaining schedulers only ever ride whatever the user already granted.
...
**Return reminders — वापसी स्मरण** (`notifications/returnReminderPure.ts` planner + copy, `returnScheduler.ts` glue, armed by the headless `<ReturnReminderScheduler>` in `App.tsx`). The "we miss you" family: a nudge that fires **only when the app has not been opened for a while**.

- **Mechanism, not memory.** Every app open (cold start and every return to the foreground) cancels the family and re-arms a ladder from today: slots at **3, 6, 9, 12 and 15 days** after the open (`RETURN_INTERVAL_DAYS = 3`, `RETURN_REMINDER_CAP = 5`), each at **08:00 local** (`RETURN_HOUR`/`RETURN_MINUTE` — after the 07:00 daily verse and the 07:30 festive slot so an absent user never gets two pushes in one instant). Nothing is persisted: the pending queue is the state, so a slot survives to fire only if the app was not opened after it was armed, and a user who opens the app daily never hears one. After five unanswered nudges (~two weeks) the ladder ends — the user has answered.
- **Default ON, no setting** (product decision, Sep 2026): the family is deliberately absent from Reminder Settings. It rides the shared OS permission grant only — it schedules when the grant is already `granted` and never prompts. The mute paths are the OS's: its **own Android channel `return-reminders`** (name "Weekday practice reminders", importance DEFAULT, `sound: 'default'`) so it can be silenced without touching the daily verse or festivals, and the app-wide notification switch on iOS. See RULEBOOK §6.2's posture on surfaces with no in-app off switch; a Settings toggle is the one-line mitigation if store review or feedback asks for it.
- **Day-specific copy, baked at schedule time.** Each slot carries the copy for the **weekday it lands on**, hand-authored in hi/en per weekday in `RETURN_REMINDER_COPY` and aligned with the routine builder's vaar map (`data/routine/vaar.ts`): title = `<वार> · <deity> का दिन` (`मंगलवार · हनुमान का दिन` / *Tuesday · Hanuman's day*), body = one invitation naming a bundled text for that deity (Sun Surya Ashtakam · Mon Rudrashtakam · Tue Hanuman Chalisa · Wed Ganesh Chalisa · Thu Vishnu Chalisa/Sahasranama · Fri Durga Chalisa · Sat Shani Ashtakam/Hanuman Chalisa). gu/kn re-script the Devanagari via `contentByLang` (§10). The reading language is an effect dep, so a language change re-arms.
- **A tap lands on Home** (deep-link table below), where the FOR TODAY row's tier 4 already leads with the same weekday deity's texts on an ordinary day (§50) — the named reading is one tap away without opening a reader.
- **Budget.** Five more pending slots on the shared iOS cap; the family is sized at 5 for exactly that reason and the cap must not grow without re-auditing the §38 sum.
- **Tests.** `notifications/__tests__/returnReminderPure.test.ts` (tsx): ladder shape (cap × interval, 08:00, calendar-day spacing, first slot never sooner than the interval), unique prefixed identifiers, weekday = fire day, seven-weekday copy that names the vaar deity, every named `sourceId` an active library entry tagged for that deity, gu/kn re-scripting; the deep link is covered in `deepLink.jest.test.tsx`. E2E deliberately none — a notification tap cannot be driven by Maestro and the family never appears in-app.

**Japam alarms** — see §35 for the scheduling tiers; they participate in deep-linking below.

**Remote push groundwork — device-id registration** (`notifications/pushTokenPure.ts` pure half + `pushToken.ts` glue, armed by the headless `<PushTokenRegistrar>` in `App.tsx`). Every family above is local and on-device; this is the one piece of *remote*-push plumbing, and it has **no visual surface at all** — no screen, no copy, no setting, no notification of its own. It registers this install's device id with the registry so a server can address a single install.
- **The device id is the Expo push token.** `ExponentPushToken[…]` is the address a server pushes to, and — since the app captures no phone number or account — the only handle it has on one install. No OS identifier is used: IDFV and `ANDROID_ID` both carry privacy-disclosure weight this needs none of. The token can *rotate* (reinstall, device restore, an FCM refresh); the once-only guard keys on the token value itself, so a rotated token is a new device id and registers afresh. A separate **install id** (`@vedansh/install-id`), derived once from the first token via the repo's FNV-1a hash, is ours alone — never sent — and serves only as the stable per-device seed for the retry-backoff jitter.
- **Additive by construction**, which is the whole design constraint. It **never prompts** (it reads `permissionState.ts` and stands down unless the grant already exists — the permission moment stays with §38's opt-in flow and §7's routine card), **never schedules or cancels** anything (so the shared iOS 64-pending budget is untouched), and **never throws or blocks** (every path returns a status; the work runs behind `InteractionManager`). Deleting `<PushTokenRegistrar />` restores the previous behaviour exactly.
- **No network call until an endpoint *and* an API key are both configured.** The endpoint is pinned in source (`PUSH_REGISTRY_ENDPOINT = https://api.incardible.in/api/mobile/devices`), overridable per build profile via `EXPO_PUBLIC_PUSH_REGISTRY_URL` (`resolvePushEndpoint`); either source must be `https:`. Registration additionally needs `EXPO_PUBLIC_DEVICE_REGISTRATION_API_KEY` — with no key the app is **capture-only**: the token is stored locally (`@vedansh/push-token`, readable via `getCapturedPushToken()`) but nothing leaves the device, so the plumbing is verifiable on a real device *before* the key is switched on. ⚠️ Because babel inlines `EXPO_PUBLIC_*` at build time the API key **ships inside the app bundle and is extractable** — keep it a low-privilege key scoped to device registration only; the read must stay the full literal, and Jest needs `expo/virtual/env` stubbed (`jest.config.js` `moduleNameMapper`) for the same reason `expo-speech` is.
- **Registered once, keyed on the device id.** The last successfully-registered token is stored (`@vedansh/push-token-sync`); `shouldRegisterDeviceId` skips the call entirely while it matches the current token, so a settled install costs no request. Only a rotation (a different token) registers again.
- **Fire-and-forget, with a bounded retry.** `PUSH_SYNC_MAX_ATTEMPTS = 3` per sync, each bounded by `PUSH_SYNC_TIMEOUT_MS = 8 s`, separated by a tripling backoff (`PUSH_SYNC_BASE_DELAY_MS = 2 s` → 6 s, clamped at 30 s) plus a **per-device jitter that is a hash of the install id, not a random number** — it still spreads a herd of devices retrying after an outage across a one-second window, but it is reproducible in a test and needs no entropy source. Worst case is ~32 s and nothing waits on it. The retry *policy* lives in the pure half (`runUploadWithRetry` takes both the request and the wait as parameters), so the whole schedule is asserted under `tsx --test` with a fake clock and no network.
- **The status code is the protocol** (`classifyUploadResponse`), because it decides what the client does next: **2xx** accepted (so only return it once the row is durable — the client then records the device id and stops calling); **5xx / 429 / 408 / 425** retried with the backoff above; **any other 4xx** abandoned for this attempt without a retry, since an identical body would be refused identically. Only an accepted call records the device id, so both a 4xx (e.g. a rejected key) and an exhausted retry leave nothing recorded and are re-attempted on a later foreground — a fix on either side needs no client release. Response bodies are ignored.
- **Re-arms** on the grant arriving, on every foreground, and on an OS token-rotation event (`addPushTokenListener`). Concurrent callers collapse onto one attempt.
- **Request is the minimum a server needs to address one install** — `POST` with `authorization: Bearer <key>` and body `{ "deviceIds": ["ExponentPushToken[…]"] }` (a one-element array; the shape is pinned by `pushTokenUpload.jest.test.ts`). Nothing from the private Smaran/Kundali/Namkaran records and no location, ever.
- **Not yet wired for delivery.** Receiving a remote push still needs a `type` in the `resolveNotificationTarget` table below — an unrecognised payload resolves to `null`, i.e. an ordinary Home launch — and an Android `channelId` on the send side. Registration is this app's first outbound request, so switching the key on is also a privacy-disclosure decision: it changes the store privacy label and the policy copy.
...
`RoutineShell` titled with the routine's name; right slot is a `saffron` `＋` → Add Content. For weekday routines, a read-only 7-column vaar grid tops the screen: each day chip shows its short label over the presiding-deity label (`deityLabelForWeekday`) in `saffron-deep`. Below the vaar grid (or first, for daily routines) sits the **स्मरण / Reminder card** (next subsection). Item rows: resolved title + sub (`resolveRoutineItem` — "अध्याय 1 / Chapter 1", "पूरा पाठ / Whole text", "N माला / N mala") plus the item's scheduled day shorts; tapping the row opens the item's reader (`navigateToRoutineItem`); a `×` removes it. Bottom: ghost "इस साधना को हटाएँ / Delete this routine" — deletion drops the routine from context and a focused-effect guard pops back exactly once (goBack in the handler double-popped; the comments document the bug).

### Routine Detail — स्मरण / Reminder card (PRD-07 Phase 3)

The per-routine reminder opt-in (the final PRD-07 phase; PRD `docs/roadmap/prds/07-routine-reminders-phase3.md`, prototype `docs/routine-reminders-prototype.html`). A `sectionLabel` eyebrow `स्मरण / Reminder` over a `parchment-soft` card (1 px `goldTint` border, `radii.lg`, `spacing.md` padding) between the weekday strip and the item list:

- **Toggle row** — title `दैनिक स्मरण / Daily reminder` (weekday routines: `साप्ताहिक स्मरण / Weekly reminder`) in the card-title face (`scriptTitleFont` at the `cardHindi` size), a state sub-line in `cardMeta`/`meaning` ("बंद — चालू करने पर समय चुनें / Off by default — turn on to pick a time" when off), and a `saffron`-track Switch (a11y "Toggle routine reminder"). **Off by default** — the presence of the `reminder` field on the routine record *is* the switch; no parallel boolean.
- **Turning ON is the permission moment** (never the scheduler's): `undetermined` → the shared `requestPermission()`; the reminder time persists **only after a grant** (a refusal leaves the toggle honestly off — the Pitru-Smaran honesty pattern). Effective status comes from `NotificationPreferencesContext` / `permissionState.ts`, so a never-asked Android install reads `undetermined`, not `denied`. On a **hard denial** (`canAskAgain: false`) the whole toggle row is replaced by the settings-path banner ("सूचनाएँ बंद हैं — Settings में सक्षम करें" → `Linking.openSettings()`), the §38 banner's second flavour.
- **When on**: a `TimeStepper` row (§52a control, same as Reminder Settings / Japam Alarms) above a 1 px `divider` hairline, defaulting to **07:00** — user-chosen, so no collision-offset games. Under it a centred weekday-aware caption at 12/18 (`meaning`/`cardMeta` face, `ink-muted`): daily → "प्रतिदिन इसी समय / Every day at this time"; weekday → "केवल सोम · शनि — इस साधना के दिनों पर / Only Mon · Sat — this sadhana's days" (the union of item weekdays via `WEEKDAY_LABELS[..].shortHi`/`short`); an **empty union** (no items yet) warns "कोई दिन निर्धारित नहीं — पहले सामग्री जोड़ें / No days scheduled — add content first" and schedules nothing.
- Scheduling itself is the §38 `routine-reminder` family: pure planner `notifications/routineReminderPure.ts` (weekday filter + `completedToday` suppression are planner inputs; `ROUTINE_REMINDER_CAP = 12`, `ROUTINE_WINDOW_DAYS = 7`), glue `routineScheduler.ts`, headless `<RoutineReminderScheduler>` in `App.tsx`. Copy: title `नित्य साधना स्मरण`, body `«nameHi» · आज की साधना` (`nameEn` fallback for a blank Hindi name); a tap lands on `HomeTab → RoutineToday`. Pinned by `.maestro/routine-reminder-smoke.yaml` (card presence only — flipping the toggle raises the native permission dialog) plus the tsx/Jest suites named in the flow's header.
...
**First-tap CPU contract (Sept 2026).** Deferring a solve until after paint is not sufficient: Home, its rolling warm-up, widget preparation and reminder re-arming share the JavaScript thread with presses. Cold day solves now yield inside tithi/nakshatra/karana and lunar-conjunction searches through `backgroundWork` (4 ms budget checked between units; native astronomy primitives are not pre-emptible). Synchronous APIs drive the same generators, preserving dates and end times. Public Pitru Paksha and opted-in Pitru/Janma reminder dates hydrate persisted answers before cooperative scans; muhurat reminders fill missing days cooperatively before grading. Cancellation prevents obsolete scans from publishing. Release-device first-tap latency remains a profiling gate; desktop timer probes are diagnostic only.
...
- Data comes from **one solve**: `useMuhurat(today, calendarSystem, { live: isFocused })` supplies both the muhurat windows **and** the day's `PanchangData` (cached, off the render path); observances ride the lighter `useObservancesForDate` (split out of `usePanchangForSelection` so the strip never pays for the upcoming-window resolution it doesn't render). The minute tick is enabled only while Home is focused and re-renders the live `prevailingTithi()` headline without re-solving astronomy; on refocus, the headline recomputes immediately. The civil date rolls over via **`useTodayKey()`** (`utils/useTodayKey.ts`): a timer just past local midnight plus an AppState foreground re-check, so an overnight-backgrounded app never shows yesterday's panchang. A mid-session observance-store upgrade re-resolves the chips **without** clearing them first (no blink; `useObservancesForDate` resets only when the day/city/system changes). The calendar system itself is a **module-level store** (`usePanchangCalendarSystem`), so a purnimant/amanta change on the Panchang tab propagates to the mounted Home strip immediately.
...
- Accessibility: single button, label `"Today's Panchang. {vara}, {live tithi}. {observances}. Tap to open."`; the spoken tithi changes with the visible headline.
...
**Detailed Daily Rashifal (September 2026).** The existing `Panchang → Jyotish → Open Daily Rashifal` route is retained. The landing's `JyotishGuidanceRows` remain fully visible and unchanged. On `RashifalScreen`, the existing back/share header, disclaimer, active-person chips, source card and twelve-sign picker remain in place. Three pill buttons below the sign picker select Yesterday / Today / Tomorrow (44 pt minimum, theme gold tint for the selected state). `useRashifalDay` supplies the selected date at the shared 06:00 IST anchor, refreshing at India midnight and foreground; the relative offset remains selected across rollover. All date-dependent guidance, personal context, practice and sharing use that date. Historical/future headings use “Daily Rashifal” rather than claiming today; share-card Hindi branding is “दैनिक राशिफल”.

The guidance block retains its sign/date and personal pills, then adds a calculated headline and short introduction. Favour / Pause / Reflect rows are available behind one initially collapsed button, exposing the original supporting chips. Immediately below, `RashifalLifeAreas` renders one parchment-soft bordered group of six expandable sections: relationships, work/studies, money/spending, wellbeing/routine, family/home and reflection/practice. Relationships opens first; opening another closes the previous one. Each contains bilingual authored interpretation contextualised by an actual transit, a practical reflection step on the existing gold-tint/left-rule treatment, any matching running-dasha note, and an expandable “Why this guidance?” explanation listing the relevant grahas, Moon houses and (natal selection only) Lagna houses. Mixed signals are labelled as balanced attention rather than hidden or converted into a score. A sign, day or active-person change resets the expanded content and closes any share preview.

Detailed prose uses `meaningToken(lang, typography)` so M/L reading size and all four scripts are respected; section titles use the existing script-aware title family and card-title size. Cards use `radii.lg`, existing dividers, parchment surfaces and saffron/gold accents. No new palette, new tab, alternate profile mode, timing card or luck score. The one `JyotishPracticeCard` follows the detailed group and uses the existing reader handoff. Its default Navagraha subtitle describes the prayer (नवग्रहों की स्तुति का पाठ / A prayer to the nine grahas), not its availability in the app. Public sharing receives a fresh **general** `computeRashifal` result for the selected day; personal notes, chart fields and detailed birth-derived context never enter the no-birth-details share card. Share preview titles stay short; the card itself displays the selected date.
...
- **आज का पंचांग — `VedanshPanchangWidget` / `VedanshPanchangWidgetProvider` (small · wide · large · lock; best at small).** Represented **IST** date + tithi headline + तक line + vrat line, carrying the selected city and calendar system. **Small** is the glance (date, tithi, तक, vrat, sunrise); **wide** adds Rahu Kaal on the timing line; **large** gives sunrise / Rahu Kaal / Abhijit a labelled row each from the PRD-14 (§48) engine. Tap → Panchang on the represented date.
  - **The headline is the tithi RUNNING NOW, and the widget turns over when it does (Sept 2026 fix).** The payload carried one tithi per civil day — the sunrise (udaya-vyapini) one — and both native readers drew it until midnight. But a tithi lasts ~20–27 h and so hands over *mid-day*: on 17 Sep 2026 in Bengaluru षष्ठी ended at 10:48 AM, and the placed widget still read षष्ठी that evening while Home's own glance (§48's `MuhuratGlanceCard`, which resolves `prevailingTithi` live) had long since read सप्तमी. Two halves fix it, and neither works without the other. **(1)** Every dated entry now carries `tithiSegments` — the day's tithi chain from `panchang/prevailingTithi.ts`'s `tithiChain`, link 0 always the sunrise tithi, each solved link with its instant and a rendered `तक 10:48 AM` line; the last link's end is omitted, never guessed, exactly where the app omits its own तक line. **(2)** Each platform refreshes ON those instants: iOS puts a timeline entry on every handover (WidgetKit draws a prebuilt timeline, not a live clock), Android schedules one `setAndAllowWhileIdle` wake-up per boundary — `updatePeriodMillis` is floored at 30 minutes and does not wake a dozing device, which is also why the card could sit on yesterday's date through a morning. All three readers (planner.ts `runningTithi`, Swift, Kotlin) run the same forward scan, so the widget and the app are one answer by construction.
...
**Architecture.** A versioned, bundle-only document — `WidgetPayloadV1` (`mobile/src/widgets/contract.ts`: `schemaVersion`, provenance, `locale`, a ~14-day IST Panchang window, a device-local verse window, and a Japam snapshot). A **pure planner** (`planner.ts` — no React/storage/network/wall-clock/native) builds dated entries, each Panchang entry carrying the day's `tithiSegments` chain so native can pick the running tithi and schedule its own refresh without doing astronomy; a **deferred coordinator** (`WidgetCoordinator.tsx`, mounted in `App.tsx` outside `NavigationContainer`) runs *after* `InteractionManager` settles — it dynamically `import()`s the planner so the Panchang graph never loads on Home's first-frame path — dedupes by a stable key over day/location/calendar/language/japam revision, throttles writes, atomically persists, then requests a native reload. Transport is the App Group file on iOS (`group.com.prashantsharma.vedansh.widgets`, `mobile/modules/home-widgets-ios/`) and a dedicated SharedPreferences payload on Android. Deep links use the `vedansh://widget/{verse|panchang|japam}` scheme (`deepLink.ts`), dispatched on cold + warm start from `App.tsx`.

**Preparation cache.** `widgets/planCache.ts` reads one persisted 14-day window before invoking astronomy. Its identity includes the running OTA/build, day-cache version, both civil start dates, coordinates/elevation, calendar system, device time zone and the displayed observance rows (so a location-scan upgrade replaces fallback labels). Language, labels and current Japam totals are always recomposed; no personal practice history is stored in this derived cache. Midnight, scope/build changes, malformed/truncated data or unreadable storage trigger a fresh plan. A cold plan makes 14 full day solves, reusing tomorrow's already-solved sunrise instead of making another 14 full solves. Calculations yield within engine searches; stale generations stop between units. `@vedansh:widget-plan:` joins the build-reset allowlist, while the existing native-write dedup key remains separate. The IST window never enters the app's device-local day store.
...
**Tests.** Pure/fixture suites under `mobile/src/widgets/__tests__/` — `contract` (schema round-trip; missing/corrupt/incompatible/expired rejection; all-four-languages required; dedup-key sensitivity), `catalog` (content↔size parity across the gallery, the Swift `supportedFamilies`, the Kotlin providers, the manifest receivers in `withHomeWidgets.js` and the `appwidget-provider`/layout resources they name — plus the verse-is-wide-first / Panchang-is-small-first rule this section exists for), `planner` (japa streak, >108, IST vs device-local boundaries, two-line determinism), `planPayload` (real 14-day payload, process-TZ isolation, and an hourly sweep pinning the payload's running tithi to the app's live `prevailingTithi` across the whole window), `deepLink` (verse/Panchang/japam parse + routing + cold-start retry), `startup` (coordinator does no static Panchang import; one iOS kind per content type; both native readers select the running tithi, validate the chain's ordering, and carry their platform's handover refresh). A committed fixture (`fixtures/widget-payload-v1.json`) is decoded by TypeScript, Swift, and Kotlin to pin cross-language parity. Maestro `mobile/.maestro/home-widgets-smoke.yaml` covers the More-row → gallery path and the three deep links. **Device-only gates** (PRD-15 §5/§8, not automatable here): EAS multi-target sign + physical-device install, per-size render/VoiceOver/large-text screenshots — now **every kind × every advertised size**, since each is a real placement a user can choose — and the per-kind Android pin flow.
...
**Data.** `mobile/src/data/vidhi/` — `types.ts` adds optional `anchor:'personal-tithi'` and a `gita` ref alongside katha/section refs. Festival entries retain `festivalIds` and deity tags; the personal entry deliberately has neither. **Verification boundary (PRD-28 Phase B, RULEBOOK §26):** `VidhiEntry.status?: 'draft' | 'verified'` — absent means verified (the seven PRD-19 entries). `ALL_VIDHI_ENTRIES` is the review/test surface; **`VIDHI_ENTRIES`, `VIDHI_BY_ID`, `getVidhiById` and `getVidhiForFestival` expose verified entries only**, so a draft is indistinguishable from no entry on the catalog, search rows, day-panel pill, Observance Detail card and the §65.2 arc door. The **visarjan family** — `ganesh-visarjan-uttar-puja` (hook `anant-chaturdashi`) and `durga-visarjan` (hook `dussehra`) — is authored **draft**: instruction-only, mantra-free, the permanent-vs-festival-murti boundary stated; it becomes visible with zero code change when its status flips after the two-source review. The catalog count (`VIDHI_ENTRIES.length`) therefore stays 7 until then. **Convention/source fields and `sourceUrl` are review-only and never render** — pinned across every page of all seven entries. Checklist + resume state remain under `@vedansh/vidhi-checklist`: samagri is keyed by vidhi + occurrence date and conduct progress by vidhi + civil day. Personal routes carry only `{vidhiId,dateMs}`; no remembered name, relation or entry id enters Vidhi state.
...
5. **Pitru Paksha overview** (`PitruPakshaOverviewScreen`) — title पितृ पक्ष `<year>`, then the **standing hero** (`pitru-paksha-standing`, Sept 2026 UX review): the screen answers *where today stands in the fortnight* before it answers *what the dates are*. One 21/30 title plus one pill, in three states — **during**: today's short tithi name (`heroHi`, the row label without its qualifier clause) over a `saffronTint` pill `दिन N / M · आज`; **before**: `पितृ पक्ष आरम्भ` over `N दिन शेष · <start>` (`कल से` at one day); **after the year has rolled**: `पितृ पक्ष <year+1>` over a *quiet* `goldTint` pill `अगले वर्ष · <start> से`, plus a second 12/18 `inkMuted` line naming the paksha that ended. The rolled year is never carried by a bare numeral in the title alone. Under it, the 12/18 `भाद्रपद कृष्ण पक्ष · <range>` line.
   Then **one strip** (`radii.md`, `parchmentSoft` on `divider`, 52 pt, 8 px dot): with family-matched days, `pitru-paksha-family-strip` — saffron dot, `आपके परिवार के N दिन` + the dates; tapping **opens** the first family day and scrolls to it (captured by `onLayout`) — one tap, not scroll-then-tap; with an **empty ledger**, `pitru-paksha-smaran-door` — gold dot, `अपने पितरों की तिथियाँ जोड़ें` → `PitruSmaranList`, the screen's only zero-entry door.
   Then the fortnight as §33.6 Upcoming rows. **This is the app's ONE fortnight surface** (Sept 2026): §74's verified tithi teachings ride these dated rows, because the fortnight used to be rendered twice — undated on the परिचय screen and dated here — and neither list knew about the other, which is what made the two screens read as unrelated.
   A row **opens where it sits** when it holds something — a verified `fortnightDay` teaching, a family match, or being today (which always opens, since it is the day the screen is about). Open row = a card (`pitru-paksha-today` for today, else `pitru-paksha-day-<key>`; 1 px `saffron` on `parchmentSoft`, `radii.md`): the short tithi name, `आज · <date>` or the date, **each family member as a 44 pt row** (`pitru-paksha-person-<entryId>`, `parchmentHighlight` on `divider`, `radii.sm`, `॥ <name>` 13.5/21 `saffronDeep` + `›`) that opens that person's own page (`PitruSmaranDetail {entryId}`) — a name on a day is a door, never dead text — the teaching paragraphs at 13/21 `inkSoft`, and **one** 44 pt `goldTint`/`gold` action — `किस दिन किसका ›` → `PitruParichayReader {lessonId:'kis-din-kiska'}`, which opens the §74 reader **on** the lesson that explains how a tithi is matched. The card carries **no guide button of its own**: the bar's filled door is the one guide door and follows the open day (below) — a dated copy on the card put the same door on screen twice whenever today was open (Sept 2026 review). Tapping the head closes it; only one day is open at a time. A day holding none of the three is not pressable at all — no chevron, no empty card — and its title reads `inkMuted`; an openable closed row carries a 17 pt `inkSoft` `›` (the shared `PanchangTimelineRow` gained optional `trailing` / `muted` props for this; its Panchang caller is unchanged). The card carries **no line on days no one is saved for**; silence is the absence, not a caption about it.
   **Both standing doors sit in a sticky 74 pt action bar** (`pitru-paksha-actions`, `parchmentHighlight` on `divider`, `elevation.lifted`, absolute bottom, 50 pt `radii.pill` buttons; scroll pads by `spacing.xxl + 74`) — the §73 `daan-home-actions` pattern: **परिचय** outlined `saffron`/`saffronDeep` (`pitru-paksha-shiksha-door`, rendered only when `hasPitruShiksha()` is true, never a teaser) and filled **॥ तिल-तर्पण** (`pitru-paksha-vidhi-door`, flex 1.3). They were a door above the table and a door below fifteen rows until the Sept 2026 review found neither was on screen while the days were being read. **The guide door follows the day in view**: it opens on the open day (and its label carries that date — `॥ तिल-तर्पण · 2 अक्टू`), else today while the paksha runs, else the first family-matched day, else the fortnight start (plain label). The public reminder tap therefore still lands education-first, calendar second, vidhi third with no deep-link change. No engine/device reassurance footer.
...
7. **Vrat & Parv catalog ledger bar, middle column** — the second of the three columns of the §33 ledger bar (`CatalogLedgerBar`), between मेरा व्रत and क्षेत्र: ॥ glyph, `पितृ स्मरण`, and one 11 pt state line — `अगला <short date>` off the soonest solved occurrence, `N स्मरण` while none has solved yet, `जोड़ें` while empty. Tap → the same More-stack remembrance list. It was a full-width pinned card until the bar replaced the three of them; the live count survives in the accessibility label (`Pitru Smaran. N entries`), which `PitruSmaranTouchpoints.test.ts` pins. The empty invitation is standing and cannot be dismissed, so the planning surface never loses its only zero-entry door.
...
**Privacy & non-goals.** Nothing syncs, nothing shares, no export from this surface; the entries ride the full backup (§75) like every other store. Public season notification copy is identical on every device; personal notification copy exists only after the user deliberately saves that person's entry and the OS notification grant succeeds. No Gregorian-anniversary mode, no gotra/genealogy fields.
...
### 65.2 पर्व-अर्क — the festival-arc strip (PRD-28)

**Purpose.** Answer the live question of a multi-day festival — *where am I, what is today, what is left* — and let the family record the one decision the calendar cannot make for them: how many days Ganesh stays. It also carries the arc's two hand-offs: PRD-23's Kanya Pujan bhog/grocery list a day before it is needed, and the visarjan vidhi door on the concluding day (verified-only, §62).

**Placement.** `ArcStrip` renders on **Observance Detail** (§33) directly under the ask row, for rules carrying `arcId` (`festivals.ts`; the shape lives in `panchang/arcs.ts` — `ganesh-utsav`, `sharad-navratri`, `deepavali`). It returns **null** for every other rule, so no other detail page changes. It is not a §33 "How to observe" state and never displaces the katha card or the §65/§65.1 blocks.

**Card.** `parchment-soft`, `divider` border, `radii.lg`, `elevation.card`, 14/12 padding, `marginTop` 14; a11y label `Festival arc <ArcNameEn>`.
- **Head row** — `पर्व-अर्क · Festival arc` eyebrow (`pillTextStyle`, `interSemiBold` 11, tracking 0.6, `saffron-deep`, 1.25 cap), the arc name (script title face 15, `ink`, one line), and — during the arc only — a `दिन N` chip (`saffronTint` pill, `interSemiBold` 11, `saffron-deep`, 1.15 cap).
- **Strip** — one column per *drawn* ordinal (`pickStripOrdinals`: first, last, every rule-bound or labelled day, today; a 22 pt `…` column between non-adjacent picks, so a 12-day Ganesh arc is `1 … 4 … 12`). Each column: a 26 pt circle — **future** `parchment` fill + `divider` ring, `ink-muted` numeral; **done** `gold` fill, `ink` numeral; **now** `saffron` fill, `onPrimary` numeral, a 4 pt `saffron` halo (shadow) — 2 pt connectors between (`gold` up to today, `divider` after), and a two-line 10/13 label beneath (script body face, `ink-muted`; `saffron-deep` on today; `आज` when today has no label). Numerals are Devanagari in hi (gu/kn re-script), Latin in en. Rule-bound columns other than the page's own rule are `Pressable`s (a11y `<LabelEn>, day N[, today]`, ≥ 44 pt tall) that **push** that day's `ObservanceDetail`; the current rule's own slot and gap days are plain.
- **Status line** — centred 12.5/18 `ink-soft`: before → `प्रारम्भ · <vara> · <d Mon> · N दिन में`; during → `आज दिन N / M · K दिन शेष · विसर्जन <vara> · <d Mon>` (an **open** chooser arc says `आज दिन N` alone — no total, no presumed end); on the concluding day → `आज विसर्जन`. Dates via `formatShortDate` + `VARA_NAMES_*`.

**Duration chooser** (chooser arcs, the **sthapana rule's** page only, hidden once the arc is over) — a `divider`-topped section: `कितने दिन विराजेंगे?` (title face 15) + the stance note (`meaning` face 12/18, `ink-muted`: *…जो आपके घर की परम्परा है, वही चुनें। ऐप केवल तिथि निकालता है, सुझाव नहीं देता।*), then a wrap grid of six ≥ 44 pt tiles (`radii.md`, 30 % basis): `डेढ़ दिन · ३ दिन · ५ दिन · ७ दिन · १० दिन` (the last with a 10 pt `अनन्त चतुर्दशी` sub-line) and `बाद में`. Unselected: transparent on a `goldTint` border, `ink` text; selected: `saffron` fill, `onPrimary` text, `elevation.raised`. **Nothing is selected by default** — the choice is the family's; tapping the selected tile or `बाद में` clears it (`clearArcChoice`). With a choice: a `goldTint` fact box (`radii.md`) — `स्थापना` / `आपका विसर्जन` (`saffron-deep`) rows as `<vara> · <d Mon>` — and, only when notification permission is granted, the 11/16 `ink-muted` reminder fact (*एक दिन पहले सायं 6 बजे और उस दिन प्रातः 7 बजे*). The choice persists per arc per sthapana date at `@vedansh/parv-arc` (`panchang/arcChoiceStore.ts`, the §62 checklist-store shape).

**Preparation hand-off** (`arc.prepare`; Navratri from three days before Vijayadashami to its eve) — a ≥ 48 pt row on `goldTint` with a `gold` border, `radii.md`: saffron-deep ॥ glyph, `कन्या पूजन की तैयारी` (title face 14) over the note (12/17 `ink-soft`: *अष्टमी या नवमी — परिवार की परम्परा अनुसार। भोग सूची और रसोई की खरीदारी तैयार है।*), `›`. Tap → `VidhiDetail {vidhiId:'navratri-ghatasthapana', dateMs: sthapana}` — the तैयारी tab already shows the PRD-23 bhog panel + grocery ledger (§65.1), keyed to the sthapana date so checks persist through the arc.

**Visarjan vidhi door** (eve and day of the arc's visarjan slot, only when `getVidhiById(arc.visarjanVidhiId)` resolves — a draft is absent) — the same row on `parchment`/`divider`: saffron ॥, the vidhi title, `N चरण · लगभग M मिनट`, `›` → `VidhiDetail {vidhiId, dateMs: visarjan}`.

**Stance and degradation.** Offered set only, no default, no recommendation copy, no modal, no push. Never chosen ⇒ the strip shows sthapana + the chooser and an honest day count, no visarjan slot, no reminder (the vrat-family input list simply has no arc entry). Diwali and Navratri are calendar-fixed and show no chooser. Ordinals and totals come from resolved **dates** (Navratri 2026 is 11 civil days; the ten-day Ganesh arc of 2026 is 12) — the strip never prints the customary count as fact.

**Localisation.** All copy through `contentByLang` / `meaningByLang` (gu/kn derive); dense chrome capped at 1.15/1.25 (§3.0). No emoji — ॥ and `…` only (§5).

**Files.** `mobile/src/panchang/arcs.ts` (pure: definitions, solver, occurrence builder, `arcDayFor`, `prepareActive`, `visarjanReminderInputs`) · `mobile/src/panchang/arcChoiceStore.ts` + `useArcChoices.ts` · `mobile/src/components/ArcStrip.tsx` · the strip in `mobile/src/screens/ObservanceDetailScreen.tsx` · arc fields in `mobile/src/panchang/{types,festivals}.ts` · the visarjan inputs in `mobile/src/components/VratReminderScheduler.tsx` + the `titleHi` override in `mobile/src/notifications/vratReminderPure.ts`. Tests: `src/panchang/__tests__/arcs.test.ts` (tsx), `src/notifications/__tests__/vratReminderPure.test.ts` (tsx), `src/panchang/__tests__/jest/arcChoiceStore.jest.test.ts`, `src/components/__tests__/ArcStrip.test.tsx`, `src/screens/__tests__/ObservanceDetailScreen.test.tsx` (Jest); `.maestro/parv-arc-smoke.yaml` (e2e). PRD: `docs/roadmap/prds/28-parv-arc.md`.
...
## 68. Compiled Kundali Report (पूर्ण कुंडली विवेचन) — PRD-20 Phase 6, rebuilt by PRD-43

**Purpose.** The one-shot long reading compiled from the saved chart. Everything is composed from typed phrase tables in a pure engine into a **versioned, fully serializable `KundaliReportModel` v2** (plain JSON, no `Date` instances — pinned by a serde round-trip test; deliberately the grounding object a future AI phase would consume, PRD-20 §5). Since PRD-43 the reading moves **placement → combination → indication**, and every interpretive section carries the `basis` chain it was derived from (RULEBOOK §14.3.1), rendered by `BasisChain` (§72).

**Structure.** One **as-of stamp line** at the top (`As of 14 Sep 2026 · traditional guidance, not a certain prediction · every section shows its basis` — 11 pt `inkMuted`, script body face; this DATES the report and frames it once) → North Indian chart card → **thirteen** section cards in fixed order, ids stable across age bands:
`snapshot` (**आपकी कुंडली, एक नज़र में · Your Kundali in sixty seconds**; for a minor `<name> की कुंडली · <name>'s Kundali in sixty seconds` — 5–7 label:value rows, each a PROJECTION of a later section; the fact id IS the source section id, pinned by test; never a new claim. Rows since the September 2026 child-chart review: `lagna` (`Kanya Lagna · lord Mercury in the 3rd bhava`) · `moon` (`Meena Moon · Revati 3 · 7th bhava`) · `summary` = **उल्लेखनीय स्थितियाँ · Notable placements** (`notablePlacements()`: up to three grahas in own/exalted sign or seated in a trikona/kendra, ranked exalted → own → trikona → kendra, ties by `GRAHA_ORDER`, nodes skipped — e.g. `Saturn own sign in the 6th bhava · Jupiter in the 9th bhava · Venus in the 5th bhava`; this REPLACED the "Key combination" row, which for most charts was only "Lagna lord in a neutral house") · `vimshottari` = **चल रही दशा · Running period**, `until` the running **Antardasha's** end (`Mercury Mahadasha · Rahu Antardasha · until 14 May 2028` — it used to print the Mahadasha end, five years late) · `observations` (the transit headline · as of) · for a minor `career` = **बाल-विवेचन · Child reading** (`Age 1 y 9 m · learning · communication · creativity · social development — for a parent to observe`; 13–17 **किशोर-विवेचन · Teen reading**)) · `summary` (birth facts + **age as of this date** + the `notable-placements` fact the snapshot projects) · `lagna` · `moon` (names the Moon's bhava; under 13 a third paragraph: *At this age, treat these qualities only as something to observe — a child's nature is still forming*) · `combinations` (**ग्रह एक साथ क्या कहते हैं · What the placements say together** — ≤6 lines from `kundaliYoga.ts`: the Lagna lord's seat always first, then same-sign groups, allow-listed yogas, dignities, ranked by weight then id. **A same-sign group is titled `<A> and <B> in the same bhava · Nth bhava` (fact label एक भाव में · Same bhava), never "together"**, and its body always states the degree gap — `about 15°00′ apart — the whole-sign reading places them in one bhava; this is not a tight conjunction` or, within `CLOSE_CONJUNCTION_ORB_DEGREES` (10°), `within about 2°15′ of each other — a close conjunction by degree as well as by sign` — plus `<planet> is retrograde.` for any retrograde planet (never the nodes). Budhaditya and Chandra-Mangal are named **`Sun–Mercury association · Budhaditya yoga (traditional same-sign rule)`** / **`Moon–Mars association · Chandra-Mangal yoga (traditional same-sign rule)`**, carry the same gap sentence, and say the closer pair reads stronger — an association, not a yoga of equal strength in every chart. Under 13 every same-bhava line closes *At this age, treat this only as something to observe over time — not a fixed trait* and every yoga line *…something to watch, not a settled trait*; the Lagna-lord and dignity lines are untouched) · six life areas (**age-aware, ids stable**: for an adult `career` 10 · `relationships` 7 · `wealth` 2/11 · `wellbeing` 1/6 · `learning` 4/5 · `dharma` 9; for 13–17 `career` becomes *सीख, प्रतिभा और आगे की दिशा · Learning, talents and future direction* (10/5) and `relationships` becomes *मित्रता, सहकार्य और सामाजिक स्वभाव · Friends, teamwork and social nature* (7/3/11); under 13 the register is parent-facing throughout; each area closes with a REGISTER line — what to do with the section — never a disclaimer) · `observations` (Sade Sati always for 13+, **prefixed `As of <date>:`**, followed by Saturn's next sign-change date or an explicit "dated, not permanent" line; **under 13 the card is de-emphasised to `वर्तमान शनि गोचर · Current Saturn transit`** — fact `saturn-transit` `Saturn over <name>'s Moon sign` / `in the Nth bhava from the Moon sign`, body *traditional Jyotish places Saturn over <name>'s Moon sign at present (an adult reading would call this a Sade Sati period). For a child, no adult reading is attached to this transit — the report simply records the classical transit position*, no phase headline, no `practiceSourceId`; the basis node is the same gochar node; **Mangal Dosha display-gated off**; **Kaal Sarp excluded by decision**, pinned absent) · `vimshottari` (**विम्शोत्तरी दशा — तिथियों में जीवन-यात्रा · a dated life view**: three composed Maha×Antar pair paragraphs from `buildDashaPairReading` — both date ranges, the lords' naisargika relation, the Antar lord's natal house; **under 13 (`{ band, subjectName }`) the three paragraphs switch to the parent's observe register from `DASHA_LORD_CHILD_HI/EN`** — *In the traditional reading, Mercury highlights learning and communication. At this age, parents can simply observe how <name> responds to sounds, words, picture books, imitation and exploratory play* · the relation line ends *For a child this is a classical note, not a forecast* · the Antar seat line records the house and names only what a parent may notice, never "<lord>'s colour is on relationships just now"; Hindi observe lines are built around `बच्चे का/की/के + noun` so the verb agrees with the noun, never with the child (gender unknown; the name replaces the oblique `बच्चे`) — then nine period lines that lead with **dates** (`4 Jul 2029 → 4 Jul 2046 (ages 15–32)`), the period running at birth labelled **`active at birth → <end> (until age 15 y 11 m) · balance at birth 16 y 2 m of the full 19 y`**, current flagged; under 13 the current line carries the observe register and the other eight the three-word `tradition: …` shade instead of the adult theme sentence; facts carry both current date ranges; **every dasha basis node is an IST `indiaDateKey`** — the pair reading used to slice UTC and put `2016-04-28` under a `29 Apr 2016` paragraph) → **the full disclaimer exactly once**, at the foot. **Address:** an adult reads *your chart / Your Moon*; a minor's report says `<name>'s chart` (or *the child's chart*) everywhere — the summary line, Lagna, Moon, every life area — never the second person. Section cards carry one full accessibility label each; a practice link renders only from the allow-listed `practiceSourceId`.

**Grammar.** Bhava ordinals come from `reportFormat.ts` — `1st/2nd/3rd … 12th bhava` in English, the classical **प्रथम … द्वादश भाव** in Hindi (never a bare digit before भाव); the `\b[123]th bhava` regression is pinned across all twelve lagnas in the engine test and the Jest suite.

**Sharing.** Unchanged in shape: one header Share → the warned Kundali 4:5 card + the `पूर्ण पाठ साझा करें` full-text handoff (`kundaliHandoff.ts`). The export now carries the as-of line and the subject's age in its header and a **`Basis: bhava → lord → …` line under every interpretive section**, so a reader (or an AI assistant) can audit each sentence; the JSON tail is the v2 model.

**Files.** `mobile/src/panchang/kundaliReportModel.ts` (v2 types), `kundaliReport.ts` (engine), `kundaliBasis.ts` (`BasisNode`, dignity, naisargika maitri, age band, chip labels), `kundaliYoga.ts` (combinations + `YOGA_DEFINITIONS`), `dashaReading.ts` (`buildDashaPairReading`), `reportFormat.ts` (ordinals, IST dates, age spans), `kundaliHandoff.ts`; `mobile/src/components/BasisChain.tsx`; `mobile/src/screens/KundaliReportScreen.tsx`. Tests: `kundaliReport.engine.test.ts` (order + all-12-lagna sweep, ordinal sweep, single disclaimer, dated dasha + balance at birth, as-of stamp, snapshot traceability, age-aware ids, and the September 2026 child-chart fixture — Antardasha `until`, one IST key per boundary, same-bhava + degree gap + Budhaditya nuance, the under-13 transit card, the observe register, no second person, notable placements), `kundaliBasis.test.ts`, `kundaliYoga.engine.test.ts` (detectors on a seated chart, cap, determinism, no false `verified`), `reportFormat.test.ts`, `kundaliHandoff.engine.test.ts`, `KundaliReportExperience.test.tsx`, `.maestro/kundali-report-smoke.yaml`.
...
## 70. कुल परम्परा — Janma Tithi & the Family Record (PRD-29)

**Purpose.** The record-keeping inverse of §63: the app kept the tithis of the dead with real care and not one tithi of the living. Part A surfaces every saved birth profile's **जन्म तिथि** — the Hindu birthday is a tithi, not a Gregorian date — with this year's solved date, one opt-in reminder, and the day's traditional practice pointing only at shipped sections. Part B is the one private **कुल परम्परा** record — kuldevta/kuldevi, family temple, gotra, the family's kept observance, free-text notes — and the export that is its point. Stance, structural: every field is *chosen, never inferred* (no gotra→kuldevta mapping exists in the binary, no caste/community classification, no directory of families); the birthday stays **devotional, never social** — no greeting card, no share prompt, no age arithmetic. Nothing leaves the device except through the explicit export door.

**Engine.** Zero new solves: `janmaTithi.ts` derives the rule via `deriveTithiRuleFromDate` (the sunrise tithi of the birth civil date — udaya-vyapini, Ujjain/purnimant, the identical §63 convention, stated in words on the detail screen), dates it via `solveNextOccurrence`, and matches days via the new exported `tithiRuleMatchesDate` (the bare matcher — `entryMatchesDate` would also fire on the Pitru-Paksha mapped day, which is the dead's and wrong for the living). Answers persist through the SAME `pitruSmaranSolves` layer (tithi-keyed, person-free); the warm-first/hydrate-immediately/solve-deferred reading rules of §63 apply verbatim (`useJanmaTithi.ts`).

**जन्म तिथि list (`screens/JanmaTithiListScreen.tsx`).** Two §33 ObservanceList row sections: **जीवित · जन्म तिथि** — one row per roster person (✦ lead glyph in the §63 muted circle; title = `personLabel` — name or birth date, never an invented placeholder; caption = rule in words; right column = this year's date + relative label) opening the detail; **पितृ · स्मरण तिथि** — the saved §63 entries as quiet rows into the shipped `PitruSmaranDetail`. The empty state explains that the tithi comes from saved birth details and offers ONE door to the Kundali screen — the roster is never editable here (RULEBOOK §14.5: one store, one owner). No engine or device-status footer.

**Person detail (`screens/JanmaTithiDetailScreen.tsx`).** §33 detail-hero pattern (✦ ॐ ✦ leading, `personLabel` title 24 pt, tithi caption, the saffron-tint `इस वर्ष · date · in N days` pill, published warm-first before the rows below). Information rows (§63 anatomy): जन्म (date · time · city, with the sunrise-convention note as an 11 pt italic second line), अगले वर्ष, जन्म नक्षत्र (the Moon nakshatra at the IST birth instant — the shared Lahiri primitive). Then the **स्मरण toggle** — default OFF per person, persisted in `@vedansh:janma-tithi:v1` only after the shared OS grant succeeds (a refusal leaves it honestly off; the §63 save-flow rule). Then **इस दिन की परम्परा** rows, shipped sections only: विष्णु सहस्रनाम (the dirghayu paath) plus up to two of the kuldev's own texts when the §Part-B record names a registry deity — all through `buildEntryStartTarget`, never a hand-rolled route. Birth details stay Kundali's to edit: the header's सम्पादन cross-tabs to the Kundali screen via `panchangTabTarget`.

**Home Today chip (`components/JanmaTithiDayChip.tsx`).** `✦ जन्म तिथि — <label>` beside the §63 chip in the Today strip's row, same muted-gold register (`goldTint` fill, `gold` border, `inkSoft` text — NEVER festive saffron), one per matching person, resolved off the render path (`useJanmaTithiForDate`, the `usePitruSmaranForDate` deferral). Tap → that person's detail via `moreTabTarget`.

**Reminder (eighth local family).** `notifications/janmaTithiReminderPure.ts` (pure planner) + `janmaTithiScheduler.ts` + headless `<JanmaTithiReminderScheduler>`: prefix `janma-tithi-reminder`, own Android channel `janma-tithi-reminders`, **one notice per person per year** — the eve at 18:00 (`कल <label> की जन्म तिथि है · <tithi>`; no day-of slot — the chip owns the day), `JANMA_TITHI_REMINDER_CAP = 8` soonest-first (dated one-shots have no user ranking; the roster caps at 8 people, so worst case = 8 pending against the shared iOS budget). Payload carries only the person id + occurrence key; tap deep-links to the detail.

**कुल परम्परा record (`screens/KulParampara{,'Edit','Export'}Screen.tsx`, store `panchang/kulParampara{,'Store','Share'}.ts`).** One record per device at `@vedansh:kul-parampara:v1` (versioned payload, ids validated against their registries on parse — a retired id degrades to the free-text half, never a crash). View: kuldev hero card (registry `DeityIcon` glyph or the family's own word), then §63-anatomy fields — कुल मन्दिर (chip `तीर्थ सूची से · <city>` when linked; row opens `TheerthDetail`), गोत्र, कुल व्रत (chip shows the next solved date via the festival engine's precomputed table; row opens `ObservanceDetail`), परिवार की बात — then the `आगे सौंपें` door. Edit: kuldevi/kuldevta pill pair, the 21-deity glyph grid (72 pt cells, selection = `saffronTint` + `saffronDeep` ring; choosing a glyph clears the free-text name and vice versa), registry search inputs for temple and vrat with free text as a first-class fallback, gotra and notes as plain inputs; the stance-guard line closes the screen. Export: a summary card listing EXACTLY what is shared; introductory device metaphors and OS/cloud/server footers are absent. The content summary names kul fields · janma tithis count · pitru tithis count, followed by one साझा करें action → versioned JSON (`format: 'vedansh-kul-parampara', version: 1`, display strings denormalized beside ids) written to cache and handed to the OS share sheet (`expo-sharing` + `expo-file-system`, the §46 share mechanics). PRD-06's full backup (§75) has since landed and carries the same record at `@vedansh:kul-parampara:v1` alongside every other store; this narrower hand-on file stays as the *give-it-to-a-relative* path (a full backup would hand them your bookmarks too) and is recognised by the restore screen, which names it rather than rejecting it as garbage. Import of the hand-on file itself still waits for a dedicated importer.

**Doors.** More hub साधना group, after पितृ स्मरण: **जन्म तिथि** (`more-janma-tithi`, state = NEW while the roster is empty, else `count · soonest date`) and **कुल परम्परा** (`more-kul-parampara`, state = the saved kuldev's name, else NEW) — §37 lists both.

**Files.** `mobile/src/panchang/janmaTithi.ts` · `janmaTithiPrefs.ts` · `useJanmaTithi.ts` · `kulParampara.ts` · `kulParamparaStore.ts` · `kulParamparaShare.ts` · `pitruSmaran.ts` (`tithiRuleMatchesDate`) · `components/JanmaTithiDayChip.tsx` + `JanmaTithiReminderScheduler.tsx` · `notifications/janmaTithiReminderPure.ts` + `janmaTithiScheduler.ts` + `deepLink.ts` · `screens/JanmaTithi{List,Detail}Screen.tsx` + `screens/KulParampara{,Edit,Export}Screen.tsx` · doors in `screens/MoreScreen.tsx`, chip in `components/TodayStrip.tsx` · routes in `navigation/{types,MoreStackNavigator}.tsx`. Tests: `panchang/__tests__/{janmaTithi,kulParampara}.test.ts` (tsx), `notifications/__tests__/janmaTithiReminderPure.test.ts` (tsx), `panchang/__tests__/jest/{kulParamparaStore,janmaTithiPrefs}.jest.test.ts`, `screens/__tests__/{JanmaTithiScreens,KulParamparaScreens}.test.tsx`; device path `.maestro/kul-parampara-smoke.yaml`. See `docs/roadmap/prds/29-kul-parampara.md`.

## 71. जिज्ञासा — Ask Vedansh: answer-first search, आज का विधान, answers that act (PRD-41)
...
**Answer card (`components/AskAnswerCard.tsx`).** The §8 active-card gradient (`cardActiveFrom/To`, `cardActiveBorder`, `radii.lg`, `elevation.card`). Structure: a `goldChipBg` eyebrow **tag** (`eyebrowTextStyle` 10.5) · the **headline** in the language's title face, 19/28 · optional italic **sub** · **label · value rows** (78 pt micro label, value 14.5/22; a `tone: 'avoid'` row — the निषेध half — takes `avoid` / `avoidDeep`, the §65.1 split) · a hairline-ruled **`▸ गणना देखें / Show the working`** toggle revealing the answer's plain-language **basis** (`AskAnswer.basis`, localized, body face 11/17 muted; collapsed by default) — the raw engine-call `working` trail (`computeMuhuratDay(…)`, rule ids) is never shown to users: it renders only in `__DEV__` builds when an answer has no `basis`, and in release the toggle is hidden for such answers · the registry's own **tradition note** (12.5/19 muted) and a one-line **provenance** (11/16, never a URL) · up to **three actions**, the first a filled `saffron` pill with `onPrimary` text, the rest outlined in `cardActiveBorder`. All copy flows `contentByLang`, so gu/kn derive from the Devanagari half. `compact` tightens padding for the briefing.

**Abstain card (`AskAbstainCard`).** Deliberately as designed as the answer: `parchment-soft`, dashed `cardActiveBorder`. Two registers — an ordinary miss (*इसका उत्तर अभी नहीं दे सकते।* + "here's what the library found" / "the library found nothing either") with **did-you-mean chips** (`saffronTint` pills naming an intent's example question, tap re-asks), and the **stance-guard decline** (*यह प्रश्न भविष्य या व्यक्तिगत निर्णय का है — वेदांश़ इसका उत्तर नहीं देता।* + a redirect to what it does answer) with no chips. The ordinary-miss card appears only when the query is **question-shaped** (`looksLikeQuestion`: interrogatives, a trailing "?", or a domain word like मुहूर्त / भोग / दिशा) — a plain library query renders exactly as 1.4.6. The **decline** renders whenever the engine takes that stance, even past the question-shape gate: `DECLINE_LEXEMES` only match predictive/personal framings ("will i get the job" carries no interrogative lexeme), so a plain library query still can never see it.

**Search box (`screens/SearchScreen.tsx`).** Same field, same position. `useAsk()` loads the engine on mount (one dynamic import, warmed) so the first keystroke resolves; the resolution is memoised per trimmed query and re-runs live per keystroke (~0.3 ms V8 / ~1.3 ms Hermes-derated per resolve on a 2,900-form lexicon; no debounce needed). The answer or abstain card is the results list's `ListHeaderComponent`; when an ask header owns the space the zero state is not drawn. The placeholder **rotates through real answerable questions** every 3.2 s while idle — the feature's own discovery surface — and stops the moment the user types. An action tap commits the query to Recent, dismisses the keyboard and routes through `ask/actions.ts` (Home targets push in place; Panchang/More targets carry `initial: false` via the §60 `panchangTabTarget` / `moreTabTarget` helpers). Route params: `Search: { seed?, initialQuery? }` — an arriving `seed` (ask-from-context, §71 Phase 3) clears any previous query, since the hand-off `navigate()`s to the existing Search instance and the stale text would otherwise prefix the follow-up.

**आज का विधान (`screens/TodayVidhanScreen.tsx`, Home stack `TodayVidhan`).** The Phase 2 briefing is **standing questions asked on the user's behalf through the same resolver** (`ask/briefing.ts`): *आज* (panchang.day) · *आज का व्रत / पर्व* — the observance being kept, seeded with today's rule — or *आगे आ रहा है* — the next within 7 days · *शुभ-अशुभ समय* (muhurat.now) · *आपका संकल्प* (sadhana.progress, only when one is running). Each section is an eyebrow (11, tracked) over a `compact` AskAnswerCard. `ReaderHeader variant="index"`, an italic lede, and a closing *कुछ और पूछें* row into Search. The briefing module is a dynamic import composed after `InteractionManager` — pushing the screen never blocks a frame, and its panchang solves come from the shared day store the Today strip has usually warmed. Discovery: a Home DISCOVER `FeatureCard` (`jijnasa`, "?" glyph, NEW) — the §32 awareness-door convention — rather than a change to the Today strip, whose fold budget §50 records as contested.

**Answers that act / ask-from-context.** Every answer's actions are typed `AskTarget`s (ObservanceDetail, MuhuratDetail, MuhuratResults prefilled with the occasion, VidhiDetail with the next date, VratKathaReader, JapamCounter preset to the mantra, VastuDisha, PanchangHome on the day, SadhanaProgramDetail). `ObservanceDetailScreen` carries an *इस व्रत के बारे में पूछें* row beneath the Follow / Katha actions that opens Search **seeded** with that rule, so "iska bhog kya hai" / "kaise kare" resolve without the user naming it again.

**Stance guards (pinned).** No LLM, no network, no conversation; no prediction or personal advice (`declined`); no draft content; no luck score, dosha alarm or remedy upsell (§51 carries over); the library search never regresses (§3.6 of the PRD). Voice input (Phase 4) is a native STT dependency and therefore a store release — not shipped here.

**Deferred with reasons.** The briefing's widget variant (native extension bridge → store release) and a notification family (needs a decision on the shared iOS pending budget, §38).

**Files.** `mobile/src/ask/{types,fold,aliases,lexicon,resolve,engine,useAsk,actions,briefing}.ts` · `mobile/src/ask/intents/index.ts` · `components/AskAnswerCard.tsx` · `screens/{SearchScreen,TodayVidhanScreen,ObservanceDetailScreen,HomeScreen}.tsx` · `navigation/{types,HomeStackNavigator}.tsx`. Tests: `src/ask/__tests__/{fold,lexicon,resolve,corpus,briefing,launchPath}.test.ts` (tsx, `npm run test:ask`; the golden corpus gates ≥ 85% top-1 and zero wrong answers — 199/199 at ship), `components/__tests__/AskAnswerCard.test.tsx` (Jest); device paths `.maestro/ask-answer-smoke.yaml`, `ask-abstain-smoke.yaml`, `ask-briefing-smoke.yaml`. See RULEBOOK §25, `docs/roadmap/prds/41-jijnasa-ask-vedansh.md`, and the clickable prototype `docs/jijnasa-ask-prototype.html`.


---

## 73. दान-पुण्य — Educate-First Giving: Journey, Ledger & Daan-Dwaar (PRD-26)

**Purpose.** The third limb of the japa–vrat–daan tripod: *educate first* (why the tradition gives, in the shastra's own words), a private on-device ledger second, and an external hand-off to verified organizations strictly last. The IA contract (PRD-26 §2.7, **relaxed**) makes *educate* the DEFAULT, not a hard gate: the home carries **two standing doors in a sticky action bar** — a filled `दान करें` button and an outlined `दान-द्वार` button, both present every day and one tap away at every scroll position — so giving is never buried without the home ever becoming a pitch (the content above the bar is what opens first; the bar is a floor, not a wall). The September 2026 UX review replaced the earlier centred door block (a small floating pill over a 13pt text link with a ~26pt tap height, closed by an 11.5pt gold line at ~3.1:1) with this bar, and made the verse spine and the kathas horizontal so the home is one scroll, not three. The directory is still never a tab; the journey's terminal actions are not step-gated but sit at the end of one skippable scroll, with a quiet skip affordance near the top for the already-informed, and "record in my register" still leads "give elsewhere". **The app still never transacts** (Resolution A, links-only): every hand-off is `Linking.openURL` to the organization's own site behind the honest interstitial, and the ledger is never gated. No punya scores, streaks, totals, goals or fear copy anywhere — pinned by the copy-guard test.

**दान-पुण्य home (`screens/DaanPunyaScreen.tsx`).** ReaderHeader (index) whose **right slot is the खाता door** (`daan-ledger-door`, a 44pt `parchmentSoft`/`divider` circle carrying a 20pt `saffronDeep` stroke ledger glyph in `react-native-svg`, a11y label `Daan ledger`) → `DaanLedger`; ungated, in reach from the first frame. Under it the 13pt `inkSoft` subtitle, then the sections in §4a order:
- `आज के दान का महत्व` — the first daan-covered observance of today (`useObservancesForDate` → `getDaanOccasionForRule`; card absent on uncovered days, never a placeholder) with the journey door (`daan-today-journey`).
- `इस वार का दान` — a label row whose right end names today in `saffronDeep` (`आज · सोमवार`), then **7 equal-width 36pt chips** (`daan-vaar-*`, muhurat chip idiom) carrying the **readable short name** (`रवि · सोम · मंगल · बुध · गुरु · शुक्र · शनि`, derived by stripping `वार`; `Sun · Mon …` in en — never the old two-letter codes), each over a 5pt dot that is `saffron` under today's chip and transparent elsewhere (the label row names the day, so colour is never the only carrier). The `goldChipBg` line (`daan-vaar-line`) sets `<वार> — <ग्रह>:` in the title face on `ink` and the items in the body face.
- `शास्त्र क्या कहते हैं` — a label row with a quiet `स्वाइप करें →` hint, then the verse spine as a **horizontal carousel** (`daan-principle-carousel`: paged `ScrollView`, card width = screen − 2×`readingGutter` − 28pt peek, 10pt gap, `snapToInterval`, `decelerationRate="fast"`). Each `daan-principle-*` card (`parchmentSoft`, `divider`, `radii.lg`, `elevation.card`) shows the verse in full (14.5/24 title face, centred; Devanagari via `verseLinesByLang`), the cite at 10.5 uppercase in **`inkMuted`** (was `gold` — 3.1:1 at that size), and the meaning **clamped to two lines** with a `पूरा अर्थ ›` / `संक्षेप ‹` toggle (`daan-principle-meaning-*`, 32pt min-height) that unfolds it in place; the Gita card alone also carries the `गीता में पढ़ें ›` chip (`daan-gita-link`) deep-linking `GitaReader {chapter:17}`. Position dots under the carousel (active 16×6 `saffron`, rest 6×6 `dotRest`; hidden from a11y).
- `दान की कथाएँ` — a **horizontal shelf** (`daan-katha-shelf`) of 156pt tiles (`daan-katha-*`, `parchmentSoft`, `divider`, `radii.md`, `elevation.card`, min-height 96): a `gold` `॥` glyph, the title 14.5/**22** and subtitle 11.5/17 `inkMuted` (the §2 ≥1.5× floor — at 20 the semibold Devanagari matras of दानवीर/शिबि were sliced), each → `DaanKatha`.
- **Nothing after the shelf.** The scroll ends on the kathas: the stance line went in the Sept 2026 copy pass (§1), and a closing line about where the register is stored is banned outright by the customer-copy guard (`userFacingImplementationCopy.test.ts` — no `on this device` / `इस उपकरण पर` copy anywhere in `screens/` or `components/`). The ledger's privacy is a property of the feature (AsyncStorage, no network, no totals), not a caption.

The **sticky action bar** (`daan-home-actions`) is absolutely positioned at the bottom of the SafeAreaView, above the tab bar: 74pt, `parchmentHighlight`, 1pt `divider` top rule, `elevation.lifted`, `readingGutter` sides, two 50pt pills 10pt apart — the OUTLINED `दान-द्वार ›` (`daan-home-dwaar`, flex 1, 1.5pt `saffron` border, 15pt `saffronDeep`) → `DaanDirectory {}`, then the FILLED `दान करें` (`daan-home-donate`, flex 1.3, `saffron`, 16pt `onPrimary`) → `DaanJourney` — carrying `{occasionId}` when today's observance carries a daan occasion, else `{}` for the daily variant. The scroll content pads its bottom by the bar height so nothing hides under it.

**दान-यात्रा (`screens/DaanJourneyScreen.tsx`).** The §4a journey as ONE skippable scroll (`ScrollView`, root `daan-journey-screen`) — महत्व → शास्त्र → कथा (present only when the occasion carries one) → क्या दें → संकल्प-भाव — each under a gold uppercase section label; the stepper (dots, पीछे/आगे pills, `daan-journey-next`/`-prev`) is **gone**. Near the top, under the header, a quiet text skip button (`daan-journey-skip`, copy `पहले से जानता हूँ — सीधे दान-द्वार ›` / `Skip to the daan-dwaar ›`) scrolls straight to the terminal block via a `ScrollView` ref + the terminal's `onLayout`-captured Y. **Terminal actions are ALWAYS present** at the end of the sankalp section (§2.7 relaxed — no step gating): `daan-journey-record` (filled saffron, leading), `daan-journey-directory` (outlined, trailing, copy `दान-द्वार (बाहरी)`), preceded by the **recommended-daan chip row** (`daan-journey-causes`, Sept 2026): a 12.5pt `saffronDeep` body-face lead (`इस दिन का दान —` in occasion mode, `आज का दान —` in daily mode) followed by one 36pt pill chip per cause (`daan-journey-recommended-<cause>`, `goldChipBg` fill on a 1pt `gold` border, `radii.pill`, 13pt `saffronDeep` title face, `<cause name> ›`), wrapping. Each chip → `DaanDirectory {causes: [thatOne]}` — a single cause, so the द्वार skips its grid and opens that shelf directly (Wednesday's gau-gras → गौ-सेवा). Chips derive from the day's causes ∩ causes a verified org serves (a chip never lands on an empty grid); the row is absent when none survive. The outlined door beside it is the **full listing** — the recommendation and the listing are two different doors. **Nothing follows the buttons** — the "रुक जाना भी पूर्ण है" line that used to close the block is gone (copy rule below); the `(बाहरी)` in the directory button's own label already says the door leaves the app. **Two modes.** *Occasion mode* (`route.params?.occasionId` resolves via `getDaanOccasion`): title/why/items/causes/katha from the occasion; record → `DaanEntry {occasionId}`, directory → `DaanDirectory {causes}`. *Daily mode* (param absent/unresolved): built from today's vaar (`DAAN_VAAR_ENTRIES[new Date().getDay()]`) — title `दान-यात्रा`, a vaar-derived mahatva, the `sattvik-daan` (fallback `shraddhaya-deyam`) principle for shastra, the vaar's `itemsHi/itemsEn` for क्या दें, and the vaar row's `causes` for the recommended chips (Sun/Mon अन्न · Tue/Fri/Sat अन्न + वस्त्र · Wed गौ · Thu विद्या + अन्न); no katha; record → `DaanEntry {}`, directory → `DaanDirectory {}` (the full listing — never pre-filtered in daily mode). Shipped kathas open `VratKathaReader` cross-tab; teaching-kathas open `DaanKatha`.

**खाता (`screens/DaanLedgerScreen.tsx` + `DaanEntryScreen.tsx`, `contexts/DaanLedgerContext.tsx`).** A smaran register: entries grouped by civil month, each row category + panchang tithi stamp (`makeTithiStamp` — lunar month, paksha, tithi, vara, hi/en) + optional note; गुप्त entries render date + `गुप्त दान` on a `goldChipBg` card — their note/amount/occasion are stripped BEFORE persistence (`sanitizeLedgerEntry`; validator rejects unsanitized gupt rows). Storage `@vedansh/daan-ledger:v1` (versioned payload, tolerant hydration — the PRD-17 pattern). No totals anywhere; the CSV export (`buildLedgerCsv`, gupt rows stay bare even there) leaves via the OS `Share` sheet only. The entry form: auto tithi field, 9 category chips (`daan-cat-*`; only date+category required — the 10-second U6 form), the गुप्त switch (removes the detail fields from the tree), note + amount via `TextField variant="form"`. The ledger's caption is the privacy fact alone (`निजी — केवल इस डिवाइस पर`); the **empty state states the state and the next step** (`अभी कोई दान दर्ज नहीं है। ऊपर से पहला दान दर्ज करें, या सुदामा की कथा पढ़ें।`) over the `daan-ledger-empty-katha` chip — it names Sudama as a door, not as a parable told to an empty screen. The entry form's footer names what else may be recorded (`गेट पर, हुंडी में, पड़ोस में दिया दान भी यहाँ दर्ज किया जा सकता है।`) and stops there.

**दान-द्वार, by प्रयोजन (`screens/DaanDirectoryScreen.tsx` + `DaanDirectoryDetailScreen.tsx`).** The द्वार is organised on the **cause axis** (`data/daan/causes.ts` — अन्न-जल · गौ · बाल · वृद्ध · विद्या · आरोग्य · वस्त्र · जीव · आपदा), NOT the dravya axis the ledger records. The screen **opens on a cause grid, then drills into one cause** — under the `देशे काले च पात्रे` line and the subhead that states the screen's order (*first why each cause matters, then the places doing that seva, and the giving on their own website*), the default state is a grid (`daan-directory-grid`) of cause tiles (`daan-cause-tile-*`, `parchment-soft`, `radii.lg`, `elevation.card`), each showing the cause name + its "whom this serves" line and carrying **no place counts**. Occasion-supplied causes ring gold (`goldChipBg` + `cardActiveBorder`). **Tiles are derived from the rows present**, so a cause with no verified row renders no tile and no shelf. There is no give/open affordance on this screen.

**Tapping a tile drills into the single filtered cause** (`daan-directory-filtered`): an active chip `daan-cause-${id}` (muhurat chip idiom; tapping it clears back to the grid), then the `CauseHeading` — the uppercase cause label, then the **mahatva card** (`daan-cause-mahatva-*`, `parchment-soft` on `divider`, body 13/21 `ink-soft`) carrying `mahatva{Hi,En}` — why this daan is held dear — with a gold citation line beneath it where the mahatva makes a textual claim (अन्न, गौ, बाल, विद्या, वस्त्र carry one; वृद्ध, आरोग्य, जीव, आपदा are plain tradition-register prose and carry none) — and only then the org rows (`daan-org-*`). Each row shows name → its ONE about line → a gold `kind` meta line (अन्नक्षेत्र · देवस्थान ट्रस्ट · सेवा-संस्था · सेवा-पोर्टल, plus `· धन नहीं` where the hand-off is non-monetary). The teaching is the larger half of the drilled view by construction; the places are thin rows underneath.

**A single-cause arrival opens filtered directly.** When `route.params.causes` from an occasion resolves to one cause the screen skips the grid and lands on that cause's filtered view, headed by a `saffron-deep` `इस दिन की सेवा — …` line naming the day's causes (`daan-directory-occasion-line`); the active chip still clears back to the grid. The footer states plainly that the app does not represent these places.

**Detail is a place, not a profile.** Centred name → the single `daan-org-about` line → the non-monetary card where it applies → related teaching-katha → the action row (record filled + leading; `आधिकारिक वेबसाइट` outlined) over the "ऐप इस लेन-देन का हिस्सा नहीं है — दान संस्था की अपनी आधिकारिक वेबसाइट पर होगा" line. There is **no registration/80G text and no verification badge**: verification stays an editorial discipline in `directory.ts` (`verifiedOn` drives the 18-month staleness drop and is never rendered). The outlined button opens the inline interstitial (`daan-org-interstitial`, naming the host) before `Linking.openURL(officialUrl)` — one link per row, the organization's own official page; after a hand-off ONE gentle record-offer card renders (`daan-org-return-offer`) — declining is silent.

**दान-कथा (`screens/DaanKathaScreen.tsx`).** Story paragraphs 14/25 `ink-soft` via `commentaryByLang`, then the `शिक्षा` panel (`daan-katha-teaching`: `goldTint` on a 1pt **`gold`** border, `radii.lg`, `elevation.card`; a 12.5pt `saffronDeep` title-face label — the Devanagari label is never tracked — over the teaching at **16/26** `saffronDeep` title face), the rendered canonical source line (12/18 `inkMuted`), the ॥ ॐ ॥ endcap. **The story never dead-ends.** Under a `divider` rule comes the onward block: the `अगली कथा` label row with **five position dots** (active 14×6 `saffron`, rest 6×6 `dotRest`; a11y-hidden) over a 62pt row card (`daan-katha-next`: `gold` `॥` in a 40pt `goldTint` circle, the next teaching-katha's title 15.5/22 and subtitle 12/17, a `saffronDeep` chevron) that `navigation.replace`s to the next katha in registry order, wrapping from the last to the first (rendered only when more than one katha is verified); then the `इस भाव से` label over the FILLED 52pt `दान करें` pill (`daan-katha-donate`, `saffron`, 16pt `onPrimary`) → `DaanJourney {}` (the daily variant — a katha carries no occasion), and the QUIET `खाते में दर्ज करें` text door (`daan-katha-record`, 13.5 `saffronDeep`, 36pt min-height) → `DaanEntry {}`. Nothing follows the doors — see the copy rule below. No give/pay control — §2.7 holds here as on every daan surface.

**Doors.** More hub → साधना group row `दान-पुण्य / Daan Punya` (`more-daan-punya`, NEW state for one release). **Home tab presence (PRD-26 v3):** a non-content launcher card `दान-पुण्य` (icon key `daan`, `shortNameEn: 'Giving'`, NEW badge, §18) closes the category grid full-width → `DaanPunya`; the duplicate full-width नित्य साधना category card was removed because the dedicated RoutineBanner above the grid already owns that Home door. A **DISCOVER spotlight** card (`key: 'daan-punya'`, `CategoryIcon iconKey="daan"`, NEW) → `DaanPunya`; and a **FOR TODAY** strip card (`for-today-daan`, §50) that appears on daan-covered observance days → `DaanJourney {occasionId}`. Observance Detail → `इस दिन का दान` — **always the last section** (`observance-daan-door`), rendered only when `getDaanOccasionForRule(ruleId)` matches (exact ids beat suffix families; uncovered days render nothing). **Purpose bridge:** `PurposeList` (the text-intent screen) renders a quiet `दा` educate door (`purpose-daan-door`) → `DaanPunya`, and ONLY for the two intents the app's own vocabulary shares with a seva — विद्या→`knowledge`, आरोग्य→`health` (`causeForPurpose`). `wealth`/`prosperity` are deliberately unbridged: pointing "give here" at a prosperity intent is the fruit-promise §2.2 bans; a source-shape test pins that this door opens the educate home and never the द्वार. All daan screens are registered on the More, Panchang AND Home stacks (PRD-19 multi-stack pattern) so every door pushes in place.

**Copy rule — the §1 house rule, and where it started.** The feature shipped with a muted stance line closing almost every screen — *"यहाँ रुक जाना भी पूर्ण है"*, *"मात्रा नहीं, भाव मापदण्ड है। यहाँ कोई अंक, कोई लक्ष्य, कोई तुलना नहीं"*, *"यह सूची जान-बूझकर छोटी रखी गई है"*, *"यही प्राथमिक मार्ग है"*, a screen narrating its own section order. They restated the product's stance to a reader who never asked, and read as filler under the very buttons they sat beneath, so they are **gone from every daan surface**. What stays is a line that carries a **fact the user acts on**: what else may be recorded in the register, that the giving happens on the organization's own website, and that the app neither transacts nor represents these places (§6.1 — kept once per screen, never twice). The stance itself is enforced where it belongs: in the data (no totals, the gupt guarantee, the copy-guard test's banned vocabulary), not in a caption. The teaching register stays fully intact in the **content** — the verse meanings, the cause mahatva, the kathas' शिक्षा — which is what the reader came for.

**Release.** Pure JS + AsyncStorage — OTA-safe. The store-policy review gate (PRD-26 §6.1) still applies to the release *carrying the directory*: it rides a store release for review visibility even without a native module.

**Files.** `mobile/src/data/daan/{types,principles,occasions,vaar,kathas,directory,ledger,index}.ts` · `contexts/DaanLedgerContext.tsx` · `screens/Daan{Punya,Journey,Ledger,Entry,Directory,DirectoryDetail,Katha}Screen.tsx` · doors in `screens/MoreScreen.tsx` + `screens/ObservanceDetailScreen.tsx` · registrations in `navigation/{MoreStackNavigator,PanchangStackNavigator}.tsx` + `navigation/types.ts` (DaanStackParamList) · provider in `App.tsx`. Tests: `src/data/daan/__tests__/daanContent.test.ts`, `src/contexts/__tests__/DaanLedgerContext.test.tsx`, `src/screens/__tests__/DaanScreens.test.tsx`; device path `.maestro/daan-punya-smoke.yaml`. See RULEBOOK §27 and `docs/roadmap/prds/26-daan-punya.md`.

---

## 72. प्रश्न — current phase, direction and expandable Jyotish evidence (PRD-43)

**Purpose.** Explain what the person's natal graha, running Mahadasha/Antardasha and today's gochar mean together for their question. Career and business are the first pilot. `buildPrashnaReading` returns the full `analysis`, legacy `guidance`, and an optional serializable `phase`; pilot consumers use `phase`. The other seven purposes retain the earlier theme/checklist model pending separate review. All app computation remains offline and deterministic. The current comparison lives in `docs/evaluations/prashna-phase-2026-09-21/`; its source register is `docs/roadmap/conventions/prashna-phase-v1.md`.

**Customer copy.** Explain the phase, dates, Jyotish reasons and direction. Ask’s expandable working contains localized basis chains, without function names, version IDs, scoring traces or review-status prose. An age-gated answer needs no technical working trace. Meaningful sharing-content notices and deletion consequences remain.

**Entry and context.** Keep the full-width Prashna lead card on the saved Jyotish landing and the existing Ask entry. The screen identifies the active person, age and as-of date. Nine purpose buttons preserve their age gates. Explicit question choices distinguish first job, switching, growth, starting a business and partnership; changing purpose resets to general. Missing personal circumstances are never inferred. Person/purpose/question changes reset expanded evidence (and legacy checklist state). Question rows grow with translations, have a filled radio indicator and at least 48 dp height.

**Career/business reading order.**
1. **Your current phase / अभी का समय** — plain headline and a synthesis naming the active topic, support and demands; named Mahadasha/Antardasha and the current Antardasha's actual dates.
2. **Why this reading / ऐसा क्यों** — everyday meanings of the natal foundation, running periods and relevant Jupiter/Saturn transits. Identical explanations for the same lord in maha/antar are shown once. Technical house/rule details stay in the expandable basis.
3. **Guidance for this phase / इस समय दिशा** — question-specific direction selected from the combined phase and a reason tied to named period signals. There is no generic checklist for these two purposes. Existing Full reading and share and business Muhurat handoffs remain.
4. **Next period change / अगली दशा का बदलाव** — the next actual Antardasha boundary, including across Mahadasha boundaries, with its meaning. This is not the next transit change or a promised improvement date; future gochar is not silently assumed equal to today's.
5. **See Jyotish basis** — collapsed by default. Shows the calculation date/convention and every signal's full reasoning and BasisChain. Pilot versions, source-review progress and excluded-method inventories belong in internal documentation and structured export metadata, not screen copy. Moon and Lagna transit reference frames remain explicit.
6. **Optional practice and context** — the short traditional-reading limit, existing allow-listed practice, and dated footer. No added commerce or new navigation surface.

**Bilingual layout.** Existing parchment surfaces, saffron accents, rounded cards and tabs remain. Pilot body is Cormorant 18/25 in English and script serif 15/25 in Hindi; hero headings 22/29 and 18/29; section labels 10/20 and 12/20. Reason headings are 19/26 and 16/26. Cards have 18 dp padding and 10 dp gaps. Buttons wrap, maintain a 44 dp minimum target and use script-safe leading; the basis toggle is at least 48 dp and announces expanded state. No fixed content heights or truncated body text. Basis chips wrap within the card with localized labels. Native screenshots, including compact/large-type evidence and remaining review limits, are indexed in the evaluation README.

**Computation and provenance.** The pilot uses a question-priority natal house lord; actual lordship/occupancy links for maha/antar; and current Jupiter/Saturn placement/aspects from Lagna with Moon-relative transit conditions and vedha. Rahu/Ketu occupancy establishes topic only; no blanket favourable/difficult polarity is assigned without their missing association/dispositor analysis. The phase is a transparent decision table, not a score or probability. Contradictions survive. `origin: phase-interpretation` directions retain signal IDs. Source review stays `verified:false`; tests prove contracts, not predictive validity. The other seven purposes retain basis-linked themes and `origin: editorial` practical suggestions.

**Freshness and integration.** `useJyotishNow` refreshes on focus, foreground, IST midnight and the next exact dasha boundary. Both Prashna and report rebuild against that instant. `gocharScanDays:0` skips future ingress scans, not the current snapshot. Ask uses the same phase engine for general career/business questions; its parser still selects purpose only. Full reading passes `{purposeId, questionId}` and recomputes for the active chart. The warned text export includes the selected phase, meanings, detailed signals, direction provenance and next period in a separate JSON block. The final report JSON tail remains unchanged and parse-back equal.

**Files and verification.** `prashna.ts` gathers factors; `prashnaPhase.ts` combines the pilot; `prashnaGuidance.ts` composes the result and preserves legacy guidance; `PrashnaPhaseContent.tsx` renders it. Screen, Ask and handoff tests cover shared consumption. `prashnaPhase.engine.test.ts` covers factual basis, temporal change, exact period edges, vedha, contradictory signals and age/question gates. `useJyotishNow.jest.test.tsx` covers midnight, dasha and foreground refresh. `.maestro/prashna-smoke.yaml` retains legacy-study coverage and adds phase assertions for business. Evaluation-only scripts record public/synthetic cases and real external model responses; they do not introduce an app AI path.

**Status.** Career/business pilot implemented for review. Remaining seven topics, fuller Jyotish interpretation, independent source sign-off and release certification remain open. The earlier generic plan was rejected in user review; it is not the accepted endpoint for those topics.

## 73. क्षेत्रीय पंचांग — the regional & sampradaya lens (PRD-42 W2)

**Purpose.** A calendar the user turns on, so a Maithil family sees Sama Chakeva where they expect it, a Jain family can reach Paryushana, and a user in Ujjain sees exactly what they see today. Two promises govern every pixel below, and both are stated **on** the surfaces rather than in a help page: **a calendar only ever adds days**, and **nothing here asks who you are**. The lens set is `@vedansh:panchang-lenses` — a registry of 22 calendars (20 regional + `jain`, `sindhi`), an empty set meaning today's app byte for byte.

**Offered ≠ registered.** Every user-facing surface below offers only the calendars that **add at least one observance in this build** — `getLensesWithContent()` (`vratCatalog.ts`), today `तमिऴ` and `जैन`. The other 20 are registered (names, examples, seeding) but never shown, counted or seeded: a switch that changes nothing was the Sept 2026 report ("I selected Jain and it still shows everything"). "n उपलब्ध" everywhere is the offered count, `useLenses().lenses` is the stored set narrowed to offered ids (a Mumbai seed from before content shipped stays on disk but is not displayed), and a calendar joins the moment its first `default` + `lens` rule lands, with no registration step. **If the offered count is ever zero, the ledger column, the quiet-day row and the More row do not render at all** — a filter that can choose nothing is not a filter.

**Structure — one sheet, four doors.** The sheet is identical from every entry; only the doorway moves.

| Surface | Where | What it says |
|---|---|---|
| **The sheet** (`LensPickerSheet.tsx`) | Bottom sheet, `parchmentHighlight`, 22 pt top radii, grabber | Title `क्षेत्रीय पंचांग`, then the "days are only ever added, never removed" line at 12.5/19 `inkSoft`, then `n सक्रिय · m और उपलब्ध` at 11 pt `inkMuted` (`सभी n सक्रिय` once every offered calendar is on). Under the count, **two pill buttons** — `सभी चुनें` and `सभी हटाएँ` — 36 pt tall, 1 pt `saffron` outline, 12.5 pt `saffronDeep` label, centred with a 10 pt gap; **each hides when it would be a no-op** (no सभी चुनें when all are on, no सभी हटाएँ with none on), so the header never offers a dead button. Rows are **only the offered calendars**, grouped उत्तर · पूर्व · पश्चिम · दक्षिण · परंपरा under 10 pt/1 pt-tracked `inkMuted` eyebrows; a group with no offered calendar is not rendered (today the sheet shows दक्षिण → तमिऴ and परंपरा → जैन). Each row: name at 15 pt `ink`, its two or three recognisable observances at 11 pt `inkMuted` (one line, ellipsised), then a **third line naming what this calendar adds** — `जोड़ता है: रोहिणी व्रत` at 11 pt `saffronDeep`, two lines max (`getRulesForLens`) — and a 22 pt circular check filled `saffron` when on. **Checkboxes, not radios** — lenses compose — and a tap applies immediately with no confirm step, because the calendar behind the sheet answers it. 52 pt row floor, ≥ 44 pt tap target. |
| **The ledger column** (`PanchangScreen.tsx`, व्रत-पर्व) | Third column of the §33 ledger bar, after मेरा व्रत and पितृ स्मरण | A `CatalogLedgerCell` inside the shared `goldTint`-on-`gold` bar: ❖ glyph, `क्षेत्र` at 14 pt, and one 11 pt line of state that still **names a calendar** — `तमिऴ +1` (the first active calendar plus a count of the rest), `सभी n` when every offered calendar is on, or `n उपलब्ध` with nothing on. The column holds ~106 pt, so the names are truncated to one plus `+n` rather than turned into a bare count: a count does not tell a user where their extra dates came from, and the leading name still does. Renders even with nothing on — for a user whose city seeded nothing it is the only path to the sheet — but **not at all when the offered count is zero** (the bar falls back to two columns). |
| **आपके पंचांग से** (`PanchangScreen.tsx`, व्रत-पर्व, `LensAdditionsSection`) | Card directly under the ledger bar, **only with ≥ 1 lens on** | The additive contract leaves a lensed day unmarked among the universal ones, so this card is the pointer to what the active calendars actually added. `parchmentSoft` on `divider`, `radii.md`, `elevation.card`, 14 pt gutters. Head: `आपके पंचांग से` at 14 pt `ink` over an 11 pt `inkMuted` count line (`1 तिथि जो केवल इन पंचांगों से आती है`, or `चुने हुए पंचांग अभी कोई विशेष तिथि नहीं जोड़ते`), with a `बदलें` text button (12 pt Inter SemiBold `saffronDeep`) that opens the sheet. Then **one group per active offered calendar in registry order** under the sheet's 10 pt uppercase eyebrow: each added rule is a `resultRow` (15 pt `ink` name, 13 pt `inkMuted` other-script caption, the next date as a 11 pt `saffronDeep` uppercase short date, `›`) opening `ObservanceDetail`. (The empty-calendar line and the zero-count head copy remain in code as a guard but cannot render, since only offered calendars can be on.) Data: `getLensAdditions` + `getNextOccurrence` (`vratCatalog.ts`). |
| **The quiet-day row** (`PanchangScreen.tsx`, पंचांग) | Inside the day card, **only** where it would otherwise read `इस तिथि पर कोई व्रत या पर्व नहीं है।`, and only with ≥ 1 offered calendar | `❖ क्षेत्रीय पंचांग जोड़ें · n उपलब्ध` on `goldTint`/`divider`/`radii.md`, 13 pt title over 11 pt caption. Fixes the one structural gap of the ledger row: the tab opens on **पंचांग**, the lens changes **पंचांग**, but the row lives on the other segment. Deliberately **not dismissible** — the dismissible seed line disappears forever on one tap. Costs nothing on a day that has content, because it is not rendered then. |
| **The More row** (`MoreScreen.tsx`) | परिवार group, after जन्म तिथि; absent when the offered count is zero | `SettingsRow`, ❖ on `gold`, state = the active calendars by name (`+n` past two), `सभी n`, or `n उपलब्ध`. A second **home**, not a doorway: where a user goes looking once they half-remember the setting. |

**The seed notice.** When a chosen city seeds its calendars — and only calendars with content are ever seeded — one line appears on the पंचांग day view — where the new dates are — on `goldTint`, naming the **city's** calendar and never the person: `बेंगलुरु का कर्नाटक · तेलुगु पंचांग जोड़ा गया — बदलें`, with a ✕. Tapping the line opens the sheet. Never a modal, never a question.

**Type & motion.** Devanagari through `cardFontByLang` / `scriptBodyFont(lang, typography.meaning.fontFamily)` — never a Cormorant token, which `typographySafety.test.ts` enforces. The sheet is the standard `animationType="slide"`; a lens toggle has **no** animation of its own, because the day behind it changing is the feedback.

**A11y.** Each lens row is `accessibilityRole="checkbox"` with `checked`, labelled in English even in a Hindi UI, naming the observances and what it adds (`Jain calendar. Paryushana · Samvatsari · Gyan Panchami. Adds 1: Rohini Vrat`), so two regional calendars are distinguishable by ear. The bulk pills are buttons `Select all n calendars` / `Clear all calendars` (testIDs `lens-select-all` / `lens-clear-all`). The ledger, quiet-day and More rows are buttons labelled `Regional calendars, n on` / `, all n on` / `, none selected`. The आपके पंचांग से card is `testID="lens-additions"`; its rows are buttons `<NameEn>, next <date>`, its `बदलें` is `Change regional calendars`.

**Files.** `mobile/src/panchang/lenses.ts` (type, ids, set, parse/serialise — I/O-free and launch-path-safe), `lensRegistry.ts` (names, examples, `CITY_LENS`/`STATE_LENS` — behind a `require()` thunk), `lensStore.ts` (AsyncStorage + once-per-install seeding of offered calendars only + `setAllLenses`/`clearLenses`), `useLenses.ts` (display set via `withContentOnly`, `availableCount`, `selectAll`/`clearAll`/`allSelected`), `vratCatalog.ts` (`getLensesWithContent`, `withContentOnly`, `getRulesForLens`, `getLensAdditions`); `mobile/src/components/LensPickerSheet.tsx` (`React.lazy` in both screens); entry points and `LensAdditionsSection` in `PanchangScreen.tsx`, the row in `MoreScreen.tsx`; the gate in `festivals.ts` (`getObservanceCatalog`) and `festivalEngine.ts` (day/month/upcoming). Tests: `panchang/__tests__/lens.test.ts` (incl. the lens-by-lens agreement between `getRulesForLens` and the catalog gate), `.maestro/regional-lens-smoke.yaml` (authored; device run owed). Contract: RULEBOOK §23a.5, §23a.11–12.

---

## 74. पितृ पक्ष परिचय — the education layer beside the reminder and the vidhi (PRD-44)

**Customer copy.** Show available tithi teachings without a source-review-progress footer; review status remains in the content registry.

**Purpose.** Pitru Paksha in the app was a *reminder* (two public season notices), a *calendar* (the sixteen-row fortnight) and a *door* (the tila-tarpana guide). §74 adds the missing layer — **जानें**: what the fortnight is, why it is kept, whose day is which, what the bundled texts say, the stories, and honest answers to the common questions — and puts it **first** in the fortnight's journey (understand → see the day → act → remember) while the reminder and the vidhi stay exactly as §63 / §62 specify. It explains and never prescribes; it never names a dosha, a remedy or an obligation, and never claims the app's guide is complete shraddha (RULEBOOK §28).

**Tone.** Pitru Smaran's register (§63): Home gradient, `ReaderHeader variant="index"`, `spacing.xxl` gutters, `goldTint` / `gold` / `parchmentSoft` / ink tones; `saffron` and `saffronDeep` appear only as interactive affordance (hand-off pills, और पढ़ें, chevrons) and the quoted verse line. No NEW badge inside the feature, no streaks, no completion, no share surface, no emoji. The word is **परिचय**, never "guide" or "how-to".

**Structure — three routes in `MoreStackParamList` (`PitruPakshaShiksha: undefined` · `PitruParichayReader: {lessonId?} | undefined` · `PitruKatha: {kathaId}`), registered on `MoreStackNavigator` after `PitruPakshaOverview` so Back retraces overview → परिचय → reader / katha. Sept 2026 UX review: the परिचय screen was one scroll of six content types with three interaction models (unlock a lesson, read a verse card, tap into a story) and ~39 blocks — the content was fine, the stacking was the problem. It is now a **hub shaped by use**, and the lessons are read in the app's own reader shell:**

1. **परिचय hub** (`PitruPakshaShikshaScreen`) — Home gradient, `ReaderHeader variant="index"`, a gold `॥ ॐ ॥` (letter-spacing 6). No lede line (the section-order caption went under §1's copy rule) and no section rail (the screen no longer needs one). Each section is present only when it has verified rows; labels are the `typography.sectionLabel` uppercase idiom in `inkMuted` on the 24 pt (`spacing.xxl`) gutter. In order:
   - **Introduction — read** (`pitru-lesson-<firstId>`): one card (`parchmentSoft`, 1 px `divider`, `radii.md`) with the first lesson's title 16/24 `ink` and its **first paragraph only** at 15/25 `inkSoft`, then one filled 48 pt door (`saffron`, `radii.sm`): **पूरा परिचय पढ़ें** + `<n> पाठ ›` → `PitruParichayReader`. No lesson is unfolded in place; nothing is "and more".
   - **लोग क्या पूछते हैं — scanned** (`pitru-prashna-list`): the questions people arrive with, second, as an accordion in one ruled card. Question rows 14.5/22 title face, 48 pt, a `›` that rotates 90° when open; tapping opens the answer (13.5/22 `inkSoft`) beneath and closes any other — one answer open at a time, none at first. `accessibilityState.expanded` tracks it.
   - **शास्त्र-वचन — browsed** (`pitru-principle-carousel`): the §73 verse carousel, same geometry (card = width − 2 × gutter − 28 pt peek, 10 pt gap, `snapToInterval`, `radii.lg`, `elevation.card`) — title, quoted line(s) 14.5/24 `saffronDeep` centred (only when `ref` resolves to the verse cited), cite in the uppercase label face, meaning clamped to 2 lines with **पूरा अर्थ ›** / **संक्षेप ‹** unfolding it in place, and the reader pill **गीता में पढ़ें ›** → More-stack `GitaReader {chapter, initialIndex}` or **रामायण में पढ़ें ›** → `HomeTab/ValmikiRamayanReader`. Position dots beneath (6 pt, active 16 pt `saffron`).
   - **कथाएँ — browsed** (`pitru-katha-shelf`): the §73 tile shelf — 156 pt tiles, gold `॥`, title 14.5/22 (the ≥1.5× floor), subtitle 11.5/17 `inkMuted` → `PitruKatha`.
   - **शब्द — looked up** (`pitru-shabd-door`): a door row `<n> शब्द` + the first five terms, `›` rotating open; it unfolds the ruled glossary card beneath (term 14/21 `ink`, one line 13/20 `inkSoft`, 1 px `divider` between entries). Eleven terms are a lookup, not part of the reading flow.
   - **अब** — the three doors LAST, in the `goldTint` / `gold` door idiom (52 pt): **पक्ष की सोलह तिथियाँ** ("इस वर्ष की तारीख़ें · किस दिन किसका · आपके परिवार के दिन" — the only way from here to the fortnight, so it names what it opens) → `PitruPakshaOverview` · **॥ पितृ तिल-तर्पण स्मरण** → `VidhiDetail {vidhiId}` (undated; only when `getVidhiById('shraddha-tarpan-vidhi')` resolves) · **॥ पितृ स्मरण** → `PitruSmaranList`. No footer line.
   - **No पक्ष की तिथियाँ list.** The fortnight is the dated overview's, and its verified tithi teachings ride those rows (§63.5 item 5). The registry keeps all sixteen `tithi` rows and its coverage test.
2. **परिचय reader** (`PitruParichayReaderScreen`) — **the Vrat Katha reader shell** (§33 `VratKathaReaderScreen`), component for component, so the lessons read like every other long text in the app: flat `parchment`; `ReaderHeader` (44 pt circular back, 16 pt title, italic `n / m` counter, `sideWidth` 80); `ReadingProgressBar`; the toggle row — `LanguageToggle` centred, `ReadAloudButton` pinned right (`useReaderReadAloud`, `sourceId: 'pitru-parichay'`, the adapter's prose branch); a horizontal `pagingEnabled` `FlatList` of `KathaSectionPage`s — pill **`परिचय · n/m`** / `Introduction · n/m` (the page's `pillHi`/`pillEn` props; the katha default stays `प्रसंग`/`Part`), 20 pt title, compressed `॥` ornament, every paragraph at the shared `meaning` token (20/34 `inkSoft` hi, 20/33 `ink` en), long pages scroll inside; §5 pager dots overlaid (6 pt, current 18 × 6 `saffronDeep`); light haptic per page. Swipe only — no prev/next buttons. Pages are the **verified** `parichay` lessons in registry order; `lessonId` sets `initialScrollIndex` (an unknown or draft id falls back to page 1). The list ends on a `NextChapterCard` (`अगला · पक्ष की सोलह तिथियाँ ›`, `__type: 'transition'` so read-aloud never speaks it); settling on it fires, once, after the house 400 ms, **`navigation.popTo('PitruPakshaOverview')`** — back to the calendar if it is below in the stack (a reader opened from a dated day), a replace otherwise — so no second overview is ever stacked. The counter holds at `m / m` on the card.
3. **कथा reader** (`PitruKathaScreen`) — the §73 `DaanKathaScreen` pattern in Pitru's register: subtitle 12.5 `inkMuted` centred, paragraphs 14/25 `inkSoft` via `commentaryByLang`, the **शिक्षा** panel (`goldTint`, 1 px `gold`, `radii.lg`; gold uppercase label; teaching 14/23 `saffronDeep`), the rendered canon line (`स्रोत: …`, 11.5 `inkMuted`), an optional **मूल श्लोक … में पढ़ें ›** pill when the katha carries a `ref`, and the `॥ ॐ ॥` endcap at 0.7 opacity. An unresolvable (draft) id renders the header only.
4. **Overview door** — §63.5.

**Data (`mobile/src/data/pitru/`) — 61 rows authored, 39 verified.** `lessons.ts` (`kind: 'parichay' | 'tithi' | 'shabd'`, `fortnightDay` on tithi rows — 10 concept incl. the draft पञ्चबलि lesson, all 16 fortnight tithis, 13 glossary incl. the draft पञ्चबलि term), `principles.ts` (`ref: {kind:'gita'|'valmiki', chapter, verseIndex}` — Gita 1.42 / 9.16 / 9.25 / 10.29 / 2.20, Valmiki 2.102.27 / 1.41.19), `kathas.ts` (four verified, each retold from the bundled corpus and anchored by a `ref`: Ayodhya 102, Bala 38–44, Aranya 67–68, Ayodhya 77), `prashna.ts` (9); `index.ts` exposes **verified-only** accessors behind `require()` thunks (the More stack imports the overview eagerly — launch-graph rule) and the single door predicate `hasPitruShiksha()`. Scripture is deep-linked, never re-typed; review-only `source` blocks are never rendered.

**A11y.** Every pressable carries an English constant label for Maestro (flow: `.maestro/pitru-paksha-smoke.yaml`): hub — `Read the introduction`, `Question <id>` (with `expanded` state), `Expand meaning` / `Collapse meaning`, `Open gita reader for <id>` / `Open valmiki reader for <id>`, `Open katha <id>`, `Glossary` (with `expanded` state), `Open Pitru Paksha overview`, `Open Tila-Tarpana remembrance guide`, `Open Pitru Smaran list`; reader — `Back`, `Reading language`, the read-aloud pill, pages `pitru-parichay-page-<id>`, counter `pitru-parichay-counter`; overview — `Open Pitru Paksha introduction`, `Open Tila-Tarpana remembrance guide`, `Open the first family shraddha day`, `Open day <en date>` / `Close day <en date>`, `Open smaran <en name>`, `Open the lesson on how a tithi is matched`. Cards are plain information panels; only interactive rows carry chevrons.
...
## 75. बैकअप व पुनर्स्थापन — Backup & Restore (PRD-06 Track C)

**Purpose.** Every date a family typed — birth details, janma and pitru tithis, the kul record — and every bit of practice (saved verses, progress, streaks, routines, follows, alarms, preferences) lives only in this phone's AsyncStorage. A new phone or an uninstall wipes it. The bundle-only constraint (roadmap README) forbids a cloud copy, so the transfer is **user-driven and file-shaped**: one JSON file the user hands to the OS share sheet (Files, iCloud Drive, **Gmail**, AirDrop — their choice; the copy on the mail client is the "sync") and picks back with the OS document picker on the new device. Vedansh never sees the blob and keeps no copy; the help copy says so twice, because a lost file is unrecoverable.

**Door.** More hub → ऐप group, after Home-Screen Widgets, before Share (§37): ⇅ on a `saffron-deep` tile, label **बैकअप व पुनर्स्थापन / Backup & Restore**, state = `NEW` until the first export, then the export date via `shortDate` — PRD-06 §10.4's "last backup" nudge, on the row itself so a stale backup is visible without opening the screen. a11y `Backup and Restore, new` / `Backup and Restore`; `testID` `more-backup`. Route `Backup` on the More stack, loaded through a `getComponent` thunk like the other tools.

**Screen (`screens/BackupScreen.tsx`)** — the §70 export-screen anatomy (Home gradient, `ReaderHeader` index variant, `xxl` gutters, `parchmentSoft` cards at `radii.lg`), single reading language throughout:

1. **Lede** (`meaning` face 14/21, `inkSoft`): what lives on the phone, why to back up before switching, where to keep the file (Files, Gmail, anywhere safe), and that nothing is uploaded.
2. **"What a backup holds" card** (`बैकअप में क्या जाएगा`; the copy-lint test bans "on this phone"-style device metaphors, so the card is named for the file, not the device) — title 15.5 in the title face; under it the **last-backup line** (`backup-last`: `अंतिम बैकअप · <shortDateWithYear>` or `अभी तक कोई बैकअप नहीं`); a hairline; then **one row per group** in registry order — परिवार व ज्योतिष · साधना व पठन · स्मरण व अलार्म · पसंद — label left, count right (`backup-count-<group>`; a group with nothing reads `कुछ नहीं` in `inkMuted`). The counts come from the same reader the export uses (`readBackupEnvelope` → `summarizeEnvelope`), so the preview can never disagree with the file; they refresh after every action. Below: the filled `saffron` primary **बैकअप फ़ाइल बनाएँ / Export backup file** (`backup-export`, a11y `Export backup file`) and a centred 12.5 `inkMuted` hint naming the share sheet, Files and Gmail.
3. **"Bring it back on a new phone" card** — one paragraph (choose the `vedansh-backup` file you saved; you see what it holds first; nothing is written until you confirm) and the outlined `saffron` secondary **फ़ाइल से पुनर्स्थापित करें / Restore from file** (`backup-restore`, a11y `Restore from backup file`).
4. **Notice line** (`backup-notice`, 13 `saffronDeep`, centred) — one honest sentence per outcome: share sheet unavailable · could not create · not a Vedansh backup · from a newer version, update first · a Kul Parampara hand-on file, not a full backup (§70) · holds nothing · could not restore · restored, close and reopen the app (when the runtime cannot reload).
5. **Footer** (12.5 `inkMuted`): the file is the backup, Vedansh keeps no copy, a lost file cannot be recovered, back up from time to time.

**Interaction.** *Export:* `Sharing.isAvailableAsync` → read the registry keys in one `multiGet` → write `vedansh-backup-YYYY-MM-DD.json` to the cache dir → `Sharing.shareAsync` (`application/json`, `public.json`) → only after the sheet was presented, stamp `@vedansh/backup-meta`. Both buttons disable while either action is busy (`Preparing…` / `Reading file…`). *Restore:* `DocumentPicker.getDocumentAsync` (JSON/text/any — the parser is the gate) → `File.text()` → `parseBackupText` → on success a native **confirm `Alert`** listing the file's date and its non-empty groups with counts, and the sentence that current saved data for those sections **will be replaced**, Cancel / Restore (destructive) → one `multiSet` → `Updates.reloadAsync()` so every context rehydrates (contexts read storage once at mount; there is no live re-hydrate path). In Expo Go / dev client `reloadAsync` is unavailable → the "close and reopen" notice instead. Cancelling the picker or the confirm writes nothing and shows nothing.

**Format (`backup/envelope.ts`, pure).** `{ format: 'vedansh-backup', version: 1, exportedAt, appVersion, entries: { '<key>': { json } | { raw } } }`. Values travel **parsed** when the store wrote JSON (every store does), so the file is legible in a mail client years later; a non-JSON string travels as `raw` and round-trips byte-for-byte. Unset keys are omitted, never written empty. *Parse* names every failure (`corrupt` · `not-a-backup` · `kul-parampara-file` · `newer-version` · `empty`) and drops unknown keys and malformed values **before** the screen sees the envelope, so `entries` is always safe to write verbatim — a hand-edited file cannot plant `@vedansh/install-id` or a foreign key. *Restore semantics are replace-per-store*: each store the file carries is overwritten, each it lacks is untouched; PRD-06's field-level merge is deferred because the two real journeys (fresh install, new phone) have nothing to merge with and a wrong merge of a ledger or a family record is worse than a confirmed replace.

**Registry (`backup/registry.ts`) — what is in, what is out.** `BACKUP_KEYS` lists every user-authored store by exact key with a group and bilingual label (RULEBOOK §29 says when to add one). Excluded, and why: derived calendar caches (recomputed for free — `derivedCacheReset.ts`), OS-mirror bookkeeping (`notif-meta`, `notif-permission-asked`, `japam-alarms/once-armed`, the widget write-dedupe key — carrying them desyncs the schedulers, which rebuild them from the preferences that ARE carried), install identity (`install-id`, `push-token*`), per-install lifecycle flags (tour/what's-new/setup, rating prompt, the derived-cache fingerprint) and the backup stamp itself. The regional-lens pair travels together. `backup.test.ts` walks `src/` for every `@vedansh` key literal and fails on any that is in neither list.

**Files.** `mobile/src/backup/{registry,envelope,backupIo,backupMeta}.ts` · `screens/BackupScreen.tsx` · the row in `MoreScreen.tsx` · route in `navigation/types.ts` + `MoreStackNavigator.tsx`. Native dep added: `expo-document-picker` (store release; export alone would have been OTA-able). Tests: `src/backup/__tests__/backup.test.ts` (tsx — registry completeness, round-trip, every parse failure, tamper-drop, restore plan, counts), `screens/__tests__/BackupScreen.test.tsx` (counts, last-backup line, export outcomes, picker cancel, every invalid-file notice, confirm-cancel writes nothing, confirm → write + reload, no-reload notice, write failure); e2e `.maestro/backup-restore-smoke.yaml` (door + screen; the share sheet and picker are never opened).
