...
- **No network call until an endpoint is set.** `PUSH_REGISTRY_ENDPOINT` ships `null`, and the pure suite fails if that changes: the token is captured and stored locally (`@vedansh/push-token`, readable via `getCapturedPushToken()`) so the plumbing is verifiable on a real device *before* anything leaves it. When set it must be `https:` — a push token is enough to notify one identified person's phone.
- **Uploads are deduped by fingerprint, not by token.** The stored fingerprint covers install id + token + platform + app version + reading language + IANA timezone, so a language switch or a move across time zones re-syncs (a server should push in the language the reader chose, at a sane local hour) while an unchanged install costs no request. Only an **accepted** upload advances the fingerprint, so a 500 or an offline launch is retried rather than remembered as done; requests are bounded by `PUSH_SYNC_TIMEOUT_MS = 8 s`.
...
- **Payload is the minimum a server needs to address and segment an install** — install id, token, platform, app version, language, timezone. Nothing from the private Smaran/Kundali/Namkaran records and no location, ever (the field set is pinned by `pushTokenUpload.jest.test.ts`).
