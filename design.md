...
**Grahas and Dasha.** Graha rows show the traditional sign plus its plain-English equivalent, degree/minute, house, actual nakshatra/pada, and `℞` for retrograde. The Dasha view leads with the current Mahadasha/Antardasha, then gives each window its own labelled date line (`Mahadasha 16 Jan 2012 — 16 Jan 2028` above `Antardasha 23 Aug 2025 — 16 Jan 2028`), because a single unlabelled range under a two-lord headline reads as the Antardasha lasting the whole Mahadasha. The progress bar, elapsed time and remaining time measure the **Antardasha** — the innermost running period — and fall back to the Mahadasha only when no Antardasha brackets now; the Mahadasha's own span is the labelled line plus the timeline below. Durations drop to day granularity under one month so a just-begun Antardasha never reads `0 m`. An inline `Now` Antardasha chip row follows under the caption “इस महादशा की नौ अन्तर्दशाएँ / The nine Antardashas within this Mahadasha”, and a connected vertical timeline under the “महादशा समयरेखा / MAHADASHA TIMELINE” eyebrow then shows all nine Mahadashas with dates. Both levels are legitimately `Now` at once (the running Antardasha chip and its enclosing Mahadasha row), so each level is explicitly named — two unlabelled `Now` tags on different lords read as a bug. Both views include a short explanation before raw data, and Dasha timing is included in the accessible summary.
