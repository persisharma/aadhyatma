...
**Performance.** The occasion-independent per-day inputs (panchang solve + asta flags) are memoised module-side in the framework-free **`panchang/muhuratFinderScan.ts`** (`DAY_INPUT_CACHE`, keyed by location + calendar system + start-day, bounded to a few keys). One cache is shared by **all three surfaces** — the picker warmup (`useMuhuratFinderWarmup`), the occasion scan (`useMuhuratFinder`), and the abujh scan (`scanAbujhDays`) — so a day solved by any of them is free for the others. The first occasion pays the ~90-day astronomy scan once (~4.5 s); every subsequent occasion, the abujh calendar, and re-entry reuse the cache and resolve near-instantly (`evaluateDay` / `pushyaYogaFor` only). Scans stay chunked behind `InteractionManager`, and yields are skipped for fully-cached chunks so a warm surface never flashes the spinner. The scan core lives outside the React hook precisely so it is unit-testable under the tsx engine suite (`muhuratFinderScan.test.ts`).
