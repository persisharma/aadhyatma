...
1. **Top bar**: 44 px back circle · the screen name `जप / Japam / જપ / ಜಪ` (the mantra itself is shown once, in the tap surface — short mantras used to appear twice) · a 34 px ⏰ alarm button (opens the shared `AlarmEditorSheet` pre-locked to this mantra) · Share button.
...
3. **Tap surface** — the whole remaining column is one large Pressable (inside a ScrollView so a large-type mantra scrolls rather than clips; pressed state dims to 0.92):
   ...
   - `॥` Ornament.
   - **Turning mala** (`components/JapamMala.tsx`, pure geometry in `components/japamMalaGeometry.ts`) replaces the old big numeral + progress track. 108 wooden beads on a gold thread with a gap at the bottom for the brass Sumeru + tassel; a fixed `saffron` marker sits at the top. Each bead turns the ring (Animated rotate, 260 ms ease-out) so the bead under the fingers is always under the marker: it is brass with a `saffron-deep` stroke, chanted beads darken, beads ahead stay light wood. Every completed round **flips the direction** (`malaDirection(rounds)`) — the Sumeru is never crossed, the mala is turned — and a `सुमेरु · माला पलटें` notice shows under the count for 2.2 s. The centre holds a faint `gold` ॐ and a small `N / 108` caption (Inter 12, `ink-muted`). Box size 300 (260 under 720 px tall, 230 under 640), capped by width. Bead wood/brass colours are material constants, not theme roles. The ring is hidden from screen readers; the tap surface's label carries the count.
   - **Completed-malas tray** (`JapamMalaTray`, above a `divider` hairline): the completed-round count as a large Cormorant numeral (34, `saffron-deep`) over `माला पूर्ण / Malas done`, beside one mini mala (26 px dashed wooden ring + brass Sumeru) per completed round — capped at 9, then a `+N` pill on `saffron-tint`. A round completed while the screen is open drops its mini in (spring). Zero rounds shows italic `पहली माला आरम्भ / First mala on its way`.
   - `आज N जप / N japa today` line (reader-title face at 15, `saffron-deep`) — today's beads for this mantra from `UserActivityContext`.
   - Italic hint `जप के लिए स्पर्श करें · Tap to chant` (swipe-hint token); while the audio loop plays it reads `सुनें · मंत्र के साथ मनके आगे बढ़ते हैं / Listening · beads advance with the chant` and the top marker pulses (0.35↔1 opacity, 1.4 s). All motion drops to instant under reduced motion.
