...
**दान-द्वार, by प्रयोजन (`screens/DaanDirectoryScreen.tsx` + `DaanDirectoryDetailScreen.tsx`).** The द्वार is organised on the **cause axis** (`data/daan/causes.ts` — अन्न-जल · गौ · बाल · वृद्ध · विद्या · आरोग्य · वस्त्र · जीव · आपदा), NOT the dravya axis the ledger records. A centred `प्रयोजन चुनें` chip row (`daan-cause-*`, muhurat chip idiom) sits under the `देशे काले च पात्रे` line; **chips are derived from the rows present**, so a cause with no verified row renders no chip and no shelf. Unfiltered, the list groups by cause in `DAAN_CAUSES` order — each heading followed by its muted "whom this serves" line, a row appearing under every cause it serves (the §12 theerth multi-group-tag precedent). Filtered (chip tap, or `route.params.causes` from an occasion) it is the flat set for that cause, with a `saffron-deep` `इस दिन की सेवा — …` line naming the day's causes (`daan-directory-occasion-line`); tapping the active chip clears back to all. Each row shows name → about → a gold `kind` meta line (अन्नक्षेत्र · देवस्थान ट्रस्ट · सेवा-संस्था · सेवा-पोर्टल, plus `· धन नहीं` where the hand-off is non-monetary). Detail is educate-first even here: the work → the verification card (`card-active-border`: registration *kind*, official domain, `✓ दो स्वतंत्र स्रोतों से सत्यापित · date`) → related teaching-katha → the action row (record filled + leading; `दान करें (बाहरी)` outlined) over the "ऐप इस लेन-देन का हिस्सा नहीं है" line. `दान करें` opens the inline interstitial (`daan-org-interstitial`) before `Linking.openURL(donateUrl)`; after a hand-off ONE gentle record-offer card renders (`daan-org-return-offer`) — declining is silent.
