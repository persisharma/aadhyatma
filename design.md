...
- `दान की कथाएँ` — a **horizontal shelf** (`daan-katha-shelf`) of 156pt tiles (`daan-katha-*`, `parchmentSoft`, `divider`, `radii.md`, `elevation.card`, min-height 96): a `gold` `॥` glyph, the title 14.5/20, the subtitle 11.5/16 `inkMuted`, each → `DaanKatha`.
...
**Purpose.** A calendar the user turns on, so a Maithil family sees Sama Chakeva where they expect it, a Jain family can reach Paryushana, and a user in Ujjain sees exactly what they see today. Two promises govern every pixel below, and both are stated **on** the surfaces rather than in a help page: **a calendar only ever adds days**, and **nothing here asks who you are**. The lens set is `@vedansh:panchang-lenses` — 22 calendars (20 regional + `jain`, `sindhi`), an empty set meaning today's app byte for byte.

**Structure — one sheet, four doors.** The sheet is identical from every entry; only the doorway moves.

| Surface | Where | What it says |
|---|---|---|
| **The sheet** (`LensPickerSheet.tsx`) | Bottom sheet, `parchmentHighlight`, 22 pt top radii, grabber | Title `क्षेत्रीय पंचांग`, then the "days are only ever added, never removed" line at 12.5/19 `inkSoft`, then `n सक्रिय · m और उपलब्ध` at 11 pt `inkMuted`. Rows are grouped उत्तर · पूर्व · पश्चिम · दक्षिण · परंपरा under 10 pt/1 pt-tracked `inkMuted` eyebrows. Each row: name at 15 pt `ink`, its two or three recognisable observances at 11 pt `inkMuted` (one line, ellipsised), and a 22 pt circular check filled `saffron` when on. **Checkboxes, not radios** — lenses compose — and a tap applies immediately with no confirm step, because the calendar behind the sheet answers it. 52 pt row height, ≥ 44 pt tap target. |
| **The ledger column** (`PanchangScreen.tsx`, व्रत-पर्व) | Third column of the §33 ledger bar, after मेरा व्रत and पितृ स्मरण | A `CatalogLedgerCell` inside the shared `goldTint`-on-`gold` bar: ❖ glyph, `क्षेत्र` at 14 pt, and one 11 pt line of state that still **names a calendar** — `राजस्थान +1` (the first active calendar plus a count of the rest), or `22 उपलब्ध` with nothing on. The column holds ~106 pt, so the names are truncated to one plus `+n` rather than turned into a bare count: a count does not tell a user where their extra dates came from, and the leading name still does. Renders even with nothing on — for a user whose city seeded nothing it is the only path to the sheet. |
| **The quiet-day row** (`PanchangScreen.tsx`, पंचांग) | Inside the day card, **only** where it would otherwise read `इस तिथि पर कोई व्रत या पर्व नहीं है।` | `❖ क्षेत्रीय पंचांग जोड़ें · 22 उपलब्ध` on `goldTint`/`divider`/`radii.md`, 13 pt title over 11 pt caption. Fixes the one structural gap of the ledger row: the tab opens on **पंचांग**, the lens changes **पंचांग**, but the row lives on the other segment. Deliberately **not dismissible** — the dismissible seed line disappears forever on one tap. Costs nothing on a day that has content, because it is not rendered then. |
| **The More row** (`MoreScreen.tsx`) | परिवार group, after जन्म तिथि | `SettingsRow`, ❖ on `gold`, state = the active calendars by name (`+n` past two) or `22 उपलब्ध`. A second **home**, not a doorway: where a user goes looking once they half-remember the setting. |

**The seed notice.** When a chosen city seeds its calendars, one line appears on the पंचांग day view — where the new dates are — on `goldTint`, naming the **city's** calendar and never the person: `बेंगलुरु का कर्नाटक · तेलुगु पंचांग जोड़ा गया — बदलें`, with a ✕. Tapping the line opens the sheet. Never a modal, never a question.
...
**A11y.** Each lens row is `accessibilityRole="checkbox"` with `checked`, labelled in English even in a Hindi UI and naming the observances (`Karnataka calendar. Ugadi · Champa Shashthi`), so two regional calendars are distinguishable by ear. The ledger, quiet-day and More rows are buttons labelled `Regional calendars, n on` / `, none selected`.

**Files.** `mobile/src/panchang/lenses.ts` (type, ids, set, parse/serialise — I/O-free and launch-path-safe), `lensRegistry.ts` (names, examples, `CITY_LENS`/`STATE_LENS` — behind a `require()` thunk), `lensStore.ts` (AsyncStorage + once-per-install seeding), `useLenses.ts`; `mobile/src/components/LensPickerSheet.tsx` (`React.lazy` in both screens); entry points in `PanchangScreen.tsx` and `MoreScreen.tsx`; the gate in `festivals.ts` (`getObservanceCatalog`) and `festivalEngine.ts` (day/month/upcoming). Tests: `panchang/__tests__/lens.test.ts`. Contract: RULEBOOK §23a.5, §23a.11–12.
