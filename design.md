...
**Sharing.** One header Share action → the existing warned Kundali 4:5 share (chart summary card; `इस कार्ड में नाम, जन्म तिथि, समय और नगर शामिल हैं`). Never the Rashifal no-birth-details path. Full-document PDF export is explicitly deferred (`expo-print` would be a new dependency needing its own review).
