...
- `mobile/src/navigation/` — `TabNavigator` + the hand-rolled `AppTabBar` and its leaf `tabBarMetrics` (§17), `panchangSectionStore` (the §73 sync rule) + per-tab stacks (`HomeStackNavigator`, `MoreStackNavigator`, `PanchangStackNavigator`, `AudioStackNavigator`), `entryRoutes.ts` (bookmark / entry navigation targets), `startTarget.ts` (launch destinations).
...
**Purpose.** Persistent navigation chrome across the app's top-level surfaces. The bar is **hand-rolled** (`navigation/AppTabBar.tsx`, passed to the navigator as `tabBar`), not React Navigation's default — see "Five buttons, three routes" below. Geometry constants live in the dependency-free leaf `navigation/tabBarMetrics.ts` (`TAB_BAR_BASE_HEIGHT`, `TAB_BAR_SLOT_COUNT`), shared with `MiniPlayer` (§26) and the tour's ring fallback (§47); importing them from `AppTabBar` drags untranspiled `@react-navigation` ESM into leaf suites and breaks them under Jest.
...
- **5 buttons**, equally distributed:
  - **होम · Home** — the Home stack (catalog, readers, routine, theerth, search)
  - **भक्ति · Bhakti** — Daily Bhakti verse of the day (§23); भजन is a segment inside it
  - **पंचांग · Panchang** — the shared almanac screen, panchang section (§73)
  - **व्रत · Vrat** — the SAME screen, vrat section (§73)
  - **ज्योतिष · Jyotish** — the SAME screen, jyotish section (§73)
- Tab labels: **reading-language localized** via `contentByLang` (gu/kn transliterate; English labels in en). The bar was the last chrome surface still English-only under a fully Indic screen. Type: en = `fontFamilies.inter` 10 @ **`0.4`** tracking (was `0.02`, a no-op: RN `letterSpacing` is in **px**, not em, so 0.02 px is invisible; 0.4 matches the `cardMeta` chrome token); hi/gu/kn = their bold serif title faces (`scriptTitleFont`, hi → `devanagariBold`) at 10 with **no tracking** — tracking splits the shirorekha (§3).
- **Devanagari label line-height is a floor, not a preference.** भक्ति and ज्योतिष carry matras both above and below the baseline; at RN's default line-height for 10 pt they clip top and bottom. The label is fixed at `lineHeight: 14` / `height: 14` with `numberOfLines={1}` and `includeFontPadding: false`, verified at **360 dp with the OS font scale set to Large**. Pinned by `navigation/__tests__/panchangSectionSync.jest.test.tsx`.
- Maestro flows tap buttons by `testID` (`tab-home` · `tab-bhakti` · `tab-panchang` · `tab-vrat` · `tab-jyotish`), never by label text, since the exposed text is language-dependent.
- Icons, in the tint colour. Home, Bhakti and Panchang stay hand-built from `View` strokes. **व्रत takes a crescent moon** and **ज्योतिष a five-point star**, both single SVG paths — the moon is the literal instrument of tithi-reckoned observance, and neither may be a second sacred/swastika-adjacent glyph sitting beside पंचांग's cross. `MusicIcon` (the reference filled eighth note: round filled head, vertical stem, square flag, `d="M12 3v10.55A4 4 0 1 0 14 17V7h4V3h-6z"` — the head **must stay filled**) and `MoreIcon` remain exported and tested; they now serve the भजन segment and the अन्य hub rather than bar slots.
- Active tint: `saffron`; inactive: `ink-muted`. No active dot indicator — the tinted icon+label is the cue. The transition is **animated over 175 ms** (`Animated.timing`, `useNativeDriver: false` — colour cannot run on the UI thread; the icon cross-fades two stacked copies) so a highlight that moves in response to a *segment* tap reads as an answer to the tap rather than a glitch.
- Tap targets: full button width × full bar height (well above 44×44 minimum)
...
**Five buttons, three routes.** The navigator registers `HomeTab`, `DailyBhaktiTab` and `PanchangTab` with buttons, plus `AudioTab` and `MoreTab` with **`tabBarButton: () => null`**. पंचांग/व्रत/ज्योतिष are three entry points into ONE `PanchangTab` screen: registering three tab routes would mount three copies of `PanchangScreen` — three Hindu-calendar solves, three independent selected dates (so a segment switch would silently reset the date), and the Jyotish copy would drag the Kundali graph in behind it. Keeping `AudioTab`/`MoreTab` registered means every existing cross-tab `navigate`, notification deep link and tour step into those stacks is untouched; only the buttons left the bar.

**What moved off the bar (July→Sept 2026 restructure):**

| Was a tab | Now |
|---|---|
| **भजन** | A segment inside भक्ति. Playback is unaffected — the mini-player (§26) already docks above the bar app-wide from `App.tsx`, on every tab. |
| **अन्य** | `AppHeaderMenuButton` (§17a), the header icon on every tab root. |

---

## 17a. Header Menu Button (अन्य)

**Purpose.** अन्य as a header control instead of a bar slot (`components/AppHeaderMenuButton.tsx`). It opens `MoreTab` → `MoreHome`, the unchanged hub of settings, language, notifications, japam alarms, wishlist, profile, widgets, पितृ स्मरण and कुल परम्परा (§24).

**Spec:**

- 36×36 circle, `parchment-soft` fill on a 1px `divider` border, `radii.pill`, `hitSlop={8}` — 36 + slop clears the 44 pt touch floor without making the chrome row taller than the segment control beside it.
- Glyph: a **drawn** `menu-2` — three `View` bars (15 / 10 / 15 px wide, the short one right-aligned) in `ink-soft`. Drawn rather than a text character so the bar widths stay exact at any OS font scale (§5, no emoji).
- `accessibilityLabel="More"` and `testID="header-menu"` — English, like the rest of the chrome's a11y strings, so Maestro matches it in any reading language.
- **One row of chrome, always.** It sits *on* the screen's existing top row, never above or below it: beside the segment control on the shared screen (§73), absolutely positioned at the right of the wordmark row on Home (§18) and of the title block on Daily Bhakti (§21), so those titles stay optically centred on the screen rather than centred in the space the icon leaves.
- Registers the `headerMenu` tour target (§47), since अन्य no longer has a bar slot for the tour's ring-the-tab fallback to point at.
...
1. **The one row of chrome** (`styles.chromeRow`) — **outside the ScrollView, so it is sticky**: the section segmented pill `पंचांग · Panchang` / `व्रत · Vrat` / `ज्योतिष · Jyotish` (13 pt, `flex: 1`, active `saffron-tint`/`saffron-deep`, `testID` `segment-panchang`/`segment-vrat`/`segment-jyotish`) with **अन्य** (§17a) beside it at the right.
   - **The segment control REPLACES the screen title; it does not stack below one.** Panchang already renders its own chip row (item 2) directly underneath, so a segment row *plus* a title would put three tiers of chrome above the first line of content. Replacing the title costs zero extra height.
   - **Sticky is load-bearing, not polish.** A segment row that scrolled away would leave that chip row looking like the top of the screen. Pinned structurally by `screens/__tests__/KundaliExperience.test.tsx`, which asserts the whole chrome row precedes the `ScrollView` in source.
   - Labels are single words now (`व्रत`, not `व्रत-पर्व`), because they must read identically in the bar and the segment. **Maestro selects segments by `testID`, never by label text** — a `tapOn: "Vrat"` would also match "My Vrat" beside it.
2. **Contextual Panchang header** — the first SCROLLING child, shown for the panchang and vrat sections only; hidden in jyotish because current city, lunar-month convention, and followed-vrat state do not apply to a birth chart. One compact row, equal-width flex sides so the centre toggle stays screen-centred:
...
## 73. The Shared Screen: पंचांग · व्रत · ज्योतिष behind three tabs (Sept 2026)

**Purpose.** पंचांग, व्रत and ज्योतिष are **three entry points into ONE screen**, not three screens. Tab taps, notifications and deep links all resolve to the same route with a different `section`. §33 owns what that screen renders; this section owns the navigation contract.

**Route.** `PanchangHome({ section?: 'panchang' | 'vrat' | 'jyotish' })`. One screen component, one route, one param. `PanchangSection` (`navigation/types.ts`) is the **single vocabulary** — the route param, the segment control, the bar highlight and the logged `section` field all use these three names. It replaced an internal `PanchangHomeMode = 'calendar' | 'catalog' | 'jyotish'`: two spellings would have made a month of logged `section` values unreadable against the routes that produced them. `analytics/events.ts` declares a structurally identical `AnalyticsSection` so the analytics layer carries no navigation dependency, and `navigation/panchangSectionStore.ts` holds two mutual type assignments that fail to compile if the two ever drift.

### The sync rule

**The active segment is the single source of truth. The bottom-nav highlight mirrors it.**

- Tap the पंचांग tab → `section=panchang`, panchang segment active, panchang tab highlighted.
- While on that screen, tap व्रत **in the segment control** → content switches to vrat **and the bottom-nav highlight moves to व्रत**. Same in reverse, same for ज्योतिष.
- The highlight transition is animated over **175 ms** (§17), so the change reads as a response to the tap rather than a glitch.

The alternative — the bottom highlight staying pinned to the tab that was originally tapped — is **explicitly rejected**. A bar showing पंचांग while vrat content is on screen is the nav reporting the wrong location, and it is the specific thing that makes this pattern feel broken.

Mechanically: the state lives in `navigation/panchangSectionStore.ts` (module store: snapshot + subscribe + `setPanchangSection`), read through `usePanchangSection()` in `contexts/PanchangSectionContext.tsx`. It is a **module store, not a React context**, because one of the four writers is not a component — a notification tap is resolved in `notifications/deepLink.ts` through `navigationRef`, outside the React tree, and it is the writer that has to attribute `entry_point: 'notification'`. `AppTabBar` derives its highlight from the section whenever `PanchangTab` is focused, never from which button was pressed. Module state resets with the process, which **is** the "reset on cold start" requirement — do not persist it.

### Shared date state

The selected date is already lifted into the screen that hosts all three segments, so it is shared for free: set Panchang to next Tuesday, switch to व्रत, and next Tuesday's vrat is what shows. Panchang's internals are unchanged — the date is one `useState` in `PanchangScreen` above the section branch. It resets to today on cold start because the state mounts with the process.

### Navigation semantics

- Segment switching is a **lateral move, not a push**: callers navigate to the single `PanchangHome` route and never stack a screen, so **no back-stack entries are created** and Android back from any segment exits to होम.
- **Re-tapping the already-active tab** scrolls that segment to top. The store only bumps a `scrollToTopNonce`, so it can never be confused with a section change — and such a re-tap is **not logged as a view**, since counting taps that navigated nowhere would inflate exactly the `tab` number this section's analytics compares against swipes.
- **Per-segment scroll position is preserved** for the session: ONE `ScrollView` with offsets remembered per section in a ref, restored in a `useLayoutEffect` (`animated: false` — a lateral move should simply *be* where the user left it). Not three mounted `ScrollView`s: that would mount the Jyotish branch, and so a Kundali solve, the first time any section was opened, undoing the lazy-Panchang-stack work that keeps this graph off Home's startup path.
...
Every view of the shared screen logs `{ entry_point, section }` through `analytics/analyticsStore.ts` → `logSharedScreenView`, from the one place all four writers funnel through (`setPanchangSection`), so the attribution cannot drift from what happened.

| `entry_point` | Written by |
|---|---|
| `tab` | `AppTabBar` |
| `segment_swipe` | `PanchangScreen`'s segment control |
| `notification` | `notifications/deepLink.ts` via `applyStartTargetSection` |
| `deeplink` | Widget taps, Home's launcher tiles, tour steps, a cold-start route param |

**This is the instrument that decides whether the व्रत and ज्योतिष tabs keep their slots.** If segment swipes do all the work, a slot is reclaimed and भजन gets its tab back — so the four values must stay distinguishable, which is why the set is pinned as a set in `analytics/__tests__/events.test.ts`.

The app has **no analytics backend** and no network egress (content is bundled, §1). So this layer is a **seam, not a client**: a bounded on-device ring buffer (`MAX_EVENTS = 500`) under `@vedansh:analytics-log:v1`, plus a sink list that is empty by default. `countByEntryPoint(log, section?)` answers the question above without the reader re-deriving it. Two postures differ deliberately from every other store in the app: a failed **write** is swallowed (nothing renders this log, so a storage error must never break the tab tap that triggered it), and a **throwing sink** is dropped from the fan-out for the session rather than turning every logged event into an unhandled rejection.

**Out of scope for this restructure:** any change to Panchang's internal computation, layout or chip behaviour; any redesign of the Vrat or Jyotish *content* (this was IA and navigation only); Home content redesign.
