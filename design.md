...
## 70. दान-पुण्य — Educate-First Giving: Journey, Ledger & Daan-Dwaar (PRD-26)
...
**Files.** `mobile/src/data/daan/{types,principles,occasions,vaar,kathas,directory,ledger,index}.ts` · `contexts/DaanLedgerContext.tsx` · `screens/Daan{Punya,Journey,Ledger,Entry,Directory,DirectoryDetail,Katha}Screen.tsx` · doors in `screens/MoreScreen.tsx` + `screens/ObservanceDetailScreen.tsx` · registrations in `navigation/{MoreStackNavigator,PanchangStackNavigator}.tsx` + `navigation/types.ts` (DaanStackParamList) · provider in `App.tsx`. Tests: `src/data/daan/__tests__/daanContent.test.ts`, `src/contexts/__tests__/DaanLedgerContext.test.tsx`, `src/screens/__tests__/DaanScreens.test.tsx`; device path `.maestro/daan-punya-smoke.yaml`. See RULEBOOK §25 and `docs/roadmap/prds/26-daan-punya.md`.
