...
| **The ledger row** (`PanchangScreen.tsx`, व्रत-पर्व) | Third, after मेरा व्रत and पितृ स्मरण | The `myVratRow` anatomy verbatim (`goldTint` on `gold`, `radii.lg`, `elevation.card`): ❖ glyph, `क्षेत्र` at 15 pt, and one line of state that **names the calendars** — `राजस्थान · कर्नाटक सक्रिय · और 20 उपलब्ध`, or `कोई क्षेत्रीय पंचांग नहीं · 22 उपलब्ध`. Names, never a count: a count does not tell a user where their extra dates came from. Renders even with nothing on — for a user whose city seeded nothing it is the only path to the sheet. |
