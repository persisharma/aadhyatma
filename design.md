...
**Regional coverage of the observance catalog** (Sept 2026 — the “Goga Navami is missing” audit). The catalog is **148 rules, 134 of them default-visible**; the rest are `advanced`/`regional` and reachable only through catalog search. Two rules about what goes where:
...
**Wave 3 addition (1 rule, Sept 2026)** — the remaining-September audit. **वामन जयंती** (Bhadrapada S12; the appearance of Vishnu's fifth avatar) was the last unnamed Dashavatara jayanti in the catalog: the day between परिवर्तिनी एकादशी and अनंत चतुर्दशी carried only the generic monthly `dwadashi-vrat-shukla`. It is the catalog's third `madhyahna` rule (after राम नवमी and गणेश चतुर्थी), because the published day follows the midday-prevailing Dwadashi under Shravana nakshatra, and it is pinned to its published civil date in `observanceDates.test.ts` and re-derived in `verify-observances.mts`. The same audit confirmed the rest of the fortnight already ships (तेजा दशमी · परिवर्तिनी एकादशी · शुक्ल प्रदोष · अनंत चतुर्दशी · पूर्णिमा/सत्यनारायण · संकष्टी चतुर्थी), with पितृ पक्ष owned by `pitruSmaran.ts` rather than by a festival rule (RULEBOOK §27). **उमा-महेश्वर व्रत** (Bhadrapada Purnima) is a known, deliberate gap: as a `vrat` it needs a verified bhog profile (`bhogContent.test.ts`), and this session's egress block put both of its published sources out of reach.
...
1. **परिचय** (`PitruPakshaShikshaScreen`) — one reverent scroll in reading order under a gold `॥ ॐ ॥` (letter-spacing 6) and a 13 pt lede. Each section is present only when it has verified rows; section labels are the `typography.sectionLabel` uppercase idiom in `inkMuted`.
   - **परिचय** — concept-lesson cards (`parchmentSoft`, 1 px `divider`, `radii.md`; title 15 `ink`, body 13.5/22 `inkSoft`). A multi-paragraph lesson opens on its first paragraph and **और पढ़ें ›** (13, `saffronDeep`, 28 pt min-height) unfolds the rest in place — a परिचय is read, not navigated.
