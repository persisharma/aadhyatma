...
1. **परिचय** (`PitruPakshaShikshaScreen`) — one reverent scroll in reading order under a gold `॥ ॐ ॥` (letter-spacing 6) and a 13 pt lede. Each section is present only when it has verified rows; section labels are the `typography.sectionLabel` uppercase idiom in `inkMuted`.
   ...
   - **पक्ष की तिथियाँ** — one §33.6-style row per fortnight day (8 px `gold` marker · title 14.5 · note 13/20 `inkSoft`, `divider` rule). With verified rows missing, an italic Latin footer (12/18 `inkMuted`) says the remaining tithis follow their source review.
...
**Data (`mobile/src/data/pitru/`).** `lessons.ts` (`kind: 'parichay' | 'tithi' | 'shabd'`, `fortnightDay` on tithi rows), `principles.ts` (`ref: {kind:'gita'|'valmiki', chapter, verseIndex}`), `kathas.ts`, `prashna.ts`; `index.ts` exposes **verified-only** accessors behind `require()` thunks (the More stack imports the overview eagerly — launch-graph rule) and the single door predicate `hasPitruShiksha()`. Scripture is deep-linked, never re-typed; review-only `source` blocks are never rendered.
