...
- **AlarmEditorSheet** (exported and reused by the counter's ⏰) in a fade modal, internally scrollable (`maxHeight` 88 %): `TimeStepper` at **1-minute** steps (chevrons auto-repeat on hold — `TimeStepper` presses fire on press-in and repeat every 90 ms after 350 ms), a **Repeat** row of seven 38 px circular day chips (S M T W T F S; all on = Daily, none = Once — the summary line then warns "turns off after ringing"), the mantra picker (locked when opened from a counter), a **Label** `TextInput` (40 chars, optional), an edit-mode-only **Skip next** toggle chip showing the date it would skip, and a live "Rings in …" preview line above the filled confirm button (a11y label `Confirm alarm`).
- **Model** (`notifications/japamAlarms.ts`): `repeatDays?: number[]` (getDay() indices; absent = daily, `[]` = one-time, subset = weekly) and `skipNextDate?: 'YYYY-MM-DD'`. Pure helpers: `nextAlarmFireTimestamp(s)` (honours days + skip), `repeatSummary`, `describeUntilFire`, `formatTimeLabel`, `prefers12HourClock`. The context clears `skipNextDate` whenever time/repeat change, prunes past skip dates on load/foreground, and **auto-disables fired one-time alarms** via the scheduler's once-armed bookkeeping (`firedOnceAlarmIds`).
- Scheduling (`notifications/japamAlarmScheduler.ts`) is tiered: Android uses the native module (`AlarmManager.setAlarmClock` + lock-screen notification with the looping mantra sound and **Stop / Snooze 5 m** actions — survives Doze and reboot; the Kotlin receiver re-arms the next *repeat day* after each fire, never for one-shots); iOS 26+ uses AlarmKit (weekly recurrence on the selected days, `.fixed` one-shots for Once, native **Snooze** countdown button, overrides silent/Focus like the Clock app); older iOS / Expo Go falls back to `expo-notifications` — DAILY trigger for daily, one WEEKLY trigger per selected day, DATE one-shots for Once, plus a `japam-alarm` notification category carrying a **Snooze 5 min** action (handled in `maybeHandleJapamSnoozeResponse`, wired in App.tsx before deep-link routing). A pending skip-next on recurrence-owning tiers (AlarmKit / expo repeating triggers) is armed as 3 discrete one-shots and reverts to plain recurrence on the next foreground reconcile. `scheduleJapamAlarms` is idempotent — cancel-then-reschedule on any change (in-flight `:snooze` one-shots are spared). Tapping the alarm deep-links into the counter with `autoPlay` (§38).
