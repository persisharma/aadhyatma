...
3. **आज · Today card** (§48) — one card answering the day: date · city eyebrow with a `विधान ›` door, vara + tithi headline, masa · samvat · live tithi-handover line, the **live choghadiya** row (quality tag, progress bar, next auspicious), the Rahu Kaal / Abhijit chip row, then a **व्रत-पर्व** row (today's observance or the next one + its `तैयारी ›` door), a **राशिफल** row for the active person, and an **ask** field into जिज्ञासा. Header tap → Panchang tab; every row carries its own door.
...
## 48. Home Today Card (आज का पंचांग)

**Purpose.** Make Home answer *"what matters today"*, not only *"what can I read"*. One card between the wordmark hero and the CATEGORIES grid (§18) answers the day in full — the almanac line, the live moment, what to prepare for, the day's Rashifal, and a way to ask anything else; the exhaustive detail stays on the Panchang tab. The July 2026 strip (eyebrow + headline + one chip row) closed the daily-freshness gap the competitive review found; the **September 2026 card** replaced it because the strip still answered only *what day is it* — the choghadiya, the next vrat, the Rashifal and जिज्ञासा each sat a tab-switch away, and Home never said *what to do with the day*.

**Structure (`TodayStrip.tsx` — the file keeps its name; every `§48` pointer still lands):**

1. **Header** (one `Pressable` → Panchang tab; a11y `"Today's Panchang. {vara}, {tithi}. {observances}. Tap to open."` — the Maestro anchor of `home-today-smoke.yaml` and five other flows) beside a **`विधान ›` / `Vidhan ›` door** (its own button → `TodayVidhan`, the §71 briefing; `titleFont 14`, `saffron-deep`):
   - **Eyebrow** — `आज · 5 सितंबर · उज्जैन` / `Today · 5 September · Ujjain`: the word, `formatLongDate()` (`muhuratFormat.ts`, full month names in the app's `सितंबर` spelling, transliterated for gu/kn) and the chosen city's `labelHi/En` from `usePanchangLocation` — via **`eyebrowTextStyle()`** (12, `saffron-deep`; italic Cormorant + tracking for en, script-bold serif with no tracking for hi/gu/kn, §3).
   - **Headline** — hi/gu/kn `{vara} · {paksha} {tithi}` (`शनिवार · कृष्ण नवमी`); en `{vara} · {tithi} ({paksha})`. **22** script-bold serif / **24** `latinBold` (+0.3 tracking), `ink`, one line with `adjustsFontSizeToFit` (floor 0.8). Paksha names from `PAKSHA_NAMES_HI/EN`. Shows `—` while the day is unresolved — but on a returning launch it should never be seen: the launch prefetch (§61) warms the day from disk at process start, so the seeded `useMuhurat` composes on the first render and the headline paints with the rest of Home.
   - **Handover line** — `भाद्रपद · विक्रम संवत् २०८३ · नवमी तक 4:12 PM, फिर दशमी` (12.5 body serif, `inkSoft`; en 13.5 Cormorant medium). Masa carries `(अधिक)` when adhik; the samvat takes **Devanagari digits** in hi (`toDevanagariDigits`, transliterated onward for gu/kn), Latin digits in en. The tithi part is **live**: `prevailingTithi()` at the render instant — while the sunrise tithi still runs it reads `{tithi} तक {formatEndInstant}` plus `successorTithiToday()`'s `, फिर {next}` (the §33.5 rule: the label's end is never left implying an unnamed rest-of-day), and once the handover has happened it reads `अब {running}` (`· {clock} तक` when this day's solve knows the end). A tithi with no end in this solve reads `{tithi} पूरे दिन`. **Clock digits ride `latinSemiBold` 13** inside the otherwise script-set line (§3), which is why the line is built as segments. Reserved (a blank line) before the solve lands.
2. **Live choghadiya block** (one `Pressable` → `MuhuratDetail` for today via `panchangTabTarget`): a **12 dp dot** (`avoid` / `saffron-deep`; `divider` before the solve) + `अभी **उद्वेग** चौघड़िया` / `Now **Udveg** Choghadiya` (16 body serif, the name in the title face) + the **quality tag** (`त्याज्य`/`avoid` on `avoidChipBg` in `avoidDeep`, `शुभ`/`auspicious` on `goldChipBg` in `saffron-deep` — `latinBold 10`, uppercase, the §33 glance-card tag verbatim; quality is carried by dot AND text, never colour alone, §12); a **4 dp progress track** (`divider`, `saffron` fill, `accessibilityRole="progressbar"`) advancing once a minute — the card now takes `useMuhurat`'s **live** read (the July strip passed `live: false`; one `setState` a minute on one card is the whole cost, and the row is the reason the card exists); and a **next line** (13 body serif, `inkSoft`): while an avoid period runs, `अगला शुभ · चर 12:17 PM से` / `Next auspicious · Char from 12:17 PM` (`nextAuspiciousPeriod()`, name in `ink`, clock in `latinSemiBold` `saffron-deep`), `आज कोई शुभ चौघड़िया शेष नहीं` when nothing auspicious remains before the next sunrise; while an auspicious one runs, its end `{clock} तक` / `till {clock}`. The block reserves its height (blank next line) before the solve.
3. **Chip row** — **one fixed-height horizontal-scroll row** (`ScrollView horizontal`, no indicator, gap 6; full-bleed via −14 margin / +14 content padding so a clipped chip peeks at the card edge and signals the scroll). The July 2026 shipped version let the chips *wrap*, which on narrow devices stacked up to four pills into a tall block; the row now never wraps — overflow scrolls sideways instead, so the card height is the same on every device. Drags scroll; plain taps still bubble to the card `Pressable` (a ScrollView only claims the responder on move). **Overflow drifts ONCE, on a timer, after the launch has settled** (~24px/s, ~1.8s pause at each end; `AUTO_SCROLL_*` in `TodayStrip.tsx`): when the content is wider than the row, the row crawls to the end and back a single time so hidden chips surface without a drag, and then rests. It is **not** an `Animated.loop` — that is the whole history of this affordance, and the reason it is now a plain `setTimeout` stepping a number. The drift cannot use the native driver (it drives `scrollTo`), so as an `Animated` animation it cost a `requestAnimationFrame` tick **and** a `scrollTo` bridge call every frame, endlessly, on the one screen every launch lands on. Its first casualty was `isInteraction` — which defaults to `!useNativeDriver`, so every drift and pause claimed an `InteractionManager` handle and Home almost never reported an idle UI, starving the day's observance chips, the pitru match, the muhurat/festive schedulers, the widget writer and the strip's own midnight rollover (§61). Declaring `isInteraction: false` (Aug 2026) unblocked all of that — and by letting the observance chips finally arrive it *created* the overflow that starts the drift, so Home traded "never idle" for one endless JS-thread animation racing every deferred solve on the launch path, with taps landing late behind it. A decorative reveal cannot hold the JS thread at 60Hz, and no flag was going to fix that. Current shape: a self-scheduling `setTimeout` at **`AUTO_SCROLL_TICK_MS` 50** (≈1.2px a tick — imperceptible stepping at this speed, a third of the wake-ups), which ticks **only while actually drifting** (an end pause is one idle timer, not 108 no-op frames), pushes `scrollTo` **only when the rounded pixel changes**, waits **`AUTO_SCROLL_SETTLE_MS` 1200 after the last content change** so the launch's own churn keeps pushing it clear rather than racing it, and **stops for good after one out-and-back** — Home then goes fully idle. Lifecycle contract: the first user drag stops it **for good**; it **pauses while the Home tab is unfocused** (`useIsFocused` — bottom-tab screens stay mounted, so an ungated pass would burn JS-thread frames from other tabs); it never runs under reduce-motion (**`useReducedMotion`** — the shared subscribed hook, §11, so a mid-session preference flip stops/allows it live); it only starts once the row has a measured width and >8px of real overflow; and a **genuine content-width change** (the deferred chips landing, a language switch, the midnight rollover) is the one thing that re-arms exactly one fresh pass — re-serving the settle delay with it, so several changes in a row collapse into one drift after the last of them. Every timer it owns lives in the same ref and is cleared by `stopAutoScroll`, so nothing can tick after unmount. Pinned by `TodayStrip.test.tsx` → "chip-row auto-drift" (silent through the settle window; reaches the far offset and returns to 0; then silent for a further 60s). One normalized chip list so the pill spec exists once — **26 dp** row, chips at 10/4 padding: the contextual ★ followed-muhurat chip (§60) and the public पितृ पक्ष chip (§63) when they apply, the private Smaran / जन्म तिथि chips (§63/§70) leading, then a **Rahu Kaal chip** (`avoidChipBg` fill, **`avoidDeep`** text — the tint composites darker than the card, so raw `avoid` drops under AA there; terracotta, never red, PRD-14) and an **Abhijit chip** (`goldChipBg` fill, `saffron-deep`), both → today's `MuhuratDetail`. **Observance chips no longer ride this row** (Sep 2026) — today's observance has the व्रत-पर्व row below, where it can carry a date, a distance and a door. The kaal chip's label comes from the `KaalWindow`'s own `nameHi/nameEn` (KAAL_NAMES, `muhurat.ts`) — no duplicated literals. Chip names render via `pillTextStyle()` (Inter for en; script serif, no tracking, for Indic); the **time ranges render in `latinSemiBold` 12** — never the thin italic (§3) — and use **`formatRangeCompact()`** (`muhuratFormat.ts`): the shared meridiem written once (`3:37 – 5:13 PM`), full form when the window crosses noon/midnight. Chip a11y labels carry the range (`Rahu Kaal 9:13 – 10:45 AM`).
4. **`divider`** (1 dp, `colors.divider`, 12 above) — the two answer rows below share one anatomy: a **64 dp label column** (12 body serif, `inkMuted`), a body (title 16 body serif `ink` at 23 leading, up to two lines, the *answer* in the title face; a 12.5 `inkSoft` sub-line), and an **outlined pill** on the right (`cardActiveBorder` 1 dp, `parchmentSoft` fill, `radii.pill`, 32 dp min height, `titleFont 13` `saffron-deep` + `latinSemiBold ›`). **One button per row** — the pill is the affordance, not a nested control; the a11y label reads the whole row (`Vrat and Parv. None today · next Ganesh Chaturthi. Monday 14 September · in 9 days. Prepare.`).
5. **व्रत-पर्व row.** Today's observance when there is one — `आज **गणेश चतुर्थी**`, sub `आगे {next} · {n} दिन में` or the rule's deity — else the next one inside a **30-day** horizon — `आज कोई नहीं · आगे **गणेश चतुर्थी**`, sub `सोमवार 14 सितंबर · 9 दिन में` (`कल` / `tomorrow` at one day) — else `आज कोई व्रत-पर्व नहीं` / `अगले ३० दिनों में कोई नहीं`. The upcoming list is **`useUpcomingObservances()`** (`usePanchang.ts`, new): the same interaction-deferred, no-blink shape as `useObservancesForDate`, over `getUpcomingObservances` with the same-day slot dropped, so Home still never pays `usePanchangForSelection`'s day solve. **The door follows the content**: `तैयारी ›` / `Prepare ›` → `VidhiDetail { vidhiId, dateMs }` on the Home stack when the observance resolves a **published** vidhi (`getVidhiById(rule.vidhiId) ?? getVidhiForFestival(rule.id)` — drafts are invisible, §65/RULEBOOK §26; the PRD-23 bhog panel + samagri ledger are inside); `विवरण ›` / `Details ›` → the Panchang tab's `ObservanceDetail` otherwise; `सूची ›` / `All ›` → the vrat catalog when nothing is coming up.
6. **राशिफल row.** `मेष · दिनचर्या और सेवा का दिन` / `Mesha · a day for routine and service`, sub `चन्द्र राशि से · शनिवार` (`<name> की चन्द्र राशि से` once the roster holds more than one person — the §51a naming rule). The theme line is **not a prediction**: it is the house theme (`HOUSE_THEME_*`) the day's supportive transit points at — the same words the full Rashifal's Favour row uses, so the card can never say more than the screen it opens. Guest (no saved person): `आज का राशिफल` / `अपनी राशि चुनें`; `—` until the roster read lands. `पढ़ें ›` → the Panchang tab's `Rashifal` with no `rashiIndex` param, so it lands on the active person (or the picker). Data: **`useHomeRashifal()`** (`panchang/useHomeRashifal.ts`) → pure **`composeHomeRashifal(roster, date)`** (`panchang/homeRashifal.ts`) — see Behaviour for why it is a dynamic import.
7. **Ask field** (`Pressable` styled as an input: `parchmentSoft`, 1 dp `divider`, `radii.md`, `⌕` in `saffron`, 14 body serif `inkSoft`): `पूछें… गणेश चतुर्थी कब है?` / `Ask… When is Ganesh Chaturthi?` — the example names the observance the row above is already about (`observance.next`, an intent the resolver answers; `आज राहु काल कब है?` when there is none) and the tap opens **`Search { initialQuery }`** with that question, so the first tap lands on an answer card (§71), not an empty box.

**Surface.** `cardActiveFrom → cardActiveTo` gradient, 1px `cardActiveBorder`, `radii.lg`, **`elevation.raised`** (theme token), 14 dp padding. Opaque `cardActiveFrom` base, no `overflow: 'hidden'` (it would clip the iOS shadow) — the gradient carries its own radius. The §19/§32 card family. Type: titles in the script-bold serif (`latinBold` for en), body in the script medium serif (`latin` for en, sized +1 — Cormorant's x-height reads a point smaller), **every clock in `latinSemiBold`**.

**Behaviour.**

- **Six doors, one card.** Header → `PanchangTab`; `विधान ›` → `TodayVidhan`; choghadiya block + both time chips → `MuhuratDetail` (today); व्रत-पर्व → vidhi / observance detail / catalog; राशिफल → `Rashifal`; ask → `Search`. Cross-tab targets go through `panchangTabTarget` / `moreTabTarget` (`initial: false`); Home-stack targets navigate in place — the card is mounted on the Home stack's root, so `useNavigation()` resolves both. Every button is wired to the shared Home first-tap controller (`onPressIn`/`onPressOut`/`onPress` → `TilePressContext`, §18; pinned for all of them), and the chip row's `onScrollBeginDrag` marks a horizontal chip swipe as a scroll so it never opens anything.
- Data comes from **one solve**: `useMuhurat(today, calendarSystem)` supplies the muhurat windows, the live `nowChoghadiya` **and** the day's `PanchangData` (cached, off the render path); observances ride `useObservancesForDate`, the next observance `useUpcomingObservances` (both interaction-deferred, both no-blink on a store upgrade). The minute tick moves only the choghadiya block and the handover line; the **date** rolls over via **`useTodayKey()`** (`utils/useTodayKey.ts`): a timer just past local midnight plus an AppState foreground re-check, so an overnight-backgrounded app never shows yesterday's panchang. The calendar system is a **module-level store** (`usePanchangCalendarSystem`), so a purnimant/amanta change on the Panchang tab propagates to the mounted Home card immediately.
- **The Rashifal row is the one place the card touches the chart engine, and it does so off the launch graph.** `useHomeRashifal` follows the three panchang-cache rules (§61): the roster comes from `birthProfileStore`'s in-memory snapshot (the store every Jyotish surface already subscribes to, so a person switch reaches Home on the same render), disk hydration starts immediately, and the only CPU — a Kundali chart plus nine transit longitudes — waits behind `InteractionManager` and is reached through a **dynamic `import('./homeRashifal')`**, so `kundali.ts`' chart code stays off the static launch graph (`launchGraph.test.ts`). A guest roster costs nothing: no import, no solve. Re-solves only when the active person, their birth details, the roster size or the civil day change.
...
- **Reserved geometry.** Every row renders from the first frame with placeholders (`—`, a blank line) — the handover line, the choghadiya next-line, the 26 dp chip row, both answer rows — so the deferred solves landing never move the CATEGORIES grid under a finger (the §55 lesson).
- Accessibility: the header keeps the historic label `"Today's Panchang. {vara}, {tithi}. {observances}. Tap to open."`; the choghadiya block reads its state in words (`Now Udveg Choghadiya, avoid, till 12:17 PM. Next auspicious Char from 12:17 PM.`); each answer row and the ask field is one button whose label reads the row.

**Files:** `mobile/src/components/TodayStrip.tsx`; `mobile/src/panchang/useHomeRashifal.ts` + `homeRashifal.ts` (Rashifal row); `useUpcomingObservances` in `mobile/src/panchang/usePanchang.ts`; `formatLongDate` in `muhuratFormat.ts`; consumed by `HomeScreen.tsx` (§18). Tests: `components/__tests__/TodayStrip.test.tsx` (24 cases: rows, doors, live row, drift, pitru), `panchang/__tests__/homeRashifal.test.ts`; e2e `.maestro/home-today-smoke.yaml`.
...
**Geometry that must stay reserved.** Surfaces that hydrate after the reveal must occupy their final space from the first frame — never insert or grow later: the Today card reserves its 26 pt chip row, its handover line, its choghadiya block and both answer rows before the deferred solves land (§48); `RoutineBanner`'s three states share one height (`minHeight 57` = the progress variant's natural height) so the nudge → progress flip on routine hydration cannot move the CATEGORIES grid (§31); `FestiveToran` is a fixed `TORAN_HEIGHT` box resolved synchronously from the bundled catalog (§55). Anything new above the Home grid inherits this rule.
