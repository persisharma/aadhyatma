...
2. **Warm-up order.** After `NavigationContainer.onReady`, screens are warmed breadth-first by tap-distance from Home (depth 2, then 3, …), one module per idle slice: each waits for `InteractionManager` to go idle and yields 60 ms before the next, so an in-flight touch, navigation or animation always wins the thread. Non-screen warm-ups ride the same walk — the search index (built one library entry per slice) and the Panchang stack chunk, both at depth 2.
