...
5. **Pitru Paksha overview** (`PitruPakshaOverviewScreen`) — title पितृ पक्ष `<year>`, then the **standing hero** (`pitru-paksha-standing`, Sept 2026 UX review): the screen answers *where today stands in the fortnight* before it answers *what the dates are*. One 21/30 title plus one pill, in three states — **during**: today's short tithi name (`heroHi`, the row label without its qualifier clause) over a `saffronTint` pill `दिन N / M · आज`; **before**: `पितृ पक्ष आरम्भ` over `N दिन शेष · <start>` (`कल से` at one day); **after the year has rolled**: `पितृ पक्ष <year+1>` over a *quiet* `goldTint` pill `अगले वर्ष · <start> से`, plus a second 12/18 `inkMuted` line naming the paksha that ended. The rolled year is never carried by a bare numeral in the title alone. Under it, the 12/18 `भाद्रपद कृष्ण पक्ष · <range>` line.
   Then **one strip** (`radii.md`, `parchmentSoft` on `divider`, 52 pt, 8 px dot): with family-matched days, `pitru-paksha-family-strip` — saffron dot, `आपके परिवार के N दिन` + the dates, tapping scrolls to the first (captured by `onLayout`); with an **empty ledger**, `pitru-paksha-smaran-door` — gold dot, `अपने पितरों की तिथियाँ जोड़ें` → `PitruSmaranList`, the screen's only zero-entry door.
   ...
   A row **opens where it sits** when it holds something — a verified `fortnightDay` teaching, a family match, or being today (which always opens, since it is the day the screen is about). Open row = a card (`pitru-paksha-today` for today, else `pitru-paksha-day-<key>`; 1 px `saffron` on `parchmentSoft`, `radii.md`): the short tithi name, `आज · <date>` or the date, the day's family names in `saffronDeep`, the teaching paragraphs at 13/21 `inkSoft`, and **one** 44 pt `goldTint`/`gold` action — `किस दिन किसका ›` → `PitruPakshaShiksha {lessonId:'kis-din-kiska'}`, which opens **on** the lesson that explains how a tithi is matched rather than at the top of a scroll. The card carries **no guide button of its own**: the bar's filled door is the one guide door and follows the open day (below) — a dated copy on the card put the same door on screen twice whenever today was open (Sept 2026 review). Tapping the head closes it; only one day is open at a time. A day holding none of the three is not pressable at all — no chevron, no empty card. The card carries **no line on days no one is saved for**; silence is the absence, not a caption about it.
...
**Structure — two routes in `MoreStackParamList` (`PitruPakshaShiksha: {lessonId?} | undefined` · `PitruKatha: {kathaId}`), registered on `MoreStackNavigator` after `PitruPakshaOverview` so Back retraces overview → परिचय → katha. `lessonId` opens that concept lesson already unfolded, its card bordered in `gold`, scrolled to on its first layout (once) — the hand-off a dated day makes when it raises the question a lesson answers:**

1. **परिचय** (`PitruPakshaShikshaScreen`) — one reverent scroll in reading order under a gold `॥ ॐ ॥` (letter-spacing 6). Each section is present only when it has verified rows; section labels are the `typography.sectionLabel` uppercase idiom in `inkMuted`. **No lede line** — the *क्या है, क्यों है, किस दिन किसका — फिर स्मरण, फिर विधि* caption went in the Sept 2026 UX review: it described the screen's own section order, which is exactly the pattern §1's copy rule removes (pinned by a case in `PitruPakshaShikshaScreen.test.tsx`).
   - **Section rail** (`pitru-shiksha-rail`, Sept 2026) — a horizontal chip row pinned between the header and the scroll (1 px `divider` base, 30 pt chips, `radii.pill`; active chip `saffronTint` on `saffronDeep`, the rest `parchmentSoft` on `divider`): परिचय · वचन · कथाएँ · प्रश्न · शब्द, one chip per section the scroll actually renders. Tapping scrolls to that section's captured `onLayout` offset; `onScroll` (throttle 64) marks the section the reader is in. **Reading order is unchanged and this is not a door** — the chips open nothing the scroll does not already hold, so the "no door above the lessons" rule stands. It exists because the scroll is ~39 blocks and had no way back.
   - **परिचय** — concept-lesson cards (`parchmentSoft`, 1 px `divider`, `radii.md`; title 16/23 `ink`, body 15/25 `inkSoft` — the Hindi meaning/commentary register the verse pages and the §73/§74 katha screens use, lifted from 13.5/22 because at that size the clipped excerpt read as a dull caption; size carries the contrast, the colour stays consistent with the rest of the पितृ journey). A multi-paragraph lesson opens on its first paragraph and **और पढ़ें ›** (14.5/21 title face, `saffronDeep`, 32 pt min-height) unfolds the rest in place — a परिचय is read, not navigated.
   - **No पक्ष की तिथियाँ section** (Sept 2026). This screen owns the *concepts*; the fortnight is the dated overview's, and its verified tithi teachings ride those dated rows (§63.5 item 5). It was rendered in both places — one list undated here, one dated there, neither aware of the other — and that duplication is what made the pair feel disconnected. The reader reaches the fortnight through the **अब** door, and a dated day reaches back into this screen through `{lessonId}`. The registry still carries all sixteen `tithi` rows and its coverage test is unchanged; only the second rendering of them is gone.
   - **शास्त्र-वचन** — verse cards: title · quoted line(s) 14.5/24 `saffronDeep` centred (Devanagari in hi/gu/kn via `verseLinesByLang`, IAST in `latinItalic` for en) · cite 11.5 `gold` italic · meaning 13.5/22 · a hand-off pill (1 px `gold`, `radii.pill`, 32 pt) **गीता में पढ़ें ›** → the More stack's local `GitaReader {chapter, initialIndex}` or **रामायण में पढ़ें ›** → `HomeTab/ValmikiRamayanReader {chapter, initialIndex}`. A row is quoted only when its `ref` resolves to the bundled verse it cites.
   - **कथाएँ** — §63 door rows (`parchmentSoft`, `divider`): ॥ + title 15, subtitle 12/18 `inkMuted`, chevron → `PitruKatha`.
   - **प्रश्नोत्तर** — Q/A cards (question 14.5 `ink`, answer 13.5/22 `inkSoft`).
   - **शब्द** — one glossary card, **ruled**: term 14/21 `ink`, one line 13/20 `inkSoft`, and a 1 px `divider` between entries (none after the last; the card's own vertical padding drops to 0 and each row carries 11 pt). Eleven terms stacked on 10 px of margin read as a single paragraph, and this is the section people scan rather than read (Sept 2026 UX review).
   - **अब** — the three doors LAST, in the `goldTint` / 1.5 px `gold` door idiom (52 pt): **पक्ष की सोलह तिथियाँ** ("इस वर्ष की तारीख़ें · किस दिन किसका · आपके परिवार के दिन" — it is now the only way from here to the fortnight, so it names what it opens) → `PitruPakshaOverview` · **॥ पितृ तिल-तर्पण स्मरण** → `VidhiDetail {vidhiId}` (undated; rendered only when `getVidhiById('shraddha-tarpan-vidhi')` resolves) · **॥ पितृ स्मरण** → `PitruSmaranList`. **No footer line** — the *यह परिचय है, विधान नहीं — परिवार की परम्परा सर्वोपरि है।* caption that used to close the scroll went in the Sept 2026 copy pass (§73 "Copy rule"): the doors are the end of the screen. The screen's honesty stays where it is load-bearing — the verified-only registry, the hollow marker on a tithi the app can name but not yet explain, and the प्रश्नोत्तर answers themselves.
2. **कथा reader** (`PitruKathaScreen`) — the §73 `DaanKathaScreen` pattern in Pitru's register: subtitle 12.5 `inkMuted` centred, paragraphs 14/25 `inkSoft` via `commentaryByLang`, the **शिक्षा** panel (`goldTint`, 1 px `gold`, `radii.lg`; gold uppercase label; teaching 14/23 `saffronDeep`), the rendered canon line (`स्रोत: …`, 11.5 `inkMuted`), an optional **मूल श्लोक … में पढ़ें ›** pill when the katha carries a `ref`, and the `॥ ॐ ॥` endcap at 0.7 opacity. An unresolvable (draft) id renders the header only.
3. **Overview door** — §63.5.
...
**A11y.** Every pressable carries an English constant label for Maestro: `Open Pitru Paksha introduction`, `Expand lesson <id>` / `Collapse lesson <id>`, `Go to section <key>`, `Open gita reader for <id>` / `Open valmiki reader for <id>`, `Open katha <id>`, `Open Pitru Paksha overview`, `Open Tila-Tarpana remembrance guide`, `Open Pitru Smaran list`, and on the overview `Go to the first family shraddha day`, `Open day <en date>` / `Close day <en date>`, `Open the Tila-Tarpana remembrance guide for <en date>`, `Open the lesson on how a tithi is matched`. Cards are plain information panels; only interactive rows carry chevrons.
...
**Files.** `data/pitru/{types,lessons,principles,kathas,prashna,index}.ts` · `screens/PitruPakshaShikshaScreen.tsx` · `screens/PitruKathaScreen.tsx` · the door in `screens/PitruPakshaOverviewScreen.tsx` · routes in `navigation/types.ts` + `MoreStackNavigator.tsx`. Tests: `data/pitru/__tests__/pitruShikshaContent.test.ts` (Jest — registry invariants, reader-ref resolution against the bundled JSON, stance guard), `screens/__tests__/PitruPakshaShikshaScreen.test.tsx`, the two overview cases in `PitruSmaranScreens.test.tsx`. PRD: `docs/roadmap/prds/44-pitru-paksha-shiksha.md`; prototype: `docs/pitru-paksha-shiksha-prototype.html`. Contract: RULEBOOK §28.
