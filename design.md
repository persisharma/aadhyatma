...
**Regional coverage and calendar lenses** (PRD-42, Sept 2026). The catalog is **130 rules: 116 universal presentation rules, 2 lensed placeholders and 12 advanced entries**. `visibility: 'regional'` no longer exists. Two rules about what goes where:

- **A lens adds; it never subtracts.** A rule with no `lens` remains universal. A lensed rule appears in passive calendar presentation only when at least one of its calendars is active, but remains directly searchable/followable with its reminders intact. Raw precomputed/per-location tables contain both groups, so toggling is immediate and does no astronomy. The Wave 1 state observances stay universal; the lens is for genuinely noisy calendar depth, not a retroactive demotion.
...
**Pattern E calendar control (Wave 2; two-row split in Foundation).** The Vrat & Parv ledger reads **मेरा व्रत → पितृ स्मरण → `क्षेत्र · Calendars` (⌖) → `सम्प्रदाय · Calendars` (☸)** — two co-equal rows, each with a subtitle naming its own active calendars and opening a page-sheet **scoped to just its group**. The क्षेत्र sheet lists the ten state/region calendars; the सम्प्रदाय sheet lists the five tradition calendars (Jain, Gaudiya, Pushtimarg, Sri Vaishnava, Shaiva) — fifteen checkbox rows in all, each carrying explicit checked state, examples and a 64 pt minimum row; close/action/dismiss controls are at least 44 pt. Each sheet scrolls rather than fixing a viewport height, so small screens and Dynamic Type keep every row reachable. Hindi/English are authored and Gujarati/Kannada derive through the existing four-language path. A one-time, dismissible city-provenance line sits only on the Panchang day view (`जयपुर के अनुसार… · बदलें` / `Regional festivals for Jaipur… · Change`) and opens the क्षेत्र sheet; **क्षेत्र seeds silently from the chosen city, सम्प्रदाय never auto-seeds** — every tradition is a deliberate tap. A later city can offer, never replace, an existing calendar. Lensed detail heroes carry a quiet dual-script caption such as `राजस्थान · Rajasthan` beneath the deity line.
