...
- `festive-reminder` → **the reading its own message named**, via `buildEntryStartTarget` on the `sourceId` baked into the payload — so a festival tap behaves exactly like tapping that text's library card (`HomeTab → ChalisaReader`, `HomeTab → GitaChapters`, …). This is the deliberate opposite of the `daily-verse` rule above: the body is an explicit invitation to read a named text, so landing on a tab instead would break the promise, and moving the resume position is the normal consequence of any user-initiated "open this section" tap. The `sourceId` is validated against the shipped `library` first — these arm up to four months ahead, so a queued notification can outlive an OTA content rename; an unknown id falls back to `PanchangTab → ObservanceDetail` for the festival.
