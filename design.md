...
| Card name (primary language) | Noto Serif Devanagari (hi) / Cormorant Garamond (en) | 16–17 | 600 / 400 | Prominent top line on catalog, category, and deity cards. The **active reading language** takes this slot — Devanagari-first by default (`'hi'`), English-first when the toggle is `'en'`. Ordering/sizing is centralised in `orderTitlesByLanguage()`. |
| Card name (secondary language) | Cormorant Garamond (en) / Noto Serif Devanagari (hi) | 11–14 | 400 italic (en) / 500 (hi) | `ink-muted` supporting line below the primary title — the language *not* selected. |
