...
1. **परिचय** (`PitruPakshaShikshaScreen`) — one reverent scroll in reading order under a gold `॥ ॐ ॥` (letter-spacing 6) and a 13 pt lede. Each section is present only when it has verified rows; section labels are the `typography.sectionLabel` uppercase idiom in `inkMuted`.
   - **परिचय** — concept-lesson cards (`parchmentSoft`, 1 px `divider`, `radii.md`; title 16/23 `ink`, body 15/25 **`ink`** — the lesson body is this screen's reading content, so it takes the primary ink at reading scale, not the `inkSoft` caption register the other card notes use; at 13.5 `inkSoft` the clipped excerpt read as dull). A multi-paragraph lesson opens on its first paragraph and **और पढ़ें ›** (14.5/21 title face, `saffronDeep`, 32 pt min-height) unfolds the rest in place — a परिचय is read, not navigated.
