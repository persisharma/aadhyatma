...
- **5 buttons**, equally distributed:
  ...
  - **पंचांग · Panchang** — the shared almanac screen, panchang section (§56)
  - **व्रत · Vrat** — the SAME screen, vrat section (§56)
  - **ज्योतिष · Jyotish** — the SAME screen, jyotish section (§56)
...
- **One row of chrome, always.** It sits *on* the screen's existing top row, never above or below it: beside the segment control on the shared screen (§56), absolutely positioned at the right of the wordmark row on Home (§18) and of the title block on Daily Bhakti (§21), so those titles stay optically centred on the screen rather than centred in the space the icon leaves.
...
## 72. The Shared Screen: पंचांग · व्रत · ज्योतिष behind three tabs (Sept 2026)
...
- **Re-tapping the already-active tab** scrolls that segment to top. The store only bumps a `scrollToTopNonce`, so it can never be confused with a section change — and such a re-tap is **not logged as a view**, since counting taps that navigated nowhere would inflate exactly the `tab` number §72's analytics compares against swipes.
