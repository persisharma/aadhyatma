...
|---|---|---|
| **The sheet** (`LensPickerSheet.tsx`) | Bottom sheet, `parchmentHighlight`, 22 pt top radii, grabber | Title `क्षेत्रीय पंचांग`, then the "days are only ever added, never removed" line at 12.5/19 `inkSoft`, then `n सक्रिय · m और उपलब्ध` at 11 pt `inkMuted` (`सभी 22 सक्रिय` once everything is on). Under the count, **two pill buttons** — `सभी चुनें` and `सभी हटाएँ` — 36 pt tall, 1 pt `saffron` outline, 12.5 pt `saffronDeep` label, centred with a 10 pt gap; **each hides when it would be a no-op** (no सभी चुनें when all are on, no सभी हटाएँ with none on), so the header never offers a dead button. Rows are grouped उत्तर · पूर्व · पश्चिम · दक्षिण · परंपरा under 10 pt/1 pt-tracked `inkMuted` eyebrows. Each row: name at 15 pt `ink`, its two or three recognisable observances at 11 pt `inkMuted` (one line, ellipsised), then a **third line naming what this calendar adds in this build** — `जोड़ता है: रोहिणी व्रत` at 11 pt `saffronDeep` (two lines max), or `अभी कोई विशेष तिथि नहीं — आगामी अद्यतन में` in `inkMuted` when it adds nothing yet (`getRulesForLens`) — and a 22 pt circular check filled `saffron` when on. **Checkboxes, not radios** — lenses compose — and a tap applies immediately with no confirm step, because the calendar behind the sheet answers it. 52 pt row floor, ≥ 44 pt tap target. |
| **The ledger column** (`PanchangScreen.tsx`, व्रत-पर्व) | Third column of the §33 ledger bar, after मेरा व्रत and पितृ स्मरण | A `CatalogLedgerCell` inside the shared `goldTint`-on-`gold` bar: ❖ glyph, `क्षेत्र` at 14 pt, and one 11 pt line of state that still **names a calendar** — `राजस्थान +1` (the first active calendar plus a count of the rest), `सभी 22` when every calendar is on, or `22 उपलब्ध` with nothing on. The column holds ~106 pt, so the names are truncated to one plus `+n` rather than turned into a bare count: a count does not tell a user where their extra dates came from, and the leading name still does. Renders even with nothing on — for a user whose city seeded nothing it is the only path to the sheet. |
| **आपके पंचांग से** (`PanchangScreen.tsx`, व्रत-पर्व, `LensAdditionsSection`) | Card directly under the ledger bar, **only with ≥ 1 lens on** | The additive contract leaves a lensed day unmarked among the universal ones, so this card is the pointer to what the active calendars actually added. `parchmentSoft` on `divider`, `radii.md`, `elevation.card`, 14 pt gutters. Head: `आपके पंचांग से` at 14 pt `ink` over an 11 pt `inkMuted` count line (`1 तिथि जो केवल इन पंचांगों से आती है`, or `चुने हुए पंचांग अभी कोई विशेष तिथि नहीं जोड़ते`), with a `बदलें` text button (12 pt Inter SemiBold `saffronDeep`) that opens the sheet. Then **one group per active calendar in registry order** under the sheet's 10 pt uppercase eyebrow: each added rule is a `resultRow` (15 pt `ink` name, 13 pt `inkMuted` other-script caption, the next date as a 11 pt `saffronDeep` uppercase short date, `›`) opening `ObservanceDetail`; a calendar that adds nothing in this build keeps its eyebrow over one muted 12 pt line saying so — silence would read as "your tap did nothing", which was the report. Data: `getLensAdditions` + `getNextOccurrence` (`vratCatalog.ts`). |
...
**A11y.** Each lens row is `accessibilityRole="checkbox"` with `checked`, labelled in English even in a Hindi UI, naming the observances and what it adds (`Jain calendar. Paryushana · Samvatsari · Gyan Panchami. Adds 1: Rohini Vrat` / `… Adds no dates yet`), so two regional calendars are distinguishable by ear. The bulk pills are buttons `Select all 22 calendars` / `Clear all calendars` (testIDs `lens-select-all` / `lens-clear-all`). The ledger, quiet-day and More rows are buttons labelled `Regional calendars, n on` / `, all 22 on` / `, none selected`. The आपके पंचांग से card is `testID="lens-additions"`; its rows are buttons `<NameEn>, next <date>`, its `बदलें` is `Change regional calendars`.

**Files.** `mobile/src/panchang/lenses.ts` (type, ids, set, parse/serialise — I/O-free and launch-path-safe), `lensRegistry.ts` (names, examples, `CITY_LENS`/`STATE_LENS` — behind a `require()` thunk), `lensStore.ts` (AsyncStorage + once-per-install seeding + `setAllLenses`/`clearLenses`), `useLenses.ts` (`selectAll`/`clearAll`/`allSelected`), `vratCatalog.ts` (`getRulesForLens`, `getLensAdditions`); `mobile/src/components/LensPickerSheet.tsx` (`React.lazy` in both screens); entry points and `LensAdditionsSection` in `PanchangScreen.tsx`, the row in `MoreScreen.tsx`; the gate in `festivals.ts` (`getObservanceCatalog`) and `festivalEngine.ts` (day/month/upcoming). Tests: `panchang/__tests__/lens.test.ts` (incl. the lens-by-lens agreement between `getRulesForLens` and the catalog gate), `.maestro/regional-lens-smoke.yaml` (authored; device run owed). Contract: RULEBOOK §23a.5, §23a.11–12.
