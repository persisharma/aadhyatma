...
> **Superseded.** The v1 one-scroll, tab-less Home (hero title + flat LIBRARY card list) has been replaced by the tabbed Home in **Section 18** — wordmark hero, Today strip, साधना row, उपकरण row and one पाठ door. This heading is kept so section numbering holds; the flat library list now lives inside the Category List screens (§21), and the browse index in the Library screen (§74).
...
**Screens:**
- `mobile/src/screens/HomeScreen.tsx` — Home (§18: Today strip · साधना row · उपकरण row · पाठ door · नया).
- `mobile/src/screens/LibraryScreen.tsx` — पाठ, the library index (§74).
...
## 18. Screen: Home (आज-first, TRD-42)

**Purpose.** Answer "what is today, and what can I do with this app" in one screenful. It is **not** the library index — that is §74.
...
3. **आज · Today strip** (§48) — a one-card daily-panchang glance (vara + tithi headline, one horizontal-scroll row of observance / Abhijit / Rahu Kaal chips). Tap → Panchang tab. **Unchanged by TRD-42** — this is the strip the redesign was explicitly told to leave alone.
4. **आज के लिए · For Today recommendations** (§50) — a compact horizontal row of name-only `FeatureCard compact` strips (196×56) chosen from today's vaar deity and active festival metadata. Card tap → the text itself via `navigateToEntryStart` (§38).
5. **साधना row** (`components/SadhanaRow.tsx`) — up to three cells in one row: **routine** (`doneCount/total` → `RoutineToday`), **संकल्प** (the running vow's `दिन n/total` → `SadhanaPrograms`, omitted when no vow is running) and **जप** (the japa-only streak in days → `JapamCounter`, omitted at zero). Replaces the inline RoutineBanner, the नित्य साधना launcher tile and two DISCOVER cards that all opened the same two surfaces. With **nothing at all** to report it falls back to `RoutineBanner variant="inline"` rather than inventing a second empty state. All three cells share `ROW_MIN_HEIGHT` across every variant — the three sources hydrate from AsyncStorage *after* Home's first painted frame, and §64 forbids the row growing when they land.
6. Section label "उपकरण" (the shared `sectionLabel` token — Inter 11, uppercase, ink-muted, 0.22em tracking)
7. **उपकरण row** (`components/ToolsRow.tsx`) — **nine** launcher tiles (§19 launcher variant), four across at a fixed cell width, so the third row holds one left-aligned tile rather than a stretched one. Registry: `data/home/tools.ts`, **static and ordered**, data only — the routing lives in the component because each tool lands on a different stack.

   | | tool | opens |
   |---|---|---|
   | 1 | व्रत-पर्व | Panchang tab → `ObservanceList` (`panchangTabTarget`) |
   | 2 | मुहूर्त | Panchang tab → `MuhuratFinder` (§60) |
   | 3 | कुंडली | Panchang tab → `PanchangHome` in Jyotish mode (§51) |
   | 4 | पूजा विधि | Home stack → `VidhiCatalog` (§62) |
   | 5 | जप | Home stack → `CategoryList { japam }` (§35) |
   | 6 | वास्तु | More tab → `VastuDisha` (§66) |
   | 7 | तीर्थ | Home stack → `TheerthMap` (§26) |
   | 8 | पितृ स्मरण | More tab → `PitruSmaranList` (§63) |
   | 9 | दान-पुण्य | Home stack → `DaanPunya` (§73) |

   - **No shuffle, no ranking, no lifecycle badge.** The whole point of the row is that a tool sits in the same square on every visit — the failure it replaces is a carousel that reshuffled on every open plus two tiles (कुंडली, मुहूर्त) that hardcoded `hasNew: true` and so wore a badge that could never clear. Feature novelty is नया's job (§75), and नया clears itself. `components/__tests__/ToolsRow.test.tsx` pins both.
   - **There is deliberately no पंचांग tile.** The Today strip two rows above opens that tab, and so does the tab bar. A third door to the same screen is clutter, not a tool.
   - **दान-पुण्य is here for the same reason** (§73, merged in Sept 2026 while this pass was in flight). It shipped with **both** of its Home doors on surfaces this change deletes — a grid tile and a DISCOVER card, each carrying a hardcoded `hasNew: true`. Its PRD calls the grid tile "the giving layer's standing Home door", and *standing* is the operative word, so the tile is a tool; the launch announcement moved to नया (§75), which clears itself. The More row (§37) is additive, not a substitute.
   - **पितृ स्मरण is here because it had nowhere else.** It held a *standing* zero-state DISCOVER card — one that persisted until the user saved a tithi, because the Panchang ledger is a planning surface and the Today strip only names it on the day. नया is the wrong home (those cards clear on open); the tools row is the right one. `screens/__tests__/PitruSmaranTouchpoints.test.ts` is the guard that caught the near-miss.
   - Every cross-tab jump goes through a `*TabTarget` helper (`entryRoutes.ts`) carrying `initial: false` — see §32's note on the lazy tab bar and RULEBOOK §6.0.
8. **पाठ door** — one full-width row (`parchmentSoft`, `divider` border, radius 18, `minHeight 56`): a saffron `पा` thumb, the title `पाठ`, and a derived subtitle `{N} पाठ · {N} देवता · {N} उद्देश्य`. Tap → the **Library** screen (§74). This one row replaced the sixteen-tile CATEGORIES grid. The three counts are computed **once at module scope** from the bundled registries, never per render (§64).
9. **नया** (§75) — at most three cards for features this user has not opened yet, each gone for good once tapped. Renders **nothing at all**, heading included, when the list is empty. Replaced the DISCOVER carousel (§32).
10. Footer mantra (Section 7 — token `footerMantra`, 18 @ 55% opacity) at the end of the scroll
11. **Floating search button** (`SearchFloatingButton`) docked bottom-right → opens the Search screen (§36). Kept on Home deliberately: search is the fastest path to a named text, and pushing the library one screen away makes it *more* important, not less. (The Library carries its own search row too, §74.)

**What is NOT on Home.** The 16-tile CATEGORIES grid, the DISCOVER carousel, the देवता and उद्देश्य doors, and the daily verse. The first two were replaced (7–9 above); देवता/उद्देश्य moved to the Library (§74); the verse stays where it has always lived, the भक्ति tab (§23) — it was considered for Home and deliberately left alone. `screens/__tests__/HomeStructure.test.ts` fails if any of them comes back.

**One surface lost its Home door and did not get a replacement.** आज का विधान (§71) was reachable from a DISCOVER card; it is now two taps deep, behind the ⌕ FAB's empty state. That is a deliberate, recorded cost rather than an oversight — see §71 and `screens/__tests__/TodayVidhanTouchpoints.test.ts`.

**First-tap recovery (all Home cards).** Every tappable Home card opens on the **first** tap, not the second. iOS can cancel a child `Pressable`'s `onPress` when it lives inside a `ScrollView` even when the finger never actually drags, so a naive `onPress` intermittently no-ops the first touch. A single shared controller (`contexts/TilePressContext.tsx`, `useTilePressController` + `TilePressProvider`) remembers the action on `onPressIn`, arms a one-tick fallback on `onPressOut`, and fires it unless a real `onPress` already ran or a scroll drag intervened. `activateTile` only consumes a pending action that belongs to the current gesture: a *dragged* pending (a scroll whose press was cancelled) is never reused, so an accessibility tap — which invokes `onPress` with no preceding `onPressIn` — always runs the tapped card's own action rather than a stale one left behind by an earlier scroll. It is shared (context) across the **whole** Home surface — the साधना row, the उपकरण row, the पाठ door, नया, the Today strip (§48) and the For-Today row (§50) — because a card lives inside the outer vertical scroll, so a vertical page-drag started on a card must suppress *that card's* fallback; a per-component copy could not see the outer scroll and would navigate on a plain scroll. Each enclosing `ScrollView` (the outer vertical one and every inner horizontal row) wires `onScrollBeginDrag` to the controller's `markTileDrag`, so a swipe is always a scroll and never a tap. Introduced for the grid in July 2026 (#219), extended to the Today/Discover cards immediately after and to the routine banner in Aug 2026; the **Library screen (§74) shares the same controller**, since it inherited the grid. Pinned by `contexts/__tests__/TilePressContext.test.tsx` and `components/__tests__/RoutineBanner.test.tsx`.

(A **Continue-reading card** briefly sat between the grid and DISCOVER — retired July 2026, §49.)

**Why this shape (Sept 2026, TRD-42).** Two reviews, ten months apart, found the same failure in different clothes. A July 2026 review found a Home that never changed between visits — hero + carousel + catalog grid, nothing answering "what matters today" — and the Today strip + For Today row fixed *that*. The Sept 2026 review found what the fix left behind: the app's USP features (panchang, vrat katha, jyotish, muhurat, vidhi, vastu) were scattered across other tabs, a reshuffling carousel and a grid that sorted by literary **form** — चालीसा, अष्टकम्, सूक्तम् — which is a librarian's axis, not a devotee's. सूक्तम् (3 texts) got the same weight as the whole vrat engine, and the almanac features sat below two screenfuls. So the form grid went behind one door and the eight tools came up in its place: Home is now *today plus what you can do*, and the library is one tap away instead of sixteen tiles deep. The tax is real and acknowledged — the core journey (open → चालीसा → read) costs one extra tap. There is no analytics to settle whether that was the right trade (the app ships none by design); it is a product call, recorded here so the next reviewer argues with the decision rather than rediscovering it.

**Launch discipline.** Home is the screen every cold start lands on, so §64 applies in full: the first painted frame must be its final frame. Nothing here may reflow when AsyncStorage resolves — hence the module-scope counts (8), the shared row height (5) and नया rendering `null` rather than a placeholder (9).
...
**Purpose.** Grid tile representing a content category or a launcher door.

Two status variants — `active` (has content) and `coming` (placeholder) — and two **layout variants** (`variant` prop in `CategoryCard.tsx`): `launcher` and `card` (the classic 2-column gradient card, kept for the `coming` state and any future 2-column layout).

**Where the launcher variant is used (TRD-42).** It no longer serves a Home grid — Home has none. Today it renders **(a)** the Library's form grid, three across (§74), **(b)** Home's उपकरण row, four across (§18.7), and **(c)** the Namkaran nakshatra grid (§61, `launcherLabelPosition: 'tile'`). The tile spec below is identical in all three; only the column count and the caller's width differ.

**Launcher (active):**

- Column layout: a compact **glyph tile** with the name *below* it (myBhakti-review learning: label-below is what lets three columns breathe; the name-inside card can't shrink past two columns without truncating). At **four** across (the उपकरण row) the same spec holds — `shortNameEn` is what makes it survive one line.
...
- **Short English display names.** Under a ~⅓-row tile the full registry names ("Hymns & Praise", "Japa & Mantras") don't survive one line, so `categories.ts` carries an optional `shortNameEn` ("Hymns", "Japa", "Books", "Habits") used **only** by the launcher label; `data/home/tools.ts` carries the same field for the ¼-row tools tiles ("Vrat", "Vidhi", "Japa", "Vastu", "Theerth", "Pitru"). The **accessibility label always carries the full `nameEn`** (`"{nameEn}.{ New.?} Tap to open."`) — the Maestro smokes tap tiles by that full label, and screen readers keep the descriptive name.
- `NEW` badge: same pill as the card variant, inset 6px in the tile's top-right corner. Driven by `hasNewInCategory` on the **Library** grid; the उपकरण row never passes `hasNew` at all (§18.7).
...
**Purpose.** A banner surfacing today's नित्य साधना (daily routine). Two layouts via a `variant` prop: **`docked`** (default) — a floating chip pinned just above the tab bar, used on Daily Bhakti (§21); **`inline`** — flows in a vertical scroll rather than overlaying it (July 2026 — the docked chip on Home floated over and clipped the DISCOVER carousel). The only visual difference is positioning + shadow direction (docked lifts upward off the bar; inline casts a soft downward card shadow); state logic and copy are identical. `RoutineBanner.tsx` + `routineBannerView.ts` (pure state logic).

**On Home the inline variant is now the empty state only (TRD-42).** `SadhanaRow` (§18.5) took over the populated case — it shows routine progress beside the संकल्प day and the जप streak, which the one-line banner could not. When all three are empty the row renders this banner's `nudge` unchanged, rather than inventing a second "nothing yet" surface. Daily Bhakti's docked usage is untouched.
...
**First-tap (Home).** The whole banner is one `Pressable` that bubble-up navigates to the routine screen. In its inline Home usage it lives inside the outer vertical `ScrollView`, so — like every other Home card — it routes its press through the shared first-tap controller (`onPressIn`/`onPressOut`/`onPress` → `TilePressContext`, §18); a plain `onPress` intermittently no-opped the first tap. Outside a `TilePressProvider` (the docked usage on Daily Bhakti, §21) the controller's default is a no-op that runs the action immediately, so docked behaviour is unchanged.
...
## 32. Home Feature Spotlight (DISCOVER carousel) — RETIRED (Sept 2026, TRD-42)

**What it was.** A full-bleed horizontal carousel of `FeatureCard`s below the CATEGORIES grid, surfacing the app's cross-cutting surfaces (Daily Practice, Daily Verse, Sankalp, Pitru Smaran, Guided Pujas, Pilgrimage, Home-Screen Widgets). All cards always rendered — awareness was about *coverage* — with the order **shuffled once per app open** (`shuffleBySeed`, a seeded Fisher–Yates over a mulberry32 PRNG, stable across re-renders while the user stayed on Home).

**Why it went.** The shuffle was the defect, not a feature: a user could not learn where anything lived, because the row moved under their thumb on every open. Four cards hardcoded `hasNew: true`, so their badge could never clear. And the set overlapped everything around it — two cards opened the same surfaces as the routine banner and the नित्य साधना tile.

**What replaced it, and where each card went.**

| DISCOVER card | now |
|---|---|
| नित्य साधना · Daily Practice | साधना row, routine cell (§18.5) |
| संकल्प · Sankalp | साधना row, संकल्प cell (§18.5) |
| पूजा विधि · Guided Pujas | उपकरण tile 4 (§18.7) |
| तीर्थ यात्रा · Pilgrimage | उपकरण tile 7 (§18.7) |
| पितृ स्मरण · Pitru Smaran | उपकरण tile 8 (§18.7) — it needed a **standing** door, not a clearing one |
| Home-screen widgets · Guna Milan | **नया** (§75), which clears per user on open |
| दैनिक भक्ति · Daily Verse | nowhere on Home — it is a whole tab (§23) |

The general lesson is recorded in §75: novelty is a **lifecycle**, not a section. A surface that shows the same "new" thing forever is a banner.

**What survives it.** The `FeatureCard` shell below — its `compact` variant carries the FOR TODAY row (§50), which is why this section still exists. `utils/shuffleBySeed.ts` survives only if another caller needs it; it had no other Home caller.

**Deleted with the carousel:** the `spotlights` array and the `DISCOVER` label in `HomeScreen.tsx`. `screens/__tests__/HomeStructure.test.ts` fails if either returns.
...
**`compact` variant (Aug 2026).** A **name-only strip** form of the same shell, used by the FOR TODAY row (§50). It is now the **only** shipped variant — the tall default rendered the DISCOVER carousel, retired above; the default styling is kept for the next caller that needs a hero card. Home's today cluster (strip + FOR TODAY + routine banner) was consuming most of the first screenful before the CATEGORIES grid came into view, so the FOR TODAY cards were flattened to well under half their height (**56** vs ~130):
...
**Spotlight set.** There is no longer a Home spotlight array. The card's live callers are the FOR TODAY row (§50, built from today's vaar deity and festival metadata) and nothing else. A **नया** card (§75) deliberately does *not* use this shell — it is a 3-per-user strip with its own compact row, not a hero card. Sibling-tab/More-stack targets still navigate via the **parent** (`useNavigation()` → bubble up), never the Home stack — same pattern as `RoutineBanner`/`PanchangScreen`, and always through a `*TabTarget` helper carrying `initial: false` (RULEBOOK §6.0).
...
**Purpose.** The "browse by deity" front door. Reached from the Library's देवता rail — its 8 avatars link straight to a deity, and the `+N` overflow chip pushes this full index (§74; it was a Home grid tile until TRD-42). One card per deity, each opening the deity-filtered listing (§22): one card per deity, each opening the deity-filtered listing (§22). `mobile/src/screens/DeityIndexScreen.tsx`.
...
**Purpose.** Three onboarding surfaces that answer "what's in this app?" without a manual. The **first-launch feature tour** (`FeatureTour.tsx`) is an in-context, 23-step guided walkthrough that navigates the user through the real app, **rings the element each step describes**, and anchors a compact tooltip to it. The **first-run setup sheet** (`OnboardingSetupSheet.tsx`) opens the moment the tour closes and asks the fresh installer to pick a **reading language** and a **reading size** — the two settings the walkthrough's last steps point at. The **What's New sheet** (`WhatsNewModal.tsx`) fires once after an update and lists only that release's new features. All three are gated by `TourContext.tsx` and mounted in `App.tsx` inside the provider stack; the tour renders as a **top-level in-tree overlay** (last child of the root view, above the navigator, mini-player and tab bar), the setup sheet is a transparent `Modal` beside it, and What's New stays a `Modal` inside `NavigationContainer`. Complements §44 (NEW badge / OTA prompt) — those mark *content*; this orients the *app*.

**First-run sequence:** tour (23 steps, non-interactive) → setup sheet (language + size, interactive) → app. A returning user gets neither: What's New instead.
...
**Steps (23), in order** (`mobile/src/data/tour/steps.ts` — each: `id`, `navigateTo`, optional `targetId`, `anchor`/`pointer` fallback, bilingual title/body). The sequence is a guided walkthrough: **(1–5) the five bottom tabs** — Home, Daily Bhakti, Panchang, Bhajan, More — each a **tab-ring** (no element target); **(6–10) Home** — routine card (`routineCard`, now the **साधना row**, §18.5), the **उपकरण row** (`toolsRow`), the **पाठ door** (`libraryDoor`), then the two *inside* views that still stand on their own: Japa / mantra list (`HomeTab/CategoryList{japam}`, `japamInside`) and the Theerth temple map (`HomeTab/TheerthMap`, `theerthInside`); **(11–12) Bhakti** — Daily Verse (`dailyVerse`), Share (`shareButton`); **(13–17) Panchang** — Daily Muhurat / Choghadiya card (`muhuratCard`), the Vrat & Parv segment (`panchangSegment`), the vrat list (`PanchangTab/ObservanceList{vrat}`, `vratList`), the ★ follow affordance (`vratFollow`), My Vrat & reminders (`PanchangTab/MyVrat`, `myVrat`); **(18) Bhajan** — the library (`AudioTab`, `bhajanInside`); **(19–23) More** — Daily Reminder toggle (`MoreTab/Reminders`, `reminderToggle`), reminder times (`reminderTimes`), Japam Alarm add button (`MoreTab/JapamAlarms`, `japamAdd`), then the two rows the setup sheet asks for next: **Language** (`MoreTab/MoreHome`, `languageRow`) and **Reading Size** (`readingSizeRow`) — so the user has seen where both live *before* being asked to choose, and knows where to change them later. Both wrap their `SettingsRow` in a measurable `View` in `MoreScreen` and declare a `scrollNodeIntoView` reveal (the App group sits below the fold on smaller devices). Because the overlay is non-interactive, the vrat drill (15–17) auto-navigates and *describes* the ★-follow / 🔔-reminder affordances rather than expecting a tap. **Retargeted Sept 2026 (TRD-42 §10).** The old steps 7–9 and 11 rang `categoriesGrid`, `japaTile` and `theerthTile` — `useTourTarget` registrations that lived in `HomeScreen` and were deleted with the 16-tile grid. A step whose target cannot be measured silently falls back to ringing the destination tab, so the tour would not have crashed; it would have *described things that are not there*, which is worse. `categories` → `tools`, `japa` → `library`, and the `theerth` tile step was **deleted** (its inside step, the map, stayed). The retired ids are gone from `TourTargetId` and from `tourContent.jest.test.ts`'s `VALID_TARGET_IDS` mirror. That release also bumped `APP_TOUR_VERSION` with a What's New entry, since the tour only runs for fresh installs and a *returning* user needs to be told the library moved. A compile-time check pins every `navigateTo.name` to a real `TabParamList` tab; `TAB_ORDER` maps each to its bar index for the tab-ring fallback. `TourNavTarget` allows a nested `{ screen, params }` for `HomeTab`, `MoreTab`, and `PanchangTab` (the last widened to reach `ObservanceList`/`MyVrat`). Element targets register via `useTourTarget(id, reveal?)` in `HomeScreen`, `CategoryListScreen`, `TheerthMapScreen`, `DailyBhaktiScreen`, `PanchangScreen`, `ObservanceListScreen`, `MyVratScreen`, `AudioLibraryScreen`, and `ReminderSettingsScreen`/`JapamAlarmsScreen`.
...
**Purpose.** Make Home answer *"what matters today"*, not only *"what can I read"*. One glance-card directly below the wordmark hero (§18.3) surfaces the day's panchang state; the full detail stays on the Panchang tab. This closed the daily-freshness gap identified in the July 2026 competitive review — the previous Home rendered identically on every visit.
...
- **Home नया card** (§75 / `data/home/featureFeed.ts`, id `home-widgets`) — a per-user novelty card (`वि` glyph in the `saffronDeep`/`typography.thumb` grammar of the §46 संकल्प card) opening the Widget Gallery via `moreTabTarget('WidgetGallery')`. It was a shuffled DISCOVER spotlight until TRD-42; it now shows only to a user who has not opened it, and **disappears for good on the first tap** rather than wearing a badge that can never clear.
...
**Entries.** Home's **मुहूर्त** उपकरण tile, after कुंडली (§18.7 — no NEW badge; the tools row carries none, TRD-42); the **MuhuratFinderDoor** row on the Panchang tab (between glance card and anga grid). Both are additive.
...
**Entries — no new Home category** (the launcher grid stays closed): the day panel's `॥ पूजा विधि` ObservanceCard pill (§33.5, driven by `vidhiId` on the observance rule — the identical hook mechanism as `kathaId`), the **पूजा विधि** tile on the Vrat & Parv catalog's Browse-by-type grid (§33) — the always-available door, since the pill is date-dependent — and, since Phase 2B: **search rows** (one section-group row per vidhi, §36) and the **Observance Detail "पूजा विधि · How to observe" card** (§33). Its Home DISCOVER spotlight became the **पूजा विधि उपकरण tile** (§18.7) when the carousel was retired — a fixed square instead of a reshuffling card.
...
9. **Home उपकरण tile** — a standing door exists even with zero saved people: the eighth tile of the tools row (§18.7) → `MoreTab/PitruSmaranList`. It was a DISCOVER `FeatureCard` until TRD-42; it became a **tool** rather than a नया card precisely because नया clears on open and this door must persist for a user who has never saved a tithi. This is awareness, not the personal ledger treatment; the actual list and Panchang row remain quiet/muted. `screens/__tests__/PitruSmaranTouchpoints.test.ts` is the guard.
...
**Geometry that must stay reserved.** Surfaces that hydrate after the reveal must occupy their final space from the first frame — never insert or grow later: the Today strip reserves its 24 pt chip row before the deferred Panchang solve lands (§48); `RoutineBanner`'s three states share one height (`minHeight 57` = the progress variant's natural height) so the nudge → progress flip on routine hydration cannot move what is under it (§31); **`SadhanaRow`'s three-cell, two-cell and fallback variants share `ROW_MIN_HEIGHT`** for the same reason across three independently-hydrating sources (§18.5); the पाठ door's counts are module-scope constants, not a render-time fold (§18.8); **नया renders `null`, never a placeholder**, so an unresolved seen-map costs zero height (§75); `FestiveToran` is a fixed `TORAN_HEIGHT` box resolved synchronously from the bundled catalog (§55). Anything new on Home inherits this rule.
...
## 72. प्रश्न — ask the chart a purpose; the आधार chain (PRD-43 Wave D)

**Purpose.** People do not consult an astrologer for a report; they arrive with a purpose — *my daughter's studies*, *should I start this venture*, *is this a time to change jobs*. प्रश्न is that surface: pick a purpose, get the answer the way an astrologer gives it — the short answer, why, what supports and resists, when, what to do, and a practice — with every sentence carrying the chart facts it came from. Prototype: `docs/kundali-prashna-prototype.html`. Contract: RULEBOOK §14.3 (rewritten), §25 (the intent).

**Entry.** On the saved Jyotish landing (§51c) प्रश्न is a **full-width lead card ABOVE the 2×2 door grid**, not a fifth tile: `cardActiveFrom` fill, `cardActiveBorder`, `radii.lg`, `elevation.card`, min-height 64, title *प्रश्न · अपनी कुंडली से पूछें* (15, title face) with a 9 pt `newBadgeText` NEW tag, one 11 pt `inkMuted` line naming five of the nine purposes, a `saffronDeep` chevron; `testID="jyotish-prashna-door"`, a11y label `Open Prashna`. The grid answers *"what does my chart say"*; this card answers *"what should I do about X"*. The one-fold constraint of §51c holds (one insert). Guest/error landings carry none of it. `जिज्ञासा` (§71) deep-links here with `{ purposeId }` from the `prashna.purpose` intent.

**Screen (`PrashnaScreen`).** Back (44) + title *प्रश्न* / caption *अपनी कुंडली से पूछें* → **किसके लिए** card (name · Lagna; Moon sign · age as of the report date) → **विषय चुनें** 2-column grid of nine `Pressable` tiles (48% width, min-height 72, `radii.md`; selected = active card surface + border; **a closed purpose stays visible at 0.45 opacity WITH its reason on the second line — `18 वर्ष के बाद · From age 18` — never hidden**; `accessibilityState.disabled` + label `Purpose <name>, closed until age N`; `testID="purpose-<id>"`) → the answer in **six fixed blocks**, each a section card (`parchmentSoft`, `divider`, `radii.lg`; सार and उपाय on the active surface):

1. **सार · the short answer** — one-sentence title (16/22 title face) + the **strength pill** (`prashna-strength`: `प्रबल / मध्यम / क्षीण`, uppercase 9.5 pt tracked, `goldChipBg`/card-surface/`avoidChipBg` fills, three 3×9 bars lit to the strength — colour is never the only carrier, §12) + a body that counts factors (`3 कारक पक्ष में, 1 विपक्ष में`), states the leading support and, when any resist exists, **names the tension** (`पर — …`).
2. **आधार · why this reading** — intro line (`प्रबल संकेत — 3 स्वतंत्र कारक एक दिशा में, 1 विपरीत`) + up to five `BasisChain`s, one per top factor.
3. **बल और बाधा** — two columns (`goldTint` / `avoidTint`, `radii.md`), every factor's sentence; an empty column says so in words.
4. **काल · supportive windows** — intro line, then ≤4 window rows (`cardSurface`, 2 pt left rule `gold` when relevant / `divider` when not, 0.82 opacity when not): tabular date range `1 Sep 2010 → 4 Jul 2029` (+ `· अभी` when running) and the composed sentence. Always the running Maha and Antar; the next Antar that touches the purpose when one exists; Jupiter/Saturn transits over a purpose bhava with their next-ingress end date. **Never an event date.**
5. **दिशा · what to do** — dash-led lines: the parent register first for a minor, the purpose × strength lines, a dusthana-lord caution when triggered, and one line saying whether the running period touches the matter; then action pills — `मुहूर्त खोजें` for व्यापार/यात्रा/विवाह, `गुण मिलान` for विवाह, `पूर्ण विवेचन` always.
6. **उपाय · practice** — the purpose's allow-listed practice (`navagraha-stotram` / `surya-ashtakam` / `shani-ashtakam`) via `buildEntryStartTarget()`.

Footer (10.5/16, hairline top rule): `As of <date>. A view from traditional Jyotish — not a certain prediction. Every sentence shows its basis above.` A **closed purpose renders only the "अभी नहीं" card with the gate reason** — no सार, no factors, no windows (RULEBOOK §14.3.5). Loading/error/guest states mirror §68.

**The आधार chain (`BasisChain`).** The structural device of the whole PRD-43 surface family (report §68, प्रश्न, and the Ask card's working). A left-ruled (`cardActiveBorder`) block on `cardSurface`, a 9 pt `saffronDeep` eyebrow, then chips (`parchmentSoft`, `divider`, `radii.sm`, 10.5/14 **`fontFamilies.inter`, tabular digits**) joined by `saffron` arrows: `पंचम भाव = कुम्भ → पंचम भाव का स्वामी शनि · प्रथम भाव में → शनि · प्रथम भाव · उच्च → शनि महादशा · 1 Sep 2010 → 4 Jul 2029`. **Serif is the tradition speaking; sans is the arithmetic** — a reader who distrusts the sentence audits the chips. Labels come from `basisLabelHi/En` in the engine so the screen, the export and the Ask card never word a node three ways. One accessibility label per chain (`Basis: … , then …`). The component renders nothing for an empty chain because the ENGINE refuses to emit one (a build-failing test), so an empty prop means a fact-only section.

**Nine purposes (`prashnaPurposes.ts`).** विद्या 5·4·9 (गुरु, बुध) · व्यापार 10·7·11·3 (बुध, गुरु, मंगल) 18+ · नौकरी 10·6·11 (शनि, सूर्य) 18+ · धन 2·11·4 (गुरु, शुक्र, मंगल) 18+ · विवाह 7·2·11 (शुक्र, गुरु) 21+ · संतान 5·9 (गुरु) 21+ · स्वास्थ्य 1·6·8 (सूर्य, चन्द्र) **constitution and routine only, never a prognosis, always routes to a doctor** · यात्रा 9·3·12 (राहु, चन्द्र) · मन 4·1·12 (चन्द्र). Age is derived at read time from the chart's birth instant and never stored.

**Files.** `mobile/src/panchang/prashnaPurposes.ts`, `prashna.ts`, `kundaliBasis.ts`; `mobile/src/components/BasisChain.tsx`; `mobile/src/screens/PrashnaScreen.tsx`; the lead card in `PanchangScreen.tsx` (`JyotishLanding`); route `Prashna` in `navigation/types.ts` + `PanchangStackNavigator.tsx` (a `getComponent` thunk, off the launch graph); Ask: `src/ask/intents/index.ts` (`prashna.purpose` — triggers/blockers/examples only) + `src/ask/intents/prashnaResolve.ts` (the resolver body, loaded through a `require()` thunk so the composer's ~150 KB tree stays off Home's static launch graph), `lexicon.ts` (purpose forms, registry `require()`d inside `build()` for the same reason), `resolve.ts` (stance-guard yield), `useAsk.ts` (`ctx.kundali` = the active person's birth INPUT read from `birthProfileStore`, never the chart — computing it would put astronomy-engine on the first frame). Tests: `prashna.engine.test.ts` (purity, vocabulary scans incl. fatality/medical/commerce, registry, basis completeness, age-gate matrix, counted strength, shown tension, dated windows, स्वास्थ्य bounds), `PrashnaExperience.test.tsx`, `src/ask/__tests__/prashna.test.ts` + corpus cases, `.maestro/prashna-smoke.yaml` (authored; device run pending).
---

## 74. Screen: पाठ — the Library index (TRD-42)

**Purpose.** Everything the Home grid used to hold, on one screen, organised by the axes a devotee actually thinks in. Pushed from Home's पाठ door (§18.8) on the **Home stack**, so Back returns to Home. `screens/LibraryScreen.tsx`, registered with a `getComponent={() => require(...)}` thunk rather than a static import — it is never the initial route, so it stays off the launch import graph (§64, `data/__tests__/launchGraph.test.ts`), the same treatment GharVastu and प्रश्न take.

**Structure (top to bottom):**

1. `ReaderHeader variant="index"` — title `पाठ` / `Library`, back control labelled `Back` (English by contract, §9).
2. **Search row** — a 44 pt `parchmentSoft` field-lookalike (`radii.md`, `cardActiveBorder`, `elevation.subtle`) with a `⌕` and a rotating-language placeholder. Tap → the Search screen (§36). It **leads** because a named text is faster to type than to browse, and the screen is now two taps from a cold start.
3. **देवता rail** — a horizontal row of **8** deity discs (50 pt circle, `cardThumbRest` fill, `DeityIcon` at 30) with the name below, followed by an **`+N`** overflow chip → `DeityIndexScreen` (§42). The rail full-bleeds (`marginHorizontal: -spacing.xxl`, re-padded content) exactly like §32's carousel did.
4. **उद्देश्य chips** — every purpose from `purposes.ts` as a `saffronTint` pill, wrapping. Tap → `PurposeList` **directly**. The old Home tile opened `BrowseByPurposeScreen`, an *index of these same chips* — so this row **is** that index and the middle screen is gone, not lost. (`BrowseByPurposeScreen` remains routable, §50.)
5. **पाठ के प्रकार grid** — the former CATEGORIES grid, three across, `CategoryCard variant="launcher"` (§19), in `categories.ts` order, each `active` tile carrying its **count** below (`interSemiBold` 10.5, `saffronDeep`). A `coming` tile shows `SOON` and **no number** rather than a misleading 0.
6. **संग्रह tile** — one more launcher row → `Wishlist`, registered on the Home stack so Back returns here rather than dropping the user on the More tab.

**Counts are derived, never authored.** `data/libraryCounts.ts` folds `library` once per process (memoised — the bundled array cannot change at runtime) counting only what a user can open: `status: 'active'` and not `hidden`, the same pair Home and `getTodayRecommendationDetails` already filter on. Adding a text changes the number with no edit here. Do **not** confuse it with `panchang/vratCatalog.getCategoryCounts()`, which counts *observance rules*.

**Why form is last.** The grid it inherited sorted texts by literary **form** — चालीसा, अष्टकम्, सूक्तम् — which is a librarian's axis. It gave सूक्तम् (3 texts) the same weight as the whole vrat engine. Deity and purpose are how a devotee arrives at a text, so they lead; the forms follow, now with their real counts so the weighting is visible rather than implied.

**Shared first-tap controller.** The whole screen mounts its own `useTilePressController` + `TilePressProvider` and wires `onScrollBeginDrag` on both the page scroll and the deity rail — it inherited the grid, so it inherited §18's iOS first-tap problem verbatim.

**Tests.** `screens/__tests__/LibraryScreen.test.tsx` (all three axes present with nothing dropped, counts bound to the real registry, तीर्थ routed to the map not a text list, Back → Home, the NEW badge still asked per active category); `.maestro/library-smoke.yaml` + the shared `_open-library.yaml` subflow every category smoke now runs.

---

## 75. नया — the feature-novelty lifecycle (TRD-42)

**Purpose.** Tell **this** user about features **they** have not opened yet, at most three, then stop. Replaces the DISCOVER carousel (§32), whose failure it is designed around: a section that shows the same "new" thing forever is a banner, and a badge that can never clear is decoration.

**Placement.** Last block on Home, below the पाठ door. `components/NewFeaturesSection.tsx`.

**Present-or-absent is the whole vocabulary.** When nothing is pending the component returns `null` — heading included. There is no "you're all caught up" state, the same stance the शुभ योग card takes (§69), because that is a sentence nobody needs to read every morning. This is also what keeps it §64-safe: an empty नया costs zero height, so Home's first painted frame is its final frame whether or not the seen-map has resolved.

**Card.** A compact row on the §8 active-card gradient (`radii.md`, `cardActiveBorder`, `elevation.card`): a 30 pt `cardThumbActiveFrom` disc holding one Devanagari grapheme (`thumb`, the same convention as `LibraryEntry`), the title (14) over a one-line description (11.5, `ink-soft`), the saffron `NEW` badge (§19 geometry, `interSemiBold` 10 — the §3.0 floor), and a `›` chevron. Whole card is the press target; a11y label `"{titleEn}. {descEn} Tap to open."`.

**Resolver (`data/home/featureFeed.ts`, pure).**

- `FEATURE_FEED` is newest-version-first. An entry carries a **stable id** (never reused for a different feature), the `version` that introduced it, bilingual title + one-line description, a thumb grapheme, and a typed `FeatureTarget`.
- `pendingFeatures(seen, appVersion, feed, limit = 3)` = feed, filtered by `compareSemver(f.version, appVersion) <= 0` (an entry can land in the bundle a release before its screen is reachable), filtered by `seen[id] === undefined`, capped at 3. **Feed order, not recency of seeing.**
- `seedSeenForFreshInstall(appVersion)` — a first install starts with **everything already seen**. A first-time user has no history for anything to be new against and is about to get the 23-step tour (§47); three नया cards would be a second, worse tour.
- `feedVersionsMissingFromWhatsNew()` must return `[]` — pinned by `data/__tests__/featureFeed.test.ts`.

**Why there is no new "is this new" registry.** The app already had three (`addedInVersion` for content, the tour keys, `whatsNew`). `data/tour/whatsNew.ts` already curates *what shipped in version X* bilingually, and RULEBOOK §6.1 already makes bumping it mandatory on any release that moves `APP_TOUR_VERSION`. So a feed entry carries **only what the What's New sheet cannot**: a stable id, an icon and a route. The parity test above fails the build if a feed entry names a version `whatsNew` does not know.

**Seen state (`contexts/FeatureSeenContext.tsx`).** `@vedansh/feature-seen`, a `{ [featureId]: version }` map. Deliberately separate from `NewContentContext` (§44): that tracks *content* keyed by discoverable id and answers "is this reading new"; this tracks *features* and answers "has this user been shown this door". Folding them together would mean one blob whose halves invalidate on different events. Three behaviours worth keeping:

- **Install vs upgrade** uses the same `UPGRADER_SIGNAL_KEYS` probe as §44 — a returning user gets an empty map (so everything shipped since their last version surfaces), a fresh install gets the full seed.
- **Mark-then-navigate, in-memory first.** `markFeatureSeen` updates state before awaiting the write, so a card that hides on tap cannot read a stale map on the next render and reappear. Same order as `TourContext.markTourCompleted`.
- **An unreadable store means "all seen", not "all new".** The opposite fallback would advertise every feature to a user who has opened them all.

**Relationship to the other two novelty surfaces.** §44's `NEW` badge marks *texts* and clears per text (now on the Library grid, §74). The What's New sheet (§47) announces a *release* once, to everyone updating into it. नया is the per-user, per-feature middle: it survives the sheet being dismissed, and it clears per person rather than per version.

**Tests.** `data/__tests__/featureFeed.test.ts` (tsx — `src/data` is out of Jest): version gate, seen filter, order, cap, fresh-install seeding, feed ⇄ `whatsNew` parity, id uniqueness. `components/__tests__/NewFeaturesSection.test.tsx` (Jest): renders nothing when empty, marks seen before navigating, cross-tab targets carry `initial: false`.
