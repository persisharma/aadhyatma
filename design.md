...
- **Opt-in modal** (`ReminderOptInModal.tsx`, mounted app-wide): a `pageSheet` shown once, gated on the **third app open** with the reminder off and the prompt not yet shown ("earn the ask, never ambush") — lede, a `TimeStepper`, a filled `saffron` **Enable**, and a quiet uppercase *Not now*. Because the toggle defaults on, the provider asks the OS for permission on the **first launch that finds it unanswered**; the sheet is the second chance after a refusal. Two refinements on the gate (Aug 2026): (1) the sheet never shows while the OS is **hard-blocked** (`canAskAgain: false`) — its Enable button could not succeed, so that state belongs to the Reminders screen's Settings banner; (2) a **one-time re-offer** (`optInReofferShown` in notif meta) re-shows the sheet to installs whose `optInPromptShown` was burned while the OS permission was never actually answered — i.e. Android users the pre-fix builds silently opted out (see the permission-state bullet below); answering or dismissing the re-offer sets both flags and ends the matter.
