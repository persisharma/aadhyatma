...
4. **Abujh calendar** (`AbujhDaysScreen`) — days needing no shuddhi, resolved by the **festival engine** (`ABUJH_RULE_IDS`, `panchang/abujhMuhurat.ts`) plus computed Guru/Ravi Pushya days; each row routes to that day's `MuhuratDetail`.
...
**Shared components (no forks — §7/§9, RULEBOOK §9).** All four screens use the canonical **`ReaderHeader`** (`variant="index"`, like GunaMilan §58) — `[back] · centred title · right slot` — never a local top-bar/back copy; the day detail passes the shared **`ShareButton`** into its `right` slot. Secondary context that used to live as a header subtitle is a content line instead (the picker's "आप क्या करने जा रहे हैं?" prompt; the results' city label). The result and abujh **date cards** share one treatment — `radii.lg`, `elevation.card` (rank-1 gets the `cardActiveFrom→cardActiveTo` gradient + `cardActiveBorder` + `elevation.lifted`, the same active-card grammar as `LibraryCard`), an `alignItems:center` row with the standard 24pt saffron chevron — so the "date + best window" card looks identical in both sections. No ordinal digits: rank is carried by order + the section labels + the rank-1 hero, matching the app's no-numbered-list convention.

**Performance.** The occasion-independent per-day inputs (panchang solve + asta flags) are memoised module-side (`useMuhuratFinder` `DAY_INPUT_CACHE`, keyed by location + calendar system + start-day, bounded to a few keys). The first occasion pays the ~90-day astronomy scan once (~4.5 s); every subsequent occasion and re-entry reuses the cache and resolves in a couple of milliseconds (`evaluateDay` only). The scan stays chunked behind `InteractionManager`, and yields are skipped for fully-cached chunks so a warm occasion never flashes the spinner.
