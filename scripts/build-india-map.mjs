 ...
 * outline + state-boundary SVG paths used by <IndiaMap> — from open map data.
 * Run once at build time; the output is committed as static constants
 * (design.md §28: no runtime GeoJSON parsing).
 *
 *   node scripts/build-india-map.mjs
 *
 * Sources:
 *   - National outline: DataMeet india-composite (depicts India's official
 *     boundary, incl. the full Jammu & Kashmir / Ladakh extent; Natural Earth's
 *     de-facto boundary truncates the northern Kashmir "crown"). CC-BY — see
 *     https://github.com/datameet/maps (attribution recorded in the output).
 *   - State boundaries: Natural Earth 50m admin_1_states_provinces (public
 *     domain — https://www.naturalearthdata.com/about/terms-of-use/), which
 *     carries the post-2019 states/UTs whose names match TheerthTemple.stateEn.
 *
...
const SOURCES = {
  // National outline — DataMeet's official-boundary composite (full J&K crown).
  outline: {
    file: path.join(CACHE, 'datameet_india_composite.geojson'),
    url: 'https://raw.githubusercontent.com/datameet/maps/master/Country/india-composite.geojson',
  },
  // State/UT boundaries — Natural Earth (post-2019 states, names match temples).
  admin1: {
...
async function main() {
  const [outlineGeo, admin1] = await Promise.all([
    loadGeoJSON(SOURCES.outline),
    loadGeoJSON(SOURCES.admin1),
  ]);

  // DataMeet india-composite is an India-only FeatureCollection (the country is
  // the whole file). Project + simplify every feature's geometry into the
  // national outline — mainland plus the major-island archipelagos.
  const outline = (outlineGeo.features ?? [])
    .filter((f) => f.geometry)
    .flatMap((f) => geometryToSubpaths(f.geometry));
  if (outline.length === 0) throw new Error('no outline geometry in india-composite');
...
//   National outline — DataMeet india-composite, which depicts India's official
//     boundary including the full Jammu & Kashmir / Ladakh extent.
//     © DataMeet, CC-BY — https://github.com/datameet/maps
//   State/UT boundaries — Natural Earth 50m admin_1_states_provinces.
