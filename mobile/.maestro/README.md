...
| `ashtakam-smoke.yaml` | Ashtakam. Home → Ashtakam tile → CategoryList asserts **all 14 sections** (Sankat Mochan Hanuman Ashtak, Lingashtakam, Madhurashtakam, Achyutashtakam + the 10 PRD-A §A.4.2 deity-expansion ashtakams, scrolled in library order) → Lingashtakam reader → language toggle → back. PRD-A entries use the multi-instance AshtakamReader; the pre-existing Hanuman Ashtak keeps its dedicated reader. |
...
| `hast-rekha-smoke.yaml` | **Hast Rekha (Palm Reading)** against the **native iOS app** (like `kundali-smoke.yaml`). Clears app state, switches to English, opens Panchang → Jyotish → the Palm Reading tool card, opens the **camera guide** (permission-ungranted path: privacy copy + manual fallback; capture/review are unit-tested since Maestro cannot drive a live camera), drives the four-line self-observation picker (heart/head/life/fate testIDs), asserts the reading with the guidance disclaimer + vitality-not-lifespan framing + Navagraha practice hand-off, leaves and returns to verify the saved profile restores the reading, then exercises **Start over**. Build/install first with `npx expo run:ios --device <UDID> --port 8084`. |
