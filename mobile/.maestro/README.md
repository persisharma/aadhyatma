...
| `backup-restore-smoke.yaml` | **बैकअप व पुनर्स्थापन** (PRD-06 Track C, design.md §75). More → English → asserts the ऐप-group door (`Backup and Restore, new`) → BackupScreen: the "What a backup holds" summary with all four group rows, `No backup yet`, the **Export backup file** and **Restore from backup file** actions and the "file is the backup" footer → Back. Neither the OS share sheet nor the document picker is opened (Maestro cannot drive or dismiss them); the envelope format, the registry-completeness gate and the restore/confirm/reload path are pinned by `src/backup/__tests__/backup.test.ts` + `BackupScreen.test.tsx`. Authored 2026-09-25; not yet run on a simulator (container has none) — verify per `wiki/runbooks/e2e-verification.md`. |
