...
| `muhurat-day-cache-smoke.yaml` | **Persistent panchang day-cache** (design.md §60). Warms the finder + Abujh scans on a fresh install, then relaunches WITHOUT `clearState` — a real cold start with AsyncStorage intact — and asserts both surfaces re-render from the hydrated cache inside a 20 s budget (vs the 60 s first-run solve). Native build; the regression it guards is re-solving ~90–260 days on every app open. |
