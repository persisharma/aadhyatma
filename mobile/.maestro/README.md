...
| `home-today-smoke.yaml` | **Home today-first redesign** (design.md §18/§48/§49). Flips to English (Devanagari is unreadable to Maestro's iOS tree), asserts the "आज · Today" strip renders on Home, opens the **व्रत (Vrat & Parv)** launcher tile into the Panchang stack's ObservanceList and backs out onto PanchangHome (guards the `panchangTabTarget`/`initial: false` lazy-mount route bug), then reads a page of Hanuman Chalisa and asserts the **Continue reading** card appears below the grid (scrollUntilVisible) and resumes the reader. |
