...
| `background-art-smoke.yaml` | **Devotional background art** (PR #306). Opens the screens that resolve the NEW plates — By Deity → Shani Dev and Maa Kali (deity plates), Home → Kavacham (category plate) — asserting each list renders (a broken asset require would crash before the list draws) and screenshotting each surface to `/tmp/vedansh-bg-art-*` for visual review. Data-side completeness is pinned by `src/data/__tests__/backgrounds.coverage.jest.test.ts`. |
...
| `pitru-paksha-smoke.yaml` | **पितृ पक्ष fortnight journey** (design.md §63.5 item 5 + §74, Sept 2026 UX review). English chrome; saves one ancestor (माघ कृष्ण अष्टमी · Father), then — in season — overview standing hero → family strip opens the family day → the name on it opens the person's page → `किस दिन किसका ›` opens the paged परिचय reader on lesson 6/7 → swipe to the closing card → 400 ms hand-off returns to the same overview (`popTo`) → परिचय hub: question accordion, verse meaning fold-out, katha tile, glossary door, reader from page 1 → deletes the ancestor (net zero). **Season-gated:** the overview's only UI doors render ~30 days before purnima through amavasya, so blocks 2–4 run under `runFlow: when` and are skipped off-season; the behaviour is pinned date-independently in `PitruSmaranScreens.test.tsx` + `PitruPakshaShikshaScreen.test.tsx`. Authored 2026-09-23; not yet run on a simulator (container has none) — verify per `wiki/runbooks/e2e-verification.md`. |
...
| `share-katha-smoke.yaml` | **Prose share + multi-page series** (design.md §39.4–§39.6, PRD-45). English mode → Katha library → first katha → `Share katha` → asserts the pages strip (`Page 1`, `Page 2`), the all-pages rows (`Share all pages`, `Share as Instagram carousel`), the three single-page rows and the hashtag preview; opens `Preview page`, returns via `Back to share options`, then cancels. Asserts row presence, not enabled state, so it holds on a binary without the multi-image module too. Capture/caption/one-share-call contracts are pinned by `utils/__tests__/shareSeries.test.tsx`; the "first katha is a series" assumption by `shareContent.test.ts`. |
