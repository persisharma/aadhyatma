...
| `home-today-smoke.yaml` | **Home today-first redesign** (design.md §18/§48/§49). Asserts the "आज · Today" strip renders on Home, opens the **व्रत (Vrat & Parv)** launcher tile into the Panchang stack's ObservanceList and backs out onto PanchangHome (guards the `initial: false` lazy-mount route bug), then reads a page of Hanuman Chalisa and asserts the **Continue reading** card appears on Home and resumes the reader. |
