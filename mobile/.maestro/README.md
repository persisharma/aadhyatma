...
| `_open-library.yaml` | **Shared subflow** — from Home, scrolls to the पाठ door and opens the Library index. Home's 16-tile CATEGORIES grid moved there (TRD-42 §5.3), so every category smoke now runs `_launch.yaml` then `_open-library.yaml`. Enter on Home; exits on Library, scrolled to the top. |
...
| `gita-smoke.yaml` | Bhagavad Gītā **chaptered reader** — the path `granth-smoke` skips. Home → पाठ → Sacred Books → Bhagavad Gītā → chapter index → Chapter 1 reader; exercises horizontal verse paging and the `JumpToStartButton` "back to verse 1" pill (PR #97). Auto-advance to the next chapter (PR #29) is scroll-velocity driven (~47 swipes), so it's unit-tested in `src/screens/__tests__/gitaAutoAdvance.test.tsx` instead. |
...
| `kavacham-smoke.yaml` | Kavacham (PRD-A, multi-instance form). Home → पाठ → Kavacham tile → CategoryList (Rama Raksha Stotra) → reader → language toggle → back. First text: Rama Raksha Stotra (source-verified, 38 shlokas). |
| `stuti-smoke.yaml` | Stuti (PRD-A, multi-instance form) — **filed under Stotram, not its own tile** (स्तुति ≈ स्तोत्रम्). Home → पाठ → Stotram (Hymns & Praise) tile → CategoryList → scroll to Krishna Stuti → reader (StutiReader) → language toggle. First text: Krishna Stuti (source-verified, 12 verses). |
| `ashtakam-smoke.yaml` | Ashtakam. Home → पाठ → Ashtakam tile → CategoryList asserts **all 15 sections** (Sankat Mochan Hanuman Ashtak, Lingashtakam, Madhurashtakam, Achyutashtakam + the 10 PRD-A §A.4.2 deity-expansion ashtakams + Rudrashtakam, scrolled in library order) → Lingashtakam reader → language toggle → back. Multi-instance entries use AshtakamReader; the pre-existing Hanuman Ashtak keeps its dedicated reader. |
| `suktam-smoke.yaml` | Suktam (PRD-A, multi-instance form). Home → पाठ → Suktam tile → CategoryList (Tantrokta Devi Suktam) → reader → language toggle → back. First text: Tantrokta Devi Suktam (source-verified, 30 verses). Purusha/Narayana Suktam BLOCKED (Vedic recension divergence). |
...
| `vidhi-smoke.yaml` | **PRD-19 Puja Vidhi (Phase 1) + PRD-23 Bhog guidance.** Panchang tab → Vrat & Parv → पूजा विधि tile → vidhi catalog → Satyanarayan detail (duration-only header, sourced bhog guidance + occurrence-scoped kitchen checklist, routine-style samagri summary/checklist, तैयारी/पूजा segments) → "Begin puja" conduct mode → swipes to the dhyana reading card and back → exits with state saved. Provenance remains private; bottom reader dots replace previous/next controls and swipe-helper copy. The ObservanceCard's vidhi pill is date-dependent (purnima), so the deterministic door is the catalog tile; the pill itself is unit-tested in `VidhiScreens.test.tsx` + the vidhiId hook in `vidhiContent.test.ts`. ✅ verified on iOS 26.4 and Android 16 / API 36 (2026-08-25). |
...
| `new-content-badge-library-smoke.yaml` | NEW badge — **Library category tiles** (was `new-content-badge-home-smoke.yaml`; the grid it asserts moved off Home, TRD-42 §5.3). Dev-seeds on Home, opens पाठ, asserts `hasNewInCategory` scoping (chalisa/aarti/stotram/sanskar light up; granth, japam and the देवता rail render plain), then marks the four stotram debut-new entries seen and confirms the tile's badge clears. Distinct from Home's **नया** strip, which tracks *features* rather than texts. |
...
| `deity-browse-smoke.yaml` | **By-Deity** general browse (independent of the NEW badge). Home → पाठ → "All N deities" → asserts the deity grid (Rama/Krishna/Vishnu) → opens Shri Vishnu and asserts "Vishnu Sahasranama" is listed (PR #99 deity tagging, verified on-screen) → opens the entry and returns. |
| `background-art-smoke.yaml` | **Devotional background art** (PR #306). Opens the screens that resolve the NEW plates — पाठ → deity index → Shani Dev and Maa Kali (deity plates), पाठ → Kavacham (category plate) — asserting each list renders (a broken asset require would crash before the list draws) and screenshotting each surface to `/tmp/vedansh-bg-art-*` for visual review. Data-side completeness is pinned by `src/data/__tests__/backgrounds.coverage.jest.test.ts`. |
| `deity-expansion-smoke.yaml` | **PRD-A §A.4.2 deity expansion (9 → 21).** Home → पाठ → "All N deities" → scrolls through all 12 new deity cards (Maa Lakshmi … Navagraha, registry order) → Shani Dev → Shani Ashtakam in AshtakamReader (guards the registry-derived `entryRoutes` routing) → back → Navagraha → Navagraha Stotram in StutiReader → language toggle on both readers. |
| `discovery-purpose-smoke.yaml` | **Intent discovery** (PRD-B). Home → पाठ → उद्देश्य chips → Protection list → opens Rama Raksha Stotra and asserts the first-page "When to Recite" panel. The Home **By Purpose** tile and the purpose index behind it both retired with the grid — the chips on the Library are that index. |
...
| `ask-answer-smoke.yaml` | **जिज्ञासा answer path** (PRD-41 Phases 1+3, design.md §71). FAB → Search → types "what to offer ganesha" → asserts the answer card ("Answer. Naivedya for Shri Ganesha"), the निषेध Tulsi row, expands "Show the working" (names `ganesha-bhog`), takes the primary "Observance" action into Ganesh Chaturthi's detail, then uses its "Ask about Ganesh Chaturthi" row to ask the bare word "katha" and asserts the seeded katha answer. Authored 2026-09-02; not yet run on a simulator (container has none) — verify per `wiki/runbooks/e2e-verification.md`. |
| `ask-abstain-smoke.yaml` | **जिज्ञासा abstain + stance guard + §3.6 non-regression.** "will i get the job" → "Question declined" with the stance copy; "what is karma" → "No answer" while library results still render; plain "Hanuman" → Sections group, no abstain card. Authored 2026-09-02; not yet run on a simulator. |
| `ask-briefing-smoke.yaml` | **आज का विधान briefing** (PRD-41 Phase 2). Search empty state → "Open Today's Vidhan" → header, first answer card composes (30 s budget for the astronomy), "Ask something else" → Search, Back → briefing, Back → Home (a Home-stack push, not a tab hijack). Authored 2026-09-02; not yet run on a simulator. |
...
| `read-aloud-smoke.yaml` | **Read Aloud (पाठ सुनें) — on-device TTS** (design.md §56). **Native build only** — `expo-speech` is a native module, unavailable in Expo Go. Three parts: the deterministic More → **Read Aloud** settings sheet (asserts the `पाठ सुनें` header, steps the rate 1.0 → 1.2 → 1.0 via `Faster`/`Slower`, toggles `Read commentary` on and off, restores every preference so the persisted state doesn't leak into other flows), then the in-reader control on Hanuman Chalisa (the v1 surface), then the same control on Bajrang Baan via the Stotram category (representative of the September-2026 fan-out — every reader except Japam now carries the pill; `readerReadAloud.test.tsx` presses it on all 21). Each reader leg asserts the control is present as **either** `Read aloud` or `Read aloud unavailable`, but marks the play/pause transition `optional: true`: a CI emulator with no Hindi voice data renders the muted state by design, and that is not a failure. Part 3 authored 2026-09-04; device run owed. |
