...
| `kundali-smoke.yaml` | PRD-C Kundali + Daily Rashifal against the **native iOS app** (`com.prashantsharma.vedansh`, not Expo Go). Clears app state, switches to English, uses the permanent Home Kundali launcher, enters the Ujjain golden-fixture birth profile, checks beginner overview + North Indian chart + grahas + Vimshottari Dasha, then opens deterministic Daily Rashifal. Build/install first with `npx expo run:ios --device <UDID> --port <isolated-port>`. |
