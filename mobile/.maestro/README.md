...
| `sadhana-sankalp-smoke.yaml` | Sadhana Programs (संकल्प, PRD-11 #167) — prebuilt multi-day vows clubbed into routine creation. From the docked `RoutineBanner`: RoutineCreate 'choose' fork → **Choose a prebuilt sankalp** → opens the `consecutive` "Hanuman Chalisa — 41 Days" program (date-independent, unlike the festival/weekday sankalps) → **Begin this sankalp** enrols and lands on "Today's Practice", where the `SankalpTodayCard` shows `SANKALP · DAY 1 / 41` + the day's item → **Mark today's practice done** commits the day (card flips to the "Today's reading is done" resting state) → Back to the now-active Detail → **Set this sankalp aside** abandons it, returning the program to the **Available** shelf (badge reverts to "41 days") so back-to-back runs stay clean. The list/detail now sit on the deity sketch background with warm `LibraryCard`-style cards (gradient thumb + both languages + status pill + chevron), so the card assertions match on the title substring (cards carry an explicit `<titleEn>. <subtitleEn>. Tap to open.` a11y label) and the Today eyebrow reads `Sankalp · Day 1 / 41`. ✅ verified on iOS sim. |
...
| `feature-tour-e2e.yaml` | **First-launch feature tour** (in-context walkthrough). Driven via the replay affordance (More → "Show feature tour again") for determinism against a persisted simulator: opens the overlay, walks all 9 steps forward (Home → Panchang → Today's Practice → Bhajan → Bhakti → Japa → Wishlist → Reminder → Share) asserting each step's subtitle, exercises **Back**, and finishes with **Done**, asserting the overlay is gone. The auto-show-on-first-launch path is the same overlay; `_launch.yaml` dismisses it for every other flow. |
