    ...
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        ...
        <View style={[styles.toggleRow, { flexDirection: 'row', justifyContent: 'center', gap: 18 }]}>
          <LanguageToggle />
          <AddToRoutineButton sourceId="vishnu-sahasranama" chapter={chapter.chapter} />
              ...
              return (
                <ShivaStrotamVersePage
                  ...
                  topActions={
                    <>
                      <BookmarkButton
                        isBookmarked={isBookmarked(`vishnu-sahasranama:${chapter.chapter}:${verseIdx}`)}
                        onToggle={() => {
                          const id = `vishnu-sahasranama:${chapter.chapter}:${verseIdx}`;
                          if (isBookmarked(id)) { removeBookmark(id); }
                          else {
                            addBookmark({ id, sourceId: 'vishnu-sahasranama', chapter: chapter.chapter, verseIndex: verseIdx, savedAt: Date.now(), previewHi: item.sanskrit[0] ?? '', previewEn: item.linesEn[0] ?? '' });
                          }
                        }}
                      />
                      <ShareButton
