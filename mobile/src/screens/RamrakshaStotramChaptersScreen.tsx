...
import { ramrakshaStotramChaptersManifest, ramrakshaStotramTitleHi, ramrakshaStotramTitleEn } from '@/data/ramraksha-stotram';
...
type Props = NativeStackScreenProps<HomeStackParamList, 'RamrakshaStotramChapters'>;

export default function RamrakshaStotramChaptersScreen({ navigation }: Props) {
  const { colors, typography, spacing } = useTheme();
  const { lang } = useGitaLanguage();
  const { getChapterProgress } = useReadingProgress();

  const title = lang === 'hi' ? ramrakshaStotramTitleHi : ramrakshaStotramTitleEn;
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <BackgroundLayer source={getSourceBackground('ramraksha-stotram')} />
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        ...
        >
          {ramrakshaStotramChaptersManifest.map((chapter) => (
            <GitaChapterCard
              key={chapter.chapter}
              chapter={chapter}
              onPress={() => {
                const resumeIndex = getChapterProgress('ramraksha-stotram', chapter.chapter)?.verseIndex ?? 0;
                navigation.navigate('RamrakshaStotramReader', { chapter: chapter.chapter, initialIndex: resumeIndex });
              }}
            />
          ))}
        </ScrollView>
