...
import { getStuti, type StutiVerse } from '@/data/stuti';
...
export default function StutiReaderScreen({ navigation, route }: Props) {
  ...
  const stutiId = route.params?.stutiId ?? 'krishna-stuti';
  const audioTrack = useMemo(() => {
    const t = getTrackForText(stutiId);
    return t && hasRealAudio(t.id) ? t : undefined;
  }, [stutiId]);
  const stuti = useMemo(() => getStuti(stutiId), [stutiId]);
  const verses = stuti.verses;
  ...
  }, [stutiId, currentIndex, setProgress]);
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        <ReaderHeader
          title={contentByLang(lang, stuti.titleHi, stuti.titleEn)}
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <VersePage
                ...
                sourceId={stutiId}
                width={width}
                belowContent={index === 0 ? <WhenToRecitePanel sourceId={stutiId} /> : undefined}
                topActions={
                  <>
                    <BookmarkButton
                      isBookmarked={isBookmarked(`${stutiId}::${index}`)}
                      onToggle={() => {
                        const id = `${stutiId}::${index}`;
                        if (isBookmarked(id)) {
                          removeBookmark(id);
                        } else {
                          addBookmark({
                    ...
                    <ShareButton
                      ...
                      onPress={() => {
                        share(
                          {
                            sourceId: stutiId,
                            sectionNameHi: stuti.titleHi,
                            sectionNameEn: stuti.titleEn,
