              ...
              return (
                <ShivaStrotamVersePage
                  ...
                  topActions={
                    <>
                      <BookmarkButton
                        isBookmarked={isBookmarked(`durga-stotram:${chapter.chapter}:${verseIdx}`)}
                        onToggle={() => {
                          const id = `durga-stotram:${chapter.chapter}:${verseIdx}`;
                          if (isBookmarked(id)) { removeBookmark(id); }
                          else {
                            addBookmark({ id, sourceId: 'durga-stotram', chapter: chapter.chapter, verseIndex: verseIdx, savedAt: Date.now(), previewHi: item.sanskrit[0] ?? '', previewEn: item.linesEn[0] ?? '' });
                          }
                        }}
                      />
                      <ShareButton
