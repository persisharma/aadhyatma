...
import { adityaHridayamChaptersManifest, adityaHridayamTitleHi, adityaHridayamTitleEn } from '@/data/aditya-hridayam';
...
type Props = NativeStackScreenProps<HomeStackParamList, 'AdityaHridayamChapters'>;

export default function AdityaHridayamChaptersScreen({ navigation }: Props) {
  const { colors, typography, spacing } = useTheme();
  const { lang } = useGitaLanguage();
  const { getChapterProgress } = useReadingProgress();

  const title = lang === 'hi' ? adityaHridayamTitleHi : adityaHridayamTitleEn;
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <BackgroundLayer source={getSourceBackground('aditya-hridayam')} />
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        ...
        >
          {adityaHridayamChaptersManifest.map((chapter) => (
            <GitaChapterCard
              key={chapter.chapter}
              chapter={chapter}
              onPress={() => {
                const resumeIndex = getChapterProgress('aditya-hridayam', chapter.chapter)?.verseIndex ?? 0;
                navigation.navigate('AdityaHridayamReader', { chapter: chapter.chapter, initialIndex: resumeIndex });
              }}
            />
          ))}
        </ScrollView>
