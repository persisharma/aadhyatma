...
import { getHanumanAshtakChapter, type HanumanAshtakVerse } from '@/data/hanuman-ashtak';
...
type Props = NativeStackScreenProps<HomeStackParamList, 'HanumanAshtakReader'>;
...
export default function HanumanAshtakReaderScreen({ navigation, route }: Props) {
  ...
  const chapter = useSafeChapter(route.params.chapter, getHanumanAshtakChapter, navigation, 'HanumanAshtakChapters');
  const listRef = useRef<FlatList<HanumanAshtakVerse>>(null);
  ...
  useEffect(() => {
    ...
    setProgress({ sourceId: 'hanuman-ashtak', chapter: chapter.chapter, verseIndex: currentIndex, updatedAt: Date.now() });
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        ...
        <View style={[styles.toggleRow, { flexDirection: 'row', justifyContent: 'center', gap: 18 }]}><LanguageToggle /><AddToRoutineButton sourceId="hanuman-ashtak" chapter={chapter.chapter} /></View>
