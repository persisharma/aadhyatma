...
const { getBajrangBaanChapter } = jest.requireActual<
  typeof import('@/data/bajrang-baan')
>('@/data/bajrang-baan');
const BajrangBaanReaderScreen = jest.requireActual<
  typeof import('../BajrangBaanReaderScreen')
>('../BajrangBaanReaderScreen').default;

type ReaderProps = React.ComponentProps<typeof BajrangBaanReaderScreen>;

const chapter = getBajrangBaanChapter(1);
...
const route = {
  key: 'BajrangBaanReader-test',
  name: 'BajrangBaanReader',
  params: { chapter: 1, initialIndex: 0 },
} as ReaderProps['route'];

test('renders the first Bajrang Baan verse without throwing', () => {
  ...
  act(() => {
    tree = TestRenderer.create(
      <GitaLanguageProvider initialLang="hi">
        <ShareProvider>
          <BajrangBaanReaderScreen navigation={navigation} route={route} />
        </ShareProvider>
  ...
  assert.match(renderedText, /बजरंग बाण/);
