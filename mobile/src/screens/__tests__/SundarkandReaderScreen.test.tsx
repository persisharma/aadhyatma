...
const getActualData = jest.requireActual<typeof import('@/data/sundarkand')>('@/data/sundarkand');
...
const SundarkandReaderScreen = jest.requireActual<typeof import('../SundarkandReaderScreen')>(
  '../SundarkandReaderScreen'
).default;

type ReaderProps = React.ComponentProps<typeof SundarkandReaderScreen>;

const source = getActualData.getSundarkandChapter(1);
...
const route = {
  key: 'SundarkandReader-test',
  name: 'SundarkandReader',
  params: { chapter: 1, initialIndex: 0 },
} as ReaderProps['route'];

test('renders the first Sundarkand verse without throwing', () => {
  ...
  act(() => {
    tree = TestRenderer.create(
      <GitaLanguageProvider initialLang="hi">
        <ShareProvider>
          <SundarkandReaderScreen navigation={navigation} route={route} />
        </ShareProvider>
