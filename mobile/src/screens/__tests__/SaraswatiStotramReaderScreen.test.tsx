...
const { getSaraswatiStotramChapter } = jest.requireActual<
  typeof import('@/data/saraswati-stotram')
>('@/data/saraswati-stotram');
const SaraswatiStotramReaderScreen = jest.requireActual<
  typeof import('../SaraswatiStotramReaderScreen')
>('../SaraswatiStotramReaderScreen').default;

type ReaderProps = React.ComponentProps<typeof SaraswatiStotramReaderScreen>;

const chapter = getSaraswatiStotramChapter(1);
...
const route = {
  key: 'SaraswatiStotramReader-test',
  name: 'SaraswatiStotramReader',
  params: { chapter: 1, initialIndex: 0 },
} as ReaderProps['route'];

test('renders the first Saraswati Stotram verse without throwing', () => {
  ...
  act(() => {
    tree = TestRenderer.create(
      <GitaLanguageProvider initialLang="hi">
        <ShareProvider>
          <SaraswatiStotramReaderScreen navigation={navigation} route={route} />
        </ShareProvider>
  ...
  assert.match(renderedText, /सरस्वती नमस्तुभ्यं/);
