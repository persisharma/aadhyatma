...
const ShivaStrotamReaderScreen = jest.requireActual<
  typeof import('../ShivaStrotamReaderScreen')
>('../ShivaStrotamReaderScreen').default;

type ReaderProps = React.ComponentProps<typeof ShivaStrotamReaderScreen>;

const chapter = getShivaStrotamChapter(1);
...
const route = {
  key: 'ShivaStrotamReader-test',
  name: 'ShivaStrotamReader',
  params: { chapter: 1, initialIndex: 0 },
} as ReaderProps['route'];

test('renders the first Shiva Strotam verse without throwing', () => {
  ...
  act(() => {
    tree = TestRenderer.create(
      <GitaLanguageProvider initialLang="hi">
        <ShareProvider>
          <ShivaStrotamReaderScreen navigation={navigation} route={route} />
        </ShareProvider>
  ...
  assert.ok(renderedText.includes(chapter.titleHi));
