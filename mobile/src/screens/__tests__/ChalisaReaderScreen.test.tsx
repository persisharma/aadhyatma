...
const { getChalisa } = jest.requireActual<typeof import('@/data/chalisaRegistry')>(
  '@/data/chalisaRegistry'
);
const ChalisaReaderScreen = jest.requireActual<typeof import('../ChalisaReaderScreen')>(
  '../ChalisaReaderScreen'
).default;

type ReaderProps = React.ComponentProps<typeof ChalisaReaderScreen>;

const chalisa = getChalisa('hanuman-chalisa');
const firstVerse = chalisa.verses[0];
...
const route = {
  key: 'ChalisaReader-test',
  name: 'ChalisaReader',
  params: { chalisaId: 'hanuman-chalisa', initialIndex: 0 },
} as ReaderProps['route'];

test('renders the first Hanuman Chalisa verse without throwing', () => {
