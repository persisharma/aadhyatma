...
const getActualData = jest.requireActual<typeof import('@/data/hanuman-ashtak')>('@/data/hanuman-ashtak');
...
const HanumanAshtakReaderScreen = jest.requireActual<typeof import('../HanumanAshtakReaderScreen')>(
  '../HanumanAshtakReaderScreen'
).default;

type ReaderProps = React.ComponentProps<typeof HanumanAshtakReaderScreen>;

const source = getActualData.getHanumanAshtakChapter(1);
...
const route = {
  key: 'HanumanAshtakReader-test',
  name: 'HanumanAshtakReader',
  params: { chapter: 1, initialIndex: 0 },
} as ReaderProps['route'];

test('renders the first Hanuman Ashtak verse without throwing', () => {
  ...
  act(() => {
    tree = TestRenderer.create(
      <GitaLanguageProvider initialLang="hi">
        <ShareProvider>
          <HanumanAshtakReaderScreen navigation={navigation} route={route} />
        </ShareProvider>
