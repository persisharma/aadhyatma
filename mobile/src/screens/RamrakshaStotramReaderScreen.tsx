...
import { getRamrakshaStotramChapter, type RamrakshaStotramVerse } from '@/data/ramraksha-stotram';
...
type Props = NativeStackScreenProps<HomeStackParamList, 'RamrakshaStotramReader'>;

const DOT_COUNT = 5;

export default function RamrakshaStotramReaderScreen({ navigation, route }: Props) {
  ...
  const chapter = useSafeChapter(route.params.chapter, getRamrakshaStotramChapter, navigation, 'RamrakshaStotramChapters');
  const listRef = useRef<FlatList<RamrakshaStotramVerse>>(null);
  ...
  useEffect(() => {
    ...
    setProgress({ sourceId: 'ramraksha-stotram', chapter: chapter.chapter, verseIndex: currentIndex, updatedAt: Date.now() });
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        <View style={styles.topBar}>
          ...
          <View style={[styles.topSide, { alignItems: 'flex-end' }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              ...
              <BookmarkButton
                isBookmarked={isBookmarked(`ramraksha-stotram:${chapter.chapter}:${currentIndex}`)}
                onToggle={() => {
                  const id = `ramraksha-stotram:${chapter.chapter}:${currentIndex}`;
                  if (isBookmarked(id)) { removeBookmark(id); }
                  else {
                    const v = chapter.verses[currentIndex];
                    addBookmark({ id, sourceId: 'ramraksha-stotram', chapter: chapter.chapter, verseIndex: currentIndex, savedAt: Date.now(), previewHi: v.sanskrit[0] ?? '', previewEn: v.linesEn[0] ?? '' });
                  }
                }}
              />
              <ShareButton
        ...
        <View style={[styles.toggleRow, { flexDirection: 'row', justifyContent: 'center', gap: 18 }]}><LanguageToggle /><AddToRoutineButton sourceId="ramraksha-stotram" chapter={chapter.chapter} /></View>
        ...
        <View style={styles.listContainer}>
          <FlatList
            ...
            data={chapter.verses}
            keyExtractor={(v) => v.id}
            renderItem={({ item }) => <ShivaStrotamVersePage verse={item} sourceId="ramraksha-stotram" width={width} />}
            extraData={lang}
            horizontal
            pagingEnabled
