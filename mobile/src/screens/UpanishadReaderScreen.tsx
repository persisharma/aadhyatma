...
import UpanishadVersePage from '@/components/UpanishadVersePage';
...
type FlatListItem = UpanishadVerse | NextTransitionItem | PrevTransitionItem;

type Props = NativeStackScreenProps<RootStackParamList, 'UpanishadReader'>;

const SOURCE_ID = 'upanishad';
const DOT_COUNT = 5;

export default function UpanishadReaderScreen({ navigation, route }: Props) {
  ...
  const onViewableItemsChanged = useRef(({ viewableItems }: { viewableItems: ViewToken[] }) => {
    ...
    if ('__type' in item && item.__type === 'transition') {
      if (!hasNavigatedRef.current) {
        ...
        setTimeout(() => {
          navigation.replace('UpanishadReader', { chapter: item.nextChapter });
        }, 400);
      }
      return;
    }
    if ('__type' in item && item.__type === 'prev-transition') {
      if (!hasNavigatedRef.current) {
        ...
        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium).catch(() => undefined);
        setTimeout(() => {
          navigation.replace('UpanishadReader', {
