...
import { getKavacham, type KavachamVerse } from '@/data/kavacham';
...
type Props = NativeStackScreenProps<RootStackParamList, 'KavachamReader'>;

const DOT_COUNT = 5;

export default function KavachamReaderScreen({ navigation, route }: Props) {
  ...
  const kavachamId = route.params?.kavachamId ?? 'rama-raksha-stotra';
  const audioTrack = useMemo(() => {
    const t = getTrackForText(kavachamId);
    return t && hasRealAudio(t.id) ? t : undefined;
  }, [kavachamId]);
  const kavacham = useMemo(() => getKavacham(kavachamId), [kavachamId]);
  const verses = kavacham.verses;
  const total = verses.length;
  const listRef = useRef<FlatList<KavachamVerse>>(null);
  ...
  }, [kavachamId, currentIndex, setProgress]);
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        <ReaderHeader
          title={contentByLang(lang, kavacham.titleHi, kavacham.titleEn)}
        ...
        <View style={[styles.toggleRow, { flexDirection: 'row', justifyContent: 'center', gap: 18 }]}>
          <LanguageToggle />
          <AddToRoutineButton sourceId={kavachamId} />
        </View>

        <View style={styles.listContainer}>
          <FlatList
            ref={listRef}
            data={verses as KavachamVerse[]}
            keyExtractor={(v) => v.id}
            renderItem={({ item, index }) => (
              <VersePage
                ...
                sourceId={kavachamId}
                width={width}
                belowContent={index === 0 ? <WhenToRecitePanel sourceId={kavachamId} /> : undefined}
                topActions={
                  <>
                    <BookmarkButton
                      isBookmarked={isBookmarked(`${kavachamId}::${index}`)}
                      onToggle={() => {
                        const id = `${kavachamId}::${index}`;
                        if (isBookmarked(id)) {
                          removeBookmark(id);
                        } else {
                          addBookmark({
                    ...
                    <ShareButton
                      ...
                      onPress={() => {
                        share(
                          {
                            sourceId: kavachamId,
                            sectionNameHi: kavacham.titleHi,
                            sectionNameEn: kavacham.titleEn,
