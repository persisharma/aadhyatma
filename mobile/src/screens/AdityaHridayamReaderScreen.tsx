...
import { getAdityaHridayamChapter, type AdityaHridayamVerse } from '@/data/aditya-hridayam';
...
type Props = NativeStackScreenProps<HomeStackParamList, 'AdityaHridayamReader'>;

const DOT_COUNT = 5;

export default function AdityaHridayamReaderScreen({ navigation, route }: Props) {
  ...
  const chapter = useSafeChapter(route.params.chapter, getAdityaHridayamChapter, navigation, 'AdityaHridayamChapters');
  const listRef = useRef<FlatList<AdityaHridayamVerse>>(null);
  ...
  useEffect(() => {
    ...
    setProgress({ sourceId: 'aditya-hridayam', chapter: chapter.chapter, verseIndex: currentIndex, updatedAt: Date.now() });
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        <View style={styles.topBar}>
          ...
          <View style={[styles.topSide, { alignItems: 'flex-end' }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              ...
              <BookmarkButton
                isBookmarked={isBookmarked(`aditya-hridayam:${chapter.chapter}:${currentIndex}`)}
                onToggle={() => {
                  const id = `aditya-hridayam:${chapter.chapter}:${currentIndex}`;
                  if (isBookmarked(id)) { removeBookmark(id); }
                  else {
                    const v = chapter.verses[currentIndex];
                    addBookmark({ id, sourceId: 'aditya-hridayam', chapter: chapter.chapter, verseIndex: currentIndex, savedAt: Date.now(), previewHi: v.sanskrit[0] ?? '', previewEn: v.linesEn[0] ?? '' });
                  }
                }}
              />
              <ShareButton
        ...
        <View style={[styles.toggleRow, { flexDirection: 'row', justifyContent: 'center', gap: 18 }]}><LanguageToggle /><AddToRoutineButton sourceId="aditya-hridayam" chapter={chapter.chapter} /></View>
        ...
        <View style={styles.listContainer}>
          <FlatList
            ...
            data={chapter.verses}
            keyExtractor={(v) => v.id}
            renderItem={({ item }) => <ShivaStrotamVersePage verse={item} sourceId="aditya-hridayam" width={width} />}
            extraData={lang}
            horizontal
            pagingEnabled
