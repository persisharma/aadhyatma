...
import { getSaraswatiStotramChapter, saraswatiStotramChaptersManifest, type SaraswatiStotramVerse } from '@/data/saraswati-stotram';
...
type FlatListItem = SaraswatiStotramVerse | NextTransitionItem | PrevTransitionItem;

type Props = NativeStackScreenProps<HomeStackParamList, 'SaraswatiStotramReader'>;

const DOT_COUNT = 5;

export default function SaraswatiStotramReaderScreen({ navigation, route }: Props) {
  ...
  const chapter = useSafeChapter(route.params.chapter, getSaraswatiStotramChapter, navigation, 'SaraswatiStotramChapters');
  ...
  const isLastChapter = chapter == null ? true : chapter.chapter >= saraswatiStotramChaptersManifest.length;
  const isFirstChapter = chapter == null ? true : chapter.chapter <= 1;
  const data: FlatListItem[] = useMemo(() => {
    if (chapter == null) return [];
    const items: FlatListItem[] = [];
    if (!isFirstChapter) {
      const prev = saraswatiStotramChaptersManifest[chapter.chapter - 2];
      if (prev) {
        items.push({
    ...
    if (!isLastChapter) {
      const next = saraswatiStotramChaptersManifest[chapter.chapter];
      if (next) {
        items.push({
  ...
  useEffect(() => {
    if (chapter == null) return;
    setProgress({ sourceId: 'saraswati-stotram', chapter: chapter.chapter, verseIndex: currentIndex, updatedAt: Date.now() });
  ...
  const onViewableItemsChanged = useRef(({ viewableItems }: { viewableItems: ViewToken[] }) => {
    ...
    if ('__type' in item && item.__type === 'transition') {
      if (!hasNavigatedRef.current) {
        ...
        setTimeout(() => {
          navigation.replace('SaraswatiStotramReader', { chapter: item.nextChapter });
        }, 400);
      }
      return;
    }
    if ('__type' in item && item.__type === 'prev-transition') {
      if (!hasNavigatedRef.current) {
        ...
        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium).catch(() => undefined);
        setTimeout(() => {
          navigation.replace('SaraswatiStotramReader', { chapter: item.prevChapter, initialIndex: item.prevVerseCount - 1 });
        }, 400);
      }
      return;
    }
        ...
        <View style={styles.topBar}>
          ...
          <View style={[styles.topSide, { alignItems: 'flex-end' }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              ...
              <BookmarkButton
                isBookmarked={isBookmarked(`saraswati-stotram:${chapter.chapter}:${currentIndex}`)}
                onToggle={() => {
                  const id = `saraswati-stotram:${chapter.chapter}:${currentIndex}`;
                  if (isBookmarked(id)) { removeBookmark(id); }
                  else {
                    const v = chapter.verses[currentIndex];
                    addBookmark({ id, sourceId: 'saraswati-stotram', chapter: chapter.chapter, verseIndex: currentIndex, savedAt: Date.now(), previewHi: v.sanskrit[0] ?? '', previewEn: v.linesEn[0] ?? '' });
                  }
                }}
              />
              <ShareButton
        ...
        <View style={styles.listContainer}>
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return <ShivaStrotamVersePage verse={item} sourceId="saraswati-stotram" width={width} />;
            }}
            extraData={lang}
            horizontal
            pagingEnabled
