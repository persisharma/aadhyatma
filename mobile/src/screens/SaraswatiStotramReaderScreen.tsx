    ...
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        ...
        <View style={[styles.toggleRow, { flexDirection: 'row', justifyContent: 'center', gap: 18 }]}>
          <LanguageToggle />
          <AddToRoutineButton sourceId="saraswati-stotram" chapter={chapter.chapter} />
              ...
              return (
                <ShivaStrotamVersePage
                  ...
                  topActions={
                    <>
                      <BookmarkButton
                        isBookmarked={isBookmarked(`saraswati-stotram:${chapter.chapter}:${verseIdx}`)}
                        onToggle={() => {
                          const id = `saraswati-stotram:${chapter.chapter}:${verseIdx}`;
                          if (isBookmarked(id)) { removeBookmark(id); }
                          else {
                            addBookmark({ id, sourceId: 'saraswati-stotram', chapter: chapter.chapter, verseIndex: verseIdx, savedAt: Date.now(), previewHi: item.sanskrit[0] ?? '', previewEn: item.linesEn[0] ?? '' });
                          }
                        }}
                      />
                      <ShareButton
