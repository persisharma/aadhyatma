...
type Props = NativeStackScreenProps<RootStackParamList, 'AshtakamReader'>;

const DOT_COUNT = 5;

export default function AshtakamReaderScreen({ navigation, route }: Props) {
  ...
  const ashtakamId = route.params?.ashtakamId ?? 'lingashtakam';
  const audioTrack = useMemo(() => {
    const t = getTrackForText(ashtakamId);
    return t && hasRealAudio(t.id) ? t : undefined;
  }, [ashtakamId]);
  const ashtakam = useMemo(() => getAshtakam(ashtakamId), [ashtakamId]);
  const verses = ashtakam.verses;
  const total = verses.length;
  const listRef = useRef<FlatList<AshtakamVerse>>(null);
  ...
  }, [ashtakamId, currentIndex, setProgress]);
    ...
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        ...
        <View style={[styles.toggleRow, { flexDirection: 'row', justifyContent: 'center', gap: 18 }]}>
          <LanguageToggle />
          <AddToRoutineButton sourceId={ashtakamId} />
        </View>

        <View style={styles.listContainer}>
          <FlatList
            ref={listRef}
            data={verses as AshtakamVerse[]}
            keyExtractor={(v) => v.id}
            renderItem={({ item, index }) => (
              <VersePage
                ...
                sourceId={ashtakamId}
                width={width}
                belowContent={index === 0 ? <WhenToRecitePanel sourceId={ashtakamId} /> : undefined}
                topActions={
                  <>
                    <BookmarkButton
                      isBookmarked={isBookmarked(`${ashtakamId}::${index}`)}
                      onToggle={() => {
                        const id = `${ashtakamId}::${index}`;
                        if (isBookmarked(id)) {
                          removeBookmark(id);
                        } else {
                          addBookmark({
                    ...
                    <ShareButton
                      ...
                      onPress={() => {
                        share(
                          {
                            sourceId: ashtakamId,
                            sectionNameHi: ashtakam.titleHi,
                            sectionNameEn: ashtakam.titleEn,
