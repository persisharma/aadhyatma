            ...
            renderItem={({ item, index }) => (
              <KrishnaStotramVersePage
                ...
                topActions={
                  <>
                    <BookmarkButton
                      isBookmarked={isBookmarked(`krishna-stotram:${chapter.chapter}:${index}`)}
                      onToggle={() => {
                        const id = `krishna-stotram:${chapter.chapter}:${index}`;
                        if (isBookmarked(id)) { removeBookmark(id); }
                        else {
                          addBookmark({ id, sourceId: 'krishna-stotram', chapter: chapter.chapter, verseIndex: index, savedAt: Date.now(), previewHi: item.sanskrit[0] ?? '', previewEn: item.linesEn[0] ?? '' });
                        }
                      }}
                    />
                    <ShareButton
