...
import { getKrishnaStotramChapter, type KrishnaStotramVerse } from '@/data/krishna-stotram';
...
import KrishnaStotramVersePage from '@/components/KrishnaStotramVersePage';
...
type Props = NativeStackScreenProps<HomeStackParamList, 'KrishnaStotramReader'>;

const DOT_COUNT = 5;

export default function KrishnaStotramReaderScreen({ navigation, route }: Props) {
  ...
  const chapter = useSafeChapter(route.params.chapter, getKrishnaStotramChapter, navigation, 'KrishnaStotramChapters');
  const listRef = useRef<FlatList<KrishnaStotramVerse>>(null);
  ...
  useEffect(() => {
    ...
    setProgress({ sourceId: 'krishna-stotram', chapter: chapter.chapter, verseIndex: currentIndex, updatedAt: Date.now() });
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        <View style={styles.topBar}>
          ...
          <View style={[styles.topSide, { alignItems: 'flex-end' }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              ...
              <BookmarkButton
                isBookmarked={isBookmarked(`krishna-stotram:${chapter.chapter}:${currentIndex}`)}
                onToggle={() => {
                  const id = `krishna-stotram:${chapter.chapter}:${currentIndex}`;
                  if (isBookmarked(id)) { removeBookmark(id); }
                  else {
                    const v = chapter.verses[currentIndex];
                    addBookmark({ id, sourceId: 'krishna-stotram', chapter: chapter.chapter, verseIndex: currentIndex, savedAt: Date.now(), previewHi: v.sanskrit[0] ?? '', previewEn: v.linesEn[0] ?? '' });
                  }
                }}
              />
              <ShareButton
        ...
        <View style={[styles.toggleRow, { flexDirection: 'row', justifyContent: 'center', gap: 18 }]}><LanguageToggle /><AddToRoutineButton sourceId="krishna-stotram" chapter={chapter.chapter} /></View>
        ...
        <View style={styles.listContainer}>
          <FlatList
            ...
            data={chapter.verses}
            keyExtractor={(v) => v.id}
            renderItem={({ item }) => <KrishnaStotramVersePage verse={item} sourceId="krishna-stotram" width={width} />}
            extraData={lang}
            horizontal
            pagingEnabled
