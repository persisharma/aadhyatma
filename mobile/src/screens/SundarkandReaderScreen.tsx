...
import {
  ...
  sundarkandTitle,
  sundarkandTotal,
  type SundarkandReading,
} from '@/data/sundarkand';
import SundarkandPage from '@/components/SundarkandPage';
...
export default function SundarkandReaderScreen({ navigation, route }: Props) {
  ...
  const listRef = useRef<FlatList<SundarkandReading>>(null);
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.parchment }]}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right', 'bottom']}>
        ...
        <FlatList
          ref={listRef}
          data={sundarkandReadings as SundarkandReading[]}
          keyExtractor={(reading) => reading.id}
          renderItem={({ item }) => <SundarkandPage reading={item} width={width} />}
          horizontal
          pagingEnabled
