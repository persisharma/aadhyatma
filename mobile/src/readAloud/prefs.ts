...
/** The two voice locales v1 resolves. See `voices.ts` for why gu/kn map to 'hi'. */
export type SpeechTarget = 'hi' | 'en';

export type ReadAloudPrefs = {
  /** Speech rate, clamped to [MIN_SPEECH_RATE, MAX_SPEECH_RATE]. */
  rate: number;
  /** Probed voice identifiers, per speech target. Absent = automatic selection. */
  voiceByTarget: { hi?: string; en?: string };
