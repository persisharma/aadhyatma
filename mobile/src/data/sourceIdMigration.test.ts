...
assert.equal(canonicalSourceId('aarti-7'), 'om-jai-lakshmi-mata');
assert.equal(canonicalSourceId('aarti-8'), 'shri-ramchandra-kripalu');
// Out-of-range index falls through unchanged so the consumer can decide.
assert.equal(canonicalSourceId('aarti-9'), 'aarti-9');
