...
import type { baseTemples } from '../templeRows';

/**
 * The ORIGINAL two-line detail for all 71 temples — significance, origin story
 * and sources — as shipped before the RULEBOOK §12.6 full readings began.
 *
 * WHY IT LIVES HERE. This map is ~250 KB of prose that nothing needs until a
 * detail screen or the search index actually asks for a temple. It used to sit
 * inside `temples.ts`, which the navigator reaches statically, so every cold
 * start paid for it. `temples.ts` now pulls this module through a `require()`
 * thunk instead (the `pincodes.ts` pattern) — see `loadDetails()` there, and
 * `src/data/__tests__/launchGraph.test.ts` for the rule.
 *
 * A temple whose full §12.6 reading has been authored under `details/` has its
 * entry here REPLACED wholesale by that chunk — this map is the fallback, not
 * the base, so do not merge fields across the two.
 */
...
export const legacyDetails: Record<TempleId, TempleDetail> = {
  somnath: {
