...
// ─── 11. Library labels are precise about verse types ───────────────────────
...
assert.equal(libraryById.get('sundarkand')?.sub, '16 अनुभाग · 343 पद');
assert.equal(libraryById.get('ramcharitmanas')?.nameHi, 'रामचरितमानस — बालकाण्ड');
assert.equal(libraryById.get('ramcharitmanas')?.nameEn, 'Ramcharitmanas — Bal Kand');
assert.match(libraryById.get('ramcharitmanas')?.sub || '', /59 उपखंड/);
assert.doesNotMatch(libraryById.get('ramcharitmanas')?.sub || '', /अंश|Excerpt/i);
assert.equal(libraryById.get('vishnu-sahasranama')?.nameHi, 'विष्णु सहस्रनाम');
assert.equal(libraryById.get('vishnu-sahasranama')?.nameEn, 'Vishnu Sahasranama');
assert.doesNotMatch(
  [
    libraryById.get('vishnu-sahasranama')?.nameHi,
    libraryById.get('vishnu-sahasranama')?.nameEn,
    libraryById.get('vishnu-sahasranama')?.sub,
    libraryById.get('durga-stotram')?.sub,
  ].join(' '),
  /अंश|excerpt|excert|चयनित|selected/i
);
assert.equal(libraryById.get('durga-stotram')?.nameHi, 'दुर्गा स्तोत्रम्');

const ramcharitmanasManifest = readJson('ramcharitmanas/chapters-manifest.json');
assert.equal(ramcharitmanasManifest.length, 59, 'Ramcharitmanas Bal Kand should keep all 59 subchapters');

let ramcharitmanasVerseTotal = 0;
const ramcharitmanasSectionCounts: Record<string, number> = {
  shloka: 0,
  sortha: 0,
  chaupai: 0,
  doha: 0,
...
for (const chapter of ramcharitmanasManifest) {
  const chStr = String(chapter.chapter).padStart(2, '0');
  const data = readJson(`ramcharitmanas/chapter-${chStr}.json`);
  assert.equal(data.chapter, chapter.chapter, `Ramcharitmanas chapter-${chStr} number should match manifest`);
  assert.equal(data.verseCount, data.verses.length, `Ramcharitmanas chapter-${chStr} verseCount should match verses`);
  assert.equal(data.verseCount, chapter.verseCount, `Ramcharitmanas chapter-${chStr} manifest count should match file`);
  assert.ok(
    data.source.referenceUrls.some((url: string) => url.includes('hi.wikisource.org/wiki/') && url.includes('बालकाण्ड')),
    `Ramcharitmanas chapter-${chStr} should cite Wikisource Bal Kand`
  );
  assert.ok(
    data.source.referenceUrls.some((url: string) => url.includes('www.ramcharitmanas.iitk.ac.in')),
    `Ramcharitmanas chapter-${chStr} should cite IITK Ramcharitmanas`
  );
  assert.ok(
    data.source.referenceUrls.some((url: string) => url.includes('ramcharithmanas.com/en/balkand/')),
    `Ramcharitmanas chapter-${chStr} should cite its subchapter source`
  );
  for (const v of data.verses) {
    ramcharitmanasVerseTotal += 1;
    ramcharitmanasSectionCounts[v.section] += 1;
    assert.equal(
      v.lines.length,
      v.linesEn.length,
      `Ramcharitmanas ${v.id} should keep original and transliteration display rows aligned`
    );
  }
}
...
assert.deepEqual(
  {
    chapter: ramcharitmanasManifest[0].chapter,
    titleHi: ramcharitmanasManifest[0].titleHi,
    titleEn: ramcharitmanasManifest[0].titleEn,
  },
  { chapter: 1, titleHi: 'मंगलाचरण', titleEn: 'Invocation and Auspicious Beginning' }
);
assert.deepEqual(
  {
    chapter: ramcharitmanasManifest[ramcharitmanasManifest.length - 1]?.chapter,
    titleHi: ramcharitmanasManifest[ramcharitmanasManifest.length - 1]?.titleHi,
    titleEn: ramcharitmanasManifest[ramcharitmanasManifest.length - 1]?.titleEn,
  },
  {
    chapter: 59,
    titleHi: 'श्रीरामचरित सुनने गाने की महिमा',
    titleEn: "The Glory of Hearing and Singing Rama's Deeds",
  }
);
assert.equal(ramcharitmanasVerseTotal, 1946, 'Ramcharitmanas Bal Kand should include 1,946 verse records');
assert.deepEqual(ramcharitmanasSectionCounts, {
  shloka: 7,
  sortha: 36,
  chaupai: 1484,
  doha: 359,
...
assert.doesNotMatch(
  JSON.stringify(readJson('ramcharitmanas/chapter-01.json')),
  /कपी श्वरौ|kapī śvarau/,
  'Ramcharitmanas opening shloka should match the joined कपीश्वरौ reading'
);
