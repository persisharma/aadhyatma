 ...
 *
 * `lenses` is the DISPLAY set: the stored ids narrowed to calendars that bring
 * something in this build (`withContentOnly`). A stored id for an empty calendar
 * (a build that dropped its last rule) stays on disk but is not shown, counted or
 * offered — the sheet cannot list a switch that changes nothing. `availableCount`
 * is likewise the offered count.
 *
...
export function useLenses(): {
  ...
  hydrated: boolean;
  activeCount: number;
  /** Calendars this build offers (brings ≥ 1 observance). 0 ⇒ the filter surfaces hide. */
