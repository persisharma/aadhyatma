...
- **Full-text handoff (user-mediated AI bridge)** — one Share button: the report's warned share sheet offers the 4:5 card as primary and `पूर्ण पाठ साझा करें` as its secondary action. The pure `kundaliHandoff.ts` renders birth details, the complete graha table, the Vimshottari date table, every section, and the `KundaliReportModel` JSON as one text document; the sheet's privacy line names the birth details both actions carry before either leaves the device. This is the deliberate bridge to §5: the USER hands the grounding object to ChatGPT/any LLM; the app makes no network call, so §14.3's AI ban stays intact.
