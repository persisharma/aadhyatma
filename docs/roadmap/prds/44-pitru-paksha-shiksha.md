...
This session had **no outbound network** (proxy returned no response to every source domain). Therefore:

- **Verified rows** are limited to (a) facts the repo has already opened and pinned in [`shraddha-tarpan-source-dossier.md`](../conventions/shraddha-tarpan-source-dossier.md) — Dharma Sindhu ch. 26 and DrikPanchang's shraddha pages, opened 2026-08-19 — and (b) what the shipped engine itself computes, and (c) scripture whose exact verse exists in the **bundled** Gita / Valmiki corpora (the in-repo corpus is one reference; the corpus's own published source line is the other — RULEBOOK §27.6 precedent).
- **Draft rows** (invisible on every surface) carry `DRAFT — NOT VERIFIED` in their note and name the sources to open: the per-tithi assignments (Bharani, Matri Navami, Sannyasi Dwadashi, Ghata Chaturdashi), the three-debts / pancha-mahayajna teaching (Taittiriya Samhita 6.3.10.5, Manusmriti 3.70), the Mahalaya name lesson, the गोत्र glossary row, and the **Karna folk katha**, whose rendered canon line must say "लोक-परम्परा — महाभारत के मूल पाठ में नहीं".
...
1. **परिचय** — concept lessons as cards: title + first paragraph, **और पढ़ें ›** unfolds in place (a परिचय is read, not navigated). Verified: क्या है · श्राद्ध और तर्पण · दिन का कौन-सा समय · किस दिन किसका · हर परिवार की अपनी रीति.
2. **पक्ष की तिथियाँ** — one §33.6-style row per fortnight day (gold marker · name · note). Verified today: पूर्णिमा श्राद्ध, सर्वपितृ अमावस्या; the remaining rows are draft, and an italic footer says the traditional notes follow their source review.
3. **शास्त्र-वचन** — the verse spine: short quoted line(s) (Devanagari / IAST by language) · gold cite · meaning · **गीता में पढ़ें ›** / **रामायण में पढ़ें ›**. Verified: Gita 1.42, 9.25, 2.20; Valmiki 2.102.27 (Rama's jalanjali). A row **not** reachable in a bundled reader is never quoted (Manu 3.70 is draft and unquoted by rule).
4. **कथाएँ** — door rows → `PitruKatha`. Verified: **चित्रकूट में श्रीराम का पितृ-कर्म** (retold verse-by-verse from bundled Ayodhya Kanda 2.102.20–35).
5. **प्रश्नोत्तर** — Q/A cards. Verified: tithi unknown · why the printed calendar differs · is home tarpana complete shraddha · two ancestors one tithi.
6. **शब्द** — glossary card (पितर · तर्पण · पिण्ड · कुतप काल · अनुकल्प).
...
| **B. Why** | three debts (TS 6.3.10.5); pancha-mahayajna (Manu 3.70); Gita 1.42 / 9.25 as the tradition's own statements | Gita ✅ bundled; Manu/TS ⏳ open & record | 1 (Gita) · 2 (rest) |
| **C. Whose day** | the mapping rule; purnima rule; kshaya two-name row; **per-tithi assignments** (Bharani, Matri/Avidhava Navami, Sannyasi Dwadashi, Ghata Chaturdashi, Pratipada for maternal side) | engine ✅; assignments ⏳ Drik shraddha-days + Nirnaya/Dharma Sindhu | 1 (rule) · 2 (assignments) |
...
| **F. Kathas** | Rama at Mandakini (Valmiki 2.102) ✅; Bharata's 12th-day shraddha (2.77.1) — candidate; Karna folk katha ⏳ (label as लोक-परम्परा) | bundled ✅; Karna ⏳ | 1 · 2 |
| **G. Glossary** | पितर · तर्पण · पिण्ड · कुतप · अनुकल्प ✅; गोत्र ⏳; sapindikarana, panchabali — candidates | dossier ✅ | 1 · 2 |
| **H. FAQ** | 4 verified ✅. **Deliberately excluded**: "no new purchases / inauspicious fortnight" — a verdict the opened sources don't make; §51 stance | — | — |
