...
5. `npm run lint` 0 errors; `tsc` clean; **design.md §70 + RULEBOOK §25 land in the same PR as the build** (design-doc-sync rule); categories/enumeration mirrors refreshed if any registry list is doc-mirrored.
...
- **2026-09-01 — Full build (all phases in one go, per product direction).** Content: verse spine
  ...
  + engine 77 + widgets/data tsx suites), `npm run lint` 0 errors, `tsc` clean; design.md §70 +
  RULEBOOK §25 + wiki page in the same commit series. The §2.7 surface contract, the journey's
