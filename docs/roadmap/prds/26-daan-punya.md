...
| **Number note** | PRD-25 stays soft-reserved for सन्ध्या वन्दन. Numbering settled 2026-09-03 after two mainline waves: this built feature keeps **26**; round-2's शुभ योग/कुल परम्परा shipped on main as **27/29**, पर्व-अर्क stays **28**, कण्ठस्थ renumbered **26 → 42** (its only true collision); **31–40** are the 2027 feature bets, **41** is जिज्ञासा. PRD-40 (dakshina donation rail, 2027 bet) overlapped this feature's directory layer — **resolved via Resolution A (links-only):** PRD-40 is ON HOLD; this feature collects no payment, and its directory carries no Vedansh-owned recipient. If PRD-40 resumes it owns dakshina-to-Vedansh/partner-temples on its own surfaces, never merged here. |
...
7. **Educate first — but the door still stands (§2.7 relaxed, v3).** This is an IA contract, not a tone preference, and it is testable. Educate is the **default** — what opens first — not a hard gate in front of the door:
   - The daan home opens on **महत्व** — why the tradition gives, in the shastra's own words — with a **standing door block** right under the subtitle: a filled `दान करें` button into the §4a journey (occasion-aware) and a quiet direct `दान-द्वार` link into the directory. Both stand every day; educate content still leads the screen. The home is no longer a "zero give affordances" surface.
   - The giving directory is still **not a tab**. It is reachable from the journey's terminal, from the journey skip, OR from the home's direct link — the journey remains one skippable scroll whose terminal give action follows the why/shastra/katha/what.
   - The ledger's "record" action always appears **before** the "give elsewhere" door wherever both exist — recording what one already gives is the primary behaviour; the directory hand-off is the terminal, optional one. The ledger is never gated.
   - **The app still never transacts (Resolution A, links-only).** Every hand-off is `Linking.openURL` to the recipient's own official channel behind the honest interstitial — the app collects no payment for daan. **PRD-40 boundary:** the daan directory must never contain a Vedansh-owned recipient (every row's `officialUrl` is the org's own or a government domain); dakshina to Vedansh / partner temples belongs to PRD-40's own surfaces if that rail ever resumes, never merged here.
   - Pinned in tests: the surface-contract test asserts the home's standing doors navigate to the journey and directory, and that the directory opens as a cause grid drilling into a filtered single cause; the copy guard bans score/streak/fear vocabulary throughout.
...
- **DaanPunyaScreen** — the educate home, in §4a order: a **standing door block** under the subtitle (`दान करें` → journey, occasion-aware; `दान-द्वार` link → directory) followed by आज का दान महत्व line (occasion + vaar aware), shastra principle cards (Gita deep links; RV 10.117 / TU 1.11.3 verse cards), katha cross-links, occasion browser, and a quiet खाता door at the end. Educate leads; the doors stand every day (§2.7 relaxed, v3) — the app still never transacts.
- **DaanLedgerScreen / DaanEntryScreen** — timeline + the 10-second form; export via share sheet.
- **DaanDirectoryScreen / detail** (Phase 2) — opens as a **cause grid** (tiles, no place counts), tapping a cause drills into its filtered single-cause view with the cause's **mahatva above its places** (§5.2), thin rows, external hand-off with the honest interstitial. A single-cause occasion arrival opens the filtered view directly.
