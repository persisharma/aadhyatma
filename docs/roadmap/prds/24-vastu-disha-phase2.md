...
| | |
|---|---|
| **Status** | Plan — drafted 2026-09-04, revised the same day with buyer/renter mode, floor-plan input, tiered rules and the AI layer. **UX decided 2026-09-05** (owner, from the prototype): Variant A capture + Variant C floor-plan mark-up, the **mandala grid as the single assessment reading**; the ledger reading is dropped. **Amended 2026-09-06** (owner, from the journey prototype's grid branch): the manual/no-visit capture is **drag-placement on the mandala grid itself** — room chips dragged into the nine cells — not a per-room direction-chip picker; the drop point inside a cell is kept as the user's sketch. **R1 + R2 built 2026-09-07** (source-ladder compass, declination grid, tilt/Hold/haptics, गृह वास्तु capture → assessment → roster → compare, F0 text handoff, C4 doors, C5 `vastu.myhome`, D target; door padas + 6 new registry rows ship `draft` per §22.3; F1 waits on the next store release, F2 on PRD-32) |
