...
**Resolution pattern:** For content-heavy features, add a web-verification pass after TDD/review. Per RULEBOOK §11: every Sanskrit sloka must be cross-checked against ≥1 authoritative online source before merge. Caught discrepancies must be fixed; unverifiable citations must be relaxed (remove specific Purana attribution if not findable).
