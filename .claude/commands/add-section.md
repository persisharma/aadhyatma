...
2. **`mobile/src/data/<id>/index.ts`** — adapt `mobile/src/data/gita/index.ts` (chapter-based) or write a flat loader (subsection-less). The manifest is the ONLY eager import; every chapter payload goes behind a `require()` thunk resolved by `get<Pascal>Chapter()` (RULEBOOK §2 row 2). Invariant checks: per-chapter on first load — call `assertChapterMatchesManifest` from `../chapterInvariants` in the getter — and add the corpus to the `CORPORA` table in `mobile/src/data/__tests__/chapteredCorpusInvariants.test.ts` for the whole-corpus walk. Never a module-scope IIFE that reads the chapters: it would force every payload onto the launch path.
...
8. **`mobile/src/navigation/HomeStackNavigator.tsx`** — `Edit` to register the screen(s) in the Home stack using the same `gestureEnabled: false, animation: 'fade'` options as existing screens. Declare each screen **lazily, at module scope**, with the others: `const <Pascal>ReaderScreen = lazyScreen('<Pascal>Reader', 3, () => import('@/screens/<Pascal>ReaderScreen'));` (depth = taps from Home; a reader opened from a category list is 3, its chapters index is 3 and the reader behind that 4). Never a static `import … from '@/screens/…'` — `lazyScreenRegistration.test.ts` fails it. Then regenerate the warm-up route graph: `npx tsx scripts/gen-route-graph.mts`.
9. **`mobile/src/data/texts.ts`** — `Edit` to append a `LibraryEntry` for `<id>`. Status `'active'`. Include the `category` and `deities` fields collected in Phase A. Place it before the `coming` entries. Its `verseCount` comes from `./verseCounts`, never from the corpus: add the `<id>` count symbol to `mobile/scripts/gen-verse-counts.mts`, run `npx tsx scripts/gen-verse-counts.mts`, and import it from `./verseCounts`. `verseCounts.test.ts` fails a direct corpus import.
